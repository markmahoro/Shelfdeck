...
test('publisher atomically normalizes Plan, Nodes, Edges and initial Events with stable replay', () => {
  fixture(({ publisher, databasePath }) => {
    ...
    assert.equal(persistedPlan.planner_ref, 'helix://libra/planners/Fixture/v1');
    assert.equal(persistedPlan.catalog_digest, CATALOG_DIGEST);
    const persistedNodes = rows(databasePath, 'fx_plan_nodes');
    assert.equal(persistedNodes.length, 2);
    assert.equal(persistedNodes[1].capability_ref, 'libra.fixture.commit@1');
    assert.equal(persistedNodes[1].effect_class, 'domain_fact_commit');
...
test('publisher persists an explicit same-Plan compensation target and contract', () => {
  fixture(({ publisher, databasePath }) => {
    ...
    assert.equal(persisted.capability_ref, 'libra.fixture.observe@1');
