...
test('builds the same title-only rating Identity Evidence across years and target owners', () => {
  ...
  const entry = buildRatingTargetIdentity({
    title: '看不见的朋友 (2024) - 1080p H.264 CHDWEB', year: '2024',
  ...
  const subjectTitles = subjectQuery.identityEvidence.filter((item) => item.anchorKind === 'title').map((item) => item.anchorValue);
  const entryTitles = entryQuery.identityEvidence.filter((item) => item.anchorKind === 'title').map((item) => item.anchorValue);
  assert.deepEqual(subjectTitles, ['看不见的朋友']);
  assert.deepEqual(subjectTitles, entryTitles);
  assert.equal(subjectQuery.identityEvidence.some((item) => item.anchorKind === 'title_year'), false);
  assert.equal(entryQuery.identityEvidence.some((item) => item.anchorKind === 'title_year'), false);
