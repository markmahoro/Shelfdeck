...
test('one-member import plan is deterministic and restart returns one physical result',async()=>{
  ...
  const imported=c.buildImportedWorkspaceMediaHandle({workspaceDeliveryContract:contracts[0],workspaceMaterialHandle:first,
    producingEventId:'external-import-event-1',idempotencyKey:'external-import-key-1'});
  assert.equal(imported.productionVideoProfile,null);
