...
function seedControl(unitOfWork,material){const field={ownerDomain:'procurement',scopeType:'material_field',scopeId:'field-1'},change={action:'acquire',identity:material,expectedRevision:0,fromScope:null,toScope:field};
  const handle={schemaRef:'helix://contracts/types/ResponsibilityControlCommitHandle/v1',schemaVersion:1,handleId:'seed-control',operationKind:'acquire',ownerDomain:'procurement',processType:'seed',processId:'seed-1',
    ...
    controlScopeDigest:require('../../src/helix/foundation/persistence/material-control').controlScopeDigest([change]),expectedControlRevisions:[{materialKey:material.materialKey,revision:0}],receiptContract:{receiptSchemaRef:'helix://fixtures/SeedControlReceipt/v1',controlRevisionSetSchemaRef:'fixtures.seed-control-revision-set@1'},eventFenceDigest:D('fence')};
...
function acceptedBasis(unitOfWork,material){const projection=createMaterialControlProjectionPort({schemaManifest,unitOfWork}).getMaterialControlProjection(material.materialKey);
  const member={ordinal:0,materialKey:material.materialKey,role:'primary_payload',physicalIdentity:material,sizeBytes:100,bindingRevision:1,admittedControlRevision:1,admittedControlProjectionDigest:projection.projectionDigest,
  ...
  const relatedReferenceSetDigest=canonicalDigest({schema:'procurement.related-reference-set@1',items:[]});
  ...
  const candidatePackage={candidatePackageId:'candidate-1',packageRevision:1,packageDigest:D('package'),
    ...
    identityClaim,seasonContinuityClaims:[claim],seasonContinuityClaimSetDigest:D('claim-set'),relatedReferences:[],
    relatedReferenceSetDigest,relatedDispositionScopeDigest};
...
function resultBinding(resultId,eventId){return {resultId,eventId,
  evidenceSchemaRef:'helix://contracts/types/SubjectAndTransferReceipt/v1',
  evidence:{schemaRef:'helix://contracts/types/SubjectAndTransferReceipt/v1',schemaVersion:1,evidenceKind:'handoff_a_acceptance_commit'},
  effectReceiptId:eventId+'-effect'};}
function seedEvent(databasePath,eventId){const database=new Database(databasePath);database.prepare('INSERT INTO fx_workflow_events(event_id) VALUES(?)').run(eventId);database.close();}
...
test('commits new Subject, Binding, exact Control transfer, Receipt and Outbox atomically and replays',()=>fixture(({databasePath,unitOfWork})=>{
  const material=identity();seedControl(unitOfWork,material);createLibraIntakeStore({schemaManifest,unitOfWork}).ensureContinuityHead();const basis=acceptedBasis(unitOfWork,material);seedEvent(databasePath,'handoff-a-event');
  const store=createIntakeAcceptanceStore({schemaManifest,unitOfWork}),request={deliverySnapshot:basis.snapshot,payload:basis.payload,responsibilityControlCommitHandle:basis.handle,
    commitMarker:{commitMarker:'handoff-a-marker',commitDigest:D('commit')},resultBinding:resultBinding('handoff-a-result','handoff-a-event')};
  ...
  const outbox=db.prepare("SELECT consumer_set_digest FROM fx_outbox WHERE message_kind='libra_candidate_accepted'").get();
...
test('stale global head rolls back every accepted participant before Control transfer',()=>fixture(({databasePath,unitOfWork})=>{
  const material=identity();seedControl(unitOfWork,material);const intake=createLibraIntakeStore({schemaManifest,unitOfWork});intake.ensureContinuityHead();const basis=acceptedBasis(unitOfWork,material),subjects=intake.repositories.subjects;seedEvent(databasePath,'bad-event');
  ...
  assert.throws(()=>createIntakeAcceptanceStore({schemaManifest,unitOfWork}).accept({deliverySnapshot:basis.snapshot,payload:basis.payload,responsibilityControlCommitHandle:basis.handle,
    commitMarker:{commitMarker:'bad-marker',commitDigest:D('bad')},resultBinding:resultBinding('bad-result','bad-event')}));
...
test('Outbox crash rolls back Subject, Binding, Control, Receipt, Result and Marker',()=>fixture(({databasePath,unitOfWork})=>{
  const material=identity();seedControl(unitOfWork,material);createLibraIntakeStore({schemaManifest,unitOfWork}).ensureContinuityHead();const basis=acceptedBasis(unitOfWork,material);seedEvent(databasePath,'crash-event');
  ...
  assert.throws(()=>createIntakeAcceptanceStore({schemaManifest,unitOfWork}).accept({deliverySnapshot:basis.snapshot,payload:basis.payload,responsibilityControlCommitHandle:basis.handle,
    commitMarker:{commitMarker:'crash-marker',commitDigest:D('crash')},resultBinding:resultBinding('crash-result','crash-event')}));
