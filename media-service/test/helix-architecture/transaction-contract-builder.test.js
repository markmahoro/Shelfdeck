...
test('builds all 44 canonical transactions with stable identities and crash fixtures', () => {
  assert.equal(contracts.length, 44);
  assert.equal(new Set(contracts.map((contract) => contract.transactionId)).size, 44);
  assert.equal(contracts.reduce((sum, contract) => sum + contract.crashFixtures.length, 0), 45);
