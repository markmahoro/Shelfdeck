...
test('validates all canonical transaction participants and crash contracts', () => {
  ...
  assert.equal(result.transactionCount, 38);
  assert.equal(result.responsibilityControlCount, 12);
  assert.equal(result.crashFixtureBindingCount, 39);
});
...
test('rejects duplicate transaction inventory identities', () => {
  const root = fixture();
  try {
    mutate(path.join(root, 'manifests', 'transaction-inventory', 'entries-001-038.json'), (shard) => {
