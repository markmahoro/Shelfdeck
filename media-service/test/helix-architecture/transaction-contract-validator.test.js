...
test('validates all canonical transaction participants and crash contracts', () => {
  ...
  assert.equal(result.transactionCount, 30);
  assert.equal(result.responsibilityControlCount, 12);
  assert.equal(result.crashFixtureBindingCount, 31);
});
...
test('rejects duplicate transaction inventory identities', () => {
  const root = fixture();
  try {
    mutate(path.join(root, 'manifests', 'transaction-inventory', 'entries-001-030.json'), (shard) => {
