...
test('validates all canonical transaction participants and crash contracts', () => {
  ...
  assert.equal(result.transactionCount, 43);
  assert.equal(result.responsibilityControlCount, 12);
  assert.equal(result.crashFixtureBindingCount, 44);
});
...
test('rejects duplicate transaction inventory identities', () => {
  const root = fixture();
  try {
    mutate(path.join(root, 'manifests', 'transaction-inventory', 'entries-001-043.json'), (shard) => {
