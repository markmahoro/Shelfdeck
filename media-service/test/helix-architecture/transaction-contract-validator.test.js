...
test('validates all canonical transaction participants and crash contracts', () => {
  ...
  assert.equal(result.transactionCount, 26);
  assert.equal(result.responsibilityControlCount, 10);
  assert.equal(result.crashFixtureBindingCount, 27);
});
...
test('rejects duplicate transaction inventory identities', () => {
  const root = fixture();
  try {
    mutate(path.join(root, 'manifests', 'transaction-inventory', 'entries-001-026.json'), (shard) => {
