...
async function waitFormation(host,cookie,predicate){const deadline=Date.now()+30_000;let item;while(Date.now()<deadline){const response=await host.inject({method:'GET',url:'/v1/admin/formation',headers:{cookie}});assert.equal(response.statusCode,200,response.body);item=response.json().items[0];if(item&&predicate(item))return item;await pause(25);}assert.fail('Formation did not reach the expected state: '+JSON.stringify(item));}
...
async function waitOpenProductOffer(dataDir,runtimeError){const deadline=Date.now()+30_000;let current=null;while(Date.now()<deadline){if(runtimeError())throw runtimeError();const database=new Database(path.join(dataDir,'shelfdeck.db'),{readonly:true});try{current=database.prepare("SELECT p.on_deck_package_id,p.package_digest,p.offer_id,'open' state FROM libra_product_packages p WHERE NOT EXISTS (SELECT 1 FROM libra_delivery_receipts r WHERE r.on_deck_package_id=p.on_deck_package_id) ORDER BY p.published_at_ms LIMIT 1").get();if(current)return current;}finally{database.close();}await pause(25);}assert.fail('Libra did not publish an open Handoff B Offer: '+JSON.stringify(current));}

test('direct ratings append immutable User Perception records and republish rating-bound Acceptance Specs',async(t)=>{
  ...
  try{const cookie=await session(host,initialized.adminApiKey),headers={cookie};
    ...
    const firstOffer=await waitOpenProductOffer(dataDir,()=>runtimeError);assert.ok(firstOffer.offer_id);assert.equal(firstOffer.state,'open');
    ...
    for(const rating of [1,2,3,4,5]){response=await host.inject({method:'POST',url:'/v1/admin/perception/records',headers,payload:{targetType:'subject',targetId:noRating.subjectId,expectedRevision:rating-1,rating,idempotencyKey:'rating-'+rating}});assert.equal(response.statusCode,202,response.body);
      const formation=await waitFormation(host,cookie,(item)=>item.acceptanceSpecRevision===rating+1);assert.notEqual(formation.acceptanceSpecDigest,previousDigest);assert.ifError(runtimeError);ratingSpecs.push(formation);previousDigest=formation.acceptanceSpecDigest;}
