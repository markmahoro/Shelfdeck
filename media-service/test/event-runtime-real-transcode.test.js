...
test('real transcode is executed as atomic Workflow Events and invalidates Basedata after replace', { timeout: 120000 }, async (t) => {
  ...
  try {
    ...
    config.subLibraries = [{ uuid: 'library-1', name: 'Fixture', mediaType: 'movie', libraryAutomationMode: 'manual', maintenanceAutomationMode: 'manual', allowedCapabilities: { metadata: [], optimize: ['media.transcode', 'media.file.replace'] }, capabilityPolicyRevision: '1' }];
    configStore.saveConfig(config);
    admissionStore.upsertAdmission({ subjectId: 'item-1', admissionGeneration: 1, status: 'active', sourceRevision: 'source-1', sourceAccessDescriptor: { sourceType: 'adult_folder', subLibraryId: 'library-1', locator: { path: input } } });
    kairoxStore.publishBasedata({ subjectId: 'item-1', sourceRevision: 'source-1', facts: { path: input, codec: 'h264', bitrate: 2000000, resolution: '320x180' } });
    const task = taskStore.createTask({ subjectId: 'item-1', subjectName: 'Fixture', status: 'queued', source: 'manual', sourceAccessMappingRevision: 'identity', helixAdmission: admissionStore.getAdmission('item-1'), capabilityPolicyRevision: '1', objectiveRevisionSnapshot: 'objective-1', taskTarget: { targetGate: 'optimize', gateObjective: { targetMediaFacts: { targetCodec: 'h265', targetBitrateProfileByBucket: { '1080p': { minMbps: 0.01, targetMbps: 0.3, maxMbps: 100 } } } } }, subjectInfo: { subjectId: 'item-1', name: 'Fixture', path: input, codec: 'h264', bitrate: 2000000, resolution: '320x180', subLibraryId: 'library-1', optimizeObjectiveStatus: 'ready' } });
