...
test('260 Candidate demand stays below the hard cap while completion Work publishes before all Runs seal',async(t)=>{
  ...
  for(let group=0;group<4;group+=1){const directory=path.join(sourceRoot,'group-'+group);fs.mkdirSync(directory,{recursive:true});
    for(let ordinal=0;ordinal<65;ordinal+=1)fs.writeFileSync(path.join(directory,
  ...
  try{
    ...
    const deadline=Date.now()+180_000;let sawEarlyOffer=false,maxOpenWorks=0,last={};
    while(Date.now()<deadline){
      ...
      if(runtimeError||last.runs===2&&last.sealed===2&&last.candidates===260)break;
      await new Promise((resolve)=>setTimeout(resolve,25));
    }
    assert.ifError(runtimeError);assert.deepEqual(last,{...last,runs:2,sealed:2,candidates:260,offers:260,failedWorks:0,failedEvents:0});
