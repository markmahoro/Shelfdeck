...
test('real-shaped Douban pages are bounded, paged, replay-safe, and append source correction lineage',async(t)=>{
  ...
  try{const cookie=await session(host,initialized.adminApiKey);await configure(host,cookie);const beforeSyncCalls=state.calls.length;
    ...
    const database=new Database(path.join(dataDir,'shelfdeck.db'),{readonly:true});try{assert.equal(database.prepare("SELECT count(1) count FROM perception_records WHERE source_kind='douban'").get().count,4);assert.equal(database.prepare("SELECT count(1) count FROM perception_record_relations WHERE relation_kind='supersedes'").get().count,1);assert.equal(database.prepare('SELECT count(1) count FROM perception_acquisitions WHERE state=\'completed\'').get().count,3);assert.ok(database.prepare("SELECT count(1) count FROM perception_identity_anchors a JOIN perception_records r ON r.perception_id=a.perception_id WHERE r.source_record_key='douban:1003' AND a.anchor_kind='title_year'").get().count>=3);}finally{database.close();}
