...
function providerFetch(state){return async(input,init={})=>{const url=new URL(String(input)),headers=Object.fromEntries(Object.entries(init.headers||{}).map(([key,value])=>[key.toLowerCase(),value]));state.calls.push(url.pathname+url.search);
  ...
  if(url.pathname==='/subject/1003/')return response(200,'<div id="content"><h1><span property="v:itemreviewed">第二页丙</span><span class="year">(2014)</span></h1></div><div id="info">又名: Page Two C / 第二页C\n制片国家/地区: 中国大陆</div>');
  ...
  if(start===2)return response(200,identity+'<ul>'+item('1003','第二页丙','',3)+'</ul><div class="paginator"></div>');return response(200,identity+'<ul></ul>');};}
...
test('real-shaped Douban pages are bounded, paged, replay-safe, and append source correction lineage',async(t)=>{
  ...
  const host=await createCleanServiceHost({dataDir,adminDistDir:admin,secretRoot:SECRET_ROOT,integrationFetch:providerFetch(state),doubanRequestPaceMs:0,now:()=>++now,onExecutionRuntimeError(error){runtimeError=error;state.runtimeError=error;}});
  try{const cookie=await session(host,initialized.adminApiKey);await configure(host,cookie);const beforeSyncCalls=state.calls.length;
    ...
    assert.equal(state.calls.some((value)=>value.includes('/subject/')),false);
    const database=new Database(path.join(dataDir,'shelfdeck.db'),{readonly:true});try{assert.equal(database.prepare("SELECT count(1) count FROM perception_records WHERE source_kind='douban'").get().count,4);assert.equal(database.prepare("SELECT count(1) count FROM perception_record_relations WHERE relation_kind='supersedes'").get().count,1);assert.equal(database.prepare('SELECT count(1) count FROM perception_acquisitions WHERE state=\'completed\'').get().count,3);assert.equal(database.prepare("SELECT count(1) count FROM perception_identity_anchors a JOIN perception_records r ON r.perception_id=a.perception_id WHERE r.source_record_key='douban:1003' AND a.anchor_kind='title'").get().count,1);assert.equal(database.prepare("SELECT count(1) count FROM perception_identity_anchors a JOIN perception_records r ON r.perception_id=a.perception_id WHERE r.source_record_key='douban:1003' AND a.anchor_kind='title_year'").get().count,0);}finally{database.close();}
