...
test('metadata artifacts are revision isolated, checksummed and atomically probed', () => {
  ...
  try {
    ...
    workspace.writeArtifact(config, { subjectId: 'item/1', metadataRevision: 'rev-1', name: 'metadata.nfo', content: '<movie/>', eventId: 'event-1' });
...
test('materialize Capability consumes the current admitted source after organize/rebind without implicit Event lookup', async () => {
  ...
  try {
    ...
    workspace.writeArtifact(config, { subjectId: 'item-1', metadataRevision: 'rev-1', name: 'metadata.nfo', content: '<movie/>', eventId: 'render' });
    builtIns.registerBuiltIns();
    let fenceCheckpoint = '';
    const result = await registry.get('metadata.artifacts.materialize').execute({ config, event: { eventId: 'materialize' }, assertFence: (checkpoint) => { fenceCheckpoint = checkpoint; }, task: { id: 'task-1', subjectId: 'item-1', objectiveRevisionSnapshot: 'rev-1', sourceAccessMappingRevision: sourceAccessResolver.getRevision(), subjectInfo: { path: mediaPath, metadataArtifactRevision: 'rev-1' }, helixAdmission: { sourceAccessDescriptor: { locator: { path: mediaPath } } } } });
