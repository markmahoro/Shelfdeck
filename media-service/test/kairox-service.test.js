...
process.env.CONTROL_PLANE_DATA_DIR = fs.mkdtempSync(path.join(os.tmpdir(), 'shelfdeck-kairox-service-'));
...
test('maintenance projection consumes Lifecycle completion without recomputing it', () => {
  const admission = { subjectId: 'maintenance-1', status: 'active', admissionGeneration: 3, incidentCode: '' };
  const lifecycle = {
  ...
  const result = buildMaintenanceProjection({ subjectId: admission.subjectId }, admission, lifecycle, []);
...
test('fact bundle maps current source revision and pending refresh into Lifecycle inputs', () => {
  const item = bundleToLifecycleItem({
    ...
    subjectId: 'maintenance-2', status: 'active', sourceRevision: 'source-2', incidentCode: '',
  });
...
test('Kairox admission creates only an owned skeleton and returns Basedata as next gate', () => {
  ...
  const runtime = createKairoxRuntime({
    kairoxStore: {
      ensureSubject({ subjectId, subjectKind = '' }) { bundles.set(subjectId, { subjectId, media: { subjectId, subjectKind, maintenancePriorityClass: 'normal', priorityRevision: 0 }, basedata: null, metadata: null, optimize: null, objective: null, refreshRequests: [] }); },
      getBundle(subjectId) { return bundles.get(subjectId) || null; },
      getBundles(subjectIds) { return Object.fromEntries(subjectIds.filter((id) => bundles.has(id)).map((id) => [id, bundles.get(id)])); },
      getSubject(subjectId) { return bundles.get(subjectId) && bundles.get(subjectId).media || null; },
      getMaintenanceRun() { return null; },
    },
    admissionStore: {
      getAdmission(subjectId) { return admissions.get(subjectId) || null; },
      getAdmissions(subjectIds) { return Object.fromEntries(subjectIds.filter((id) => admissions.has(id)).map((id) => [id, admissions.get(id)])); },
      upsertAdmission(input) {
        const value = { ...input, admissionGeneration: input.admissionGeneration, status: input.status || 'active' };
        admissions.set(input.subjectId, value);
        return value;
      },
    },
    taskStore: {
...
test('Kairox objective policy lets Lifecycle close a no-op optimize objective without StrategyEngine', () => {
  ...
  const item = kairoxObjectivePolicy.applyObjectivePolicy({
    subjectId: 'objective-item', subLibraryId: 'library',
...
test('automatic terminal failure blocks only the matching generation and target', () => {
  ...
  const admission = { subjectId: 'failure-item', admissionGeneration: 3, status: 'active' };
  const failure = {
    taskId: 'failed-task', targetGate: 'optimize', admissionGeneration: 3,
    objectiveHash: 'objective-1', updatedAt: '2026-01-01T00:00:00.000Z',
  };
  const blocked = buildMaintenanceProjection({ subjectId: 'failure-item' }, admission, projection, [], failure);
  assert.strictEqual(blocked.automationBlocker.code, 'previous_automatic_task_failed');
  const recovered = buildMaintenanceProjection({ subjectId: 'failure-item' }, { ...admission, admissionGeneration: 4 }, projection, [], failure);
