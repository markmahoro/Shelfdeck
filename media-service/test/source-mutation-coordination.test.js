...
test('Libra consumes Kairox SourceMutationResult through Nexora rebind and new admission generation', () => {
  let item = { subjectId: 'item-1', subLibraryId: 'library-1', membershipStatus: 'active', desiredState: 'managed', phase: 'maintenance', quarantineStatus: 'none', quarantineReason: '', admissionGeneration: 1, sourceRevision: 'source-1', mediaKind: 'adult_file', playable: true };
  const events = [];
  const calls = { suspended: [], rebound: [], acknowledged: [], admitted: [] };
  let source = { subjectId: 'item-1', readiness: 'ready', sourceRevision: 'source-2', sourceAccessDescriptor: { subLibraryId: 'library-1' } };
  const store = {
    getLibrarySubject: () => item,
    getLibrarySubjects: () => [item],
    upsertLibrarySubject: (patch) => (item = { ...item, ...patch }),
    appendEvent: (event) => events.push(event),
  };
  const mutation = { mutationId: 'mutation-1', subjectId: 'item-1', oldSourceEvidence: { path: '/media/a.mkv' }, newSourceEvidence: { path: '/media/scraped/a.mkv' } };
  const kairoxService = {
    ...
    getMaintenanceProjections: () => ({ 'item-1': { subjectId: 'item-1', admissionCurrent: false, maintenanceRevision: 'm2' } }),
