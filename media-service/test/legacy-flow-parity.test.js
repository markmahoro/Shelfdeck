...
const LEGACY_PARITY = [
  ...
  ['transcode.large_output', ['output.media.disposition', 'staged.asset.discard']], ['transcode.replace', ['media.file.replace']], ['transcode.cleanup', ['workspace.cleanup']],
  ...
  ['upgrade.identity_verify', ['media.identity.inspect', 'media.identity.accept']], ['upgrade.replace', ['media.file.replace']], ['upgrade.cleanup', ['workspace.cleanup']],
...
test('Basedata, Metadata and Optimize FlowPlans compose the parity capabilities through typed bindings', () => {
  registerCatalog();
  const basedata = planner.planTask({ id: 'b', subjectId: 'i', targetGate: 'basedata', taskTarget: { targetGate: 'basedata' }, helixAdmission: { assets: [{ assetId: 'a', assetKind: 'movie', assetRevision: 1, canonicalLocator: { path: 'Z:/a.mkv' } }], sourceAccessDescriptor: { sourceType: 'emby' } } }, {});
  assert.deepStrictEqual(basedata.nodes.map((node) => node.capability), ['emby.item.observe', 'filesystem.layout.observe', 'basedata.verify', 'basedata.publish', 'basedata.subject.publish']);
  assert.strictEqual(graph.validateGraph(basedata, registry), basedata);

  const metadata = planner.planTask({ id: 'm', subjectId: 'i', targetGate: 'metadata', taskTarget: { targetGate: 'metadata' }, subjectInfo: { subLibraryId: 'jav' }, helixAdmission: { sourceAccessDescriptor: { sourceType: 'folder', subLibraryId: 'jav' } } }, { subLibraries: [{ uuid: 'jav', mediaType: 'adult', adultRegion: 'japanese_jav', allowedCapabilities: { metadata: ['metadata.sidecar.render', 'metadata.image.acquire'] }, capabilityParameters: { 'metadata.image.acquire': { kinds: ['poster', 'fanart'] } } }] });
