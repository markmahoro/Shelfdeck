...
export default function PeoplePage() {
  ...
  const [draft, setDraft] = useState({ name: '', aliases: '', preference: 0 });
  const [toast, setToast] = useState('');
  const query = useQuery({ queryKey: ['people', search, kind, preference], queryFn: () => people.list({ search, contentKind: kind, preference, limit: 100 }) });
  ...
  const create = useMutation({ mutationFn: () => people.create({ name: draft.name, aliases: draft.aliases.split(',').map((value) => value.trim()).filter(Boolean), preference: draft.preference }), onSuccess: () => { refresh(); setCreateOpen(false); setDraft({ name: '', aliases: '', preference: 0 }); setToast('演员已创建'); }, onError: (error) => setToast(error.message) });
  ...
  return <Page>
    <PageHeader title="演员" meta={`${query.data?.total || 0} 位演员`} actions={<><Button onClick={() => setMergeOpen(true)} disabled={!candidates.data?.candidates.length}>身份合并 {candidates.data?.candidates.length ? `(${candidates.data.candidates.length})` : ''}</Button><Button variant="primary" icon="plus" onClick={() => setCreateOpen(true)}>新建演员</Button></>} />
    <div className="toolbar"><input className="input search" placeholder="搜索演员或别名" value={search} onChange={(e) => setSearch(e.target.value)} /><select className="select filter-compact" value={kind} onChange={(e) => setKind(e.target.value)}><option value="">全部内容</option><option value="general">普通</option><option value="adult">成人</option></select><select className="select filter-compact" value={preference} onChange={(e) => setPreference(e.target.value)}><option value="">全部偏好</option>{preferences.map((item) => <option key={item.value} value={item.value}>{item.label}</option>)}</select></div>
