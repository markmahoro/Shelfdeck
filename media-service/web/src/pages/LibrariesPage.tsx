...
const adultCapabilities = { metadata: ['metadata.sidecar.render', 'metadata.poster.acquire', 'metadata.fanart.acquire'], optimize: ['media.transcode', 'media.replace', 'source.organize', 'metadata.artifacts.materialize'] };
const generalCapabilities = { metadata: [] as string[], optimize: ['media.transcode', 'source.upgrade.download', 'media.replace'] };
const allOptimizeCapabilities = [...new Set([...generalCapabilities.optimize, ...adultCapabilities.optimize])];
const capabilityLabels: Record<string, string> = { 'metadata.sidecar.render': 'NFO资料', 'metadata.poster.acquire': '海报', 'metadata.fanart.acquire': '背景图', 'media.transcode': '转码', 'source.upgrade.download': '来源升级', 'media.replace': '文件替换', 'source.organize': '文件整理', 'metadata.artifacts.materialize': '资料写入媒体目录' };
const initialLibraryDraft = { name: '', source: 'emby', embyServerId: '', sectionId: '', watchRoot: '', mediaType: 'movie', adultRegion: 'japanese_jav', ruleTemplateId: 'default', libraryAutomationMode: 'manual', maintenanceAutomationMode: 'manual', doubanEnabled: false, allowedCapabilities: generalCapabilities };
...
export default function LibrariesPage() {
  ...
  const [capabilityDraft, setCapabilityDraft] = useState<{ metadata: string[]; optimize: string[] }>(generalCapabilities);
  ...
  return <Page>
    ...
    <Dialog open={createOpen} title="新建媒体库" onClose={() => setCreateOpen(false)} actions={<><Button onClick={() => setCreateOpen(false)}>取消</Button><Button variant="primary" disabled={!draft.name || create.isPending || (draft.source === 'emby' ? !draft.sectionId : !draft.watchRoot)} onClick={() => create.mutate()}>{create.isPending ? '创建中' : '创建媒体库'}</Button></>}>
      <div className="form-grid">
        ...
        <Field label="来源"><select className="select" value={draft.source} onChange={(e) => { const source = e.target.value; const mediaType = source === 'folder' ? 'adult' : 'movie'; setDraft({ ...draft, source, mediaType, ruleTemplateId: defaultTemplateId(mediaType, draft.adultRegion), allowedCapabilities: source === 'folder' ? adultCapabilities : generalCapabilities }); }}><option value="emby">Emby 媒体库</option><option value="folder">文件夹</option></select></Field>
        {draft.source === 'emby' ? <>
        ...
        <Field label="允许的资料能力"><div className="page-actions">{adultCapabilities.metadata.map((capability) => <label key={capability}><input type="checkbox" checked={draft.allowedCapabilities.metadata.includes(capability)} onChange={(event) => setDraft({ ...draft, allowedCapabilities: { ...draft.allowedCapabilities, metadata: event.target.checked ? [...draft.allowedCapabilities.metadata, capability] : draft.allowedCapabilities.metadata.filter((entry) => entry !== capability) } })} />{capabilityLabels[capability]}</label>)}</div></Field>
    ...
    <Dialog open={!!capabilityLibrary} title="维护能力" onClose={() => setCapabilityLibrary(null)} actions={<><Button onClick={() => setCapabilityLibrary(null)}>取消</Button><Button variant="primary" onClick={() => { if (capabilityLibrary) update.mutate({ library: capabilityLibrary, patch: { allowedCapabilities: capabilityDraft } }); setCapabilityLibrary(null); }}>保存</Button></>}>
      <div className="stack"><Field label="资料产物"><div className="page-actions">{adultCapabilities.metadata.map((capability) => <label key={capability}><input type="checkbox" checked={capabilityDraft.metadata.includes(capability)} onChange={(event) => setCapabilityDraft({ ...capabilityDraft, metadata: event.target.checked ? [...capabilityDraft.metadata, capability] : capabilityDraft.metadata.filter((entry) => entry !== capability) })} />{capabilityLabels[capability]}</label>)}</div></Field><Field label="优化动作"><div className="page-actions">{allOptimizeCapabilities.map((capability) => <label key={capability}><input type="checkbox" checked={capabilityDraft.optimize.includes(capability)} onChange={(event) => setCapabilityDraft({ ...capabilityDraft, optimize: event.target.checked ? [...capabilityDraft.optimize, capability] : capabilityDraft.optimize.filter((entry) => entry !== capability) })} />{capabilityLabels[capability]}</label>)}</div></Field></div>
    </Dialog>
