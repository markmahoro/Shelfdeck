...
export default function LibrariesPage() {
  ...
  const [draft, setDraft] = useState({ name: '', source: 'emby', embyServerId: '', sectionId: '', watchRoot: '', mediaType: 'movie', ruleTemplateId: 'default', libraryAutomationMode: 'manual', maintenanceAutomationMode: 'manual' });
  ...
  return <Page>
    ...
    <Dialog open={createOpen} title="新建媒体库" onClose={() => setCreateOpen(false)} actions={<><Button onClick={() => setCreateOpen(false)}>取消</Button><Button variant="primary" disabled={!draft.name || create.isPending || (draft.source === 'emby' ? !draft.sectionId : !draft.watchRoot)} onClick={() => create.mutate()}>{create.isPending ? '创建中' : '创建媒体库'}</Button></>}>
      <div className="form-grid">
        ...
        <Field label="媒体类型"><select className="select" value={draft.mediaType} onChange={(e) => setDraft({ ...draft, mediaType: e.target.value })}><option value="movie">电影</option><option value="tv">剧集</option><option value="adult">成人</option></select></Field>
