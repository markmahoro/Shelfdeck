...
export default function DashboardPage() {
  ...
  function closeWizard() {
    ...
    setDoubanEnabled(false); setTestError('');
  }
    ...
    <div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 16, marginBottom: 24 }}>
        {[
          ...
          { to: '/system', label: '任务调度设置', desc: '执行模式与并发控制' },
        ].map(({ to, label, desc }) => (
          <Link
            key={to}
            to={to}
      ...
      <div style={{ background: '#fff', borderRadius: 10, padding: 20, boxShadow: '0 1px 4px rgba(0,0,0,0.08)' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
          <h3 style={{ fontSize: 15, fontWeight: 600, color: '#1a1a2e', margin: 0 }}>子库管理</h3>
          <button
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 14 }}>
            <thead>
              <tr style={{ background: '#f9fafb' }}>
                <th style={{ textAlign: 'left', padding: '10px 16px', borderBottom: '2px solid #eee', color: '#666' }}>名称</th>
                <th style={{ textAlign: 'left', padding: '10px 16px', borderBottom: '2px solid #eee', color: '#666' }}>豆瓣同步</th>
                <th style={{ textAlign: 'left', padding: '10px 16px', borderBottom: '2px solid #eee', color: '#666' }}>状态</th>
                <th style={{ textAlign: 'left', padding: '10px 16px', borderBottom: '2px solid #eee', color: '#666' }}>最后刷新</th>
                <th style={{ textAlign: 'center', padding: '10px 16px', borderBottom: '2px solid #eee', color: '#666' }}>操作</th>
              </tr>
            </thead>
            <tbody>
              {subLibs.map((sl) => (
