...
export default function MediaPage() {
  ...
  const startMaintenance = useMutation({ mutationFn: (subjectId: string) => helixLibrary.startMaintenance(subjectId), onSuccess: () => completed('维护已开始'), onError: (error) => setToast(error.message) });
  const prioritize = useMutation({ mutationFn: (subjectId: string) => helixLibrary.prioritizeMaintenance(subjectId), onSuccess: () => completed('已优先维护'), onError: (error) => setToast(error.message) });
  const cancelPriority = useMutation({ mutationFn: (subjectId: string) => helixLibrary.cancelMaintenancePriority(subjectId), onSuccess: () => completed('已取消优先'), onError: (error) => setToast(error.message) });
  ...
  const stop = useMutation({ mutationFn: (item: MediaItem) => helixLibrary.offboard(item.subjectId, { idempotencyKey: `stop-media:${item.subjectId}:${Date.now()}`, cleanupMode: 'retain_source', reason: 'admin_stop_management' }), onSuccess: () => { setToast('已停止管理'); setSelected(null); refresh(); }, onError: (error) => setToast(error.message) });
  const updateRating = useMutation({ mutationFn: ({ subjectId, rating }: { subjectId: string; rating: number | null }) => libraryApi.patchPerception(subjectId, rating), onSuccess: (_, variables) => { setSelected((current) => current ? { ...current, userRating: variables.rating, helix: { ...current.helix, maintenance: { ...current.helix?.maintenance, userPerceptionFacts: { ...current.helix?.maintenance?.userPerceptionFacts, userRating: variables.rating } } } } : current); refresh(); setToast('用户评分已保存'); }, onError: (error) => setToast(error.message) });
  const assets = useQuery({ queryKey: ['subject-assets', selected?.subjectId], queryFn: () => helixLibrary.getAssets(selected!.subjectId), enabled: !!selected?.subjectId && selected?.type === 'series' });
  ...
  return <Page>
    ...
    {items.length === 0 ? <section className="panel"><EmptyState title="没有匹配的媒体" /></section> : <section className="panel table-wrap"><table className="table responsive"><thead><tr><th>媒体</th><th>媒体库</th><th>来源</th><th>维护方向</th><th>维护状态</th><th></th></tr></thead><tbody>{items.map((item) => {
      ...
      return <tr key={item.subjectId}>
        <td data-label="媒体"><div className="person-cell">{item.imageUrl ? <img className="poster" src={item.imageUrl} alt="" /> : <div className="poster" />}<div><div className="table-main">{item.name || item.title || item.subjectId}</div><div className="table-sub">{item.type || item.mediaType || ''}</div></div></div></td>
    ...
    <Drawer open={!!selected} title={selected?.name || selected?.title || '媒体详情'} onClose={() => setSelected(null)}>{selected && <div className="stack">
      ...
      {selected.type === 'series' && <section><div className="field-label">季与剧集</div><div className="stack field-actions">{Object.entries((assets.data?.assets || []).reduce((groups: Record<string, any[]>, asset: any) => { const key = asset.seasonKey || '未分季'; (groups[key] ||= []).push(asset); return groups; }, {})).map(([seasonKey, entries]) => <div key={seasonKey}><strong>第 {seasonKey} 季</strong><div className="muted">{(entries as any[]).map((asset) => `第 ${asset.episodeKey || '-'} 集`).join(' · ')}</div></div>)}</div></section>}
      <section><div className="field-label">操作</div><div className="page-actions field-actions">
        {eligible && !automatic && !selectedRun && <Button onClick={() => startMaintenance.mutate(selected.subjectId)}>开始维护</Button>}
        {eligible && (automatic || !!selectedRun) && selectedPriority !== 'expedited' && <Button onClick={() => prioritize.mutate(selected.subjectId)}>优先维护</Button>}
        {eligible && selectedPriority === 'expedited' && <Button variant="quiet" onClick={() => cancelPriority.mutate(selected.subjectId)}>取消优先</Button>}
