...
export default function App() {
  return <div className="helix-shell">
    ...
    <main id="main" className="helix-main"><Routes>
      {pages.map((page) => <Route key={page.slug} path={page.path} element={page.slug === 'overview' ? <OverviewPage/> : page.slug === 'material-fields' ? <MaterialFieldsPage/> : page.slug === 'shelves' ? <ShelvesPage/> : page.slug === 'formation' ? <FormationPage/> : page.slug === 'collection' ? <CollectionPage/> : page.slug === 'offdeck'?<OffdeckPage/>:page.slug === 'settings' ? <SettingsPage/> : <HelixPage page={page}/>}/>) }
