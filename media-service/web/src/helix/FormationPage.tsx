...
function ProcessDetail({ item }:{ item:FormationSubject }) {
  ...
  return <article className="formation-process-card" aria-label={`${item.displayIdentity}的上架过程详情`}>
    ...
    <div className="formation-process-stages">
      ...
      <details className="formation-process-stage" open><summary><span className="formation-stage-index" aria-hidden="true">2</span><span><strong>媒体整理</strong><small>{detail?.mediaOrganization.summary || item.organizingAction}</small></span><StageState state={detail?.mediaOrganization.state || stageState(item)} /></summary><div className="formation-stage-body"><ol>{mediaSteps.map((step)=><li key={step.key} className="formation-stub-result" data-state={step.state==='done'?'completed':step.state==='blocked'?'attention':'pending'}><span className="formation-stub-result-mark" aria-hidden="true">{step.state==='done'?'✓':step.state==='blocked'?'×':'○'}</span><div><strong>{step.label}</strong><p>{step.state==='done'?'已完成。':step.state==='blocked'?'执行或业务验收失败，无法继续。':step.state==='running'?'正在执行。':'尚未开始。'}</p>{step.progress?.mode==='determinate' && step.progress.totalValue ? <div className="formation-stub-progress"><progress max={step.progress.totalValue} value={step.progress.currentValue || 0} aria-label={`${step.label}进度`} /><small>{progressText(step.progress)}</small></div>:null}</div></li>)}</ol></div></details>
...
export default function FormationPage() {
  ...
  return <section className="source-page workbench formation-page"><PageHeader title="媒体整理工作区" description="一张表查看全部媒体；列表显示当前进展，完整上架过程在屏幕中央查看。" actions={<Button type="button" onClick={()=>void load()} disabled={loading}>{loading?'正在刷新…':'刷新'}</Button>} />
    ...
    <section className="formation-ledger"><div className="source-registry-heading"><div><h2>全部媒体</h2></div><span>当前显示 {rows.length} 条</span></div>
      ...
      <div className="formation-table-wrap"><table className="formation-table formation-stub-table"><thead><tr><th>媒体名称</th><th>评分</th><th>目标收藏架</th><th>整理要求</th><th>当前进展</th><th>详情</th><th>用户操作</th><th>加急</th></tr></thead><tbody>{rows.map((row)=>row.kind==='ended'?<tr key={row.item.historyId} className="formation-stub-media-row"><td><strong>{row.item.displayIdentity}</strong><small>已结束</small></td><td>—</td><td>—</td><td>—</td><td><strong className="formation-stub-current">{row.item.label}</strong><small>{formatTime(row.item.endedAtMs)}</small></td><td>—</td><td>—</td><td><button type="button" className="formation-expedite-toggle" disabled>加急</button></td></tr>:<MediaRow key={row.item.subjectId} item={row.item} shelves={shelves} loading={loading} acquisitionIncomplete={perceptionSync?.completionState==='incomplete'} onOpen={open} onAction={action} />)}</tbody></table></div>
    </section>
