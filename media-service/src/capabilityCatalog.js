...
const TYPES = Object.freeze({
  NONE: 'None', ASSET_SNAPSHOT: 'AssetSnapshot', SOURCE_OBSERVATION: 'SourceObservation', LAYOUT_OBSERVATION: 'LayoutObservation',
...
const CATALOG = Object.freeze({
  ...
  'emby.item.observe': def({ asset: port(TYPES.ASSET_SNAPSHOT) }, TYPES.SOURCE_OBSERVATION, 'pure', resource('emby')),
  'filesystem.media.probe': def({ asset: port(TYPES.ASSET_SNAPSHOT) }, TYPES.SOURCE_OBSERVATION, 'pure', resource('filesystem')),
  'filesystem.layout.observe': def({ asset: port(TYPES.ASSET_SNAPSHOT) }, TYPES.LAYOUT_OBSERVATION, 'pure', resource('filesystem')),
  ...
  'basedata.subject.publish': def({ assets: port(TYPES.BASEDATA_PUBLICATION, { many: true }) }, TYPES.BASEDATA_PUBLICATION, 'commit_once'),
  'media.identity.resolve': def({}, TYPES.MEDIA_IDENTITY),
  'series.identity.resolve': def({}, TYPES.MEDIA_IDENTITY),
  'metadata.provider.fetch': def({ identity: port(TYPES.MEDIA_IDENTITY) }, TYPES.METADATA_OBSERVATION, 'pure', resource('emby', 'scraper')),
  'series.metadata.provider.fetch': def({ identity: port(TYPES.MEDIA_IDENTITY) }, TYPES.METADATA_OBSERVATION, 'pure', resource('emby', 'scraper')),
  ...
  'series.metadata.sidecar.render': def({ metadata: port(TYPES.RESOLVED_METADATA) }, TYPES.METADATA_ARTIFACT, 'staged_write', resource('filesystem')),
  ...
  'series.metadata.publish': def({ metadata: port(TYPES.RESOLVED_METADATA), artifacts: port(TYPES.ARTIFACT_MANIFEST, { optional: true }) }, TYPES.METADATA_PUBLICATION, 'commit_once'),
  ...
  'series.upgrade.identity.resolve': def({ integration: port(TYPES.INTEGRATION_EVIDENCE) }, TYPES.UPGRADE_IDENTITY, 'pure', { ...resource('moviepilot'), parameters: { seasonKey: { type: 'string' } } }),
  'source.season-upgrade.search': def({ identity: port(TYPES.UPGRADE_IDENTITY) }, TYPES.UPGRADE_CANDIDATES, 'pure', resource('moviepilot')),
  'source.season-upgrade.output.resolve': def({ transfer: port(TYPES.TRANSFER_OBSERVATION) }, TYPES.STAGED_MEDIA_ASSET, 'pure', { ...resource('filesystem'), parameters: { seasonKey: { type: 'string' } } }),
  'series.season-package.verify': def({ stagedAsset: port(TYPES.STAGED_MEDIA_ASSET) }, TYPES.VERIFIED_MEDIA_ASSET, 'pure', { ...resource('filesystem'), parameters: { seasonKey: { type: 'string' } } }),
  ...
  'media.file.replace': def({ verifiedAsset: port(TYPES.VERIFIED_MEDIA_ASSET) }, TYPES.REPLACEMENT_EVIDENCE, 'commit_once', { ...resource('filesystem'), approvalActions: ['upgrade.beforeReplace', 'transcode.beforeReplace'] }),
  'series.season.replace': def({ verifiedSeasonPackage: port(TYPES.VERIFIED_MEDIA_ASSET) }, TYPES.SOURCE_MUTATION, 'commit_once', { ...resource('filesystem'), approvalActions: ['upgrade.beforeReplace'] }),
  ...
  'series.assets.layout.verify': def({ outcomes: port(TYPES.REPLACEMENT_EVIDENCE, { many: true }) }, TYPES.LAYOUT_VERIFICATION, 'pure', resource('filesystem')),
  ...
  'series.optimization.result.publish': def({ layout: port(TYPES.LAYOUT_VERIFICATION), replacements: port(TYPES.REPLACEMENT_EVIDENCE, { many: true }) }, TYPES.OPTIMIZE_PUBLICATION, 'commit_once'),
});
