...
function apply({ capability, task, event, output }) {
  ...
  if (type === 'MediaReplacementEvidence') {
    kairoxStore.markBasedataStale({ itemId: task.itemId, reason: 'post_optimize_replace', taskId: task.id });
  ...
  if (type === 'SourceMutationEffect') {
    ...
    kairoxStore.markBasedataStale({ itemId: task.itemId, reason: 'source_mutation_pending_rebind', taskId: task.id });
    kairoxSignalBus.publish({ kind: 'source_mutation', itemId: task.itemId, mutationId: mutation.mutationId });
