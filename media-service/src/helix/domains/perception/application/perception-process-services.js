...
function createPerceptionProcessServices(options){
  ...
  function createRecord(command){if(!command||!['subject','shelf_entry'].includes(command.targetType)||!Number.isSafeInteger(command.rating)||command.rating<1||command.rating>5||!Number.isSafeInteger(command.expectedRevision)||command.expectedRevision<0||typeof command.idempotencyKey!=='string'||!command.idempotencyKey)throw Object.assign(new Error('Rating command is invalid.'),{code:'PERCEPTION_RATING_COMMAND_INVALID'});
