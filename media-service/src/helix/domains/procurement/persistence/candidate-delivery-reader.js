...
function createCandidateDeliveryReader(options) {
  ...
  const repository = createRepositoryDefinition({ repositoryId:'candidate_delivery_reader', owner:'procurement',
    schemaManifest:options.schemaManifest, statements:{
      ...
      list_primary:{ kind:'select-all', tableId:'proc_candidate_primary_materials', columns:['ordinal','material_key','role','binding_revision',
      ...
      list_run_members:{ kind:'select-all', tableId:'proc_run_materials', columns:['ordinal','material_key','binding_revision','last_snapshot_digest',
