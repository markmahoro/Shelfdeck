...
function effectReceipt(context, receipt, result, effectClass, now) {
  ...
  const value = {
    ...
    effectId: receipt.effectId || canonicalDigest([effectClass, context.idempotencyKey]),
    effectClass,
    idempotencyKey: context.idempotencyKey,
...
function createMediaProductionCapabilityPorts(options) {
  ...
  const ports = {
    ...
    'libra.product_media.verify@1': {
      ...
      execute(context) {
        const result = buildProductMediaVerification({ input:context.namedInputs.productMediaCandidateInput, verifiedAtMs:now() });
