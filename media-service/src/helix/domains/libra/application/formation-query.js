...
function createFormationQuery(options){const repository=createRepositoryDefinition({repositoryId:'libra_formation_query',owner:'libra',schemaManifest:options.schemaManifest,statements:{
  ...
  list_runs:{kind:'select-all',tableId:'libra_runs',columns:['libra_run_id','subject_id','state','state_revision','state_digest','priority_class','priority_intent_digest','package_revision_head','created_at_ms','terminal_at_ms'],keyColumns:[],safeIntegers:true},
  list_packages:{kind:'select-all',tableId:'libra_product_packages',columns:['on_deck_package_id','offer_id','libra_run_id','package_revision','package_digest','state','published_at_ms'],keyColumns:[],safeIntegers:true}
}});
  function rows(){return options.unitOfWork.execute([{participantId:'libra_formation_read',owner:'libra',repositories:[repository],execute(context){const r=context.repository(repository.repositoryId);
    return {subjects:r.invoke('list_subjects',{}),decisions:r.invoke('list_decisions',{}),receipts:r.invoke('list_receipts',{}),bindings:r.invoke('list_bindings',{}),
      routingDecisions:r.invoke('list_routing_decisions',{}),routingPolicies:r.invoke('list_routing_policies',{}),decisionHeads:r.invoke('list_decision_heads',{}),
      acceptanceSpecs:r.invoke('list_acceptance_specs',{}),runs:r.invoke('list_runs',{}),packages:r.invoke('list_packages',{})};}}]).libra_formation_read;}
  function build(){const value=rows(),receipts=new Map(value.receipts.filter((item)=>item.outcome==='accepted').map((item)=>[item.intake_decision_id,item]));
    ...
    return Object.freeze(value.subjects.map((subject)=>{const decisions=(decisionsBySubject.get(subject.subject_id)||[]).sort((a,b)=>Number(a.decided_at_ms)-Number(b.decided_at_ms));
      ...
      const currentRun=runs.find((item)=>['active','suspended','frozen'].includes(item.state))||runs[0]||null;
      const productPackage=currentRun?value.packages.filter((item)=>item.libra_run_id===currentRun.libra_run_id)
        .sort((a,b)=>Number(b.package_revision)-Number(a.package_revision))[0]||null:null;
      const productionStage=!currentRun?(acceptanceSpec?'awaiting_libra_run':null):productPackage?'handoff_b_ready':
        currentRun.state==='frozen'?'frozen':currentRun.state==='suspended'?'suspended':'production';
      ...
      return Object.freeze({formationViewId:subject.subject_id,subjectId:subject.subject_id,displayIdentity:snapshot?.candidatePackage?.displayIdentity||subject.subject_id,
        ...
        acceptanceSpecDigest:acceptanceSpec?.spec_digest||null,acceptanceSpecPublishedAtMs:acceptanceSpec?Number(acceptanceSpec.published_at_ms):null,
        productionStage,currentRun:currentRun?Object.freeze({libraRunId:currentRun.libra_run_id,state:currentRun.state,
          stateRevision:Number(currentRun.state_revision),stateDigest:currentRun.state_digest,priorityClass:currentRun.priority_class,
          packageRevisionHead:Number(currentRun.package_revision_head)}):null,
        handoffB:productPackage?Object.freeze({onDeckPackageId:productPackage.on_deck_package_id,offerId:productPackage.offer_id,
          packageRevision:Number(productPackage.package_revision),packageDigest:productPackage.package_digest,state:productPackage.state}):null,
        intakeCount:Number(subject.intake_revision),primaryMaterialCount:bindings.filter((item)=>item.authority_kind==='primary_control').length,
