...
function materialRow(packageId, member) {
  ...
  return {
    ...
    digest_algorithm: workspace?.digestAlgorithm || member.physicalIdentity.contentHashAlgorithm,
    digest_hex: workspace?.digestHex || member.physicalIdentity.contentHash,
...
function writePackage(repo, commit, decision, snapshot) {
  ...
  decision.offloadContextManifest.members.forEach((item) => repo.invoke('insert_offload', {
    ...
    content_hash_algorithm: item.physicalIdentity.contentHashAlgorithm,
    content_hash: item.physicalIdentity.contentHash,
