...
const { CAPABILITY_REFS:C,
  ...
  aftercareSettlementEventId, aftercareServiceCatalogRevision, aftercareSettlementApprovalId,
  buildAftercareSettlementHandles, projectHealth, isAftercareArtifactMaterial } = require('../model/aftercare-contract');
...
function effectReceipt(context,effectClass,result,at){return Object.freeze({schemaRef:'helix://contracts/types/EffectReceipt/v1',schemaVersion:1,effectReceiptId:stable('arca-care-effect-receipt-',{eventId:context.eventId}),effectId:canonicalDigest([effectClass,context.idempotencyKey]),effectClass,idempotencyKey:context.idempotencyKey,commitMarker:stable('arca-care-effect-marker-',{eventId:context.eventId}),externalReceiptRef:null,outputDigest:canonicalDigest(result),verificationEvidenceDigest:canonicalDigest(result),committedAtMs:at});}
function committed(context,ref,result,at,effectClass){return Object.freeze({...outcome(ref,result,at),effectReceipt:effectReceipt(context,effectClass,result,at)});}
...
function materializeArtifactWithRollback({source,location,retiredLocation,oldMaterial,mountScopeId,fingerprint,revalidate,copy=atomicCopy,rename=fs.renameSync}){
  ...
  if(oldMaterial&&!currentIsOld&&!retiredIsOld)throw Object.assign(new Error('Aftercare Artifact current-old identity is absent.'),{code:'ARCA_AFTERCARE_TARGET_CONFLICT'});
...
async function rollbackMediaReplacement({source,location,supersededLocation,staged,oldMaterial,mountScopeId,fingerprint}){
  ...
  if(oldMaterial&&fs.existsSync(supersededLocation)&&!fs.existsSync(location)){
    ...
    await fs.promises.rename(supersededLocation,location);
  }
  if(fs.existsSync(staged)&&sameBoundedMedia(staged,source,mountScopeId,fingerprint))await fs.promises.rm(staged,{force:true});
}
async function materializeMediaWithRollback({source,location,supersededLocation,staged,oldMaterial,mountScopeId,fingerprint,revalidate,copy=atomicCopyMedia,rename=fs.promises.rename}){
  const currentIsSource=fs.existsSync(location)&&sameBoundedMedia(location,source,mountScopeId,fingerprint),
    currentIsOld=exactInventoryMaterial(location,oldMaterial,mountScopeId,fingerprint),
    supersededIsOld=exactInventoryMaterial(supersededLocation,oldMaterial,mountScopeId,fingerprint);
  if(fs.existsSync(location)&&!currentIsSource&&!currentIsOld)throw Object.assign(new Error('Aftercare Primary target changed outside this Case.'),{code:'ARCA_AFTERCARE_TARGET_CONFLICT'});
  ...
  try{
    if(!currentIsSource){
      ...
      revalidate();
      if(currentIsOld)await rename(location,supersededLocation);
      revalidate();
    ...
    try{await rollbackMediaReplacement({source,location,supersededLocation,staged,oldMaterial,mountScopeId,fingerprint});}catch(rollbackError){throw Object.assign(new Error('Aftercare Primary rollback failed.'),{code:'ARCA_AFTERCARE_ROLLBACK_FAILED',cause:error,rollbackError});}
    throw error;
  }
}
...
const AFTERCARE_ENCODERS=Object.freeze({nvidia_nvenc:'hevc_nvenc',intel_qsv:'hevc_qsv',amd_vaapi:'hevc_vaapi',software_cpu:'libx265'});
...
function createAftercareCapabilityPorts(options){const now=options.now||Date.now,contextReader=options.contextReader,store=contextReader.store,controls=createMaterialControlProjectionPort(options),fingerprint=options.computeBoundedMaterialFingerprintSync;
  ...
  ports[C.custody]=pure(C.custody,['careBasis','inventoryRevision','knownBindings'],(_n,c,at,execution)=>{
    ...
    for(const item of c.raw.materials){const location=item.location;
      try{const stat=fs.statSync(location,{bigint:true});if(!stat.isFile()){findings.push(finding('custody',isPrimaryMaterial(item)?'primary_not_regular':'artifact_not_regular','critical',!isPrimaryMaterial(item)&&isRepairablePresentationArtifact(item)?'auto_repair':'attention_required',item.material_key));continue;}
        ...
        if(identityChanged){findings.push(finding('custody',isPrimaryMaterial(item)?'primary_identity_changed':'material_identity_changed','critical','attention_required',item.material_key));continue;}
  ...
  ports[C.binaryAcquire]=Object.freeze({
    ...
    async execute(execution){
      ...
      if(n.integrationHandle.allowedOperation!==C.binaryAcquire||n.integrationHandle.artifactKind!=='poster'||
        ...
        throw Object.assign(new Error('Aftercare Provider input does not match the current Case identity and operation.'),{code:'ARCA_AFTERCARE_PROVIDER_INPUT_STALE'});
  ...
  async function mediaEffect(execution,kind){const c=context(execution),care=activeCase(c),at=now(),n=execution.namedInputs,primary=c.raw.materials.find(isPrimaryMaterial);if(!primary)throw new Error('Aftercare current Primary Binding is absent.');const source=primary.location;fingerprint(source);const strategy=kind==='transcode'?n.aftercareMediaRepairStrategy:null,sourceProbe=await options.mediaProbe.probe(primaryReadHandle(c,primary)),durationUs=Math.max(0,Math.floor(Number(sourceProbe?.durationMs||0)*1000)),shouldContinue=()=>{const current=contextReader.read(c.shelfEntryId),active=current&&store.history(c.shelfEntryId).cases.find((item)=>item.state==='active');return Boolean(current&&active?.aftercareCaseId===care.aftercareCaseId&&active.careBasisDigest===current.basis.digest&&current.raw.shelf.status==='active'&&!current.raw.reservations.some((item)=>item.state==='active'));},progress=(offsetUs,totalUs)=>Object.freeze({durationUs,offsetUs,totalUs,deadlineAtMs:execution.deadlineAtMs,reportProgress:execution.reportProgress,shouldContinue});
    ...
    const root=workspaceRoot(options,store,care.aftercareCaseId),relative='media/'+kind+(strategy?'-'+strategy.strategyId.slice(-16):'')+'.mkv',target=path.join(root,relative);fs.mkdirSync(path.dirname(target),{recursive:true});if(!fs.existsSync(target)){const temporary=target+'.tmp',binary=resolveAftercareFfmpegPath(options.ffmpegPath);if(kind==='remux'){await runFfmpeg(binary,['-hide_banner','-nostdin','-y','-fflags','+genpts','-i',source,'-map','0','-c','copy','-f','matroska',temporary],progress(0,durationUs));revalidateCaseAuthority(execution,care);}else {const encoder=AFTERCARE_ENCODERS[strategy.selectedDeviceClass];if(!encoder)throw new Error('Aftercare selected media Device has no local encoder.');const video=strategy.video,encode=video.rateControlMode==='quality_bound'?(strategy.selectedDeviceClass==='software_cpu'?['-c:v',encoder,'-crf',String(video.qualityBound)]:['-c:v',encoder,'-cq',String(video.qualityBound)]):['-c:v',encoder,'-b:v',String(video.targetVideoBitrateBps),...(video.rateControlMode==='strict_abr'?['-maxrate',String(video.targetVideoBitrateBps),'-bufsize',String(video.targetVideoBitrateBps*2)]:[])],profile=video.dynamicRangeOperation==='tone_map_to_sdr_bt709'?['-vf','setparams=color_primaries=bt2020:color_trc=smpte2084:colorspace=bt2020nc:range=limited,zscale=t=linear:npl=100,format=gbrpf32le,zscale=p=bt709,tonemap=hable:desat=0,zscale=t=bt709:m=bt709:r=tv,format=yuv420p','-pix_fmt','yuv420p','-color_range','tv','-color_primaries','bt709','-color_trc','bt709','-colorspace','bt709','-map_metadata','-1']:[];
        if(video.rateControlMode==='two_pass_abr'){const passlog=temporary+'.passlog',nullTarget=process.platform==='win32'?'NUL':'/dev/null';try{await runFfmpeg(binary,['-hide_banner','-nostdin','-y','-i',source,'-map','0:v:0',...profile,...encode,'-pass','1','-passlogfile',passlog,'-an','-sn','-f','null',nullTarget],progress(0,durationUs*2));revalidateCaseAuthority(execution,care);await runFfmpeg(binary,['-hide_banner','-nostdin','-y','-i',source,'-map','0:v:0','-map','0:a?','-map','0:s?',...profile,...encode,'-pass','2','-passlogfile',passlog,'-c:a','copy','-c:s','copy','-f','matroska',temporary],progress(durationUs,durationUs*2));revalidateCaseAuthority(execution,care);}finally{for(const suffix of ['', '.log', '.log.mbtree', '-0.log', '-0.log.mbtree']){const item=passlog+suffix;if(fs.existsSync(item))fs.rmSync(item,{force:true});}}}else {await runFfmpeg(binary,['-hide_banner','-nostdin','-y','-i',source,'-map','0:v:0','-map','0:a?','-map','0:s?',...profile,...encode,'-c:a','copy','-c:s','copy','-f','matroska',temporary],progress(0,durationUs));revalidateCaseAuthority(execution,care);}}fs.renameSync(temporary,target);}
    revalidateCaseAuthority(execution,care);
  ...
  ports[C.inventoryCommit]=Object.freeze({validateInputs(c){requireNamed(c,['verifiedCareInventoryChange','responsibilityControlCommitHandle']);},execute(execution){const c=context(execution),n=execution.namedInputs,caseId=n.verifiedCareInventoryChange.aftercareCaseId,history=store.history(c.shelfEntryId),prior=history.commits.find((item)=>item.aftercareCaseId===caseId),care=prior?store.getCase(caseId):activeCase(c);if(!care||care.state!=='active'||caseId!==care.aftercareCaseId)throw new Error('Aftercare Inventory change belongs to another or terminal Case.');const sourceWorkId=preparationWorkId(execution),frozenMaterials=contextReader.inventoryMaterials(c.shelfEntryId,care.careBasis.inventoryRevision),receipts=[...options.workResultReader.read(execution.workId).filter((item)=>item.outcomeKind==='succeeded'&&item.capabilityRef===C.artifactMaterialize).map((item)=>item.result),...placementMaterialReceipts(c,options.workResultReader,sourceWorkId,care,frozenMaterials)];if(!receipts.length)throw new Error('Aftercare Inventory commit lacks durable Material Effect receipts.');let record;
    if(prior){if(prior.previousInventoryRevision!==Number(care.careBasis.inventoryRevision)||prior.newInventoryRevision!==Number(c.raw.entry.current_inventory_revision)||prior.controlChangeDigest!==n.responsibilityControlCommitHandle.controlScopeDigest)throw Object.assign(new Error('Aftercare committed Inventory does not match this recovery Event.'),{code:'ARCA_AFTERCARE_INVENTORY_REPLAY_CONFLICT'});postInventoryCaseAuthority(c,care,prior.newInventoryRevision);record=Object.freeze({inventoryCommitId:prior.inventoryCommitId,previousInventoryRevision:prior.previousInventoryRevision,newInventoryRevision:prior.newInventoryRevision,controlChangeDigest:prior.controlChangeDigest,committedAtMs:prior.committedAtMs});}
      ...
      const controlParticipant=createMaterialControlParticipant({schemaManifest:options.schemaManifest,participantId:'arca_aftercare_inventory_control',handle:n.responsibilityControlCommitHandle,changes,authorizedScopeDigest:n.responsibilityControlCommitHandle.controlScopeDigest,commitMarker:stable('arca-care-inventory-control-marker-',{caseId:care.aftercareCaseId})}),nextRevision=Number(c.raw.entry.current_inventory_revision)+1,representationDigest=canonicalDigest({schema:'arca.inventory-representation@1',shelfEntryId:c.shelfEntryId,inventoryRevision:nextRevision,sourcePackageId:c.raw.inventory.source_package_id,members:materials.map(representationMember)}),written=store.commitInventory({aftercareCaseId:care.aftercareCaseId,shelfEntryId:c.shelfEntryId,previousInventoryRevision:c.raw.entry.current_inventory_revision,representationDigest,controlChangeDigest:n.responsibilityControlCommitHandle.controlScopeDigest,materials,related:c.raw.related,facts:c.raw.facts,people:c.raw.people},controlParticipant);record=Object.freeze({inventoryCommitId:written.inventory_commit_id,previousInventoryRevision:written.previousInventoryRevision,newInventoryRevision:written.newInventoryRevision,controlChangeDigest:written.control_change_digest,committedAtMs:written.committed_at_ms});}
    const postCommitContext=postInventoryCaseAuthority(contextReader.read(c.shelfEntryId),care,record.newInventoryRevision),settlementEventId=aftercareSettlementEventId(execution.workAttemptId),
      hasSettlementEvent=options.workResultReader.readBindings(execution.workId).some((item)=>item.capabilityRef===C.settlement),
      settlementHandles=buildAftercareSettlementHandles({context:postCommitContext,aftercareCaseId:care.aftercareCaseId,receipts,
        frozenMaterials,observedOldBindings:observeKnownOldBindings(postCommitContext.raw,fingerprint),observedAtMs:record.committedAtMs});
    if(hasSettlementEvent)store.issueSettlementApproval({aftercareCaseId:care.aftercareCaseId,
      settlementScopeDigest:settlementScopeDigest(settlementHandles),serviceCatalogRevision:aftercareServiceCatalogRevision(options.registry),
      shelfStandardRevision:care.careBasis.standardRevision,careBasisDigest:care.careBasisDigest,settlementEventId});
    const at=effectOccurredAt(execution,record.committedAtMs),base={schemaRef:'helix://contracts/types/AftercareInventoryCommitReceipt/v1',schemaVersion:1,receiptId:record.inventoryCommitId,receiptKind:'aftercare_inventory_committed',ownerDomain:'arca',scopeType:'shelf_entry',scopeId:c.shelfEntryId,scopeDigest:care.careBasisDigest,effectReceiptRef:stable('arca-care-effect-receipt-',{eventId:execution.eventId}),committedAtMs:record.committedAtMs,aftercareCaseId:care.aftercareCaseId,shelfEntryId:c.shelfEntryId,previousInventoryRevision:record.previousInventoryRevision,newInventoryRevision:record.newInventoryRevision,controlChangeDigest:record.controlChangeDigest};const result=Object.freeze(base);return committed(execution,C.inventoryCommit,result,at,'responsibility_control_commit');},validateResult(_c,o){if(!o?.result?.newInventoryRevision)throw new TypeError('Aftercare Inventory Commit Receipt is invalid.');}});
  ports[C.settlement]=Object.freeze({
    ...
    async execute(execution){
      ...
      const managedLocations=[...handles.map((item)=>path.resolve(item.location)),
      ...
      const settlementDirectories=[...new Set(handles.map((item)=>path.dirname(path.resolve(item.location))))]
      ...
      for(const handle of handles){
        ...
        if(handle.finalLocation){
          const finalObserved=physicalIdentity(handle.finalLocation,c.raw.shelf.target_mount_scope_id,fingerprint);
          if(finalObserved.materialKey!==handle.finalMaterialKey||finalObserved.sizeBytes!==handle.identity.sizeBytes||finalObserved.contentFingerprint!==handle.identity.contentFingerprint)
      ...
      if(!consumedReplay){
        for(const handle of handles){settlementAuthority(c.shelfEntryId,care.aftercareCaseId,durableApproval.approvalId,scope);if(fs.existsSync(handle.location))await fs.promises.unlink(handle.location);if(fs.existsSync(handle.location))throw new Error('Aftercare superseded Material deletion was not durable.');await new Promise((resolve)=>setImmediate(resolve));}
      ...
      const entries=handles.length?handles.flatMap((handle)=>[{key:'exists:'+handle.identity.materialKey,value:false},{key:'location:'+handle.identity.materialKey,value:handle.location},{key:'final_verified:'+handle.identity.materialKey,value:handle.finalLocation?true:null}]):[{key:'settlement_count',value:0}],postBase={schemaRef:'helix://contracts/records/post-delete-reality/v1',schemaVersion:1,recordKind:'post-delete-reality',entries:Object.freeze([...entries,{key:'settlement_directory_set_digest',value:canonicalDigest(settlementDirectories)},{key:'approval_state',value:consumed.state}])},postDeleteReality=Object.freeze({...postBase,recordDigest:canonicalDigest(postBase)}),materialSetDigest=canonicalDigest(handles.map((item)=>({materialKey:item.identity.materialKey,location:item.location})).sort((a,b)=>(a.location+a.materialKey).localeCompare(b.location+b.materialKey))),base={schemaRef:'helix://contracts/types/SettlementDeletionEvidence/v1',schemaVersion:1,evidenceId:stable('arca-care-settlement-evidence-',{caseId:care.aftercareCaseId,key:materialSetDigest}),evidenceKind:'aftercare_settlement',producerRef:C.settlement,basisDigest:care.careBasisDigest,observedAtMs:at,authorizationOrApprovalRef:consumed.approvalId,materialKey:materialSetDigest,preDeleteIdentityDigest:canonicalDigest(handles.map((item)=>({identity:item.identity,location:item.location})).sort((a,b)=>(a.location+a.identity.materialKey).localeCompare(b.location+b.identity.materialKey))),postDeleteReality,effectReceiptId:stable('arca-care-effect-receipt-',{eventId:execution.eventId})};const result=Object.freeze({...base,payloadDigest:canonicalDigest(base)});return committed(execution,C.settlement,result,at,'destructive_commit');
    },
...
module.exports=Object.freeze({AFTERCARE_LONG_MEDIA_TIMEOUT_MS,createAftercareCapabilityPorts,decodeFinding,validNfo,validImageBytes,progressSample,remainingDeadlineMs,aftercareMediaProbeEvidence,resolveAftercareFfmpegPath,materializeArtifactWithRollback,materializeMediaWithRollback});
