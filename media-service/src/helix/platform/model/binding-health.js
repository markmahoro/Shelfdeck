...
function evaluateBindingHealth(input) {
  ...
  const requiredObservation = ['contentHash', 'endpointId', 'endpointReachable', 'inode', 'location',
    'locationState', 'mountScopeId', 'mountScopeRevision', 'observationEvidenceDigest'];
  ...
  if (!input.expected.identity || !/^[0-9a-f]{64}$/.test(input.expected.identity.materialKey || '') ||
      !/^[0-9a-f]{64}$/.test(input.expected.identity.contentHash || '') ||
  ...
  if (observation.endpointReachable !== true) {
    ...
    if (observation.locationState === 'resolved') {
      ...
      if (observation.mountScopeId !== identity.mountScopeId || observation.mountScopeRevision !== input.expected.mountScopeRevision ||
          observation.inode !== identity.inode) reasons.push('filesystem_object_mismatch');
      if (observation.contentHash !== identity.contentHash) reasons.push('content_hash_mismatch');
    }
  }
