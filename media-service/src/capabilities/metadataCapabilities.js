...
function registerMetadataCapabilities(register) {
  register({ capability: 'media.identity.resolve', allowedTargetGates: ['metadata'], execute: async ({ task, config }) => {
    ...
    const sourcePath = descriptor.locator && descriptor.locator.path || task.itemInfo && task.itemInfo.path || '';
    ...
    return { result: { descriptor, sourcePath, adultId, tmdbId: task.itemInfo && task.itemInfo.tmdbId || '', providerIds: task.itemInfo && task.itemInfo.providerIds || {} } };
  } });
  register({ capability: 'metadata.provider.fetch', allowedTargetGates: ['metadata'], execute: async ({ task, config, input }) => {
    const descriptor = task.helixAdmission && task.helixAdmission.sourceAccessDescriptor || {};
    const subLibrary = (config.subLibraries || []).find((entry) => entry.uuid === (descriptor.subLibraryId || task.itemInfo && task.itemInfo.subLibraryId || ''));
  ...
  register({ capability: 'metadata.image.acquire', allowedTargetGates: ['metadata'], execute: async (context) => {
    ...
    return { result: { artifact: artifacts.writeArtifact(context.config, { itemId: context.task.itemId, metadataRevision: metadataRevision(context), name: `${kind}.jpg`, content, source: embedded ? 'western_analysis' : facts[field], eventId: context.event.eventId }) } };
  } });
...
module.exports = { registerMetadataCapabilities, nfoFor };
