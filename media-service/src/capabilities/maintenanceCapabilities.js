...
function registerMaintenanceCapabilities(register) {
  ...
  register({ capability: 'output.media.verify', allowedTargetGates: ['optimize'], execute: async (context) => {
    ...
    return { result: { stagedAsset: staged, valid: true, objectiveSatisfied: ['within', 'no_profile'].includes(bitrateComparison.status), bitrateComparison, outputPath, sizeBytes, bitrateKbps, summary, sourcePath: staged.sourcePath, originalSizeBytes: staged.originalSizeBytes, workDir: staged.workDir } };
  } });
  ...
  register({ capability: 'optimization.result.publish', allowedTargetGates: ['optimize'], execute: async ({ task, event, input }) => {
