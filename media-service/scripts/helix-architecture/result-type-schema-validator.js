...
function validateResultTypeSchemas(options) {
  ...
  if (sourceEntries.length !== 99 || nominalOutputs.size !== 87 || directOutputs.size !== 11 || outcomeOutputs.size !== 1) {
    findings.push(finding('RESULT_SOURCE_COUNT_DRIFT', 'SSOT result source map must remain 87 nominal + 11 direct + 1 Outcome.'));
  }
  ...
  const knownSchemaIds = new Set([
    ...
    ...applicationRegistries.flatMap((applicationRegistry) =>
      (applicationRegistry && applicationRegistry.entries || [])
  ...
  for (const helper of ['CandidatePackage', 'OnDeckProductPackage', 'OnDeckCommitReceipt', 'OffloadCompletionFact', 'PeopleCandidateDraft', 'PrimaryInputManifest',
