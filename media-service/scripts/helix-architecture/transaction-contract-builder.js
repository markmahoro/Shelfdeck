...
const crashFixtures = Object.freeze({
  ...
  'procurement-run-admission': ['Execution Basis验证前后、active Triage Rule解析前后、逐成员Selection/Control fence重验、Run/Basis rows写入、Control acquire/assert、Command Receipt/marker前后、响应前崩溃', '完整Basis、Run、全部run_selection、同Field Procurement Control、ProcurementControlReceipt与Command Receipt全有或全无；Rule只能来自注入的Procurement Registry；任一成员stale或冲突整体rollback；不建立Event且不写Outbox', 7621],
  ...
  'procurement-run-seal': ['Seal Decision验证前后、Run revision/basis CAS、逐成员terminal Evidence形成、Reservation转换、aggregate Evidence/typed Command Receipt/marker前后、响应前崩溃', 'Run sealed head、保留的candidate_delivery、全部released terminal members及逐成员Evidence、三项可重建digest、typed Command Receipt和marker全有或全无；不建立伪Event Result、不释放Procurement Control且不写Outbox', 7623],
...
const definitions = Object.freeze({
  ...
  'Procurement Run Admission': {
    ...
    eventResultRequired: false,
    writeTables: ['proc_procurement_runs', 'proc_run_materials', 'fx_material_controls',
      'fx_material_control_revisions', 'fx_command_receipts', 'fx_commit_markers'],
    readTables: ['proc_material_fields', 'proc_field_profile_hint_revisions', 'proc_field_access_revisions', 'proc_field_observations',
  ...
  'Procurement Run Seal': {
    commitClass: 'domain_fact_commit',
    writeTables: ['proc_procurement_runs', 'proc_run_materials', 'fx_command_receipts', 'fx_commit_markers'],
    readTables: ['proc_procurement_runs', 'proc_run_materials', 'proc_candidate_packages'],
    fixtureRefs: ['procurement-run-seal'], hasOutbox: false, eventResultRequired: false
  },
...
function buildTransactionContracts(entries) {
  return entries.map((entry) => {
    ...
    if (definition.commitClass === 'responsibility_control_commit' && definition.eventResultRequired !== false &&
        !writeTables.includes('fx_event_result_bindings')) {
