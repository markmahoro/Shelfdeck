...
const special = {
  ...
  'FinalInventoryDecision.members': arrayOf(object({
    objectId: id(),
    revision: positiveInteger(),
    schemaRef: { const: 'helix://contracts/domain-types/FinalInventoryMember/v1' },
    digest: digest(),
    objectKind: { const: 'final-inventory-member' },
    ...
    finalName: text({ maxLength: 255, pattern: '^[^\\\\/]+$' }),
    targetEndpointId: id(),
    targetLocation: text({ maxLength: 4096 }),
  }), 4096),
...
function libraDeliverablePromotionDecisionSchema() {
  ...
  const properties = {
    ...
    productionAttestation: object({ attestationId: id(), libraRunId: id(), onDeckPackageId: id(), acceptanceSpecId: id(),
      ...
      unmetRequirementCodes:arrayOf(enumText('metadata_field_unmet','video_codec_unmet','container_unmet',
        'file_extension_unmet','minimum_raster_unmet','system_upscale_forbidden','primary_audio_unmet','max_size_exceeded',
        'dynamic_range_conversion_unmet','output_color_profile_unmet','dolby_vision_metadata_not_removed'),12),
...
function oldPrimaryStructuralExclusiveRelatedHandlesSchema() {
  const member = object({
    ...
    finalMaterialKey: digest(),
    finalTargetLocation: text({ maxLength: 4096 }),
    settlementExpectation: enumText('replace_or_move', 'remove_after_place'),
