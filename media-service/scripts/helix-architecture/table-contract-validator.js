...
function readInventoryEntries(contractsRoot, findings) {
  ...
  if (!manifest || manifest.status !== 'active' || manifest.targetCount !== 184 || !Array.isArray(manifest.entryFiles)) {
    findings.push(finding('INVALID_TABLE_INVENTORY_MANIFEST', 'Table inventory must be active with 184 sharded entries.'));
    return [];
  }
...
function validateTableContracts(options) {
  ...
  for (const entry of inventoryEntries) {
    ...
    for (const json of contract.jsonContracts || []) {
      ...
      if (![4 * 1024, 16 * 1024, 64 * 1024, 128 * 1024, 256 * 1024, 1024 * 1024].includes(json.maxBytes) || json.requiresJsonValidCheck !== true) findings.push(finding(
