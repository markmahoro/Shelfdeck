...
function validateTransactionContracts(options) {
  ...
  if (!manifest || manifest.status !== 'active' || manifest.targetCount !== 18 || manifest.entryFiles.length !== 1) findings.push(finding(
    'INVALID_TRANSACTION_INVENTORY_MANIFEST', 'Canonical transaction inventory must be active with 18 entries.'
  ));
