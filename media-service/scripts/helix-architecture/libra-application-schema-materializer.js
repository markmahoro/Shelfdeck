...
const { buildLibraApplicationSchemas, schemaDigest, typeId } = require('./libra-application-schema-builder');

function materializeLibraApplicationSchemas(contractsRoot) {
  ...
  const registry = {
    schemaVersion: 1,
    manifestVersion: 1,
    manifestId: 'helix.libra-application-type-registry',
  ...
  fs.writeFileSync(path.join(contractsRoot, 'libra-application-type-registry.json'), `${JSON.stringify(registry, null, 2)}\n`);
  return registry;
}

module.exports = Object.freeze({ materializeLibraApplicationSchemas });
