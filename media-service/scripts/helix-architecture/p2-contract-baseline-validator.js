...
function validateP2ContractBaseline(options) {
  ...
  const exactCounts = sourceMap.counts && sourceMap.counts.capabilities === 113 && sourceMap.counts.resultFamilies === 99 &&
    sourceMap.counts.tables === 184 && sourceMap.counts.transactions === 44 && capabilities.packageCount === 113 &&
    resultTypes.catalogResultCount + 1 === 99 && tables.tableCount === 184 && transactions.transactionCount === 44;
  if (!exactCounts) findings.push({ code: 'P2_CARDINALITY_MISMATCH', message: 'P2 baseline must close 113/99/184/44 exactly.' });
