...
function validateP2ContractBaseline(options) {
  ...
  const exactCounts = sourceMap.counts && sourceMap.counts.capabilities === 112 && sourceMap.counts.resultFamilies === 96 &&
    sourceMap.counts.tables === 161 && sourceMap.counts.transactions === 26 && capabilities.packageCount === 112 &&
    resultTypes.catalogResultCount + 1 === 96 && tables.tableCount === 161 && transactions.transactionCount === 26;
  if (!exactCounts) findings.push({ code: 'P2_CARDINALITY_MISMATCH', message: 'P2 baseline must close 112/96/161/26 exactly.' });
