...
function validateP2ContractBaseline(options) {
  ...
  const exactCounts = sourceMap.counts && sourceMap.counts.capabilities === 112 && sourceMap.counts.resultFamilies === 98 &&
    sourceMap.counts.tables === 180 && sourceMap.counts.transactions === 43 && capabilities.packageCount === 112 &&
    resultTypes.catalogResultCount + 1 === 98 && tables.tableCount === 180 && transactions.transactionCount === 43;
  if (!exactCounts) findings.push({ code: 'P2_CARDINALITY_MISMATCH', message: 'P2 baseline must close 112/98/180/43 exactly.' });
