...
function validateP2ContractBaseline(options) {
  ...
  const exactCounts = sourceMap.counts && sourceMap.counts.capabilities === 112 && sourceMap.counts.resultFamilies === 97 &&
    sourceMap.counts.tables === 178 && sourceMap.counts.transactions === 43 && capabilities.packageCount === 112 &&
    resultTypes.catalogResultCount + 1 === 97 && tables.tableCount === 178 && transactions.transactionCount === 43;
  if (!exactCounts) findings.push({ code: 'P2_CARDINALITY_MISMATCH', message: 'P2 baseline must close 112/97/178/43 exactly.' });
