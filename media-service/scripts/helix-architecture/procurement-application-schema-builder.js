...
function candidateAssemblyPlanBinding(){
  ...
  const physicalIdentity=object({schemaRef:{const:'helix://contracts/types/PhysicalMaterialIdentity/v2'},schemaVersion:{const:2},
    materialKey:digest(),mountScopeId:id(),inode:id(),sizeBytes:nonNegative(),fingerprintAlgorithm:{const:'middle-256k-sha256'},
    fingerprintVersion:{const:1},contentFingerprint:digest()});
  const readHandle=object({identity:physicalIdentity,bindingRevision:positive(),location:text()});
  const layoutReadHandle=object({...readHandle.properties,endpointId:id()});
  const typedParameter=object({parameter:{type:'string',pattern:'^[a-z][a-zA-Z0-9_.-]{0,127}$'},
    valueType:{type:'string',enum:['string','integer','number','boolean']},
    value:{type:['string','integer','number','boolean']},valueDigest:digest()});
  const boundedLayoutScope=object({schemaRef:{const:'helix://contracts/domain-types/BoundedLayoutScope/v1'},schemaVersion:{const:1},
    scopeId:id(),revision:positive(),digest:digest(),rootHandleDigest:digest(),maxDepth:positive(),
    maxMembers:{type:'integer',minimum:1,maximum:256},typedParameters:{type:'array',maxItems:256,items:typedParameter}});
