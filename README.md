...
一站式媒体库管理工具——自动盘点、智能推荐、一键优化，把你的观影记录和评分变成清晰的操作指南。

**核心能力**：拉取媒体库数据 → 分析码率/分辨率/编码质量 → 结合豆瓣评分或你的个人打分 → 为每条影片计算策略：保留、删除、压缩或洗版 → 一键入队执行。

**当前版本**：v1.0.0 仅支持 **Emby/Jellyfin** 媒体库。后续版本将逐步添加本地文件夹扫描、TMDB 直接接入等多来源支持，最终实现"一个工具管理所有媒体"。

## 功能概览

- **媒体库盘点**：同步 Emby 媒体库数据，自动分析码率/分辨率/编码格式
- **豆瓣评分**：抓取豆瓣"看过"评分，辅助决策
- **策略推荐**：可配置规则引擎，根据评分+码率条件自动推荐删除/转码/洗版
- **任务中心**：一键执行删除、转码（FFmpeg 压缩）、洗版（MoviePilot 搜索优质版本）
- **桌面客户端**：未观看内容海报浏览 + 媒体库管理 + 播放记录
- **多来源规划**：v1.0 支持 Emby，后续将接入本地文件夹、TMDB 等数据源
