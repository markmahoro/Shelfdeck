...
- `**media-tray-supervisor/`**（Windows）：ShelfDeck 小助手——与桌面客户端**同源** `effectiveBaseUrl` 健康（黄/绿/红）、左键面板展示当前地址；**已配置**后才提供统一启停入口（本机 `spawn` 与远端仅运维差异，见 [docs/design/DESIGN_TRAY_MEDIA_SERVICE_SUPERVISOR.md](docs/design/DESIGN_TRAY_MEDIA_SERVICE_SUPERVISOR.md) / [docs/operations/OPS_TRAY_MEDIA_SERVICE_SUPERVISOR.md](docs/operations/OPS_TRAY_MEDIA_SERVICE_SUPERVISOR.md)）
