...
- **战略原则**（本节）：配置权威在媒体管理服务、前后端分工、同源规则。
- **产品约定**（实施细节、API 调用方式、UI 展示要求）：见 `[API_README.md](../api/API_README.md)` **§路径映射与配置**。
...
### 4.4 MCP 与 REST 同源调用的架构约束

**原则**：MCP 工具应 **调用与媒体管理服务 REST 相同的领域服务**，禁止维护两套业务规则。

**架构约束**：

- **共享领域服务层**：MCP 工具与 REST 控制器均调用同一套领域服务（如 `LibraryService`、`RevisitQueueService`、`TaskService`），业务逻辑 **只在领域服务层实现一次**。
- **禁止重复实现**：MCP 工具 **不得** 直接操作数据库或实现独立业务逻辑；所有业务规则必须通过领域服务暴露。
- **验证策略**：
  - **代码审查检查清单**：PR 审查时确认 MCP 工具调用的是领域服务，而非重复实现。
  - **集成测试**：为关键业务场景编写集成测试，同时覆盖 MCP 和 REST 路径，确保行为一致。
  - **架构测试**（可选）：使用架构测试工具（如 ArchUnit）强制 MCP 工具层不直接依赖数据访问层。

**实施指导**：具体验证机制由实现团队在开发阶段落实；本节提供架构原则与推荐实践。
