...
| 配置字段参考（SSOT）                 | `docs/design/DESIGN_CONFIG_FIELDS_REFERENCE.md`                                                                                                                                                                                                                                                                                                             | **所有配置字段的单一事实来源**：字段名、类型、必填、默认值、验证规则                                                                                                                               |
| 配置与路径                         | `docs/design/DESIGN_CONFIG_AND_PATHS.md`                                                                                                                                                                                                                                                                                                                    | 配置中心职责边界、路径映射约定、配置归属原则（字段定义见 `DESIGN_CONFIG_FIELDS_REFERENCE`）                                                                                                       |
...
| ShelfDeck 小助手 / 托盘监督（Windows） | `docs/requirements/REQ_FEATURE_windows-tray-media-service-supervisor.md` · `docs/architecture/ARCH_TRAY_MEDIA_SERVICE_SUPERVISOR.md` · `docs/design/DESIGN_TRAY_MEDIA_SERVICE_SUPERVISOR.md` · `docs/user/USER_GUIDE_TRAY_MEDIA_SERVICE.md` · `docs/operations/OPS_TRAY_MEDIA_SERVICE_SUPERVISOR.md` · `docs/testing/TEST_TRAY_MEDIA_SERVICE_SUPERVISOR.md` | **小助手**：**独占写入** 连接文件；与 Desktop **同源读** `effectiveBaseUrl` 健康（黄/绿/红、含 **启动中** 态）；左键面板（当前地址 + **打开主界面** + **队列摘要** + **开机自启** / **退出停本机** 设置）、已配置后才启停（UI 不分本地/远端）；本机 **`spawn`**；退出默认不杀后端；`media-tray-supervisor/`                          |
| 桌面后端连接与 Windows 生命周期          | `docs/requirements/REQ_FEATURE_desktop-backend-connection-and-windows-lifecycle.md` · `docs/design/DESIGN_DESKTOP_BACKEND_ENDPOINT.md` · `docs/architecture/adr/ADR_001_windows-single-local-media-service-instance.md`                                                                                                                                     | **小助手** **独占写入** 连接文件；Desktop **只读** **同源** `effectiveBaseUrl`、顶栏小状态与强门禁；Windows 本机单实例 ADR；波及 `DESIGN_DESKTOP_MEDIA_SERVICE_AVAILABILITY` 等                                                                       |
...
| 配置中心保存/检验反馈（呈现）               | `docs/requirements/REQ_FEATURE_config-center-save-feedback.md` · `docs/design/DESIGN_CONFIG_CENTER_SAVE_FEEDBACK.md`                                                                                                                                                                                                                                        | 配置中心各分区保存与检验的反馈通道、状态与 UI 槽位；与全局错误横幅分流；文案句式仍服从 `DESIGN_DESKTOP_UI_COPY`                                                                                             |
| 桌面依赖媒体服务（门禁）                  | `docs/requirements/REQ_FEATURE_desktop-requires-media-service.md` · `docs/design/DESIGN_DESKTOP_MEDIA_SERVICE_AVAILABILITY.md`                                                                                                                                                                                                                              | 桌面壳层以 `GET /v1/health` 为可达判据；基址 **仅小助手** 写入、桌面 **只读**（`DESIGN_DESKTOP_BACKEND_ENDPOINT`）；不可达时强门禁并引导小助手；开发变量见 `DEV_SETUP`                                                                   |
...
| 迭代总结（托盘监督·历史）                 | `docs/project/PRJ_ITERATION_SUMMARY_tray_supervisor_20260420.md`                                                                                                                                                                                                                                                                                            | 2026-04-20 **本机 spawn + 托盘 UI** 整体验收摘要（T1–T6）；**小助手产品模型**以 `PRJ_ITERATION_SUMMARY_desktop_backend_connection_20260420` 与 `DESIGN_TRAY_MEDIA_SERVICE_SUPERVISOR` 为准 |
| 迭代总结（桌面中文文案）                  | `docs/project/PRJ_ITERATION_SUMMARY_desktop_ui_copy_20260420.md`                                                                                                                                                                                                                                                                                            | 2026-04-20 桌面客户端用户可见中文文案工程验收摘要（指向 REQ/DESIGN）                                                                                                                      |
| 迭代总结（配置保存与媒体服务门禁）             | `docs/project/PRJ_ITERATION_SUMMARY_config_center_media_service_gate_20260420.md`                                                                                                                                                                                                                                                                           | 2026-04-20 配置中心保存反馈与壳层媒体服务可达门禁整体验收摘要（指向 REQ/DESIGN）                                                                                                                |
| 迭代总结（桌面后端连接与小助手文档）            | `docs/project/PRJ_ITERATION_SUMMARY_desktop_backend_connection_20260420.md`                                                                                                                                                                                                                                                                                 | 2026-04-20 起修订：**小助手** **独写** 连接、桌面 **只读** + 顶栏灯 + 门禁引导；单实例 ADR；黄绿红、左键面板、启停门槛、退出不杀后端；工程待验收                                                                                              |
...
| 托盘监督（Windows）      | **ShelfDeck 小助手**（托盘；与桌面同源 `effectiveBaseUrl`） | `media-tray-supervisor` | `**shelfdeck-media-tray-supervisor**` |
...
3. **配置字段定义**（字段名、类型、默认值、验证规则）：以 `DESIGN_CONFIG_FIELDS_REFERENCE.md` 为准。
4. **HTTP 路径、模型、错误码**：以 `openapi.yaml` 为准。
5. 若 OpenAPI 与 REQ/DESIGN 冲突：**先改文档至一致，再改代码**。
