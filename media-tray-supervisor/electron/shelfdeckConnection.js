 ...
 * 与 media-desktop/electron/shelfdeckConnection.js 保持一致。
 */
...
/**
 * 小助手「保存连接」写入（Desktop 禁止调用）。
 * @param {{ baseUrl: string; apiKey?: string }} payload
 */
function writeConnectionFile(payload) {
  const dir = path.dirname(getConnectionFilePath());
  fs.mkdirSync(dir, { recursive: true });
  const body = {
    [KEYS.baseUrl]: stripSlash(payload.baseUrl),
  };
  if (payload.apiKey != null && String(payload.apiKey).trim()) {
    body[KEYS.apiKey] = String(payload.apiKey).trim();
  }
  fs.writeFileSync(getConnectionFilePath(), JSON.stringify(body, null, 2), 'utf8');
}
