...
4. **媒体库管理页（MediaManage）**：资产治理；全库列表（含已观看）、搜索与多维筛选（含**是否蓝光/原盘**）、单条/批量入队、星级与策略；列表展示**体积**、**原盘**、**当前/目标码率**、**预测体积**（电影行；按 §4.5 与策略目标估算）、**豆瓣个人评分（匹配结果）与有效星级状态**（用于策略的星级：豆瓣优先）；侧栏汇总**当前库容量**与**电影按目标码率预测占用**（洗版/删除档规则见 §4.5）；**原盘条目**不提供**码率压缩（Transcode）入队（判定在本页刷新管线**完成，见 §5.4.0；**洗版 Upgrade 仍允许**）。列表数据可持久化到本机 `localStorage`（`embyDesktopPlayerLibraryManageCacheV1`），与当前连接指纹绑定；**进入本页不自动拉取 Emby**，需用户主动「刷新媒体库列表」与服务器对齐（回写观看状态等流程可附带静默刷新以保持接近一致）。
...
### 5.4.0 蓝光/原盘资源（ISO、BDMV）与 **Transcode** 入队

- **定义**：资源为 `.iso` 映像，或磁盘上存在 `**BDMV` 目录结构**（含 Emby 返回路径经**配置中心「路径映射」转换后在本机可验证的情形），视为**原盘类**，与「常规单文件/文件夹片源」区分治理。
- **判定位置（与 `TASK_CENTER_FULL_LOGIC.md` §3.7.3 一致）**：**媒体库管理页**在「刷新媒体库列表」、结合路径映射解析本机路径时完成探测；结果进入列表缓存（`embyDesktopPlayerLibraryManageCacheV1`）。**任务中心**不对条目做原盘权威扫描。
- **产品规则**：原盘类资源**不允许**创建**码率压缩（Transcode）**任务；界面需明确提示，批量场景需跳过并汇总说明。**洗版补源（Upgrade）**不在此限。用户若需转码，须先提取或转封装为常规容器后，再纳入 §5.1 / §5.2 流程。
...
| 添加入口、互斥、**原盘类仅禁 `transcode`（媒体库判定）**、移除任务                 | **§3.7**（SSOT）              |
...
- **目标态媒体控制面（REST）**  
  - 前后端分离后的 **对外 HTTP 形状**以仓库 `[docs/api/openapi.yaml](docs/api/openapi.yaml)` 为准（索引见 `[docs/api/README.md](docs/api/README.md)`）。**当前发版**实现仍以上文 IPC / 主进程摘要为准，而非已切至独立控制面服务。
  - **路径映射与配置**：以控制面为 SSOT、设置 UI 经 API 落库、预检与 Worker 同源解析；可选「本机播放附加映射」须在 UI 上与 Worker 规则区分——见 `[CONTROL_PLANE_OPENCLAW_REVISIT.md](CONTROL_PLANE_OPENCLAW_REVISIT.md)` **§3.4**。
