...
MVP 文档以播放闭环为主；**媒体库 + 豆瓣「看过」个人评分**的完整产品定义、数据来源（`movie.douban.com/people/.../collect` 网格解析）、增量/全量同步、片名匹配规则、**有效星级（豆瓣优先）** 与合规说明，以正式版 PRD **`EmbyDesktopPlayer_PRD_v1.0.0.md` §4.4** 与 **§14.6（beta.6）** 为单一事实来源。
