...
- **隔离**：各 Flow **业务逻辑独立演进**。共享部分仅限于 **公共任务模型字段**（§3.4）、**调度与用户操作语义**（§8–§10）、**共享基础设施**（如"同视频互斥"机制 §3.7.2、部分通用 `status` 枚举 §3.5.3、日志与 IPC 形状）。**修改某一 Flow 的业务逻辑**（例如仅调整洗版重试策略、转码 DV 处理链路）**不应** 迫使修改其他 Flow 的业务实现；若出现强耦合，应重构为「共享内核 + 各 Flow 插件」而非交叉改代码。术语与槽位定义 **须先** 与 **§2.0** 一致。
- **隔离边界说明**：隔离指各 Flow 的 **业务步骤与专有状态** 独立，**不意味** 完全无共享；调度层的 **类型任务槽**、**同视频互斥**、**通用 status 枚举** 等基础设施由所有 Flow 共用，这是合理的架构分层，不违反隔离原则。
...
- **实现指导**：原盘判定应优先在 **媒体管理服务** 后端完成（路径映射解析、文件系统探测均在服务端），前端通过 API 获取判定结果，避免跨进程同步问题。判定结果应缓存，减少重复探测；文件系统探测应在后台异步完成，避免阻塞 UI。详见 `[DESIGN_LIBRARY_AND_QUEUE.md](../design/DESIGN_LIBRARY_AND_QUEUE.md)` **§4.0**。
...
- **任务中心设置 · 转码 Flow（与 §5.1、§7.4 对齐；避免双轨矛盾）**：**最终用 NVENC / QSV / AMF / CPU 中哪一个**，由 **编码资源池** 的 **（1）设备是否入池、（2）每设备子槽余量、（3）用户配置的优先级顺序、（4）CPU 参与策略** 共同决定（**§5.1.2**），**不是** 再由另一个全局下拉 **「压制编码器：自动 →先 NV 再 QSV 再 CPU」** 单独裁决 —否则与 **用户排序** 冲突。  
  ...
  - **已废除的旧开关**：整机级 **「自动 = NVENC → QSV → CPU」** 开关已被本文资源池模型完全替代；实现 **必须删除或禁用** 会与池优先级 **竞态** 的旧开关，不得保留任何历史遗留逻辑。
...
> 与 **§17** 对表；适用于 `**transcode` / `upgrade` / `delete`** 等需跨重启恢复的任务。
...
### 18.2 Checkpoint 与启动恢复（技术债务标注）

**当前状态**：v1 版本 **不实现** 完整的 checkpoint 与跨重启恢复机制。
...
**实现路线图**：
- **v1.0**：应用重启后，所有 `executing` 状态任务标记为 `interrupted`，用户需手动重试（从头重跑）。
- **v1.1+**：实现阶段边界 checkpoint，支持从安全点恢复（如 `verify` 阶段可跳过重新压制）。
- **v2.0+**：实现细粒度恢复，支持编码断点续跑（需 FFmpeg 分段编码支持）。
