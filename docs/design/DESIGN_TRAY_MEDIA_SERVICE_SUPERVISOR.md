...
4. **保存连接（与「保存 = 可用」一致）**  
   - **一条管线**：先健康检查；不通过则 **尝试按所填地址启动** 服务，再健康检查；**本机与远端流程一致**，区别仅在 **启动方式**（本机 `spawn` 等见 §4；远端自动启动可为 **占位**，见 `DESIGN_DESKTOP_BACKEND_ENDPOINT` §4）。  
   - **独立「启动 / 停止 / 重启」按钮** 仍要求 **已持久化有效基址**（§3.1），与「更改连接 → 保存」内嵌的临时启动 **不混用门槛**。
