...
Status: ShelfDeck / Helix architecture SSOT; Levels 0–10 accepted; final full-document audit and post-baseline `PBF-01`–`PBF-12`（含`PBF-09-R1`、`PBF-10-R1`、`PBF-10-R2`、`PBF-10-R3`、`PBF-11-R1`、`PBF-11-R2`、`PBF-11-R2-R1`、`PBF-11-R2-R2`、`PBF-11-R3`）bounded corrections closed; implementation not authorized by this document.
...
- Decision Basis Commit的Libra participant固定为`libra_decision_basis_revisions`、
  ...
  `libra_subject_episode_scopes`以重验Subject Snapshot；routing variant再读取
  ...
  `expectedHeadRevision=0, expectedHeadDigest=null`时为active Subject原子插入revision 1 head；后续只允许
  exact revision/digest CAS并加1。Commit只接受完整`DecisionInputSet@1`与匹配
  `DomainFactCommitHandle`；`basisRevision`为该Subject immutable Basis最高revision加1、无历史时为1，逐项保存全部typed input
  snapshot并返回`DecisionBasisRevision@1`。相同`subject+basisKind+inputSetDigest`重放既有Basis而不推进head；新的
...
- Routing Decision Commit的Libra participant固定为`libra_routing_assessments`、
  ...
  `fx_command_receipts`并要求Receipt逐字节引用当前Basis。两分支都要求head当前Basis逐字节等于ready
  `routing` Basis、Subject仍active且intake/content profile/structure revision未变；随后原子保存Assessment、resolved或
...
- Acceptance Spec Publish的Libra participant固定为`libra_acceptance_specs`、
  ...
  Routing Decision与ready `acceptance_spec` Basis逐字节匹配、Subject与Product Scope未变，并把Spec revision按
...
| `libra_decision_basis_revisions` | `decision_basis_id PK, subject_id FK, basis_kind(routing|acceptance_spec), basis_revision, expected_head_revision, routing_decision_id NULL, query_result_set_digest, routing_input_digest NULL, spec_input_digest NULL, product_scope_digest NULL, input_set_digest, status(ready|unresolved), unresolved_reason_code NULL, basis_digest, committed_at_ms` | `UNIQUE(subject_id,basis_revision)`及`UNIQUE(subject_id,basis_kind,input_set_digest)`；routing的routing input始终非NULL且spec/scope NULL；acceptance_spec的routing/spec input始终非NULL，ready时current Routing与scope非NULL，unresolved时只允许按closed reason置对应pointer/scope NULL；ready无reason，unresolved必须有reason；immutable；`INDEX(subject_id,status,basis_revision)` |
| `libra_decision_basis_inputs` | `decision_basis_id FK, input_ordinal, input_kind(subject_snapshot|routing_authority|shelf_routing_projection|routing_fact|routing_decision|shelf_standard_projection|product_scope|decision_fact|query_result), input_schema_ref, input_object_id, input_revision, input_digest, input_json, provider_domain NULL, query_contract NULL, query_version NULL, query_input_digest NULL, result_kind NULL, result_revision NULL, result_digest NULL, expires_at_ms NULL` | `PK(decision_basis_id,input_ordinal)`及`UNIQUE(decision_basis_id,input_kind,input_object_id,input_revision,input_digest)`；ordinal从0连续，逐项typed JSON≤`64 KiB`、总JCS≤`1 MiB`；query variant填写query列，其他variant全部query列NULL；全部rows按8.6.18可重建同一`DecisionInputSet@1`，不得旁读Owner/Provider或Foundation Result补值 |
...
| `DecisionBasisRevision` | `DomainFactEnvelope + decisionBasisId + subjectId + basisKind(routing|acceptance_spec) + basisRevision + expectedHeadRevision + readiness(ready|unresolved) + unresolvedReasonCode? + routingDecisionId? + queryResultSetDigest + routingInputDigest? + specInputDigest? + productScopeDigest? + inputSetDigest + basisDigest`；variant/nullability与`DecisionInputSet@1`逐项相同；`decisionBasisId=SHA-256(JCS({schema:"libra.decision-basis-id@1",subjectId,basisKind,basisRevision,inputSetDigest}))`；`basisDigest=SHA-256(JCS({schema:"libra.decision-basis@1",decisionBasisId,subjectId,basisKind,basisRevision,readiness,unresolvedReasonCode,routingDecisionId,queryResultSetDigest,routingInputDigest,specInputDigest,productScopeDigest,inputSetDigest}))`且等于`DomainFactEnvelope.factDigest`；完整Result≤`16 KiB`，全部typed inputs由Owner relation rows恢复 |
...
| Libra Routing/Decision Basis/Acceptance Spec | 首次无Decision head、Basis input relation逐项写入、head CAS、Assessment/Decision写入、Spec publish及Result/marker前后；Subject Intake/Identity/Episode变化、Field Routing head变化、Arca Projection在Query后变化、同semantic Spec但新provenance | 首次Basis原子建立revision 1 head；Basis及全部typed input rows全有或全无；Routing/Spec只写Libra/Foundation固定表且不读`arca_*`；stale Subject/Policy/head稳定拒绝；Arca Projection变化使Run Creator不建Run并触发新Preparation，不改写历史Decision；同semantic `specDigest`不强制替代Run；相同marker返回原Decision/Spec；`contentProfile=series`与`structureKind=season`不互相改名 |
...
- `8.0`–`8.10`：`ACCEPTED / JOURNEY-AMENDED / POST-BASELINE-DOC-CORRECTED`（2026-07-19；用户确认的基线保持，Level 9反向审计与`PBF-01`–`PBF-12`（含`PBF-09-R1`、`PBF-10-R1`、`PBF-10-R2`、`PBF-10-R3`、`PBF-11-R1`、`PBF-11-R2`、`PBF-11-R2-R1`、`PBF-11-R2-R2`、`PBF-11-R3`）bounded修正已回写）；
...
| 10.10 | 九条旅程、112 Capability、169 tables、113 Admin routes、1 public health route及clean-cut门禁 | preserved after bounded final-audit closure and `PBF-02`–`PBF-12`（含`PBF-09-R1`、`PBF-10-R1`、`PBF-10-R2`、`PBF-10-R3`、`PBF-11-R1`、`PBF-11-R2`、`PBF-11-R2-R1`、`PBF-11-R2-R2`、`PBF-11-R3`） |
...
post-baseline `PBF-01`–`PBF-12`（含`PBF-09-R1`、`PBF-10-R1`、`PBF-10-R2`、`PBF-10-R3`、`PBF-11-R1`、`PBF-11-R2`、`PBF-11-R2-R1`、`PBF-11-R2-R2`、`PBF-11-R3`）已经按同一纪律完成bounded合同闭合并记录在Review Section 15；实现、测试或
部署仍未由本文件授权。
...
- Level 8（`8.0`–`8.10`）：`ACCEPTED / JOURNEY-AMENDED / POST-BASELINE-DOC-CORRECTED`（2026-07-19；用户确认基线保持，`PBF-01`–`PBF-12`（含`PBF-09-R1`、`PBF-10-R1`、`PBF-10-R2`、`PBF-10-R3`、`PBF-11-R1`、`PBF-11-R2`、`PBF-11-R2-R1`、`PBF-11-R2-R2`、`PBF-11-R3`）已闭合）
