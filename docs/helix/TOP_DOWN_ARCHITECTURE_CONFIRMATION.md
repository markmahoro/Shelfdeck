...
| `libra.product_metadata.commit@1` | `LibraProductMetadataSourceBasis(metadata_observation|western_analysis) + ProductMetadataDraft + VerifiedArtifactManifest + DomainFactCommitHandle → ProductMetadataFact` | `domain_fact_commit` |
...
| `libra.product_media.verify@1` | `WorkspaceMediaHandle + MediaProbeEvidence + MediaRequirement → ProductMediaVerification` | `pure_observation` |
...
| `LibraMediaCastCommitPayload` / `LibraProductMetadataCommitPayload` | Media Cast payload固定为`{schema:"libra.media-cast-fact-commit-payload@1",sourceBasis,mediaCastDraft}`；Product Metadata payload固定为`{schema:"libra.product-metadata-fact-commit-payload@1",sourceBasis,productMetadataDraft,verifiedArtifactManifest}`。`sourceBasis`必须是对应closed union的完整typed value；`DomainFactCommitHandle.payloadDigest=SHA-256(JCS(对应完整payload))`，Handle本身不进入digest。Observation Draft匹配Set digest；Western Media-Cast Draft匹配Match Basis digest；Western Product Metadata Draft匹配Basis中的`analysisVariantDigest`并逐字节等于Normalize Result，最终Basis digest不得要求出现在Draft。Product variant的Manifest run必须等于当前Run |
...
| `ProductMediaVerification` | `VerificationEnvelope + libraRunId + producingEventId + workspaceMediaHandleId + workspaceMaterialHandleId + workspaceMaterialHandleDigest + workspaceMaterialFenceDigest + mediaRequirementDigest + probeEvidenceDigest + qualitySummary + spaceSummary`；输入WorkspaceMediaHandle内嵌的完整Material Handle与Probe source handle必须逐字节一致；`verificationKind=libra_product_media`，只有`result=passed`可供Product Staging；`verificationId=SHA-256(JCS({schema:"libra.product-media-verification-id@1",libraRunId,workspaceMaterialHandleId,workspaceMaterialFenceDigest,mediaRequirementDigest,probeEvidenceDigest}))`，完整typed Result canonical JCS≤`16 KiB` |
...
- `WorkspaceProductVerificationSnapshot@1`固定为
  ...
  `libra.product_media.verify@1`的typed `passed` Result，并必须逐项匹配同一Run、
  `WorkspaceMaterialHandle.handleId`、完整Handle JCS digest及`fenceDigest`；failed/not_applicable、另一Handle、另一Run或
...
- `ProductionProvenance@1`固定为
  ...
  `{attestationId,libraRunId,onDeckPackageId,acceptanceSpecId,productConformanceEvidenceId,
  productConformanceEvidenceDigest,unmetRequirementCount(0),attestedAtMs,attestationDigest}`；只有unmet count为0可发布。
...
| Libra Product Fact Commit variants | Handle variant selector解析前后、Run/fact revision fence前后、Source Basis Result逐项验证与relation写入中间、Artifact Verification领域digest/Foundation完整Result storage digest与64 KiB双界验证、Requirement/Registry五方比对及完整Manifest Owner row写入前后、Fact/typed Result/marker FK各边界、相同Handle重放、伪造或缺失Outbox | selector三元组必须唯一命中或按closed规则拒绝；Product Fact variant绝不fallback到generic；Fact revision、完整Source refs、typed Result与唯一有效marker全有或全无，metadata variant还必须同时冻结可历史重建的完整Manifest并验证其中全部显式Result refs、两层digest、Requirement和Artifact fence；任何崩溃/不一致均零写入可见；成功重放返回同一Fact revision/digest/Manifest且不追加Source refs；不存在孤儿/复用marker或本Commit的Outbox row |
