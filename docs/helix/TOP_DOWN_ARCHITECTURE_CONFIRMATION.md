...
| `TriageUnitSnapshot` | `unitId,mediaType(single|group),contentProfile(movie|series|jav|western_adult),structureKind(single|season),displayIdentity,identityMetadata(TriageIdentityMetadata),seasonContinuityClaims[SeasonContinuityClaim],seasonContinuityClaimSetDigest,members[...]? ,memberScope?{scopeKind(bdmv_container),procurementRunId,bdmvGroupKey,scopeDigest,memberSetDigest,memberCount,topologyDigest,selectedPayloadSetDigest},relatedReferences[RelatedMaterialReference],unitDigest`；普通Unit使用完整members；BDMV Movie Unit使用memberScope且禁止在Structure Result中内联完整成员数组，Candidate Context根据scope reference重建成员；members/memberScope二选一；unitId覆盖scope、topology、member set和selected payload digest，unitDigest覆盖完整Unit；单标题BDMV仍只形成一个Movie Unit，内部STREAM/PLAYLIST/CLIPINF/index/MovieObject不能独立成为Candidate或Related；完整Unit的JCS bytes必须≤`64 KiB` |
