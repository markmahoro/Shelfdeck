...
Planner由上述Owner链构造`MetadataObservationSelectionSnapshot@1`与完整`MetadataObservationSet@1`；任何Fact
Commit必须同时提交二者。Commit把精确Result引用写入Libra-owned
`libra_product_fact_observation_refs`，因此后来新增Observation不会改写已提交Fact所依据的集合。历史重建只允许按
该relation连接其显式引用的Work/Plan/Event/Result；这些Foundation rows在仍被Product Fact引用时禁止GC。这里
Foundation Result是正式、被关系化引用的Observation事实来源，不是隐藏业务Store或fallback；relation缺失、Result
schema/digest不符均为integrity fault。
...
| Libra Product Fact Commit | exact `media_cast|product_metadata` variant + current Run/fact revision fence + complete typed commit payload + immutable Product Fact revision + exact Metadata Observation Result references + commit marker + durable typed Result；两种variant均`outboxRequired=false`，Fact、Observation refs、Result与marker全有或全无 |
...
- `variant=libra_media_cast_fact@1`固定`outboxRequired=false`；Libra participant写
  `libra_product_fact_revisions,libra_product_fact_observation_refs`，Foundation participant只写
...
- 两个variant的exact read set固定为`libra_runs,libra_product_fact_revisions,fx_supporting_works,fx_work_attempts,
  ...
  `product_metadata` variant额外读取`fx_artifact_registry`。这些Foundation Result只可按
  `MetadataObservationSelectionSnapshot@1`的显式Result ID读取；禁止扫描无关Result、读取Provider/current文件Reality
  或把Foundation Result当成未声明的Fact来源。
...
| `libra_product_fact_revisions` | `product_fact_id PK, libra_run_id FK, fact_kind, fact_revision, aggregate_id NULL, schema_ref, fact_json, fact_digest, evidence_digest, observation_set_id NULL, observation_set_digest NULL, observation_selection_digest NULL, observation_count, commit_payload_schema_ref NULL, commit_payload_digest NULL, event_fence_digest NULL, commit_marker NULL, result_digest NULL, committed_at_ms` | `fact_kind`必须是Catalog已注册的Product Fact kind，不由Store发明open value；Fact JSON上限`64 KiB`且大Artifact只含handle；`media_cast|product_metadata`按8.6.19固定`product_fact_id/aggregate_id/revision/commit/evidence/result`，要求全部新增commit/Observation列非NULL且`observation_count=1..16`；其他已注册kind继续服从各自既有精确合同，允许这些PBF-14专用列为NULL且count=0，不能被本修正反向套用未定义公式；非NULL `commit_marker`显式FK到`fx_commit_markers`且唯一，`result_digest`逐字节等于marker所指`fx_event_result_bindings`；`UNIQUE(libra_run_id,fact_kind,fact_revision)`；`INDEX(libra_run_id,fact_kind,fact_revision)` |
| `libra_product_fact_observation_refs` | `product_fact_id FK, ordinal, work_id FK, attempt_id FK, plan_id FK, event_id FK, result_id FK, fetch_intent_digest, source_kind(related_nfo|provider), source_ref, source_priority, evidence_id, observation_digest, reference_digest` | `PK(product_fact_id,ordinal)`、`UNIQUE(product_fact_id,result_id)`；ordinal从0连续且恰等于Fact row的`observation_count`，按`source_priority,evidence_id`的UTF-8 bytes排序；Work/Attempt/Plan/Event必须构成同一Run、同一Execution Basis且Event capability固定`libra.product_metadata.fetch@1`，Result schema固定`MetadataObservation@1`；每项`reference_digest=SHA-256(JCS({schema:"libra.product-fact-observation-ref@1",productFactId,ordinal,workId,attemptId,planId,eventId,resultId,fetchIntentDigest,sourceKind,sourceRef,sourcePriority,evidenceId,observationDigest}))`；relation存在期间所指Foundation链不得GC；不得用later current Observation替换历史引用 |
...
| `libra.media_cast.resolve@1` | `MetadataObservationSet + PersonReferenceProjection → MediaCastDraft` | `pure_observation` |
| `libra.media_cast.commit@1` | `MetadataObservationSelectionSnapshot + MetadataObservationSet + MediaCastDraft + DomainFactCommitHandle → MediaCastFact` | `domain_fact_commit` |
...
| `libra.product_metadata.commit@1` | `MetadataObservationSelectionSnapshot + MetadataObservationSet + ProductMetadataDraft + VerifiedArtifactManifest + DomainFactCommitHandle → ProductMetadataFact` | `domain_fact_commit` |
...
inputs交给Libra Product Fact CommitParticipant。`MediaCastDraft.metadataObservationSetDigest`与
`ProductMetadataDraft.metadataObservationSetDigest`必须逐字节等于同次输入Set；Selection必须无损重建该Set。
...
| `MetadataObservationSelectionSnapshot` | `selectionId,libraRunId,runExecutionBasisDigest,setId,setDigest,items[1..16]{ordinal,workId,attemptId,planId,eventId,resultId,fetchIntentDigest,sourceKind,sourceRef,sourcePriority,evidenceId,observationDigest,referenceDigest},selectionDigest`；items逐项等于`libra_product_fact_observation_refs`并按同一顺序；`selectionId=SHA-256(JCS({schema:"libra.metadata-observation-selection-id@1",libraRunId,runExecutionBasisDigest,setId,setDigest}))`；`selectionDigest=SHA-256(JCS(完整value excluding selectionDigest))`；从每项Result完整展开Observation后必须逐字节重建同次`MetadataObservationSet`，否则不是可提交输入 |
...
| `ProductMetadataDraft` | `DraftEnvelope + resolvedIdentityDigest + metadataObservationSetDigest + fieldProvenance[{fieldPath,sourceKind,sourceRef,evidenceDigest}] + descriptiveFacts + providerIdentities[] + mediaCastDraftRef? + artifactRequirements[]`；Movie/Series相同有效字段按Related NFO优先、TMDB补缺，JAV/Western按`6.4.7.1`唯一来源规则；`DraftEnvelope.draftDigest=SHA-256(JCS(完整Draft excluding DraftEnvelope.draftDigest))`且完整value≤`64 KiB` |
| `MediaCastDraft` | `DraftEnvelope + subjectId + metadataObservationSetDigest + relations[{relationId,personId?,displayName,displayNameNormalized,role,source,providerIdentities[],originEvidenceDigest,confidenceClass,relationDigest}]`；`personId`允许NULL，关系按`role,displayNameNormalized,relationId`的UTF-8 bytes稳定排序；单项`relationDigest=SHA-256(JCS(完整relation excluding relationDigest))`，Draft digest排除自身后覆盖完整value |
...
| `MediaCastFact` | `DomainFactEnvelope + subjectId + relations[]完整沿用MediaCastDraft relation schema + relationsDigest + relationCount`；`relationsDigest=SHA-256(JCS({schema:"libra.media-cast-relations@1",relations}))`，`DomainFactEnvelope.factDigest=SHA-256(JCS({schema:"libra.media-cast-fact@1",subjectId,relations,relationsDigest,relationCount}))`；不是digest-only摘要，必须可直接进入On-deck Product Package而无需旁读People或Provider |
| `ProductMetadataFact` | `DomainFactEnvelope + subjectId + resolvedIdentityDigest + metadataObservationSetDigest + fieldProvenance[{fieldPath,sourceKind,sourceRef,evidenceDigest}] + descriptiveFacts + providerIdentities[] + mediaCastFactRef?{productFactId,factRevision,factDigest} + verifiedArtifactManifestDigest + productMetadataDigest`；除Draft-only envelope外逐项来自已验证`ProductMetadataDraft`及正式Media Cast/Artifact commit结果，完整JCS≤`64 KiB`；`productMetadataDigest=SHA-256(JCS({schema:"libra.product-metadata@1",subjectId,resolvedIdentityDigest,metadataObservationSetDigest,fieldProvenance,descriptiveFacts,providerIdentities,mediaCastFactRef,verifiedArtifactManifestDigest}))`且`DomainFactEnvelope.factDigest=productMetadataDigest`，不能只保存摘要而丢弃Arca/播放器需要的描述字段 |
| `LibraMediaCastCommitPayload` / `LibraProductMetadataCommitPayload` | Media Cast payload固定为`{schema:"libra.media-cast-fact-commit-payload@1",metadataObservationSelection,metadataObservationSet,mediaCastDraft}`；Product Metadata payload固定为`{schema:"libra.product-metadata-fact-commit-payload@1",metadataObservationSelection,metadataObservationSet,productMetadataDraft,verifiedArtifactManifest}`。`DomainFactCommitHandle.payloadDigest=SHA-256(JCS(对应完整payload))`，Handle本身不进入digest；Selection/Set/Draft三者的set digest必须相同，Product variant的Manifest run必须等于当前Run |
...
| `LibraProductFactEvidence` | `evidenceDigest=SHA-256(JCS({schema:"libra.product-fact-evidence@1",libraRunId,factKind,factRevision,observationSelectionDigest,commitPayloadDigest,eventFenceDigest}))`。`fact_json`保存完整typed Result；`fact_digest=DomainFactEnvelope.factDigest`；`result_digest=SHA-256(JCS(完整typed Result))`且逐字节等于同marker的Event Result。Media Cast Result逐项复制Draft relation并以当前Run的subjectId校验；Product Metadata Result逐项复制Draft、同RunMedia Cast Fact引用与Manifest digest。Owner row、Observation refs、marker与Result足以历史重建；不得回读Provider、People或later Observation补字段 |
...
| --- | --- | --- | --- |
| `domain-fact-commit@1/libra_media_cast_fact@1` | `libra_product_fact_revisions,libra_product_fact_observation_refs` + Result/marker | `libra_runs,fx_supporting_works,fx_work_attempts,fx_workflow_plans,fx_plan_nodes,fx_workflow_events,fx_event_result_bindings`及全部write tables；同`factKind`历史rows用于revision fence | no |
| `domain-fact-commit@1/libra_product_metadata_fact@1` | `libra_product_fact_revisions,libra_product_fact_observation_refs` + Result/marker | Media Cast variant全部read tables + `fx_artifact_registry`；同Run Media Cast Fact与Artifact Handle均在事务内验证 | no |

两个variant写入Observation refs前必须从Selection完整重建Set，并逐项重验Work→Attempt→Plan→Event→Result链；
...
- Product Metadata/Media-Cast Fact以Libra relation显式引用其精确Metadata Observation Result，Fact history不扫描
  later current Result，也不把Foundation Event Result当成无归属缓存；
...
- `8.0`–`8.10`：`ACCEPTED / JOURNEY-AMENDED / POST-BASELINE-DOC-CORRECTED`（2026-07-20；用户确认的基线保持，Level 9反向审计与`PBF-01`–`PBF-14`（含`PBF-09-R1`、`PBF-10-R1`、`PBF-10-R2`、`PBF-10-R3`、`PBF-11-R1`、`PBF-11-R2`、`PBF-11-R2-R1`、`PBF-11-R2-R2`、`PBF-11-R3`、`PBF-12-R1`、`PBF-13-R1`、`PBF-13-R2`、`PBF-13-R3`、`PBF-13-R4`、`PBF-13-R4-R1`、`PBF-13-R5`）bounded修正已回写）；
...
post-baseline `PBF-01`–`PBF-14`（含`PBF-09-R1`、`PBF-10-R1`、`PBF-10-R2`、`PBF-10-R3`、`PBF-11-R1`、`PBF-11-R2`、`PBF-11-R2-R1`、`PBF-11-R2-R2`、`PBF-11-R3`、`PBF-12-R1`、`PBF-13-R1`、`PBF-13-R2`、`PBF-13-R3`、`PBF-13-R4`、`PBF-13-R4-R1`、`PBF-13-R5`）已经按同一纪律完成bounded合同闭合并记录在Review Section 15；实现、测试或
部署仍未由本文件授权。
...
- Level 8（`8.0`–`8.10`）：`ACCEPTED / JOURNEY-AMENDED / POST-BASELINE-DOC-CORRECTED`（2026-07-20；用户确认基线保持，`PBF-01`–`PBF-14`（含`PBF-09-R1`、`PBF-10-R1`、`PBF-10-R2`、`PBF-10-R3`、`PBF-11-R1`、`PBF-11-R2`、`PBF-11-R2-R1`、`PBF-11-R2-R2`、`PBF-11-R3`、`PBF-12-R1`、`PBF-13-R1`、`PBF-13-R2`、`PBF-13-R3`、`PBF-13-R4`、`PBF-13-R4-R1`、`PBF-13-R5`）已闭合）
