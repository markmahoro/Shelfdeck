...
### Observation fact table and bounded page commit amendment (2026-08-10)

Field Observation is the profile-neutral, append-only source of physical file facts used by all later Procurement
processes. Each committed page writes one durable row per observed regular file to the Procurement-owned
`proc_field_observation_entries` table. Historical entries are retained indefinitely. `proc_field_observations` is the
page/observation header and cursor ledger; `proc_field_materials` remains the current-state projection.

The Page is only a batch receipt, not the storage location for file details. Its compact typed result/evidence contains
the observation/work/page identity, cursors, entry count, boundary digests, page/fact digest and commit marker. It must
not contain `materialObservations[]` or any other unbounded file array. The canonical detail set is the relationized
entry rows, ordered by the frozen field-relative path and entry ordinal; `entryDigest` and `pageDigest` are recomputable
from those rows.

Observation uses one Event/Capability per page: `procurement.field.observation.page.commit@1`. The Event enumerates and
fingerprints at most 256 files and at most 64 MiB of Physical Material reads, then atomically writes entry rows, page
header, current material projection, Field head, commit marker and compact typed Result. A retry may reread the bounded
page, but it cannot leave a partial page or advance the cursor without the complete transaction. The existing
`FieldObservationPageRequest@1` shape remains the request contract; its `pageBudget` is `1..256`.

There is no independent Layout Capability, Layout Event or Layout Result. Layout is a technical read Projection over
the pinned Observation entry rows. It is queried by Field/Observation revision and an explicit physical scope; it does
not enumerate NAS, fingerprint files, read current mutable material rows as a fallback, or embed a large layout in a
Workflow Plan. `ObservationScopeProjection.entries` carries the complete immutable `PhysicalMaterialIdentity@2`,
Endpoint and size for the selected members plus already-observed sidecar candidates in the exact direct-parent scope;
for BDMV it carries the BDMV-root topology scope and the immediate external parent scope. This is a bounded typed
projection, not a new business fact, and its entries are never obtained from current mutable Material rows. Run Creator
derives ordinary parent and nearest BDMV ancestor groups from the recorded relative paths; Observation itself remains
media-profile and BDMV-semantic neutral.

Media Probe and Structure continue to be separate Triage Events. They consume bounded Observation-scope projections;
Structure performs no NAS reads. BDMV grouping and single-title topology remain Structure/Procurement rules, not
Observation facts. Related Material is resolved from the same Observation rows and remains a reference only. The
projection is partitioned by ordinary directory, BDMV root and BDMV external parent so a sidecar from one directory
cannot be associated with a different Candidate; BDMV internal Playlist/Clipinfo/Stream/index/MovieObject entries are
structural members only and are filtered from Related Material.

### Incremental Eligibility amendment (2026-08-10)

The current `eligibility_state` on `proc_field_materials` is a material-local Decision Projection with a
`eligibility_basis_digest`; it is not invalidated merely because the Field's global Observation head advanced.
The basis covers the Physical Material Identity, current Field/Access Binding and Reality/Snapshot, Extraction Policy
revision/digest, Selection/Reservation snapshot, and Foundation Material Control projection. The global terminal
Observation revision remains an audit pointer only. `eligibility_observation_revision` records the Observation revision
used by the last decision and is allowed to lag the current head when the material-local basis is unchanged.

After a terminal Observation, Eligibility Reconcile receives only a bounded Change Set: new or reality/binding/location
changed materials, materials absent from the terminal work, materials with `unknown` or invalid basis, and materials
affected by Field/Access/Policy/Selection/Reservation/Control revision changes. An unchanged material retains its
Eligibility state, basis, and revision; no Eligibility SQL UPDATE or revision increment is performed for it. A lost
Control/Handoff signal is discovered by the durable reconcile cursor and is handled with the same exact Material Key
Change Set. `unknown` is fail-closed and never means `eligible` before a successful reconcile.

This amendment supersedes older wording that required every material's Eligibility revision to equal the global
terminal Observation revision or that described Observation Commit as an unbounded Page Evidence write. The canonical
Observation detail remains `proc_field_observation_entries`; there is no second Eligibility history store.

### BDMV container and deterministic main-title amendment (2026-08-10)

For Procurement topology, a BDMV disc is the logical container directory that directly contains `BDMV`; an optional
sibling `CERTIFICATE` directory belongs to the same container. `PLAYLIST`, `STREAM`, `CLIPINF`, `index.bdmv`,
`MovieObject.bdmv` and `CERTIFICATE/id.bdmv` are therefore one Run scope and must never be admitted as separate
ordinary directory groups. The ordinary Run limit remains 256 logical selections. A BDMV container consumes one
logical selection slot, while its physical members use a separate bounded member limit of 1024. This is the same
atomic physical scope limit as the sealed `ResponsibilityControlCommitHandle`, so Run Admission can acquire the whole
scope in one transaction. A container above that limit is closed as `procurement_selection_scope_too_large` and is
never split; the ordinary-directory 256-member rule is unchanged.

Movie BDMV Triage no longer treats `titleCount > 1` as an automatic business failure. Playlist candidates are first
normalized by their unique M2TS member set, then ranked deterministically by total unique payload bytes, duration,
distinct clip count and canonical playlist path. The highest-ranked valid title is the primary Movie title. The
selected playlist and its M2TS members form the one Movie Unit; other valid title families receive terminal
`disc_non_primary_title` Evidence and never become Candidate or Related Material. The selection evidence remains in
the durable `MediaProbeEvidence.discTopology` digest, so a later human review can identify the deterministic choice.
Only a missing/invalid topology, missing selected dependency or an actually ambiguous fence remains `not_ready`; a
multi-title disc by itself is not `disc_multi_title_unsupported`.

Media Probe executes ffprobe only for selected payload files (ordinary media or BDMV `STREAM/*.m2ts`). BDMV
structural files are represented by bounded typed structural evidence and are never sent to ffprobe. Structure consumes
that durable topology/evidence, verifies the complete container scope, and emits exactly one BDMV Movie Unit. Internal
BDMV members, including `CERTIFICATE/id.bdmv`, can never be independent Candidate or Related Material. This amendment
supersedes earlier wording that required `titleCount=1` or grouped only by the nearest `BDMV` ancestor.

### BDMV atomic Control scope alignment (2026-08-10)

The earlier BDMV note's provisional physical-member value of 4096 is superseded for the clean implementation. Run
Admission is a single atomic Material Control transaction, and the sealed `ResponsibilityControlCommitHandle` contract
allows at most 1024 exact members. Therefore the effective BDMV container member limit and the maximum physical
members of any admitted Run are 1024; ordinary directory groups remain limited to 256 members. This is not a
Foundation state-machine change or a batch-acquire fallback: a larger BDMV container is closed as
`procurement_selection_scope_too_large` and never split. The current Movie Field has no eligible BDMV container above
1024 members, so this alignment removes the previously observed Admission failure without changing its business
coverage.

The machine contract generated from this amendment is closed at 111 active Capability contracts, 97 Result families,
180 clean tables and 43 Canonical Transactions. Local implementation evidence is recorded in `CURRENT_STATUS.md`;
this SSOT remains the architecture authority and does not authorize Docker, NAS, Libra or Arca deployment.

### Procurement container, Related Scope and Handoff input-form amendment (2026-08-10)

The canonical BDMV container resolver starts from any frozen Observation member. A member under `BDMV` resolves to
the directory containing that `BDMV`; a member under `CERTIFICATE` resolves to the same container only when the frozen
scope also contains a sibling `BDMV` directory. A normal `CERTIFICATE` directory without that sibling is never a BDMV
scope. Run Creator, Evidence Planner, Observation Scope Projection, Structure and Candidate Context use the same
normalized Field-relative path algorithm and the same `bdmvGroupKey`, `scopeDigest` and member-set digest.

`TriageUnitSnapshot` no longer carries an inline `relatedReferences[]` array. It carries one bounded `relatedScope`:
`{scopeKind(ordinary_parent|bdmv_external_parent),parentRelativeLocation,stemKey,observationProjectionRevision,
relatedRuleRevision,scopeDigest}`. Structure only freezes this scope and includes it in the Unit identity/digest. Candidate
Context resolves the scope against the frozen Observation revision, applies the deterministic same-stem/standard-sidecar
rules and BDMV-internal filter, and writes the complete Related Reference set only into Candidate Package and the Handoff A
Delivery Snapshot. Missing or ambiguous sidecars do not block the Candidate; BDMV internal `M2TS/MPLS/CLPI/index/
MovieObject` files are never Related.

`materialInputForm` is a new authoritative field, with closed values `stream_file|bdmv|dvd|iso`. It is present in
`TriageUnitSnapshot`, `CandidateDraft`, `CandidatePackage`, `CandidateDeliverySnapshot` and
`AcceptedIntakePayload.sourceProvenance`, and is covered by every corresponding digest. `bdmv` is copied from
`BdmvAssessmentEvidence.discKind`; `iso` and `dvd` are copied from valid typed `MediaProbeEvidence.discTopology`;
ordinary media without disc topology is `stream_file`. Extension or path strings are never used to infer ISO/DVD.
Missing, conflicting or incomplete topology is a typed fail-closed result and cannot produce a Handoff A Offer. This
amendment adds no Capability, Result family, table, Business Object or Admin route and does not implement ISO mounting,
DVD/ISO remux or any Libra production Event.
...
PhysicalMaterialIdentity
  = filesystemObjectKey + sizeBytes + boundedContentFingerprint

filesystemObjectKey
  = mountScopeId + inode
~~~

`boundedContentFingerprint`以固定成本对现实文件中段形成高可信业务指纹；它不证明全文件逐字节完整。
`filesystemObjectKey`区分相同指纹的不同文件实例。clean Helix只支持
Linux x86-64 Docker部署，`mountScopeId`表示ShelfDeck配置的稳定挂载范围，`inode`表示该范围内的
原生文件对象。Level 7固定有界采样、算法和重验合同；Level 8固定Mount Scope Registry及持久字段。
...
+ sizeBytes and bounded content fingerprint match
= healthy
~~~
...
Material Field可以提供versioned contentProfile Hint；create未提供Hint时必须先规范化并持久化为显式`mixed`，
不能以NULL或调用者默认值表达。具体Profile准召与Shelf Standard属于Level 5。
...
versioned Profile Hint、Observation Inventory与Extraction Policy。Material Field回答“用户允许Procurement去哪里观察原料、当前
...
已经Admission的Run不得原地扩张其Selected Field Material Set。Triage在Run内发现新的必要structural dependency
时，只发布发现Evidence并使当前Run按有界结果封口；Field Management据此重新评估新增成员，后续Run以新的完整
Execution Basis选择原成员与新增dependency，复用同Field retained Control并对新成员取得Control。该过程不是
修改旧Run、分批补Control或Gate回退；Related Material Reference仍不触发新Run。
...
- Working Set：Metadata Artifact、下载结果、转码中间物、验证Evidence和其他仍可被继续加工或丢弃的
  Supporting Material；
- Product Staging Set：已经完成与Material role相匹配的Libra产品验证、可进入Product Material Manifest并准备
  交付Arca的确定Product Material。`primary_payload`使用媒体验证，Artifact/sidecar使用Artifact验证，
  结构依赖使用Manifest/结构验证；进入Staging不等于每个文件本身都必须是可播放媒体。
...
| --- | --- |
| Object Root | Person |
| Process Roots | Person Registration、Person Merge、Reference Image Maintenance |
| Canonical Facts | Person identity、Name、Alias、Provider Identity、Reference Image、内部派生Reference Face、Person Preference |
...
Person可以由用户直接注册，也可以由Provider Evidence、Arca公开的On-deck人物Evidence或人脸聚类形成Registration
Candidate后建立。Candidate不是Person，直到Registration规则或用户确认完成。用户直接注册Person与随后添加
Reference Image是两个相互独立的People事务：Person没有Reference Image仍是合法Person；Reference导入失败或
用户暂不添加Reference不得回滚、删除或降级已经成立的Person。

Reference Image是用户可见、可添加和可释放的人物参考资料。Reference Face只是从一张已验证Reference Image
内部派生的Face/Embedding事实，不是用户可单独创建、选择或删除的业务对象。一张用户上传的Reference Image
必须恰好检测到一张人脸；零张或多张都使本次Reference Image Maintenance失败，且不得形成任何Reference Fact。

Person Registry是全局人物登记，不按普通/成人、Movie/Series或其他媒体content profile拆分。媒体的
content profile与Person在媒体中的关系归拥有该媒体事实的Domain；People Person revision不得持久化
`content_scope`或由Candidate接受流程临时猜测同义字段。
...
Media-Cast关系允许在Person尚未注册时以`personId=null`保存显示名、角色、Provider Identity hint与原始
Evidence。以后Person注册成立不自动赋予People Management改写关系的权限；Pre-deck仍由Libra、On-deck仍由
Arca依据各自事实和确定性Evidence发布新Media-Cast revision。
...
- Physical Material Identity由filesystemObjectKey与contentFingerprint构成，location属于Domain-local Binding；
...
| Physical Material Identity | 由filesystemObjectKey、sizeBytes与versioned bounded content fingerprint共同构成的高可信业务文件身份；不提供全文件字节完整性证明 | 3.2.4 |
...
| contentFingerprint | 对Physical Material正中间最多256 KiB字节计算的SHA-256；读取成本固定有界，不是全文件checksum | 3.2.4、7.6.1 |
...
| Product Staging Set | Production Workspace中已经通过与Material role相匹配的Libra产品验证、可被Product Material Manifest引用并准备交付的确定Product Material集合；非媒体Artifact不接受也不需要伪造Media Probe通过 | 3.4.6 |
...
Handoff A的Offer必须携带一份由final Candidate Package唯一派生的
...
Handoff合同发布新版本，新的Acceptance Basis才能为同一immutable Deliverable形成新的顺序Offer。Beta的首次
Candidate Publication只建立当前合同版本对应的Offer。
...
+ 未被任何active/waiting Run Selection或尚未Handoff终结的Candidate Delivery Reservation占用
+ Extraction Policy允许
= eligible
~~~

“重复开采抑制”不属于Procurement Eligibility。`5.3.7`已经固定：失败Run/Candidate历史不形成隐藏抑制，用户
释放Material后可以按当前事实重新开采；Arca Off-deck Suppression属于另一个Owner和业务目的，Procurement
不得读取或复用。

Beta `ExtractionPolicy@1`固定为一份≤`16 KiB`的closed typed value：
...
- `includedDirectories[]`为空表示整个Field均满足include；非空时至少命中一个目录边界；
- `excludedDirectories[]`命中即排除，优先级高于include；
- 两类目录值必须是相对当前Field root的真实UTF-8目录路径，使用`/`分隔、禁止absolute、`.`、`..`、空segment、
  尾随`/`、glob、wildcard和regex；按Linux大小写敏感、Unicode code point不归一化；对Material的Field-root-relative
  file path `P`，目录值`D`当且仅当`P`以`D + "/"`开头时命中，禁止字符串prefix越过目录segment边界；
- `allowedExtensions[]`为空表示本阶段不限制扩展名；非空值必须是带前导`.`的ASCII lowercase extension，观察
  文件名的最后suffix只做ASCII case-fold后精确匹配；是否真为可播放媒体仍由Triage判断；
- `minimumSizeBytes`为`0..2^53-1`的non-negative integer，Material必须满足`sizeBytes >= minimumSizeBytes`；
- `excludedMaterialKeys[]`按完整Physical Material `materialKey`精确排除，不能使用路径、名称或Hash片段替代；
- 各数组去重并按UTF-8 bytes升序，目录数组各最多128项、extension最多64项、material key最多128项；
- 最终Policy Decision为`includeMatch AND NOT directoryExcluded AND extensionAllowed AND sizeAllowed AND
  NOT materialKeyExcluded`，不存在隐含default deny、失败历史或跨Domain规则。

新建Material Field时必须同事务建立显式revision `1`；Beta推荐初值为五个数组均为空且
`minimumSizeBytes=0`，因此结果是允许整片Field进入后续Triage，但这仍是一份可审计的current Policy revision，
不是缺失Policy时的隐含default allow。

Policy只能在完整schema、路径、上限与`policyDigest=SHA-256(JCS(完整policy value excluding policyDigest))`验证通过后发布current
revision。未知schema、无效值或持久化损坏是稳定contract/invariant failure；运行中的Eligibility只能形成
`unknown(policy_unavailable_or_invalid)`，禁止默认allow或默认deny。
...
Triage Rules是Procurement拥有、随ShelfDeck binary发布的immutable system decision contract，不是用户配置、
Extraction Policy或调用者可以临时提交的参数。`TriagePlanner`物理包内的versioned
`ProcurementTriageRuleRegistry@1`是唯一Authority：Registry必须保存一个active rule pointer和本clean schema
lineage已经发布的全部immutable rule revision；每项Rule以`ruleRef + revision + ruleSchemaRef +
ruleDigest`唯一标识。Run创建时只能冻结Registry当前active的Rule Snapshot；已有Run继续使用自己的冻结revision，
...
无法解析Episode、真实歧义或结构不完整形成稳定`not_ready`，不能由Executor默认成Season 1、Episode 1
或任选主标题；Movie BDMV多Playlist按已冻结的确定性主标题规则选择，不能把每个STREAM成员当作独立Unit。
现实读取、closed reason set、token grammar、Member映射与digest basis由`8.6.4`、`8.6.18`–
`8.6.20`唯一细化，不能留给实现按数组位置、Store旁读或路径猜测补齐。

#### 5.3.3 Material Field contentProfile Hint降低Triage成本但不成为事实Owner

用户可以在Material Field上提示`movie|series|jav|western_adult|mixed`。注册时未提供Hint必须规范化为
`mixed`并形成revision 1；之后只能以expected revision CAS追加immutable Profile Hint revision并切换current
pointer。有显式Profile时，Triage默认从对应Profile的规则开始；`mixed`按混合Field处理并由Triage形成Claim。
...
但不能反向修改原Candidate Package。Hint修订只影响在新revision下开始的Observation与Run；已经提交的
Observation、Run、Retry Intent、Candidate与重放继续使用各自冻结的Snapshot，不跟随current pointer。
...
并形成唯一Movie Claim和可审计的主标题topology；识别出的原盘内部成员不得退化为多个独立Candidate。包含多个
独立Movie、多个Episode/Season、真实主标题歧义、结构不完整或仍在变化的原盘不满足Candidate Readiness，
继续留在Procurement，不越过Handoff A。普通的多Playlist并不自动等于主标题歧义，按BDMV主标题选择合同收口。

BDMV的目录拓扑必须在Triage之前就保持完整。Run Creator把terminal Observation中位于同一个直接包含
`BDMV`目录的逻辑容器（以及可选的同级`CERTIFICATE`）下的全部Field Material视为一个不可拆分的
container group；该分组键使用Field-root-relative的规范化容器路径确定，属于pre-triage拓扑事实，不是
`movie`、`series`或其他mediaType判断。Run Creator不得把同一BDMV容器的`PLAYLIST`、`STREAM`、`CLIPINF`、
`index`、`MovieObject`或`CERTIFICATE`成员拆到不同Run。
Run Admission时，形成单标题所需的结构成员必须同时满足Observation、Eligibility与Control；缺失或不合格
时，Triage不得在已Admission的Run中静默扩张，当前结构只能以稳定Evidence/closed reason收口并由后续
Field Management重新观察、评估和建立新的完整Run。

BDMV container group可以与其他完整group按canonical顺序依次装入同一Run；每个BDMV group占一个逻辑
Selection slot，内部物理成员最多1024项，普通directory group仍按物理成员计入256项上限。单个group超过
自身上限时整体不建立Run，不进行拆分。Structure Event只消费完整container group，解析Playlist与其
Clip/结构依赖，形成一个有界Triage Unit；BDMV内部的多个M2TS或结构文件不得退化为多个独立Candidate。
多标题Movie按确定性主标题选择并为非主标题成员记录`disc_non_primary_title` Evidence；只有真实歧义或
结构不完整保持`not_ready`，不越过Handoff A。
Evidence Assessment对BDMV容器只签发一个`procurement.triage.bdmv.assess@1`逻辑Event，而不是为每个物理成员
签发通用Probe。该Capability读取有界Playlist/Clip元数据，按确定性规则选择主标题，并只对选定的主标题
M2TS执行一次compact、有界的媒体元数据探测；不读取完整M2TS payload，也不计算Physical Material全文件Hash。
它输出紧凑的`BdmvAssessmentEvidence@1`，成员角色和成员集合由冻结Observation/Run事实及digest重建。普通
文件仍使用`shared.material.media.probe@1`。Structure只消费durable BDMV evidence并输出紧凑
`UnitScopeReference`，不访问NAS，也不得把`STREAM`文件名当作Candidate display identity；Candidate Context
在需要时再重建primary payload和structural dependency成员。
...
Related关联必须建立在同一Procurement Run已冻结的Layout Evidence上。Observation完成后，运行时可以从
持久化的`FieldObservationPage@1` Evidence重建有界Layout索引；Evidence Assessment Planner只把当前
Candidate所在普通父目录（或BDMV外层电影目录）中、已经被Observation看到的Sidecar投影进该Candidate的
Layout Snapshot。Candidate Assembly只消费这个Snapshot和自己的Unit Context，不重新扫描整个Field或
Run。普通电影按同stem及标准Sidecar名称确定性匹配；BDMV根目录内的`PLAYLIST`、`STREAM`、`CLIPINF`、
`index.bdmv`、`MovieObject.bdmv`及`.mpls/.clpi/.m2ts`成员只能作为BDMV结构成员，永远不能成为Related
Reference。索引丢失或重启时必须从durable Observation Evidence重建，不能从当前文件系统Reality补写旧
Candidate事实。
...
Beta的版本化系统Policy必须满足以下确定性基线，不能把选择留给数组顺序、数据库返回顺序或Executor自行猜测：

- 只解析请求kind，并排除已经被有效`supersedes|retracts`关系终结的Record；其他kind的值不能参与当前结果；
- 先按当前Policy声明的Anchor matcher与strength rank求匹配，只保留命中最高strength tier的Record；
- 最高strength tier没有请求值时返回`not_found`；全部有效值一致时返回`found`，并以稳定排序后的
  `perceptionId`作为Canonical winner reference；同tier出现不同有效值时保守返回`not_found`，内部原因固定为
  `strongest_value_conflict`，不得按时间、来源、ID或数组顺序静默覆盖；
- `duplicate_of`与“本次Query可以选出winner”不是同一证明。只有Policy单独声明的exact strong-identity
  proof matcher成立，并且两条Record具有相同fact kind与相同Canonical value时，才能形成duplicate relation；
  名称模糊匹配、仅相同值或仅共同命中Query都不足以证明两条Record是重复记录。

这些规则属于可版本化的Perception Resolution Rule。未来可以以新rule revision改进Anchor、阈值或冲突算法，
但历史Resolution必须保留当时的rule revision/digest，且一次Resolver输入必须携带完整、可执行的冻结Rule
Snapshot，不能只传一个无法解析的digest。
...
Beta固定两种Person Registration入口。用户直接注册是显式People Command，立即建立Person，不要求先创建
Candidate，也不把Reference Image作为注册前置条件。系统发现人物必须先形成Registration Candidate：只有
来源明确且namespace稳定的Provider Person Identity属于strong identity，允许Candidate在可审计的系统规则下
自动接受；仅有姓名、Alias、图片、人脸相似或NFO显示文本时均属于weak identity，只能等待用户接受或忽略。
无论Candidate来自哪里，接受Person都不同时修改Libra或Arca的Media-Cast Fact。
...
- `L5-A2 — Unified Primary Input Manifest`：所有Candidate恰好拥有一份由`1..N`个Primary Material
  ...
  Manifest合同；Movie BDMV多Playlist按主标题合同自动选择并保留选择Evidence，跨Season或拓扑不明确的原盘仍不接收。Triage只完成结构分组与Claim，
...
| People Management | Person Registration、Person Merge、Reference Image Maintenance | Person Registry、Preference与Reference事实 | 2.9、3.7、4.6.3、5.9.4–5.9.5 |
...
- 本次Selected Field Material Set包含`1..256`项pre-triage `triage_input` Field Material；这是单个Run的原子
  准入上限，不限制一片Material Field的总规模，也不是资源并发上限；空Selection不得建立Run；
- Run admission能够针对全部选择成员在同一事务中取得或确认Procurement Control，禁止把同一Run拆成多次
  Control acquire后再声称原子成立；
- 计划选择的`triage_input` Field Material没有被任何`active|waiting` Run Selection或尚未完成Handoff A Decision的
  Candidate Delivery Reservation占用；
- 选择范围能够形成稳定Execution Basis，其中明确冻结所选成员和Triage rule revision；
- Material Field/Endpoint的系统级Incident没有使必要Evidence当前不可取得。

Procurement Control的稳定Owner Scope固定为`ownerDomain=procurement, ownerScopeType=material_field,
ownerScopeId=fieldId`；`procurementRunId`是本次取得/确认Control的授权Process与审计Correlation，不把Control
改成Run私有锁。Admission对当前`uncontrolled`成员执行CAS acquire；对已经由同一`fieldId`持有且不存在有效
Selection/Delivery Reservation的成员执行revision/digest assert并复用；被其他Domain或另一Material Field控制时
整体拒绝。该规则使失败Run封口后能够保留Procurement责任，也避免为新Run伪造一次Control release/acquire。

Run开工的唯一正式冻结输入是`ProcurementRunExecutionBasis@1`。它必须包含Field Access、完整
Material Field Profile Hint Snapshot、使用同一Hint revision的terminal Observation、Extraction Policy、
由Procurement Triage Rule Registry签发的active Triage Rule Snapshot和完整Selected Field
Material Set；每个成员冻结Binding、Eligibility、Reality、
Provenance、Endpoint/location及expected Control snapshot。`run_basis_digest`不得替代可恢复内容；完整Basis必须能
由Procurement自己的immutable/relationized记录确定性重建，Foundation Work/Plan或最新Field current row都不能
充当历史Basis。

Run Creator只读取同一terminal Observation与current Eligibility形成的pre-triage事实，不得先判断Movie、Series、
JAV或Western Adult再选择分片规则。普通文件按Field-root-relative直接父目录形成不可拆分组；含有`BDMV/`
子树的外层目录（以及同级`CERTIFICATE/`）形成不可拆分BDMV container group。普通组按物理成员数占用
`1..256`个logical slot；BDMV container作为一个logical slot，但物理成员最多1024个；该1024同时是Run
Admission的atomic Control scope上限。组与组内成员均按canonical UTF-8
bytes排序，再按该顺序依次装入Run；下一组加入后会超过256时关闭当前Run并从下一Run继续，不得通过重排式
bin packing改变稳定结果。同一目录组或BDMV container group必须完整进入同一Run；普通单组超过256或BDMV物理成员超过1024时不建立Run，
并以`procurement_selection_scope_too_large`形成稳定closed reason，直到Observation、Binding、Policy或Eligibility
事实改变。Field根目录下的成员属于同一直接父目录组。

一次Procurement Automation reconcile必须为当前全部可建立目录组与container group形成零到多个互不重叠的Run；允许多条
`active|waiting` Procurement Run并存，不存在全局单Run或一次reconcile只准建立一条Run的合同。每条Run分别执行
原子Admission；已建立Run不因另一分片stale而回滚。Run创建不取得CPU、Volume、Network、Device或Worker Permit，
实际执行供给和并发只由Supporting Work、Work Scheduler、Event Runtime与Resource Governor控制。
...
Run Admission成功后，`ProcurementRunCoordinator`作为该Process的Domain Work Issuer，必须幂等签发恰好一份
Evidence Assessment Work。Coordinator通过durable Run/Work/Result事实reconcile；进程内Signal只可唤醒，不是执行
真相，丢失Signal或进程重启后仍须从Owner与Foundation事实恢复。Triage在同一Procurement Run内固定使用两段
Supporting Work，而不是一条Coordinator或Executor内部状态机：
...
一次Field Observation Supporting Work为同一`fieldId + accessRevision + profileHintRevision`串行提交
`1..N`页，但它不是新的Business Process或业务对象。Coordinator在Work建立时冻结完整
`MaterialFieldProfileHintSnapshot@1`，并在每页执行前冻结`fieldObservationWorkId`、该页唯一且只在该页重试中
保持稳定的`observationId`、
...
Field Access revision。current Profile Hint在Work期间改变时，旧Work后续页必须因current-snapshot fence失败而
整页rollback；已提交Page保持历史事实，新revision必须从新的Observation Work和`cursorIn=null`开始。

分页不得通过动态扩写一份Workflow Plan、Planner预扫描文件系统或Executor内部循环调用另一Capability实现。
同一Field Observation Supporting Work按页使用多个有界Work Attempt：每个Attempt恰好发布一份immutable Plan，
只包含当前页的单一`procurement.field.observation.page.commit@1` Event；该Event同时完成有界物理读取和
Observation事实提交。成功后，Procurement Owner读取compact durable typed Result，并从
`proc_field_observation_entries`恢复明细。`hasMore=true`时Owner以同一Work Objective与Execution Basis签发下一
Work Attempt并冻结上一页`cursorOut`及已提交revision；`hasMore=false`时Owner才把Supporting Work置为`succeeded`
并触发Eligibility reconcile。Foundation只聚合Work Attempt终态，不得替Procurement判断Observation Work是否完成。
...
Retry Intent只允许选择原Run中因失败而`released`、当前仍满足Eligibility且没有Selection/Delivery Reservation的
成员，冻结精确member set、当前Field/Profile Hint/Access/terminal Observation/Extraction Policy/active Triage Rule
admission head、Eligibility/Binding/Control precondition与`retryScopeDigest`。Intent创建与消费
是两个事务：创建事务以`fieldId + idempotencyKey`幂等保存`open` Intent；消费事务CAS
`open → consumed|stale`。有效时，消费与新`ProcurementRunExecutionBasis`、新Run、全部Selection及Control
acquire/assert在同一Run Admission事务成立，并由`UNIQUE(retry_intent_id)`保证最多一份新Run；任一成员
precondition失效时原子进入`stale`且不建立部分Run。响应前崩溃以commit marker和原typed Result重放；新的
失败Run不会自动派生下一Intent。

一个Run可以发布`0..N`个彼此不重叠的Candidate Package。每份Package发布时独立建立自己的Handoff A Offer；Libra
可以在同Run其他Candidate Assembly Work尚未完成、Run尚未Seal时处理该Offer。Package发布后immutable；Run后续
封口、Handoff Rejected或另一Package Accepted都不能改写它。

Selection与交付占用是两个连续但不同的Procurement guard：

- Run Admission建立`run_selection`；
- Candidate Package发布时，其Manifest成员在同一事务中从`run_selection`转为`candidate_delivery`，并绑定
  精确`candidatePackageId + packageDigest`；同一成员不能进入两份仍可交付Package；
- Run Seal把没有进入任何Published Candidate的剩余`run_selection`原子改为`released`，但不得释放或覆盖
  `candidate_delivery`；因此Run完成不依赖Candidate命运，同时材料不会在Handoff A完成前被再次采购；
- Handoff A Accepted Receipt被Procurement幂等消费后，将对应guard置为`transferred`；Rejected Decision被幂等
  消费后置为`released`。两种终态都不修改immutable Package；Accepted的Control已经在Handoff事务转移，
  Rejected继续保留同一Material Field的Procurement Control。

Run Seal是Procurement-owned原子提交：只允许`active|waiting → sealed`，CAS expected state revision与
`runBasisDigest`，冻结`sealOutcome(completed|failed|partial_failure)`、成功Package集合、失败/未形成Package
逐成员Evidence、`finishedAtMs`和Selection disposition。每个被释放成员必须把Decision中的`evidenceDigest`
持久化到自己的Run relation row；Run head只保存aggregate `sealEvidenceDigest`，不能代替逐成员Evidence。
Seal不修改`fx_material_controls`。相同Decision重放返回原Receipt；异Decision或已sealed后重开稳定拒绝。
它由`ProcurementRunCoordinator`在terminal Work Result全部收口后执行，是不调用Capability的短Owner transaction：
以`fx_command_receipts + fx_commit_markers`保存durable typed Receipt和幂等恢复点，不建立伪造的Supporting
Work/Plan/Event/Event Result，也不申请Resource Permit。其职责只限于按既有Evidence封口，不能在Seal内部补做Triage。
...
- Rejected时，Libra保存Structured Rejection/Decision/Receipt，Procurement消费typed terminal message后保留
  Control并冻结同一Receipt Evidence；原Package不修改；
...
结束本域实时Binding/selection投影并将Candidate Delivery Reservation置为`transferred`；Rejected terminal message则
将其置为`released+handoff_rejected`并保留同一Field的Procurement Control。Receipt/Decision/message重复或延迟不会
删除Observation、释放错误Control或创建重复Subject。

#### 6.3.5 Priority与并发

Field Observation和大范围Triage是后台吞吐工作；已经发布的Handoff A Offer属于高优先级交接工作。
同一Identity可被多片Material Field并行观察，但最多被一个`run_selection`或`candidate_delivery` guard占用；
Control取得、
...
Package的交付责任在其幂等消费Handoff A Accepted或Rejected typed terminal message并关闭Delivery Reservation后
收口：Accepted的Material Control已经在Libra Accepted事务转移，Rejected继续由Procurement持有材料并决定未来
是否依据新Basis再次开采。Libra terminal Decision先成立、Procurement异步收口尚未完成是可恢复技术窗口，不是
第二个业务Decision。

#### 6.3.7 Upstream Consistency Audit

- Field Management未被改写为Process Root，Procurement Run未变成长生命周期媒体对象；Procurement Control
  稳定归Material Field scope，Run只拥有有界Selection；
- Candidate仍只含Identity Metadata，不提前生产Product Metadata；
- Candidate/Handoff Offer未指定Shelf；
- Rejected未形成反向Handoff或要求Libra承担Triage；
- Failed Run只在Procurement域内释放Run Selection，不释放Procurement Material Control；Published Candidate的
  Delivery Reservation保持到Handoff A终态。
...
Capability、Flow、设备或Task。它也是Spec或初始Execution Basis变化时唯一能够建立替代Run的业务入口；Automation、API或
...
| superseded | 当前Spec或初始Execution Basis已不再是该范围的有效生产订单；Run永久失去提交资格 | terminal history only |
...
| --- | --- |
| ready且Spec语义、Production Material Manifest及全部初始Execution Basis事实相同 | 当前Run继续；更新Freshness Evidence，不改写immutable Spec/Basis |
| ready且Spec语义变化，或Spec相同但Production Material/Binding/Control等初始Execution Basis变化 | 旧active/suspended Run变为superseded；Run Creator为同一Subject/Episode交付范围创建新Run并冻结新Basis |
| 当前Spec Basis不再fresh但新Spec暂时unresolved | 旧Run进入suspended，停止产生新外部效果；等待有界Decision Preparation恢复 |
| suspended窗口内重新ready且Spec与初始Execution Basis都相同 | 原Run回到active |
| suspended窗口内重新ready且Spec或初始Execution Basis变化 | 原Run superseded并创建新Run |
...
Integration重试和Freshness重评，不占用重型生产资源；只有Spec与初始Execution Basis都逐字节相同时原Run
回到active，任一发生变化时旧Run永久`superseded`并由Run Creator建立替代Run；预算耗尽仍不可形成可交付
产品时进入`frozen`。具体次数、退避和墙钟上限由Level 7执行合同与Level 10 Operational Baseline固定，不改变
本Level状态语义。
...
##### 6.4.7.1 Beta Product Metadata补齐链路

Product Metadata补齐由既有`LibraProductionCoordinator + ProductionPlanner`组织，不新增Metadata Center、
Provider Broker或跨Domain缓存。Production Planner只读取当前Run已冻结的Requirement、Libra Facts与durable
typed Observation，计算字段Gap、选择下一项Metadata来源并形成不可变Plan；它不得调用Provider、打开NFO、
写Product Fact或在一次`metadata.fetch`失败后偷偷切换来源。

Beta来源顺序固定为：

| contentProfile | Source order and boundary |
| --- | --- |
| movie / series | 先读取当前Candidate随单Related Material中的有效NFO，只使用可解析且语义有效的字段；仍缺失的Required字段再从TMDB取得 |
| jav | 使用已配置的JAV Provider取得Product Metadata；不把普通Movie/Series的TMDB回退套用到JAV |
| western_adult | 使用既有Libra Western frames/analysis/normalize能力形成自有Product Metadata；不伪造不存在的外部Provider身份 |

一次`libra.product_metadata.fetch@1`只观察一个明确来源。Related NFO和TMDB是两个独立Event；前一个结果不会
驱动Executor创建后一个Event。每个Event完成后，`LibraProductionCoordinator`仅以durable Result唤醒同一Run
的gap reconcile；Planner重新读取当前Observation集合，决定Gap是否已闭合、是否还需下一来源，或是否可以纯
计算完整`ProductMetadataDraft`并规划既有Commit/Artifact工作。该Draft组合只执行固定字段优先级和Requirement
投影，不访问I/O、不提交Fact，也不成为新的物理组件或Capability。

“当前Observation集合”不是对`fx_event_result_bindings`的无条件扫描，也不允许由caller cache或数组顺序猜测。
每项Metadata Fetch必须属于`owner_domain=libra,process_type=libra_run,process_id=<libraRunId>,
work_kind=product_metadata_observation`的Supporting Work；其`basis_digest`固定覆盖Run Execution Basis与完整
`MetadataFetchIntent@1`。同一Run Basis内`sourcePriority`唯一且从0连续，Work的idempotency key固定为
...
以上“当前Observation集合”只适用于`movie|series|jav`。Planner由上述Owner链构造
`MetadataObservationSelectionSnapshot@1`与完整`MetadataObservationSet@1`；对应Fact Commit必须同时提交二者。

`western_adult`不进入该集合，也不伪造`metadata.fetch`、Provider或NFO Observation。其Product Metadata basis固定为
既有Western chain的durable typed Results：`western.analysis.observe@1 → WesternAnalysisResult@1`及随后
`western.metadata.normalize@1 → ProductMetadataDraft@1`。Planner把同Run/Execution Basis的显式Event Result refs
冻结成`WesternProductMetadataBasisSnapshot@1`，Normalize Result必须逐字节等于Commit提交的Draft。Western
Media-Cast则引用`shared.face.reference.match@1 → PersonMatchEvidence@1`形成
`WesternMediaCastBasisSnapshot@1`；`matches=[]`是允许提交空relations Fact的正式Evidence，不等于无依据或扫描不到
Result。非空matches必须被Draft relations完整解释，不能静默丢弃。

两类Source Basis的精确Result引用统一写入Libra-owned`libra_product_fact_source_refs`，因此后来新增Observation、
Analysis或Match Result都不会改写已提交Fact所依据的basis。历史重建只允许按该relation连接其显式引用的
Work/Plan/Event/Result；这些Foundation rows在仍被Product Fact引用时禁止GC。Foundation Result在这里是正式、
被关系化引用的Source Evidence，不是隐藏业务Store或fallback；relation缺失、capability/schema/digest不符均为
integrity fault。

NFO与Provider对同一字段均有有效值时，Related NFO优先；TMDB只填补缺失Required字段，不默认覆盖用户已有
Sidecar内容。Observation不足、TMDB/JAV Provider无结果或Western分析无法形成Required字段时，保留明确Gap并
进入既有`temporarily_unplannable`、有界恢复或frozen语义；不得增加第三个隐藏fallback。Provider Adapter只负责
协议、认证和把单次响应规范化为`MetadataObservation`，不读取Acceptance Spec、不选择下一Provider、不创建
Supporting Work，也不写Libra Product Fact。
...
同一Reclaimer同时处理superseded与discard Cleanup Scope，但三类资格不能混用：completed Run只依据Arca Off-load
Completion Projection回收已经完成责任转移的Workspace引用；superseded Run只依据immutable replacement/terminal
revision与两轮无引用Evidence回收旧Workspace；discarded Run只依据Libra自己的Discard Commit
...
中已声明的NFO Related Material并形成Media-Cast/人物Evidence；People Management不得为“发现演员”直接扫描
Shelf Target Folder或读取Arca Material Binding。Arca把有界、分页、可游标恢复的
`OnDeckPersonEvidenceProjection`发布给People Management，Projection只含Shelf Entry/inventory revision、
关系显示名/角色、稳定Provider Person Identity（如有）、原始relation evidence digest与NFO observation digest，
不转移Media-Cast Owner。

Person注册成立后，Arca只能在Presentation Aftercare中按确定性Evidence发布新的Inventory/Media-Cast revision：

- 当前关系含相同namespace的exact稳定Provider Person Identity，且唯一命中一个active Person；或
- 该Person来自用户接受的Registration Candidate，Candidate origin evidence digest与当前未解析relation的
  evidence digest精确相同；或
- 已有personId只因正式Person Merge correlation需要解析到目标Person。

上述修正保持Primary/Related Material、Material Control和Canonical Content Identity不变，仅更新Arca拥有的
person relation snapshot并重新执行Presentation验证。只有姓名、Alias或人脸相似时不得自动改写关系；Beta
不提供手工Media-Cast编辑器，也不让People Management直接写Arca Store。无法形成确定性证明的关系继续以
`personId=null`保存，不伪造成系统故障或错误Person。
...
#### 6.8.2 Perception Acquisition与Resolution
...
Acquire必须冻结Normalize实际读取的Observation内容：每项Observation必须携带一个有界typed inline payload，
其字段只包含Normalize合同所需的来源值、原始rating/scale、观看状态、Identity hint和Provenance引用，不保存
完整Provider response。只有ID、revision或digest而没有可读取内容的Observation不满足合同；Normalize也不得
重新调用Provider取得第二份内容。`perception.source.acquire@1`是`pure_observation`，不得创建Artifact、写
Artifact Registry或隐藏任何Workspace副作用；Adapter必须通过字段投影和page budget使单项payload不超过
`16 KiB`、整页Result不超过`64 KiB`。单项必要DTO仍超限时以稳定contract failure结束本页且不推进cursor，
不能在Executor内部临时转存Artifact。
...
整体不成立。Acquisition创建时从Source current cursor head冻结`initialCursorRevision/value`：只有该Source
从未提交过cursor row时二者才分别为逻辑值`0`和`null`；已有head时revision冻结为当前真实revision。若新
Acquisition的source config revision与scope digest与head所属Acquisition兼容，初始value继承head的
`cursorOut`；不兼容时初始value为`null`以重新起扫，但expected revision仍保持当前真实revision，绝不把
revision重置为0。这里的“兼容”固定为source config revision与scope digest两者均和head所属Acquisition
精确相同，不由Adapter自行放宽。Acquisition创建必须在Owner事务中从Source head直接形成immutable initial
pair并建立active row；若head或active uniqueness在提交前变化则整次创建重试，不能发布基于过期head的
Acquisition。任一Acquisition的第一页必须使用其冻结的initial pair；同一Acquisition的后续page使用
上一页成功提交后的revision和`cursorOut`并CAS当前source head。`hasMore=false`的page同时终结Acquisition；
任何Record/Anchor/Relation/Cursor验证失败都回滚整页。
...
Resolution的Owner内数据准备与pure Decision固定为：

~~~text
CanonicalQueryHandle（含bounded typed query）
  → PerceptionResolutionInputAssembler读取Perception自己的Repository
  → PerceptionResolutionQuery
    + PerceptionResolutionRecordSet
    + PerceptionResolutionRuleSnapshot
  → perception.dedup.resolve@1（pure；禁止Store/Provider旁读）
  → PerceptionResolutionDraft
  → perception.resolution.commit@1
  → immutable Resolution revision/head
~~~

Input Assembler只按同一Rule Snapshot的candidate retrieval clauses取得完整候选超集，并冻结Record值、Identity
Anchor、Provenance以及与候选有关的既有`duplicate_of|supersedes|retracts`关系；它不得排序winner、解决冲突或
生成duplicate relation。Resolver只消费三份digest-bound typed input。候选超过Rule声明的bounded上限、
Snapshot不完整、Rule不可执行或digest不匹配时必须以稳定contract failure结束，不能把输入缺失伪装成
`not_found`，也不能旁读Store补齐。

相同`queryInputDigest + recordSetDigest + ruleDigest`已有current Resolution时，Facade可以直接返回已提交结果；
...
Person Registration、Merge和Reference Image Maintenance分别拥有自己的输入、用户确认与完成Decision：
...
Beta Person Registration只有两条正式用户结果路径：

1. **用户直接注册**：People Command Facade消费一份完整`DirectPersonRegistrationDecision`，在一个People
   transaction中建立Person首revision、Alias与可选Provider Identity。Reference Image不是该Decision的成员，
   也不是Person成立的前置条件。
2. **系统从On-deck NFO发现**：Arca按`6.6.2.1`发布`OnDeckPersonEvidenceProjection`；现有
   `PeopleEvidencePlanner`每日补偿扫描并按cursor分页消费尚未处理的Evidence，通过
   `people.registration_evidence.observe@1`形成typed Registration Evidence。exact稳定Provider Person Identity
   形成strong Candidate并可由系统规则自动接受；只有姓名/Alias/图片/人脸等弱Evidence时形成open Candidate，
   必须由用户接受或忽略。cursor保存在该People-owned Supporting Work的Foundation progress中，不新增People
   Canonical Object/Table；相同projection revision/evidence digest不得重复建立Candidate或Person。

两条路径都只建立People Person Fact，不修改Media-Cast。系统发现路径不授权People读取Arca Store或物理NFO；
Signal只缩短延迟，People自己的周期cursor与幂等Evidence receipt保证Signal丢失后仍可恢复。
...
Reference Image Maintenance输入，不能在Person Commit中伪造成已导入Reference Asset。

Reference Image Maintenance是Person注册之后的独立事务链：

~~~text
用户选择一张本地图片
  → people.reference_asset.import@1（导入People Workspace/Artifact Registry）
  → shared.face.embedding.compute@1（single-reference模式）
  → 必须且只能得到1张Face / 1个Embedding
  → PeopleReferenceMaintenanceDecision(add_image)
  → people.reference_fact.commit@1
  → Reference Asset + 内部Reference Face + Person Reference revision原子成立
~~~

零张或多张Face、Artifact digest不一致、model ref不匹配或Person/reference revision stale均使本次操作整体失败；
已注册Person保持不变，未提交Artifact由既有People Workspace reclamation回收。用户释放Reference Image时提交
`release_image` Decision，People事务同时把该Asset及其唯一派生Face置为released并发布新Reference revision；
二进制Artifact在无引用后由既有GC回收。没有独立的Face add/release命令，也不允许用户选择内部Face ID。

Person成立但从未成功提交自己拥有的Reference Image是合法状态：Person Store中的`current_reference_revision`
为SQL `NULL`，`PersonReferenceProjection.currentReferenceRevision`也必须为JSON `null`。该字段只表示查询
Person自己的local Reference pointer；没有Merge contribution时`contributions=[]`，存在正式Merge correlation时
仍可包含其他owner的`inheritedReadOnly` Contribution。它不同于“曾有local Reference、当前active集合为空”：
后者仍有正整数current Reference revision，并携带该Owner current revision及空集合的Contribution。Reference Projection的稳定revision/digest是People域内技术checkpoint，
由能够改变其semantic basis的Person Registration、Merge或Reference事务同步维护；Query和HTTP GET不得为了
补建或推进Projection revision产生写入。

“联网搜索某演员图片供用户挑选”不是Registration、Candidate或Reference Fact Commit的一部分，Beta不实现；
Beta Reference Maintenance只接受用户明确选择的本地上传图片。
...
| Procurement Run | Procurement | eligible Field Material出现且无冲突时，按6.3.2的直接父目录原子组和256项上限确定性建立全部可创建Run；多Run可并存，失败Basis不忙循环 |
...
| Person operations | People Management | 每日/Signal增量消费Arca On-deck Person Evidence并生成Candidate；strong Candidate可按系统规则自动接受，weak Identity只等待用户；direct注册与Reference Image上传只由用户命令触发 |
...
| Direct Person Registration | 用户以完整People Decision直接建立Person首revision的独立事务；不经过Candidate，也不要求Reference Image | 5.9.4、6.8.3 |
| Reference Image Maintenance | Person成立后独立导入/释放一张用户可见Reference Image，并原子建立/释放其唯一内部Reference Face的People Process | 3.7.2、6.8.3 |
| On-deck Person Evidence Projection | Arca从当前Inventory/NFO人物关系发布、供People按cursor消费的有界只读Projection；不转移Media-Cast Owner | 6.6.2.1、6.8.3 |
| Beta Product Metadata Source Order | Movie/Series按Related NFO优先、TMDB补缺，JAV使用JAV Provider，Western Adult使用Libra自有分析的固定生产来源规则 | 6.4.7.1 |
...
这里的“一次Runtime tick”只定义单个Event的原子推进语义，不表示进程内全局并发为1。Execution Runtime
Host可以同时持有多个不同Event的技术执行句柄；Beta默认`maxInFlightEvents=16`，它只是防止Promise、内存与
外部句柄无界增长的进程安全上限，不是Resource Capacity。每个Event仍必须独立取得完整Permit bundle；相同
resource key是否串行、不同resource key是否可重叠，唯一由Resource Governor当前Profile决定。Host停止时先
停止租赁新Event，再等待全部in-flight Event安全收口；不得遗留第二个executing Attempt或用进程退出伪造terminal。
...
commit payload contract/digest、typed result contract、commit idempotency key与Event Fence。这里的
`commitPayload`固定为Capability合同声明的全部typed input与typed parameter中，排除当前Commit Handle本身、
ExecutionContext、Permit及Runtime envelope后剩余的canonical object；`payloadDigest`只计算该对象，不能把
携带`payloadDigest`的Handle再次纳入自身digest。对于`perception.record.commit@1`，`commitPayload`恰好是
`PerceptionAcquisitionCommitDraft`。
...
- `fingerprintAlgorithm=middle-256k-sha256`、`fingerprintVersion=1`。每次Observation对每个Regular File读取
  正中间最多`262144`字节并计算SHA-256；这是Physical Material的正式bounded content fingerprint，
  不提供全文件逐字节完整性证明；
- `sampleLength=min(sizeBytes,262144)`，`sampleOffset=floor((sizeBytes-sampleLength)/2)`；零长度文件固定
  `sampleOffset=0,sampleLength=0`并使用空字节SHA-256。任何Physical Material Identity观察或重验都不得
  读取超过`262144`字节，读取成本不得随文件大小增长；
- `BoundedFingerprintProfile@1`固定为`profileRef=middle-256k-sha256@1,revision=1,algorithm=middle-256k-sha256,maxSampleBytes=262144`，
  `profileDigest=SHA-256(JCS(完整value excluding profileDigest))`；Integration Port与Capability不得接受调用者改变采样位置、
  样本长度或算法；
- `inode`在typed JSON中使用无符号十进制字符串，避免JavaScript number精度差异；`materialKey`固定为
  `sha256(JCS({schema:"physical-material-identity@2",mountScopeId,inode,sizeBytes,fingerprintAlgorithm:"middle-256k-sha256",fingerprintVersion:1,contentFingerprint}))`
...
- 每次Observation统一重新计算bounded fingerprint，不建立“存量/增量”两套执行模式；新增、变化、不变和
  missing只是与上一轮Inventory对账后的结果；
- 每次采样前后都必须重验`mountScopeId+inode+size+mtimeNs+ctimeNs` stat fence。任一字段在采样期间变化，
  本次Evidence失效并defer/retry；受保护Effect/Commit前按同一有界算法重验，不得升级为全文件读取；
- 启动发现mount replacement、fingerprint冲突、inode不稳定或无法提供可信纳秒级stat时，该root不进入Ready，
  不能按路径生成替代Identity，也不能静默关闭Fingerprint；
- Fingerprint读取按volume resource分片、发布有界progress并服从Level 10的I/O容量；Field Observation Page最多
  256项，因此单个Page Event的Physical Material读取上限为`64 MiB`；两个上限任一先达到即提交当前页。

该Identity用于ShelfDeck业务中的高可信文件对象识别，不承担NAS bit rot、坏道、RAID一致性或档案级完整性证明。
同一filesystem object、相同size与中段样本相同而未采样区域发生变化时，Identity可以保持不变；这是明确接受的
残余风险，由NAS scrub、校验文件系统与备份体系负责。
...
Resource Demand只能由typed Adapter、Platform Resource Projection和Capability正式输入中的Handle解析为
`volume_read:<volume>`、`volume_write:<volume>`、`volume_mutation:<volume>`、`integration:<endpoint>`、
`cpu_heavy`、`sqlite_write`、`control_commit`、`encoder:<device>`、`ai_device:<device>`或`worker:<node>`等
正式key。禁止从Capability ref拼造`capability:*`资源；Capability identity不是设备、volume或容量事实。
...
三个排队点读取同一`DomainExecutionProjection`：
`priorityClass,localPriority,priorityRevision,supplyRole(expansion|completion)`。`supplyRole`不是新的Business
Priority：`expansion`表示该Work主要扩大尚待处理frontier，`completion`表示它消费已经terminal的上游Evidence
并形成当前Process的既定交付物或terminal收口。达到soft cap后Work Supply停止建立新的`expansion` Work
Attempt，但允许`completion` Work在hard cap以内逐个补入；达到hard cap后两者均不得突破。Safety Liveness与
Handoff reserved lane继续使用独立保留机会，不得借`completion`绕过Correctness Breaker。

Procurement的固定Projection为：Field Observation=`background_observation/local 0/expansion`；Evidence
Assessment=`normal_foreground/local 100/expansion`；Candidate Assembly=
`normal_foreground/local 200/completion`。因此已经形成resolved Unit时，Candidate Assembly可在兄弟Evidence
继续运行期间发布独立Handoff A Offer，但不把普通Procurement Run伪装成expedited或handoff acceptance。
...
| Commit Payload | Capability合同声明的全部typed input/parameter排除当前Commit Handle与Runtime context后的canonical object；是Commit Handle payload digest的唯一basis | 7.5.1 |
...
为使Composition Root能够在不取得Domain internal implementation的前提下完成Foundation接线，每个已接入
Execution Foundation的Domain可以在同一`public/index.js`暴露一个typed Foundation construction port。该port只
允许构造并返回本Domain拥有的Planner registration、Capability registration、input-binding projection、Process
service与Reconciler；不得暴露Repository、raw Store、Executor dispatcher、Event Runtime、Resource Governor或
generic `execute` authority。Foundation Runtime本身仍只由Composition Root创建。Procurement的正式端口为
`ProcurementExecutionRegistration`，方法固定为`createCapabilityRegistration`、`createProcessServices`和
`createPlanningRegistration`。
...
│  └─ ProcurementExecutionRegistration
├─ application/
...
`MaterialFieldManager`拥有Field Access Binding、versioned Profile Hint、Field Observation、`ExtractionPolicy@1`发布、Extraction
...
Candidate Package、完整Primary Input Manifest及逐Primary Physical Identity/size/Location Evidence组成的immutable
...
注入Run Coordinator、TriagePlanner和Procurement transaction participant；HTTP、Foundation或调用者不能注册、
覆盖或选择Rule。Registry upgrade只能追加revision并切换active pointer，不能删除本clean schema lineage已经发布的
revision。Run/Intent只冻结active Rule Snapshot及其`authorityDigest`，不冻结整个`registryDigest`；仅追加无关历史
entry且active tuple不变时不应使Admission Basis失效。
...
它也是`SubjectContinuityResolutionDecision@1`、`AcceptedIntakePayload@1`与`IntakeRejectionDecision@1`的唯一Application assembler：可以读取
...
计算N:M Binding Draft，不决定new/extension。Continuity Resolution只在Candidate/Material contract verification通过后形成；
任一正式Intake验证失败时，Coordinator按Handoff A closed precedence形成Rejection Decision，不伪造Resolution、Subject或global head变化。
...
`LibraCommandFacade.selectShelf(ManualShelfSelectionIntent@1)`只接受Routing unresolved Subject的一次性用户选择，
不改写Field Routing Policy；Command Receipt与对应routing Decision Basis同事务成立，后续Resolver和Routing
Decision Commit只验证该Receipt。`AcceptanceSpecResolver`只按ready Basis中的`contentProfile`选择唯一Profile
Rule Set：未声明`rating`时直接使用`baseRequirements`；声明时`found`选择对应1..5分支，`not_found`选择
`no_rating`分支。每个分支已经携带完整六类Requirement，Resolver不得partial merge、补默认值、查询Provider或
把Capability availability写进Spec。
...
`ArcaShelfFacade`同时发布只读`ShelfRoutingProjectionQuery@1`与`ShelfStandardProjectionQuery@1`。前者按
调用方给出的`1..64`个`shelfId`返回同序、逐项typed的`ShelfRoutingTargetProjection@1`；后者只按精确
`shelfId + routingProjectionRevision + standardRevision + standardDigest`返回
`ShelfStandardProjectionReadResult@1(found|not_found|stale|unavailable)`。两类Query都只从Arca Owner rows
重建，不触发刷新、不写Arca Store，也不把Repository row、Template Store或Placement Policy暴露给Libra。
Libra把返回值冻结进自己的Decision Input；它不能在Routing或Spec事务中直接读取`arca_*`表。Arca Projection
在Libra提交后发生变化，只会使后续Run Creator/Freshness Reconcile拒绝旧Basis并建立新Decision，不反向改写
历史Routing Decision或Acceptance Spec。
...
Presentation Assessment同时负责读取Arca当前Inventory中已声明的NFO Reference，并通过既有
`ArcaInventoryQueryFacade`发布分页`OnDeckPersonEvidenceProjection`。它不注册Person；People注册后需要修正
On-deck Media-Cast时，仍由同一个Aftercare Coordinator按`6.6.2.1`建立确定性Presentation修复，并通过既有
Aftercare Inventory Commit发布新Inventory revision。该路径不新增第四个Aftercare维度或跨Domain Store访问。
...
│  ├─ PerceptionResolutionInputAssembler
│  └─ PerceptionResolver
├─ planning/
...
People Management只维护Person、Alias、Provider Identity、Preference、Reference Image、内部派生Reference Face和候选/合并事实。
...
`PersonRegistrationCoordinator`明确区分direct registration与Candidate acceptance；前者消费完整用户Decision，
后者消费精确Candidate revision，两者共享People Owner事务纪律但不互相伪装。`PeopleEvidencePlanner`只从
Arca公开Projection或正式Provider hint准备Evidence，不打开NFO/媒体文件。`ReferenceAssetCoordinator`只接受
用户已选定的上传图片，组织Artifact import、single-reference Face Embedding与一个Reference commit；它不
承担联网图片搜索，也不把Reference失败并入Person Registration事务。

`PeopleCommandFacade`的Beta正式命令至少为`registerPerson(DirectPersonRegistrationDecision)`、
`acceptRegistrationCandidate(PeopleCandidateAcceptanceDecision)`、`addReferenceImage(command)`和
`releaseReferenceImage(command)`；后两者都只返回同一个Reference Maintenance operation/ref。Facade不暴露
`addReferenceFace|releaseReferenceFace`，也不允许调用方传Embedding、model或内部Face ID；这些值只能由
`ReferenceAssetCoordinator`完成既有Workflow后固化进`PeopleReferenceMaintenanceDecision`。
...
Work/Event的technical lease只保护一次推进机会，不是业务事实，也不替代Resource Permit。Lease的初始
TTL必须覆盖正常的bounded critical section；Planner或其他异步推进可能超过初始TTL时，持有者必须通过
Foundation的renew/heartbeat延长同一lease fence，并在完成后释放。Lease续租只更新内存中的技术过期点，
不写业务表、不改变Work/Event状态；renew或release发现fence已过期、被替换或不存在时必须fail closed，
不能把陈旧lease当成成功释放。默认clean Runtime使用60秒安全TTL和有界heartbeat，具体值属于Level 8
实现合同；长操作仍必须可通过durable Work/Attempt恢复，不能依赖无限期租约。
...
`MaterialControlAuthority`只对Physical Material Identity执行`acquire|transfer|release|assert`；其写方法只接受
由Domain Owner签发的Control command或Responsibility Control Commit Handle。它不能：
...
同一public port另提供bounded read-only `getMaterialControlProjection/getMaterialControlProjections`，但不把
Foundation升级为业务Decision Owner。返回的`MaterialControlProjectionSnapshot`必须绑定精确`materialKey`、
Query结果、current `controlRevision/state/owner scope`或typed failure，以及`projectionDigest`：

- 查询成功且从未存在Control row：`state=uncontrolled,controlRevision=0`；
- current row为released：`state=uncontrolled`并保留该row的正整数controlRevision；
- current row为controlled且`ownerDomain=procurement|libra|arca`：分别投影为
  `procurement|production|finished_goods`；
- 查询不可用、row损坏、未知state或未知owner不能伪装成uncontrolled，必须返回
  `resultKind=unavailable`及稳定`failureCode/evidenceDigest`，使消费者保持unknown；Unavailable结果不得携带
  猜测的revision、state、owner或Region；
- batch Query按materialKey排序、最多500项，一次Repository查询完成，禁止N+1。

该Query只提供Foundation current Control Evidence；Procurement仍独占Extraction Eligibility Decision，Run
admission仍必须对`fx_material_controls.control_revision`执行最终CAS。
...
- Libra Production Planner通过`PlatformComputeRuntimePort@1.readDeviceSnapshot(PlatformComputeDeviceQuery@1)`
  取得已enabled且probe有效的`MediaExecutionDeviceSnapshot@1`；Port只从
  `platform_compute_devices + platform_compute_device_probes`形成typed Snapshot，不返回Device driver/config、
  不发Permit且不选择业务方案。Planner选择并冻结Snapshot，Resource Governor随后只按Plan的resource key发Permit；
...
Procurement CandidatePublisher
  ...
  → Libra Intake Acceptance
      rejected: IntakeRejectionDecision + RejectionReceipt
                + LibraCandidateRejectedMessage atomically; no Subject/head/Control transfer
      accepted: Subject + Libra Binding + Control transfer + receipt atomically
  → Procurement consumes Accepted or Rejected typed terminal message
      and atomically closes Reservation/delivery responsibility
~~~
...
不能写Libra Subject。Rejected consume只把对应Candidate Delivery置为rejected，并把精确Reservation成员释放为
`handoff_rejected`；Procurement继续拥有这些Physical Material的Control，后续是否形成Retry必须走已有Procurement
规则，不能由Libra反向启动Run。Accepted与Rejected消息对同一Offer互斥，迟到的相反终态必须稳定拒绝。
...
`OffloadCompletionPort.read(OffloadCompletionQuery@1)`是Arca对Libra开放的唯一完成事实读取口，只返回versioned
`OffloadCompletionReadResult@1(found|not_found)`；它不遍历Shelf Entry、不触发On-deck、不写Libra Store。Neutral
Signal只携带Package/Projection ref用于快速wake；Signal丢失时Libra仍按Package/Run历史周期查询同一Port。

Handoff B Acceptance在同一SQLite Unit of Work中使用`ProductDeliveryPort`的read-only
`acceptance_fence` participant重验Run/Spec/Package/Delivery/Control；它只返回typed Fence，不暴露或写Libra
Repository。SQLite writer serialization保证Fence通过到Arca Custody/Control transfer commit之间，Libra Run transition
不能插入。Arca Accepted事务仍只写Arca/Control/Foundation；随后typed Outbox由Libra Lifecycle complete消费。
...
| Libra | Subject、Domain-local Binding、每Field Routing Policy head/Target/Assessment/Decision、Decision Basis、Acceptance Spec、Run Admission head/Libra Run revision/Discard Decision、Production Material/Episode Delivery Manifest、Workspace revision/Material reference/Cleanup Scope、Product Fact、On-deck Package/Fact/Artifact/Material/Off-load Context relation、delivery receipt |
...
| People Management | Person、Alias、Provider Identity、Preference revision、Reference revision/Asset/内部Face、registration/merge candidate、merge record |
...
逐表合同显式使用机器`rowMutability`时只允许两种closed值：

- `append_only`：row插入后禁止任何UPDATE；后续语义只能追加新revision/relation row；
- `cas_lifecycle`：允许合同逐项枚举的一次或多次expected-state CAS transition；未被列入transition write set的
  列均为write-once，进入terminal state后除相同Evidence幂等重放外禁止再次UPDATE。

“terminal字段写后锁定”“历史事实不覆盖”或某些列write-once不能被materializer推导成整表`append_only`。
本次合同中`proc_run_materials`与`proc_candidate_deliveries`使用显式`rowMutability=cas_lifecycle`消除机器歧义；
Repository注册对这两张表只按各自声明允许列出的CAS，不能按自然语言中是否出现`immutable`封禁合法transition，
也不能把该声明外推成其他表或全局UPDATE放行。后续若为另一张表新增同row lifecycle machine contract，必须在
该表自己的bounded设计中显式声明，不从这两张表继承默认值。

#### 8.5.4 Canonical transaction boundaries

以下提交必须在一个SQLite事务内整体成立：

Material Field注册与Profile Hint修订沿用Procurement现有owner-local Admin Command UoW，不新增下表的顶层
Canonical Transaction：注册事务必须原子写`proc_material_fields`、Access revision、Extraction Policy revision、
Profile Hint revision 1，以及既有`fx_command_receipts`、`fx_commit_markers`、`fx_audit_records`；因此注册
exact read set固定为`[proc_material_fields,fx_command_receipts]`，
exact write set固定为`[proc_material_fields,proc_field_access_revisions,proc_extraction_policy_revisions,
proc_field_profile_hint_revisions,fx_command_receipts,fx_commit_markers,fx_audit_records]`。未传Hint时在计算Command
`requestDigest`前规范化为显式`mixed`。Profile修订事务的exact read set为目标
`proc_material_fields`、expected `proc_field_profile_hint_revisions`及`fx_command_receipts` idempotency row，
exact write set固定为`[proc_material_fields,proc_field_profile_hint_revisions,fx_command_receipts,
fx_commit_markers,fx_audit_records]`；URL/body
`fieldId`、expected revision/current pointer与idempotency receipt必须在任何写入前验证。同key同规范化payload返回
原typed Result，同key不同payload冲突；同值修订是稳定no-op，不追加revision、不切pointer或推进
`updated_at_ms`。该应用事务不写Outbox。
...
| Field Observation Page Commit | compact `ObservationPageCommitResult@1`/Receipt（只含游标、entry count、边界和digest）+ current Field Profile Hint pointer/exact revision row验证 + current Field Observation head CAS + immutable page header revision + Procurement-owned `proc_field_observation_entries` rows + current Material physical projection（新/Reality或Binding变化才置`unknown`，不变材料不写Eligibility列）+ durable typed Result + commit marker；全部同事务成立；同一Work全部Page使用同一Hint Snapshot，Hint current pointer变化时旧Work后续Page整页rollback；Page不保存完整文件数组，不计算最终Eligibility/Region，也不发布Outbox |
| Field Eligibility Reconcile Commit | bounded `ExtractionEligibilityDecisionBatch` + exact Material Key Change Set + current Field/Access/Policy/Selection/Reservation/Control facts和既有Eligibility revision CAS + 同事务重读验证Material-local basis；只更新Decision或Basis实际变化的Field Material current Eligibility/Control Projection，unchanged rows为no-op且不执行SQL UPDATE；更新后的rows是唯一durable output，事务只返回typed summary，不写Event Result、commit marker或Outbox；全局terminal Observation revision只作审计输入，不能单独使全Field失效 |
| Procurement Run Admission | complete `ProcurementRunExecutionBasis@1` + prospective Run identity + `ResponsibilityControlCommitHandle` + current Field/Profile Hint/Access/terminal Observation/Policy/active Triage Rule authority/Material/Selection/Control fence + Run及relationized Basis rows + 全部`run_selection` guard + 对全部成员的同Field Procurement Control acquire/assert + durable `ProcurementControlReceipt` + `fx_command_receipts` + commit marker；这是Run Creator的短Owner transaction，不建立Supporting Work/Plan/Event且不申请Resource Permit；Profile Hint Snapshot必须逐字节命中current Field pointer、精确immutable revision row及terminal Observation冻结值；每个Selection member必须从同事务current Field Material row冻结完整Physical Identity四元组与size，写入`proc_run_materials`并纳入Basis member digest，不能只保留不可逆materialKey/摘要；Triage Rule必须由注入的Procurement Registry解析并匹配当前active snapshot，不能信任调用者值；`hasOutbox=false`，全部事实/Control/Command Receipt全有或全无，禁止分批Control后补建Run |
| Procurement Candidate Publication | complete `CandidateDraft@1` + exact Run Basis/Triage Rule/Structure Unit/Identity/Manifest Draft fence + Run `candidate_package_revision_head` CAS + immutable Candidate Package + final Primary Input Manifest及N:M Episode Claim/完整Related Material Reference relation + package members是Run Selection精确非空子集 + 成员`run_selection → candidate_delivery` + 从final Package唯一派生的`CandidateIntakeAcceptanceBasis@1`和stable `offerId` + open Handoff Offer + durable typed Result/commit marker + `ProcurementCandidateOfferAvailableMessage@1` Outbox；final Manifest与Candidate Primary relation逐项复制immutable Run member的完整Physical Identity/size，并把它们纳入member/package/delivery digest；CommitParticipant只消费Draft/Handle，不旁读Foundation Event Result或Provider；Run revision head、Package、Manifest、Relation、Reservation、Offer、Result/marker与Outbox全有或全无；Canonical Transaction的Procurement domain write participant固定为`proc_procurement_runs`、`proc_candidate_packages`、`proc_candidate_season_continuity_claims`、`proc_candidate_primary_materials`、`proc_candidate_primary_material_episode_claims`、`proc_candidate_related_references`、`proc_candidate_deliveries`、`proc_run_materials`，不得从概括性“Run fence/Manifest/Relation”文字推断或漏表；Related relation必须逐列保存`RelatedMaterialReference@1`的完整Physical Identity、association Evidence和reference digest，不能把Foundation Event Result或current Material row当成历史Package payload |
| Procurement Run Seal | `ProcurementRunSealDecision@1` + Run expected state revision/basis CAS + sealed outcome/逐成员terminal Evidence/aggregate Evidence/finished time + 未成Package成员`run_selection → released`，保留`candidate_delivery`与全部Procurement Control + durable typed command receipt/commit marker；这是Coordinator收口terminal Work Result的短Owner transaction，使用`fx_command_receipts`而非伪造Event Result，不建立Work/Plan/Event且不申请Permit；candidate reservation、released member及seal Evidence的三项digest按8.6.18唯一公式形成并可由Owner rows重建；`hasOutbox=false` |
...
| People Candidate Acceptance | current Candidate revision/payload digest CAS + Candidate terminal `accepted` revision + Person create或Merge完整fact set +必要Preference resolution + merge correlation + 所有受影响Person的Reference Projection checkpoint + durable typed result + commit marker + People-internal Outbox |
| Direct Person Registration | complete DirectPersonRegistrationDecision + Person首revision + Alias/Provider Identity + 初始Reference Projection checkpoint + durable command result + command receipt + People-internal Outbox；不包含Reference Fact |
| People Reference Image Commit | PeopleReferenceMaintenanceDecision + Person/reference revision CAS + Asset/唯一派生Face同时active或released + new Reference revision/head + 所有受影响Person的Reference Projection checkpoint + durable typed result + commit marker + People-internal Outbox |
| Procurement Retry Intent Commit | complete `ProcurementRetryIntent@1` + failed sealed Run/Basis与精确released member scope + current Field/Profile Hint/Access/terminal Observation/Policy/active Triage Rule admission head + Eligibility/Binding/Selection/Control precondition + `open` intent + durable `ProcurementRetryIntentReceipt`/commit marker + 唯一`ProcurementRetryIntentAvailableMessage` Outbox；Profile Hint必须命中current revision及terminal Observation Snapshot；相同`fieldId+idempotencyKey`重放原结果 |
...
| Rule Template Publish | immutable Template revision + all currently bound Shelf effective Standard revisions + Template/Shelf current head switches + 每座受影响Shelf的routing projection revision/digest CAS + Outbox |
| Libra Decision Basis Commit | complete `DecisionInputSet@1` + exact pre-CAS `SubjectDecisionHeadSnapshot@1` + Subject/current Decision head CAS + immutable Decision Basis revision + relationized typed input snapshots + durable `DecisionBasisRevision@1`/commit marker；manual selection variant同事务写Command Receipt；Routing与Spec使用同一事务但不同`basisKind`，不创建Routing Decision、Acceptance Spec或Libra Run；`hasOutbox=false` |
...
| Libra Workspace Material Reference Commit | complete `LibraWorkspaceMaterialReferenceDecision@1` + expected Workspace/Reference revision + exact `WorkspaceMaterialHandle`/role-aware verification Evidence + current Run fence + immutable Reference revision + Workspace revision CAS + durable `LibraWorkspaceMaterialReferenceResult@1`/commit marker；closed operation为`attach_working|promote_to_product_staging`，同一handle不得同时处于Working与Product Staging，`hasOutbox=false`；`released`只由持有删除/absence Evidence的Workspace Cleanup Commit追加，不能借普通Reference command绕过清理合同 |
...
| Shelf Deregistration Commit | Shelf/Shelf Entry administrative terminal facts + Shelf routing projection revision/digest CAS + precise Control release + release receipt |

`PBF-22`对四项既有Procurement Canonical Transaction的machine contract只扩充read fence，不新增顶层
Transaction、variant或write participant；materializer必须生成以下精确表集：

- `helix.transaction.field-observation-page-commit@1`
  `readTables=[proc_material_fields,proc_field_profile_hint_revisions,proc_field_access_revisions,
  proc_field_observations,proc_field_observation_entries,proc_field_materials,fx_supporting_works]`；
  `writeTables=[proc_material_fields,proc_field_observations,proc_field_observation_entries,proc_field_materials,
  fx_event_result_bindings,fx_commit_markers]`，`outboxRequired=false`；
- `helix.transaction.procurement-run-admission@1`
  `readTables=[proc_material_fields,proc_field_profile_hint_revisions,proc_field_access_revisions,
  proc_field_observations,proc_extraction_policy_revisions,proc_field_materials,proc_procurement_runs,
  proc_run_materials,fx_material_controls]`；
  `writeTables=[proc_procurement_runs,proc_run_materials,fx_material_controls,
  fx_material_control_revisions,fx_command_receipts,fx_commit_markers]`，`outboxRequired=false`；
- `helix.transaction.procurement-retry-intent-commit@1`
  `readTables=[proc_procurement_runs,proc_run_materials,proc_material_fields,
  proc_field_profile_hint_revisions,proc_field_access_revisions,proc_field_observations,
  proc_extraction_policy_revisions,proc_field_materials,fx_material_controls]`；
  `writeTables=[proc_procurement_retry_intents,proc_procurement_retry_intent_materials,
  fx_event_result_bindings,fx_commit_markers,fx_outbox]`，既有`outboxRequired=true`及唯一Procurement consumer不变；
- `helix.transaction.procurement-retry-admission@1`
  `readTables=[proc_procurement_retry_intents,proc_procurement_retry_intent_materials,
  proc_procurement_runs,proc_run_materials,proc_material_fields,proc_field_profile_hint_revisions,
  proc_field_access_revisions,proc_field_observations,proc_extraction_policy_revisions,
  proc_field_materials,fx_material_controls]`；
  `writeTables=[proc_procurement_retry_intents,proc_procurement_retry_intent_materials,
  proc_procurement_runs,proc_run_materials,fx_material_controls,fx_material_control_revisions,
  fx_event_result_bindings,fx_commit_markers]`，`outboxRequired=false`。

四项transaction的`participants[].tables`继续只列各自既有write tables；
`proc_field_profile_hint_revisions`是Procurement read-only fence，禁止出现在writeTables。共同rollback invariant新增：
current Profile pointer、精确Hint revision、Observation Snapshot或Run/Retry关系化tuple任一不一致时，全部domain/
Foundation writes为零；成功后历史Result只从已冻结Snapshot恢复，不重读current Field。
...
- 上述Foundation participant可按职责读作2张Material Control Authority表加3张commit/result/outbox表；该逻辑分组
  不改变物理participant、Owner或总表集，完整事务仍固定为10张Libra加5张Foundation、合计15张；
...
- new Subject分支要求global head CAS、Libra分配的全新`subjectId`和nullable identity pointer；extension分支额外
  要求唯一target Subject `active`且`intake_revision/current continuity set/current episode scope`逐项CAS；两分支都把
  `AcceptedIntakePayload.sourceProvenance`完整写入Intake Decision，new分支同时把`structureKind/contentProfile`写入
  Subject并固定`routingAnchorIntakeDecisionId=intakeDecisionId`，extension分支必须与既有Subject逐字节相等且不得
  改写该anchor；这些Libra-owned provenance是后续Routing的唯一Material
  Field/Candidate来源，不允许Routing回读`proc_*`；
- `ResponsibilityControlCommitHandle`必须固定`operationKind=transfer,ownerDomain=libra,receivingDomain=libra,
  transferPoint=handoff_a_accepted`；其`basisRef=accepted_intake_payload:{intakeDecisionId}`、
  `basisDigest=AcceptedIntakePayload.payloadDigest`，`bindingSetDigest/controlScopeDigest/expectedControlRevisions[]`
  分别逐字节等于Payload中的
  `bindingDraft.bindingSetDigest`、`controlTransferScope.controlScopeDigest`及全部Control item的
  `{materialKey,expectedControlRevision}`稳定集合，`receiptContract`固定为
  `{receiptSchemaRef:"SubjectAndTransferReceipt@1",controlRevisionSetSchemaRef:"libra.handoff-a-transferred-control-set@1"}`；
- Handoff A每项`expectedControlRevision`必须为正整数；其同revision historical post-state必须是由
  `procurement/material_field/{controlTransferScope.fieldId}`控制的available Projection，且重算的projection digest
  逐字节等于Payload中的`expectedControlProjectionDigest`，revision 0或无法恢复previous revision均fail closed；
- 每个Control participant必须执行exact expected revision CAS并建立一条`operation_kind=transfer`的
  `fx_material_control_revisions` row；新revision严格等于expected revision加1，
  `basis_digest=ResponsibilityControlCommitHandle.basisDigest=AcceptedIntakePayload.payloadDigest`、
  `commit_marker`等于本次Handoff A marker，from/to scope逐项等于Payload；全部Committed Control Projection按
  8.6.19唯一公式从同事务post-CAS `fx_material_controls` terminal row重算并与新revision的historical post-state
  逐字节相等，再共同形成Receipt的`controlRevisionSetDigest`，不得由调用方直接提供该摘要；
...
`Procurement Handoff A Acceptance Consume`不是新的Capability或Handoff；它是Procurement消费既有Handoff A
Accepted终态消息的Owner application transaction：

- exact input为`LibraCandidateAcceptedMessage@1`；`participants[domain=procurement].tables`固定为
  `proc_candidate_deliveries`、`proc_run_materials`，Foundation participant固定为`fx_inbox`；
  `writeTables/readTables`均为这3张表的精确并集；
- 事务以`offerId/candidatePackageId/packageRevision/packageDigest`重验open Delivery，再展开同Candidate全部且非空的
  `candidate_delivery` Reservation；每个member只允许expected state
  `candidate_delivery → transferred`并写`terminal_disposition=handoff_accepted`、
  `terminal_evidence_digest=receiptDigest`；本事务不得写Material Control，因为Control transfer已经在Libra Accepted
  canonical transaction成立；
- Delivery同事务执行`open → accepted`，写`handoff_decision_id=intakeDecisionId`、nullable Decision digest、
  Receipt ID/digest、`terminal_evidence_digest=receiptDigest`和closed time；Inbox保存
  `ProcurementCandidateAcceptanceClosureResult@1.closureDigest`后才能ack Outbox；
- 相同message/dedup重放必须从terminal Owner rows重建同一Result；若Delivery已经以相同Evidence accepted则幂等返回，
  已rejected、member scope不完整或任一digest不一致均稳定拒绝，不能覆盖终态、部分转移或重复操作Control。

`Handoff A Rejected`的机器Transaction Contract必须精确物化为：

- `participants[domain=libra].tables`固定为`libra_intake_decisions`、
  `libra_intake_rejection_reason_evidence`、`libra_handoff_a_receipts`共3张；
- `participants[foundation=execution-foundation].tables`固定为`fx_event_result_bindings`、
  `fx_commit_markers`、`fx_outbox`共3张；
- `writeTables/readTables`均为上述6张表的精确并集；只重验相同Offer尚无Decision、相同marker/result/outbox的
  replay及row-to-typed digest，不读取或写入Subject、Continuity、Binding、Material Control或任何`proc_*`表；
- Domain participant只接受Owner组装的完整`IntakeRejectionDecision@1`及匹配Handle；本Capability的
  `commitPayload`就是完整Decision，`DomainFactCommitHandle.payloadDigest=SHA-256(JCS(完整typed Decision))`，
  `resultSchemaRef=IntakeRejectionReceipt@1`；reason/evidence rows必须
  按closed precedence完整重建`IntakeStructuredRejection@1`，Decision row和Receipt必须分别重建同一Decision/Rejection
  digest；Foundation participant只接受固定`LibraCandidateRejectedMessage@1`；
- 任一participant失败时Decision、全部Reason/Evidence、Receipt、Result/marker和Outbox全部rollback；相同marker
  重放返回原`IntakeRejectionReceipt@1`，不得产生第二个Decision、Receipt或Outbox message。

`Procurement Handoff A Rejection Consume`不是新的Capability或Handoff；它是Procurement消费既有Handoff A终态消息的
Owner application transaction：

- exact input为`LibraCandidateRejectedMessage@1`；`participants[domain=procurement].tables`固定为
  `proc_candidate_deliveries`、`proc_run_materials`，Foundation participant固定为`fx_inbox`；
  `writeTables/readTables`均为这3张表的精确并集；
- 事务先以`offerId/candidatePackageId/packageRevision/packageDigest/acceptanceBasisDigest`重验open Delivery，
  再展开同Candidate全部且非空的`candidate_delivery` Reservation；每个member只能执行
  `candidate_delivery → released`并写`terminal_disposition=handoff_rejected`、
  `terminal_evidence_digest=receiptDigest`；Procurement Material Control不得释放或改revision；
- Delivery同事务写`state=rejected`及Decision/Receipt/digest终态，Inbox同事务标记consumed并保存
  `ProcurementCandidateRejectionClosureResult@1.closureDigest`；随后Outbox delivery才可ack；
- 相同message/dedup重放必须从terminal Owner rows重建相同Result；若Delivery已经以相同Evidence rejected则幂等返回，
  已accepted、member scope不完整或任一digest不一致均为integrity conflict，不能覆盖、部分释放或重新打开Run。

`Handoff B Rejected`的机器Transaction Contract必须精确物化为：

- `participants[domain=arca].tables`固定为`arca_acceptance_attempts`、`arca_acceptance_decisions`、
  `arca_handoff_b_receipts`共3张；`arca_acceptance_checks`只属于read set，用于在事务内重建并重验
  `acceptanceEvidenceSetDigest`；
- `participants[foundation=execution-foundation].tables`固定为`fx_event_result_bindings`、
  `fx_commit_markers`、`fx_outbox`共3张；`writeTables`是上述6张participant表，`readTables`额外包含
  `arca_acceptance_checks`；
- Domain participant只接受完整`ArcaAcceptanceRejectionDecision@1`及匹配Handle，重验Attempt仍为active、
  Offer/Package/Shelf/Standard/Placement逐项一致且同一Attempt尚无Decision；Capability的`commitPayload`就是完整
  Decision，`DomainFactCommitHandle.payloadDigest=SHA-256(JCS(完整typed Decision))`，
  `resultSchemaRef=RejectionReceipt@1`；
- accepted与rejected共用同一`acceptance_decision_id`唯一空间和同一Attempt terminal CAS；Rejected把Attempt置
  `rejected`并写finished time，但不写Custody、Binding、On-deck Run或Material Control；
- Foundation participant只接受固定`ArcaProductRejectedMessage@1`。任一participant失败时Attempt terminal、
  Decision、Receipt、Result/marker和Outbox全部rollback；相同marker重放返回原`RejectionReceipt@1`。

`Libra Handoff B Rejection Consume`不是新Capability或反向Handoff，而是Delivery Owner消费既有Rejected终态的
application transaction：exact input为`ArcaProductRejectedMessage@1`，Libra write participant固定为
`libra_delivery_receipts`，`libra_product_packages`只属于read set，Foundation participant固定为`fx_inbox`；事务重验Package仍为
同一published immutable digest且尚无相反终态，写一条可完整重建Message Decision/Receipt摘要的rejected Delivery
Receipt并消费Inbox，返回`LibraProductRejectionClosureResult@1`。同一message重放返回相同closure digest；Package已accepted或任一digest冲突稳定拒绝，不能
修改Package、重开Libra Run或写Arca Store。

`Libra Decision Basis Commit`、`Libra Routing Decision Commit`与`Libra Acceptance Spec Publish`共同形成
Subject接管后的唯一Decision前半链，但仍是三个独立事务：

- Decision Basis Commit的Libra participant固定为`libra_decision_basis_revisions`、
  `libra_decision_basis_inputs`、`libra_subject_decision_heads`，Foundation participant固定为
  `fx_event_result_bindings`、`fx_commit_markers`；普通variant的`writeTables`固定为这5张表。manual selection
  variant额外把`fx_command_receipts`加入Foundation participant/write/read set，Receipt的target固定为本次
  `DecisionBasisRevision@1`，相同idempotency key/request digest重放原Basis，不新增第二个Intent或Decision head。
  共同`readTables`额外包含
  `libra_subjects`、`libra_intake_decisions`、`libra_product_identity_revisions`、
  `libra_subject_episode_scopes`以重验Subject Snapshot；每份Basis必须把
  `DecisionInputSet.expectedDecisionHead`作为恰一条`decision_head_snapshot` input row保存：该Snapshot描述本次
  Basis提交前的Head，而不是提交后的current row。routing variant再读取
  `libra_field_routing_heads/libra_routing_policy_revisions/libra_routing_policy_targets`，spec variant再读取
  `libra_routing_decisions`。首次提交允许在
  `headState=absent,headRevision=0,headDigest=null`且三个pointer均NULL时验证没有current Head row，并为active
  Subject原子插入revision 1 head；后续只允许`headState=present`Snapshot与current row的revision/digest/三个
  pointer逐字节相等后CAS并加1。Commit只接受完整`DecisionInputSet@1`与匹配
  `DomainFactCommitHandle`；`basisRevision`为该Subject immutable Basis最高revision加1、无历史时为1，逐项保存全部typed input
  snapshot并返回`DecisionBasisRevision@1`。Basis row的`expected_head_snapshot_digest`必须等于这条input row的
  `input_digest`，且`expected_head_revision`等于Snapshot revision。相同
  `subject+basisKind+inputSetDigest`的semantic replay先从既有Basis及全部relation rows重建原
  `DecisionInputSet@1`并逐字节验证，然后返回既有Basis/Result，不再要求current Head仍停留在历史前置状态，也不推进
  head；relation缺失或digest不一致是integrity failure。新的
  routing Basis把head切为`currentRoutingDecisionId=null,currentDecisionBasisId=new,currentAcceptanceSpecId=null`，
  新的acceptance_spec Basis保留current Routing、切换current Basis并把current Spec置NULL；
- Routing Decision Commit的Libra participant固定为`libra_routing_assessments`、
  `libra_routing_decisions`、`libra_subject_decision_heads`，Foundation participant仍为Result/marker两张表，
  合计5张write table；`readTables`再包含`libra_decision_basis_revisions`、`libra_decision_basis_inputs`、
  `libra_subjects`；policy authority再加入`libra_field_routing_heads`、`libra_routing_policy_revisions`、
  `libra_routing_policy_targets`并要求Field Routing head仍等于Basis冻结Policy，manual authority则加入
  `fx_command_receipts`并要求Receipt逐字节引用当前Basis。两分支都先从Basis及relation rows完整重建并重算
  `DecisionInputSet@1/inputSetDigest`，再要求current Head恰为该Basis提交后的确定post-state：revision等于所存
  pre-CAS Snapshot revision加1，current Basis等于本Basis，current Routing/Spec均为NULL；Subject仍active且
  intake/content profile/structure revision未变；随后原子保存Assessment、resolved或
  unresolved Decision并切换current Routing pointer；`decisionRevision`为该Subject immutable Routing Decision最高revision加1、无历史时为1。
  head同时保留当前routing Basis并确保current Spec为NULL；
  `shelfId`只在resolved分支非NULL；unresolved reason只能取
  `RoutingDecision@1` closed set；
- Acceptance Spec Publish的Libra participant固定为`libra_acceptance_specs`、
  `libra_subject_decision_heads`，Foundation participant仍为Result/marker两张表，合计4张。它要求head当前
  Routing Decision与ready `acceptance_spec` Basis逐字节匹配，并从Basis及relation rows重建相同完整
  `DecisionInputSet@1`；current Head必须恰为该spec Basis提交后的确定post-state：revision等于其pre-CAS
  Snapshot revision加1，保留Snapshot中的resolved Routing、current Basis等于本Basis、current Spec为NULL。
  Subject与Product Scope未变后，才把Spec revision按
  同Subject immutable Spec最高revision加1、无历史时为1，随后原子保存完整`AcceptanceSpec@1`与切换current Spec pointer；`readTables`除4张write table外
  固定加入`libra_subjects`、`libra_subject_episode_scopes`、`libra_routing_decisions`、
  `libra_decision_basis_revisions`、`libra_decision_basis_inputs`；
- 三个事务都`hasOutbox=false`。`RoutingDecisionCommitResult@1`与
  `AcceptanceSpecPublicationResult@1`是Owner application transaction result，不是Catalog Capability Result
  family；相同marker重放返回原ID/revision/digest，不重算Decision或另建Spec；
- Arca Shelf/Standard只以`ShelfRoutingTargetProjection@1`或`ShelfStandardProjection@1`的immutable typed
  Query snapshot进入`DecisionInputSet@1`。Libra事务不得把`arca_*`加入read/write set。跨Domain Projection在
  提交后变更时，历史Basis/Decision/Spec保持不可变；Run Creator在创建Run前必须重新Query并逐字节比较
  `routingProjectionRevision/projectionDigest/standardRevision/standardDigest`，不相等则不创建Run并唤醒新的
  Decision Preparation。该Freshness检查不是跨Store CAS，也不允许回读旧Snapshot“补成最新”。

上述三项把本节Canonical Transaction由35项增加为38项；不新增Domain、Owner、Store、Handoff、Capability或
关系表，也不提前创建Libra Run/Workspace。

`PBF-13`把Level 6已经固化的Run→Workspace→Product Package→Discard/Reclamation责任链物化为以下机器合同：

- **Run Admission**的Libra participant固定写`libra_run_admission_heads`、`libra_runs`、
  `libra_run_revisions`、`libra_run_material_manifests`、`libra_run_material_members`、
  `libra_run_material_episode_claims`，replacement variant还写旧Run及其revision；Foundation participant固定为
  Result/marker两张表。事务读取Subject、Subject Decision Head、Acceptance Spec、全部current Material Binding/
  Episode relation、Material Control及同Subject non-terminal Run scope，先重建`LibraRunExecutionBasis@1`，再CAS
  Subject级Run admission head。初次logical expected head为revision 0且active scope set为空；提交后revision加1。
  `single` active scope set至多一项；`season`按episodeKey检查全部具有提交资格的`active|suspended|frozen` Run，
  任一重叠拒绝；所有structure还必须拒绝与另一具有提交资格Run的Primary/structural materialKey重叠，确保
  Discard的exact Input release不会释放其他Run仍使用的材料。replacement只允许同一Subject/Episode交付scope的
  旧`active|suspended` Run与已变化的Spec或Execution Basis，在同一事务把旧Run
...
- **Run Lifecycle Transition**写`libra_run_admission_heads + libra_runs + libra_run_revisions + libra_delivery_receipts + fx_inbox`及Result/marker，读取Run admission head、
  ...
  `ArcaProductAcceptedMessage@1`，同事务插入`libra_delivery_receipts`与`fx_inbox`并移除active scope；Arca事务
...
- **Workspace Admission**固定写`libra_workspaces`、`libra_workspace_revisions`、`fx_workspace_registry`及
  ...
  active Run/Basis、`libra_run_material_manifests`与`libra_run_material_members`，按Run的immutable Manifest ref验证
  manifest identity/member count/member set连续性，并只对完整`role=primary_payload` members的`sizeBytes`求和；由该值
  重算`inputPrimaryTotalBytes/requiredFreeBytes/requestDigest`后验证完整Space Evidence的workspace/run/root/snapshot/
  request/bytes/expiry。Episode Claim不影响空间字节，事务明确不读取`libra_run_material_episode_claims`。resolved root只在Platform
  participant内部用于Evidence生成，不能进入Decision或Libra row。首次建立固定在同一事务先插入
  `libra_workspaces(current_revision=1)`、再插入`libra_workspace_revisions(revision=1)`；aggregate current-head复合FK为
  `DEFERRABLE INITIALLY DEFERRED`并在commit时验证，revision→aggregate父FK保持immediate。任一步失败时aggregate、
  revision、Registry、Result与marker全部回滚，不允许关闭FK、占位revision、nullable后补或事务外预写。同一Run至多一个
  Workspace stream，`workspaceId`稳定派生且首次revision为1。`helix.transaction.libra-workspace-admission`的
  Libra `readTables`精确固定为
  `libra_runs`、`libra_run_revisions`、`libra_run_material_manifests`、`libra_run_material_members`及全部write tables；
  Platform read participant另含`platform_workspace_roots`。`libra_run_material_episode_claims`不属于该read set。
  Manifest/member重建、Primary size求和、space request/Evidence交叉验证和提交必须处于同一SQLite事务，禁止事务外
  预计算bytes或用Binding、Workspace、current Reality补值。**Workspace Material Reference Commit**固定写
  `libra_workspaces`、`libra_workspace_revisions`、`libra_workspace_material_refs`及Result/marker，读取
  `fx_workspace_materials`、Artifact分支所需的`fx_artifact_registry`、Run current fence和Decision携带的完整typed
  Verification；Foundation row必须逐字段重建
  `WorkspaceMaterialHandle@1`并重算handle/fence digest，不能信任caller或读取current Registry重造Handle。
  promote的完整`WorkspaceProductVerificationSnapshot@1`必须按Material role选择closed
  `media|artifact|structural`分支、匹配该Handle/Run且result=passed，并原样冻结进Reference row；
  transaction不读取`fx_event_result_bindings`，因为Foundation Result不得成为隐藏业务Store。Reference rows append-only并以max revision重建current，
  只允许`absent→working→product_staging→released`或`working→released`。Promotion必须引用已验证Product
  Evidence；同一handle任何时点只有一种current role，不能靠目录扫描或数组位置判断。
- **Deliverable Promotion**的Libra participant固定写`libra_product_packages`、
  `libra_product_package_materials`、`libra_product_package_material_episode_claims`、`libra_product_package_fact_refs`、
  `libra_product_package_artifact_refs`、`libra_offload_context_materials`、`libra_runs`；Foundation participant固定写
  `fx_material_controls`、`fx_material_control_revisions`、
  Result/marker/outbox。read set还必须包含Run Basis/Material Manifest及Episode relation、Workspace/current References、
  Foundation Workspace Registry/Material current fence、Material Bindings及Episode relation、Product Facts、
  Acceptance Spec。每个Product member显式声明
  `controlOperation=assert_existing_input|acquire_workspace_product`：前者必须逐项等于Run冻结Input并assert当前
  Libra Control，后者必须来自current Product Staging Reference并原子取得新的Libra Product Control；禁止从
  material handle种类猜分支。Off-load Context逐列冻结Arca可能接管的原Input Physical Identity、Location、Binding
  与Evidence。完整Package/relations、Control、Run delivery revision、Offer/Outbox及typed Result全有或全无；
  Package publication不把Run置completed，Handoff B Accepted consume才以Lifecycle contract终结Run。
- **Run Discard**写`libra_run_discard_decisions`、`libra_run_discard_receipts`、`libra_runs`、
  `libra_run_revisions`、`libra_run_admission_heads`、`libra_workspace_cleanup_scopes`、
  `libra_workspace_cleanup_members`、非空Scope对应的`libra_workspaces/libra_workspace_revisions`、Material Control
  current/revision及Result/marker/outbox。它从immutable Run Material
  Manifest形成exact original Input release set，从current Workspace Reference形成Cleanup member set；两集合不得从
  current Subject Binding、目录扫描或caller cache补齐。只有expected frozen revision可提交，Run terminal、active scope
  set移除、Input Control release、Cleanup Scope和Receipt全有或全无；物理删除继续由独立Cleanup工作完成。
- **Cleanup Scope Admission**固定写`libra_workspace_cleanup_scopes/members`、非空Scope对应的
  `libra_workspaces/libra_workspace_revisions`及Result/marker。Workspace在同事务从active进入reclaiming；no-op不改
  Workspace。`run_discarded`
  scope已经由Discard事务原子建立，不重复admit；`offload_completed`只消费Arca
  `OffloadCompletionReadResult@1`，要求Package/Run匹配、completion revision/digest稳定、24小时grace已满，并以两轮
  separated orphan/reference observation证明当前Workspace handle无其他active Libra引用；`run_superseded`只读取
  old Run immutable terminal revision与replacement relation，同样以两轮observation建立旧Workspace回收资格，防止
  Spec replacement遗留无人收口的Product。完整trigger snapshot与
  member eligibility Evidence都进入Owner rows；Signal只wake，不是输入事实。**Workspace Cleanup Commit**固定写
  Cleanup scope/member、Workspace/current revision、Workspace Material Reference release revision、Foundation
  Workspace Registry、必要的Material Control current/revision及Result/marker；每次只提交一个member，要求互斥的
  `WorkspaceMaterialDeletionEvidence@1|WorkspaceCleanupBlockingEvidence@1`与expected member revision/digest逐字节
  匹配。只有文件删除或already-absent Evidence成立才允许release Reference/Control；Blocking Evidence只把member/
  Scope置blocked并保留责任。全部member completed后才CAS scope/workspace/Registry为completed/reclaimed；中途崩溃只恢复同一Scope/member。

以上新增五项Canonical Transaction：Run Admission、Run Lifecycle Transition、Workspace Admission、Workspace
Material Reference Commit与Workspace Cleanup Scope Admission；既有Deliverable Promotion、Run Discard、Workspace
...
Canonical Transaction；该bounded修正落地时inventory保持112/97/177/43，后续`PBF-22`新增一张Procurement
Profile Hint历史表后当前总数为112/97/178/43。
...
不新增关系表、Capability、Result family或Canonical Transaction；该bounded修正落地时inventory保持
112/97/177/43，后续`PBF-22`只把当前table总数增至178。
...
Event Runtime validates Commit Handle and Fence
  ...
  → SqliteUnitOfWork
       ...
       Foundation writes commit marker / durable typed Event Result / contract-declared optional Outbox
  → typed Receipt / Domain Fact Result
~~~

CommitParticipant不是一个可被Planner任意调用的Store API，也不出现在Executor依赖中。它只能处理manifest
预声明的fact schema、Owner和Effect Class；Handle payload digest与合同定义的`commitPayload`不一致即拒绝。
...
Outbox是否参与Unit of Work只由本次精确Transaction Contract的`outboxRequired`决定，不由
`domain_fact_commit|responsibility_control_commit` Effect Class给出全局默认：

- `outboxRequired=true`时，Coordinator必须收到至少一条合法消息并装配Outbox participant；
- `outboxRequired=false`时，Coordinator必须接受零消息且不得装配Outbox participant或伪造空消费者消息；
- 通用Commit Coordinator必须先解析精确Transaction Contract，再校验Outbox cardinality；不能把generic
  Domain Fact Commit模板中的要求覆盖到声明为false的具体事务。

Field Observation Page Commit固定`outboxRequired=false`。其同一Workflow后续节点由durable typed Result解锁，
启动恢复和周期Field reconcile负责最终恢复；这两条通路都不需要为每页扫描制造Outbox。

既有`helix.transaction.domain-fact-commit@1`注册两个精确Libra Product Fact variants，而不是新增顶层Canonical
Transaction：

- `variant=libra_media_cast_fact@1`固定`outboxRequired=false`；Libra participant写
  `libra_product_fact_revisions,libra_product_fact_source_refs`，Foundation participant只写
  `fx_event_result_bindings,fx_commit_markers`；
- `variant=libra_product_metadata_fact@1`使用相同write set并固定`outboxRequired=false`；它额外验证
  `VerifiedArtifactManifest@1`、Manifest显式引用的`ArtifactManifestVerification@1` Result、对应
  `fx_artifact_registry` Handle与可选同Run Media Cast Fact，但不得产生Outbox；完整Manifest作为Product Fact
  Owner row的一部分同事务冻结，不能只保存不可逆digest；
- 两个variant的exact read set固定为`libra_runs,libra_product_fact_revisions,fx_supporting_works,fx_work_attempts,
  fx_workflow_plans,fx_plan_nodes,fx_workflow_events,fx_event_result_bindings`及全部write tables；
  `product_metadata` variant额外读取`fx_artifact_registry`。Observation variant只可按
  `MetadataObservationSelectionSnapshot@1`的显式Result ID读取；Western Product Metadata/Media-Cast variants分别只可
  按`WesternProductMetadataBasisSnapshot@1`或`WesternMediaCastBasisSnapshot@1`的显式Result ID读取；Artifact验证只可
  按同一Commit Payload内`VerifiedArtifactManifest.items[].verificationResultRef.resultId`读取。三类ref集合分别验证，
  禁止扫描无关Result、读取Provider/current文件Reality或把Foundation Result当成未声明的Fact来源。
...
head。相同commit marker只返回原Result；同marker不同payload、相同expected revision不同payload或Result/Owner row
缺失均稳定拒绝。generic Domain Fact模板不得再为这两个variant注入`fx_outbox`或要求伪造消息。

Field Eligibility Reconcile Commit不经Event Runtime或`DomainCommitRegistry`伪装成Capability commit。它由
`MaterialFieldManager`装配一个scoped `SqliteUnitOfWork`：Procurement current-fact participant只重读/写入
`proc_*`表，Foundation Control validation participant只批量重读`fx_material_controls`并核对Decision携带的
revision/digest或typed unavailable Evidence。双方都不能取得对方Repository或SQL；任一basis验证失败使该项
进入stale summary，任一事务错误使整批rollback。该只读Foundation participant不改变Control，不产生receipt、
marker或Outbox。

所有typed JSON digest使用同一Canonical Digest Rule：对合同明确命名的digest basis执行UTF-8 RFC 8785
JSON Canonicalization Scheme（JCS），再计算SHA-256并保存lowercase hex。一个字段不得包含在它自己的digest
basis中；其他被basis正常包含的嵌套digest字段不被删除。`fx_event_result_bindings.result_digest`精确覆盖
`result_json`中保存的typed Result value，digest列本身不进入JSON；Domain receipt表引用同一result value时
必须保存相同digest，禁止为同一Result定义第二套basis。
...
- digest使用`digest_algorithm + digest_hex`，首版算法固定为`sha256`、hex为64个小写字符；Physical Material
  的`content_fingerprint`继续遵守Level 3定义，不被普通payload digest替代；
...
Catalog digest核对；Composition Root随后执行Procurement Triage Rule Registry的active/append-only/digest与durable
Run/Retry引用核对；任一失败即拒绝可写启动。
...
| `fx_reconcile_cursors` | `owner_domain, reconciler_key, revision, opaque_cursor NULL, updated_at_ms` | `PK(owner_domain,reconciler_key)`；revision从1递增；opaque cursor为不超过`4 KiB`的bounded string或NULL；只保存Foundation调度位置，不保存或解释Domain Fact；每页Scope全部完成幂等reconcile后才CAS推进，崩溃前未推进时允许整页重放 |
| `fx_event_result_bindings` | `result_id PK, event_id FK, outcome_kind, result_schema_ref, result_json, result_digest, evidence_schema_ref, evidence_json, evidence_digest, effect_receipt_id, committed_at_ms` | terminal Event至多一个current result：`UNIQUE(event_id)`；两个JSON的UTF-8 JCS bytes各自≤`65,536`，大Payload必须只含typed handle；`result_digest=SHA-256(JCS(result_json解析后的完整typed Result，包含其中所有领域内digest字段))`，领域对象内部排除自身计算的digest不是Foundation storage digest；Observation Page Commit的`result_schema_ref`固定为`ObservationPageCommitResult@1`，`evidence_schema_ref`只保存同一批次的compact envelope与digest，不保存文件明细；文件明细唯一来源是Procurement-owned `proc_field_observation_entries` |
...
| `fx_commit_markers` | `commit_marker PK, effect_id, owner_domain, scope_type, scope_id, commit_digest, result_id FK, result_schema_ref, result_digest, committed_at_ms` | `commit_marker`全局唯一；Event驱动的`domain_fact_commit|responsibility_control_commit`其result字段必填并与对应`fx_event_result_bindings`同事务写入；唯一例外是非Event的Procurement Run Admission，其result字段为NULL，完整`ProcurementControlReceipt`由同事务`fx_command_receipts`承载并以marker scope/digest交叉验证；不能更新或删除 |
...
| `fx_workspace_materials` | `workspace_id FK, material_handle_id, material_key, endpoint_id, mount_scope_id, inode, fingerprint_algorithm, fingerprint_version, content_fingerprint, relative_path, digest_algorithm, digest_hex, size_bytes, reference_revision, owner_domain, process_id, root_handle_ref, access_scope, handle_schema_ref, handle_json, handle_digest, fence_digest, state(active|reclaimed), reclaimed_effect_id NULL, reclaimed_effect_receipt_digest NULL, reclaimed_at_ms NULL` | `PK(workspace_id,material_handle_id)`；`UNIQUE(workspace_id,relative_path)`；`rowMutability=cas_lifecycle`且只允许幂等filesystem cleanup effect以expected Handle fence执行一次`active→reclaimed`，三个reclaimed列在active时全NULL、terminal时全非NULL且write-once；`handle_schema_ref=WorkspaceMaterialHandle@1`、canonical JSON≤`4 KiB`、`handle_digest=SHA-256(JCS(handle_json typed value))`，全部热列与Handle逐字段一致，`fence_digest=handle_json.fenceDigest`；同一inode的hard-link location允许共享materialKey但必须是不同Handle/relative path；endpoint/mount/root handle必须等于Workspace Registry/root snapshot；完整Physical Identity由Workspace write effect在同一Effect commit按实际文件stat与中段最多256 KiB fingerprint形成，系统生成Artifact的完整digest只允许从原写入流形成且不参与`material_key`；路径必须通过containment validator；只有bytes关闭、fingerprint/stat/containment及独立Artifact digest完成且完整Handle row与Effect receipt同事务durable后Executor才能返回该Handle。Domain Cleanup Commit只读取该terminal row/Evidence并收口Libra Reference/Control，不再次删除bytes或改写本row |
...
| Table | Primary/core columns | Required uniqueness and hot indexes |
| --- | --- | --- |
| `proc_material_fields` | `field_id PK, name, status(active|deregistered), extraction_policy_id, extraction_policy_revision, current_access_revision, current_profile_hint_revision, current_observation_revision NULL, created_at_ms, updated_at_ms` | active name可重复但`field_id`唯一；注销Field保留历史row并置`deregistered`，不删除Run/Observation事实；Policy/Access/Profile Hint均显式FK；`current_profile_hint_revision`从1开始且永不为NULL，以`(field_id,current_profile_hint_revision)`显式FK指向`proc_field_profile_hint_revisions`；注册时Field aggregate先插入、Profile revision后插入，因此该current pointer FK固定`DEFERRABLE INITIALLY DEFERRED`；`current_observation_revision IS NULL`表示尚未提交任何Observation page，非NULL时以`(field_id,current_observation_revision)`显式FK指向`proc_field_observations`；该pointer与Observation→Field的循环FK均`DEFERRABLE INITIALLY DEFERRED`；`INDEX(status,field_id)` |
| `proc_field_profile_hint_revisions` | `field_id FK, revision, content_profile_hint(mixed|movie|series|jav|western_adult), hint_schema_ref, hint_digest, effective_at_ms, actor_id` | `PK(field_id,revision)`；`hint_schema_ref=helix://contracts/application-types/MaterialFieldProfileHintSnapshot/v1`；revision从1连续递增且row immutable；`hint_digest`逐字节匹配8.6.18唯一公式；注册未提供Hint时必须保存显式`mixed` revision 1，禁止NULL/空字符串/调用者默认旁路；同值修订不追加row；Field注销不删除历史；`proc_material_fields.current_profile_hint_revision`显式指向current row，`INDEX(content_profile_hint,field_id,revision)`只服务Field管理Projection，Triage不得按该索引扫描或旁读current值 |
| `proc_extraction_policy_revisions` | `extraction_policy_id, revision, policy_schema_ref, policy_json, policy_digest, effective_at_ms` | `PK(extraction_policy_id,revision)`；首版schema只允许`ExtractionPolicy@1{includedDirectories[],excludedDirectories[],allowedExtensions[],minimumSizeBytes,excludedMaterialKeys[]}`，JSON UTF-8 JCS bytes≤`16 KiB`且digest覆盖完整typed value；目录/extension/key排序、数量、normalization与precedence严格遵守5.3.1，`additionalProperties=false`；immutable；Material Field引用精确revision而非可变配置 |
...
| `proc_field_observations` | `field_id FK, revision, observation_id, field_observation_work_id, access_revision, content_profile_hint, profile_hint_revision, profile_hint_digest, page_ordinal, expected_revision, cursor_in, cursor_out, page_digest, fact_digest, commit_marker, result_digest, observed_at_ms, completed` | `PK(field_id,revision)`、`UNIQUE(observation_id)`、`UNIQUE(field_id,observation_id)`供same-Field composite FK、`UNIQUE(field_observation_work_id,page_ordinal)`、`UNIQUE(commit_marker)`；`field_observation_work_id`显式FK引用`fx_supporting_works.work_id`，`(field_id,access_revision)`显式FK引用`proc_field_access_revisions`，`(field_id,profile_hint_revision)`显式FK引用`proc_field_profile_hint_revisions`，`commit_marker`显式FK引用`fx_commit_markers.commit_marker`；revision从1开始且跨Observation Work持续递增；同一Work的pageOrdinal从0连续递增，每页使用不同observation_id且该ID只在该页重放时复用，且三项Profile Hint列逐字节相同；`expected_revision=revision-1`，首个revision的expected为0；`completed=1`恰好对应terminal page；Page只保存compact receipt，不保存文件明细，明细由同一`observation_id/page_ordinal/entry_ordinal`在`proc_field_observation_entries`恢复；`page_digest`必须匹配compact ObservationPageCommit Result，`result_digest`必须匹配typed Result；历史Page通过`commit_marker → fx_commit_markers.result_id → fx_event_result_bindings`恢复，后续Material或Profile current-row更新不改变该链；该Observation revision存在时对应Result Binding/Evidence不得GC或压缩；`observation_id/work/page`冲突重放必须拒绝；`INDEX(field_id,completed,observed_at_ms)` |
| `proc_field_materials` | `field_id FK, material_key, mount_scope_id, inode, fingerprint_algorithm, fingerprint_version, content_fingerprint, endpoint_id, access_revision, mount_scope_revision, size_bytes, mtime_ns, ctime_ns, fingerprint_verified_at_ms, current_location, binding_revision, reality_digest, provenance_digest, last_snapshot_digest, last_observation_id, eligibility_revision, eligibility_state, eligibility_reason_code, eligibility_basis_digest, eligibility_field_status NULL, eligibility_observation_revision NULL, eligibility_policy_revision NULL, selection_basis_digest NULL, control_projection, control_projection_revision NULL, control_projection_digest NULL, eligibility_reconciled_at_ms NULL` | `PK(field_id,material_key)`；Identity components unique within Field；`(field_id,last_observation_id)`显式复合FK引用同Field唯一Observation page；Fingerprint evidence只在mount/inode/size/mtime/ctime全部一致时有效；state enum为`unknown|eligible|ineligible`，Control enum为`unknown|uncontrolled|procurement|production|finished_goods`；新row固定`binding_revision=1,eligibility_revision=1,unknown/observation_pending_reconcile/unknown`且Eligibility Field/Policy/Selection/Control basis列为NULL；同Identity的endpoint/location变化才递增binding revision；相同reality重放保留Eligibility，reality变化原子递增eligibility revision并写`unknown/reality_changed/unknown`、清空Field/Policy/Selection/Control basis；Eligibility Basis必须由Material-local Identity、当前Field/Access Binding与Reality/Snapshot、Extraction Policy revision/digest、Selection/Reservation digest及Material Control projection组成，global terminal Observation revision只作审计指针；终态Observation后仅对new/reality_changed/binding_changed/location_changed/missing、unknown或basis失效、以及Field/Access/Policy/Selection/Reservation/Control影响的Material Key执行Reconcile；完全相同basis幂等no-op，未变化Material不执行SQL UPDATE、不递增eligibility revision；`eligibility_observation_revision`记录上次实际评估所用Observation revision，允许在basis不变时落后于current head；非unknown Control必须有revision/digest，uncontrolled允许revision 0或正整数；`INDEX(field_id,eligibility_state,control_projection,material_key)`及`INDEX(field_id,eligibility_observation_revision,eligibility_policy_revision,material_key)`；stored state只有在`eligibility_field_status`仍匹配Field current status且material-local basis仍匹配当前Access/Policy/Selection/Control时才可用于候选筛选，否则effective projection为unknown；最终Run admission仍必须CAS `fx_material_controls` |
| `proc_procurement_runs` | `procurement_run_id PK, field_id FK, run_basis_schema_ref, access_revision, access_digest, content_profile_hint, profile_hint_revision, profile_hint_digest, terminal_observation_revision, field_observation_work_id, extraction_policy_id, extraction_policy_revision, extraction_policy_digest, triage_rule_ref, triage_rule_revision, triage_rule_schema_ref, triage_rule_digest, triage_rule_authority_digest, run_basis_digest, retry_intent_id NULL, state(active|waiting|sealed), state_revision, candidate_package_revision_head, seal_outcome NULL, seal_decision_id NULL, seal_decision_digest NULL, seal_evidence_digest NULL, admission_commit_marker, admission_result_digest, seal_commit_marker NULL, seal_result_digest NULL, priority_class, created_at_ms, finished_at_ms NULL` | `run_basis_schema_ref=ProcurementRunExecutionBasis@1`；Run及其`proc_run_materials` immutable basis列共同构成完整Basis，禁止只凭digest恢复；Field Access/Profile Hint/Observation/Policy均显式FK，`(field_id,profile_hint_revision)`指向immutable Hint revision且三项Hint列必须逐字节一致；Admission还要求terminal Observation冻结同一Hint Snapshot；Triage tuple必须由`ProcurementTriageRuleRegistry@1`当前active entry签发，Admission时验证active，历史恢复时验证精确immutable entry仍存在且digest匹配，不建立调用者可写Rule表；初始`state_revision=1,candidate_package_revision_head=0`；Candidate Publication CAS exact expected head并将其加1，Result/package revision等于新head；只有expected revision CAS可改变state；sealed时Decision ID/digest、Outcome、Evidence digest、finished time与Seal marker/result必须同时非NULL且写后immutable，非sealed时全部NULL，`UNIQUE(seal_decision_id) WHERE seal_decision_id IS NOT NULL`；`UNIQUE(field_id,run_basis_digest)`禁止同一Basis建立第二个Run，Retry Basis因包含唯一`sourceRetryIntentId`形成新digest；`retry_intent_id`为空或唯一引用一次性Intent并加`UNIQUE(retry_intent_id) WHERE retry_intent_id IS NOT NULL`；Admission/Seal marker各自唯一且显式FK；Retry建立的Run与Intent共享同一consume/admission marker和outer result digest；Supporting Work通过`owner_domain+process_type+process_id`关联；`INDEX(state,priority_class,created_at_ms)` |
| `proc_procurement_retry_intents` | `retry_intent_id PK, field_id FK, failed_run_id FK, failed_basis_digest, retry_field_status, retry_content_profile_hint, retry_profile_hint_revision, retry_profile_hint_digest, retry_access_revision, retry_access_digest, retry_terminal_observation_revision, retry_field_observation_work_id, retry_extraction_policy_id, retry_extraction_policy_revision, retry_extraction_policy_digest, retry_triage_rule_ref, retry_triage_rule_revision, retry_triage_rule_schema_ref, retry_triage_rule_digest, retry_triage_rule_authority_digest, retry_admission_head_digest, retry_scope_digest, retry_member_count, precondition_set_digest, actor_id, idempotency_key, intent_digest, state(open|consumed|stale), state_revision, new_run_id NULL, primary_stale_reason_code NULL, consume_admission_head_schema_ref NULL, consume_admission_head_json NULL, consume_admission_head_digest NULL, create_commit_marker, create_result_digest, consume_commit_marker NULL, consume_result_digest NULL, created_at_ms, consumed_at_ms NULL` | `UNIQUE(field_id,idempotency_key)`；`UNIQUE(failed_run_id,failed_basis_digest) WHERE state IN ('open','consumed')`，stale后可按新current precondition建立新Intent，consumed后必须改为对其new Run重试；`retry_*` head按`ProcurementRetryAdmissionHead@1`关系化冻结，Profile Hint tuple必须指向精确immutable revision并与terminal Observation一致，Triage tuple必须来自创建时Registry active entry；初始`state_revision=1`；`new_run_id`只在consumed时非NULL并与`proc_procurement_runs.retry_intent_id`互证，两个方向FK均`DEFERRABLE INITIALLY DEFERRED`；terminal时`consume_admission_head_json`必须是schema验证且JCS bytes≤`16 KiB`的完整current head，digest精确匹配；stale时`primary_stale_reason_code`等于8.6.18固定precedence下`staleReasonCodes[0]`，created时为NULL；create与consume marker/result分别唯一且可重放，terminal Intent必须有consume pair；消费时CAS expected state/digest并重验全部relationized precondition，不修改旧Run |
| `proc_procurement_retry_intent_materials` | `retry_intent_id FK, ordinal, material_key, failed_run_material_digest, expected_binding_revision, expected_eligibility_revision, expected_eligibility_basis_digest, expected_selection_basis_digest, expected_selection_has_conflict, expected_control_revision, expected_control_state, expected_control_owner_domain NULL, expected_control_owner_scope_type NULL, expected_control_owner_scope_id NULL, expected_control_region_projection, expected_control_evidence_digest, expected_control_projection_digest, member_precondition_digest, consume_snapshot_schema_ref NULL, consume_snapshot_json NULL, consume_snapshot_digest NULL, consume_outcome NULL|matched|stale, consume_stale_reason_code NULL, consumed_at_ms NULL` | `PK(retry_intent_id,ordinal)`及`UNIQUE(retry_intent_id,material_key)`；成员必须来自failed Run中`released+triage_failed`的Material，按materialKey UTF-8 bytes升序且`1..256`项；`CHECK(expected_selection_has_conflict=0)`；完整expected rows重建Intent member/precondition set，aggregate digest必须匹配Intent head；Control列必须完整映射available `MaterialControlProjectionSnapshot`，包括evidence/projection两个不同digest；expected列immutable；Intent消费时全部member在同事务一次性写完整`ProcurementRetryConsumeMemberSnapshot@1`（JCS bytes≤`4 KiB`）及digest/outcome，之后immutable；stale只保存8.6.18 closed set中的唯一primary code，matched不得保存code；terminal Intent不允许存在未写consume snapshot的member |
| `proc_run_materials` | `procurement_run_id FK, ordinal, material_key, selection_role, mount_scope_id, inode, fingerprint_algorithm, fingerprint_version, content_fingerprint, size_bytes, binding_revision, eligibility_revision, eligibility_basis_digest, last_snapshot_digest, last_observation_id, endpoint_id, location, reality_digest, provenance_digest, expected_control_revision, expected_control_state, expected_control_owner_domain NULL, expected_control_owner_scope_type NULL, expected_control_owner_scope_id NULL, expected_control_region_projection, expected_control_evidence_digest, expected_control_projection_digest, admission_control_action(acquire|assert_same_field), admitted_control_revision, admitted_control_projection_digest, basis_member_digest, selection_state(run_selection|candidate_delivery|released|transferred), candidate_package_id NULL FK, terminal_disposition NULL|completed_without_candidate|triage_failed|handoff_accepted|handoff_rejected, terminal_evidence_digest NULL, selected_at_ms, reservation_updated_at_ms` | `rowMutability=cas_lifecycle`；closed transitions仅为`run_selection → candidate_delivery|released`及`candidate_delivery → transferred|released`。`PK(procurement_run_id,ordinal)`及`UNIQUE(procurement_run_id,material_key)`；ordinal从0连续，`1..256`项且`selection_role=triage_input`；Physical Identity四元组与`size_bytes`必须从Admission同事务重读的`proc_field_materials` current row逐字节复制，满足8.6.18 `PhysicalMaterialIdentity@2`的materialKey公式且size为non-negative safe integer，并作为immutable Run Basis列参与`basis_member_digest`；expected Control列完整映射Admission前的available `MaterialControlProjectionSnapshot`，包括evidence/projection两个不同digest；admitted pair冻结事务后同Field Procurement Control output并共同形成Receipt的`controlRevisionSetDigest`；Identity/size、expected字段、`basis_member_digest`和admitted pair均为write-once，禁止旁读最新值覆盖；`UNIQUE(material_key) WHERE selection_state IN ('run_selection','candidate_delivery')`强制跨Run/Candidate占用唯一；`run_selection`无Candidate/terminal pair，`candidate_delivery`要求Candidate且无terminal pair；Candidate publish只允许`run_selection → candidate_delivery`并一次性填Candidate FK，之后该FK不可改写；Run Seal只把剩余Selection置`released+completed_without_candidate|triage_failed`、要求Candidate为NULL并保存Decision同一逐成员Evidence digest；Handoff终态只允许已有Candidate的Reservation变`transferred+handoff_accepted`或`released+handoff_rejected`并保存对应Receipt/Decision Evidence digest；terminal disposition/evidence是同一CAS的write-once bundle，终态后只能相同Evidence幂等重放，不得再次UPDATE |
| `proc_candidate_packages` | `candidate_package_id PK, procurement_run_id FK, package_revision, field_id, field_access_revision, field_context_digest, media_type(single|group), content_profile(movie|series|jav|western_adult), material_input_form(stream_file|bdmv|dvd|iso), structure_kind(single|season), display_identity, identity_metadata_schema_ref, identity_metadata_json, identity_metadata_digest, identity_claim_schema_ref, identity_claim_json, identity_claim_digest, structure_evidence_id, structure_evidence_payload_digest, structure_unit_id, structure_unit_digest, triage_rule_ref, triage_rule_revision, triage_rule_authority_digest, primary_input_manifest_id, manifest_digest, related_reference_set_digest, member_control_evidence_set_digest, package_digest, state, published_at_ms` | Identity Metadata/Claim JSON各≤`16 KiB`；`UNIQUE(procurement_run_id,package_revision)`且revision必须等于Run head CAS后的值；Field Context、Profile/mediaType/structure/Identity/Unit/Rule/Manifest/Control/Related必须与完整CandidateDraft逐项一致；`seasonContinuityClaimSetDigest`由immutable `proc_candidate_season_continuity_claims`、`relatedReferenceSetDigest`由immutable `proc_candidate_related_references`按8.6.18唯一公式重建，不新增opaque JSON或旁读；published内容immutable；Publication必须与Manifest、Episode Claim/完整Related relation、对应`proc_run_materials → candidate_delivery`、Offer/Outbox、typed Result/marker同事务成立；`INDEX(state,published_at_ms)` |
| `proc_candidate_season_continuity_claims` | `candidate_package_id FK, claim_kind(provider_season_identity|triage_grouping_lineage), claim_namespace, claim_key, claim_digest, evidence_digest` | `PK(candidate_package_id,claim_kind,claim_namespace,claim_key)`；逐行无损映射`SeasonContinuityClaim@1`，禁止任何别名或转换值写入Store；仅Series；不存在row是合法粗入库；title/year/path/folder禁止编码为claim key |
| `proc_candidate_primary_materials` | `candidate_package_id FK, ordinal, material_key, role(primary_payload|structural_dependency), mount_scope_id, inode, fingerprint_algorithm, fingerprint_version, content_fingerprint, size_bytes, binding_revision, admitted_control_revision, admitted_control_projection_digest, member_digest` | `PK(candidate_package_id,ordinal)`及`UNIQUE(candidate_package_id,material_key)`；ordinal从0连续；每行必须与Candidate Draft Unit、final Manifest member及对应immutable Run member的material/Physical Identity/size/binding/admitted Control逐项一致；Identity/size随Candidate Publication原子复制且published row immutable，历史Delivery不得旁读current Field Material补值 |
...
| `proc_candidate_related_references` | `candidate_package_id FK, reference_id, primary_ordinal, role, material_key, mount_scope_id, inode, size_bytes, fingerprint_algorithm, fingerprint_version, content_fingerprint, endpoint_id, location, association_evidence_digest, reference_digest` | `PK(candidate_package_id,reference_id)`及复合FK`(candidate_package_id,primary_ordinal) → proc_candidate_primary_materials(candidate_package_id,ordinal)`；逐列无损保存一个`RelatedMaterialReference@1`，`primaryMaterialKey`只由该immutable FK目标重建；Physical Identity/materialKey及reference digest均按8.6.18重验，Related不再携带或要求全文件checksum；published row immutable，Related不进入Material Control；Candidate关闭、Run封口或Offer关闭不得删除，历史Delivery Snapshot仍从本Owner relation恢复 |
| `proc_candidate_deliveries` | `offer_id PK, candidate_package_id FK, package_revision, package_digest, acceptance_basis_digest, state(open|accepted|rejected), handoff_decision_id NULL, handoff_decision_digest NULL, handoff_receipt_id NULL, handoff_receipt_digest NULL, terminal_evidence_digest NULL, offered_at_ms, closed_at_ms NULL` | `rowMutability=cas_lifecycle`；唯一closed transition为expected `open → accepted|rejected`，terminal state不得转换或改写。`offer_id`与`acceptance_basis_digest`必须分别等于8.6.18从final `CandidatePackage@1`计算的`CandidateIntakeAcceptanceBasis@1`公式结果，且同事务存在唯一`ProcurementCandidateOfferAvailableMessage@1` Outbox；`UNIQUE(candidate_package_id,package_digest,acceptance_basis_digest)`；同一Candidate至多一个open Offer。open时全部handoff/terminal/closed列为NULL；accepted CAS一次性写Decision ID、Receipt ID/digest、terminal Evidence及closed time，Decision digest沿用既有nullable合同；rejected CAS一次性写全部handoff/terminal/closed列并逐项等于`LibraCandidateRejectedMessage@1`，`terminal_evidence_digest=receiptDigest`；这些终态列是write-once bundle，相同Offer/相同Evidence重放返回既有终态，不得执行第二次UPDATE、改写相反Outcome或新增row/Outbox；`INDEX(state,offered_at_ms)` |
...
| `libra_intake_decisions` | `intake_decision_id PK, decision_revision, decision_kind(accepted_resolution|rejected_acceptance), offer_id, candidate_package_id, package_revision, package_digest, acceptance_basis_digest, candidate_delivery_snapshot_digest, source_field_id, source_field_access_revision, source_field_context_digest, candidate_structure_kind, candidate_content_profile, candidate_identity_claim_digest, expected_continuity_head_revision NULL, expected_continuity_head_digest NULL, committed_continuity_head_revision NULL, candidate_continuity_set_digest NULL, candidate_episode_scope_digest NULL, match_cardinality NULL|none|one|multiple, matched_subject_set_digest NULL, episode_overlap_digest NULL, accepted_result NULL|new_subject|season_extension, target_subject_id NULL, expected_target_status NULL, expected_target_intake_revision NULL, expected_target_continuity_set_digest NULL, expected_target_episode_scope_digest NULL, committed_target_intake_revision NULL, committed_subject_continuity_set_digest NULL, committed_subject_episode_scope_digest NULL, accepted_payload_digest NULL, rejection_schema_ref NULL, rejection_id NULL, primary_rejection_code NULL, rejection_reason_set_digest NULL, rejection_digest NULL, decision_digest, decided_at_ms` | `decision_revision=1`且`UNIQUE(offer_id)`；`intake_decision_id=SHA-256(JCS({schema:"libra.intake-decision-id@1",offerId}))`；common Offer/Package/Acceptance Basis/Snapshot逐项匹配正式Delivery；六项source/profile provenance逐项复制Delivery Snapshot并成为Libra-owned immutable事实，不能在Routing时回读Procurement。无法取得完整Snapshot属于Delivery integrity/system failure，不伪造Rejected Decision。`accepted_resolution`要求continuity/match/overlap、accepted result/target、committed Subject digest及accepted payload完整非NULL，全部rejection列为NULL；其`decision_digest`逐字节等于`SubjectContinuityResolutionDecision@1.decisionDigest`。`rejected_acceptance`要求全部continuity/head/match/target/accepted列为NULL，rejection列完整非NULL；其`decision_digest`逐字节等于`IntakeRejectionDecision@1.decisionDigest`。`new_subject`要求target是Libra分配的新ID；`season_extension`要求one match/zero overlap、Candidate与Subject的`structure_kind/content_profile`逐字节相等及完整target CAS。Continuity无匹配、多匹配或Episode overlap仍是`new_subject`，不得成为rejection。同一Candidate/Package digest只允许一个accepted Decision的partial unique index；不同Acceptance Basis的新Offer可以各自保留Rejected Decision，但任何后续Accepted仍受该唯一约束；immutable，不接受调用者指定既有Subject或伪造head值 |
...
| `libra_intake_rejection_reason_evidence` | `intake_decision_id FK, reason_ordinal, reason_code, evidence_ordinal, evidence_schema_ref, evidence_id, evidence_digest, reason_digest` | `PK(intake_decision_id,reason_ordinal,evidence_ordinal)`及`UNIQUE(intake_decision_id,reason_ordinal,evidence_schema_ref,evidence_id,evidence_digest)`；每个reason ordinal至少一行，reason code按8.6.18 Handoff A closed precedence去重排序，组内Evidence按`evidence_schema_ref,evidence_id,evidence_digest`的UTF-8 bytes升序且唯一；同一reason ordinal全部row的reason code/digest相同；`reason_digest=SHA-256(JCS({schema:"handoff-a-rejection-reason@1",reasonCode,evidenceRefs}))`；全部rows必须精确重建`IntakeStructuredRejection@1`的Reason集合，immutable；`INDEX(intake_decision_id,reason_ordinal,evidence_ordinal)` |
| `libra_handoff_a_receipts` | `receipt_id PK, intake_decision_id FK, outcome(accepted|rejected), offer_id, candidate_package_id, package_revision, package_digest, candidate_delivery_snapshot_digest, subject_id NULL, subject_intake_revision NULL, subject_continuity_head_revision NULL, subject_continuity_set_digest NULL, subject_episode_scope_digest NULL, accepted_payload_digest NULL, libra_binding_set_digest NULL, control_revision_set_digest NULL, rejection_id NULL, primary_rejection_code NULL, rejection_reason_set_digest NULL, rejection_digest NULL, receipt_digest, committed_at_ms` | `UNIQUE(intake_decision_id)`；同一Candidate/Package digest最多一个accepted Receipt的partial unique index，新的Acceptance Basis/Offer可以分别保留rejected Receipt。accepted要求Subject、accepted payload、Binding/Control列全部非NULL且rejection列全NULL，重建`SubjectAndTransferReceipt@1`，其`ReceiptEnvelope.scopeDigest=accepted_payload_digest`，且`control_revision_set_digest`必须由同一Handoff marker的全部Control transfer revision及各自previous revision按8.6.19唯一公式重建；rejected要求Subject/accepted/Binding/Control列全NULL、rejection列全非NULL，重建`IntakeRejectionReceipt@1`，其`ReceiptEnvelope.scopeDigest=IntakeRejectionDecision.decisionDigest`。两种Receipt的Snapshot digest均逐字节等于对应Decision；相同marker重放不新增Receipt或消息 |
| `libra_subjects` | `subject_id PK, structure_kind(single|season), content_profile(movie|series|jav|western_adult), routing_anchor_intake_decision_id FK, status, intake_revision, current_continuity_set_digest, current_episode_scope_digest, current_identity_revision NULL, created_at_ms, updated_at_ms, terminal_at_ms` | `status`至少区分`active|abandoned|completed`；new Subject从Accepted Candidate固定`structure_kind/content_profile`、把创建它的Intake Decision固定为唯一Routing anchor且`intake_revision=1`；Season extension要求structure/profile相同、CAS加1并不得改写anchor，因此后续不同Field到达的Episode仍沿用该Season Subject既有Routing责任。`series`必须对应`season`，其余Beta profile对应`single`。continuity/Episode digest由本域relation rows按8.6.18重建；`current_identity_revision=NULL`是尚未发布Resolved Product Identity的唯一合法初值，正整数时显式复合FK指向`libra_product_identity_revisions(subject_id,revision)`；`INDEX(status,subject_id)`；Subject不保存Arca Shelf Entry/Deck状态 |
...
| `libra_routing_assessments` | `routing_assessment_id PK, subject_id FK, decision_basis_id FK, routing_authority_kind(policy|manual_selection), routing_policy_id NULL, routing_policy_revision NULL, manual_selection_digest NULL, routing_input_digest, assessment_schema_ref, assessment_json, assessment_digest, created_at_ms` | policy与manual列variant互斥；JSON≤`64 KiB`并完整重建`RoutingAssessmentEvidence@1`；`UNIQUE(subject_id,decision_basis_id,routing_input_digest)`；immutable |
| `libra_routing_decisions` | `routing_decision_id PK, subject_id FK, assessment_id FK, decision_revision, decision(resolved|unresolved), shelf_id NULL, unresolved_reason_code NULL, routing_authority_kind(policy|manual_selection), routing_policy_id NULL, routing_policy_revision NULL, manual_selection_digest NULL, routing_input_digest, shelf_priority_set_digest, decision_digest, decided_at_ms` | authority列variant互斥；resolved要求shelf非NULL/reason NULL，unresolved相反；同一assessment只有一个Decision；`UNIQUE(subject_id,decision_revision)`；immutable；`INDEX(shelf_id,decided_at_ms)` |
| `libra_decision_basis_revisions` | `decision_basis_id PK, subject_id FK, basis_kind(routing|acceptance_spec), basis_revision, expected_head_revision, expected_head_snapshot_digest, routing_decision_id NULL, query_result_set_digest, routing_input_digest NULL, spec_input_digest NULL, product_scope_digest NULL, input_set_digest, status(ready|unresolved), unresolved_reason_code NULL, basis_digest, committed_at_ms` | `UNIQUE(subject_id,basis_revision)`及`UNIQUE(subject_id,basis_kind,input_set_digest)`；`expected_head_revision>=0`且必须等于本Basis唯一`decision_head_snapshot` input的head revision，`expected_head_snapshot_digest`逐字节等于该input row digest；routing的routing input始终非NULL且spec/scope NULL；acceptance_spec的routing/spec input始终非NULL，ready时current Routing与scope非NULL，unresolved时只允许按closed reason置对应pointer/scope NULL；ready无reason，unresolved必须有reason；immutable；`INDEX(subject_id,status,basis_revision)` |
| `libra_decision_basis_inputs` | `decision_basis_id FK, input_ordinal, input_kind(subject_snapshot|decision_head_snapshot|routing_authority|shelf_routing_projection|routing_fact|routing_decision|shelf_standard_projection|product_scope|decision_fact|query_result), input_schema_ref, input_object_id, input_revision, input_digest, input_json, provider_domain NULL, query_contract NULL, query_version NULL, query_input_digest NULL, result_kind NULL, result_revision NULL, result_digest NULL, expires_at_ms NULL` | `PK(decision_basis_id,input_ordinal)`及`UNIQUE(decision_basis_id,input_kind,input_object_id,input_revision,input_digest)`；ordinal从0连续，ordinal 0固定为唯一`subject_snapshot`，ordinal 1固定为唯一`decision_head_snapshot`，其后依次为routing authority、按rank的Shelf projection、Routing Decision、Shelf Standard、Product Scope、按正式ID/digest排序的Facts、按正式tuple排序的Query Result；不存在的variant段跳过但不得改变剩余相对顺序。`decision_head_snapshot`固定`input_object_id=subjectId,input_revision=headRevision,input_digest=snapshotDigest`且该variant的input revision允许0；其他variant按各自typed contract使用正整数revision；逐项typed JSON≤`64 KiB`、总JCS≤`1 MiB`；query variant填写query列，其他variant全部query列NULL；全部rows按8.6.18可重建同一`DecisionInputSet@1`，不得旁读Owner/Provider、current Head或Foundation Result补值 |
...
| `libra_workspaces` | `workspace_id PK, libra_run_id FK UNIQUE, platform_workspace_root_id, platform_workspace_root_kind, platform_workspace_revision, platform_workspace_endpoint_id, platform_workspace_mount_scope_id, platform_workspace_mount_scope_revision, platform_workspace_capability_digest, platform_workspace_snapshot_digest, root_handle_ref, space_admission_evidence_schema_ref, space_admission_evidence_id, space_admission_evidence_json, space_admission_evidence_digest, space_admission_required_bytes, space_admission_available_bytes, space_admission_expires_at_ms, workspace_scope_digest, admission_decision_digest, current_revision, state(active|reclaiming|reclaimed), state_digest, created_at_ms, completed_at_ms NULL` | Platform root snapshot/space evidence/scope/admission列在创建后immutable，typed JSON≤`16 KiB`且必须完整重建`WorkspaceSpaceAdmissionEvidence@1`；root/snapshot/handle与同事务Platform read participant及`fx_workspace_registry.root_handle_ref`逐字节一致。只有`admitted`且`committed_at_ms<=expires_at_ms`的Evidence可建立Workspace，required/available bytes等于typed Evidence；current aggregate row使用`rowMutability=cas_lifecycle`；`FOREIGN KEY(workspace_id,current_revision) REFERENCES libra_workspace_revisions(workspace_id,workspace_revision) DEFERRABLE INITIALLY DEFERRED`，禁止物化为immediate FK、nullable后补或sentinel revision。首次Admission在同一事务先插入`current_revision=1`的aggregate，再插入revision 1，commit前复合FK与state/digest必须全部成立；只有Cleanup全部完成才写reclaimed/completed time；同一Run恰好至多一个Workspace stream，历史Admission Decision只从该row、Run revision与Registry重建，不依赖后来Platform root current row或resolved path |
| `libra_workspace_revisions` | `workspace_id FK, workspace_revision, state, material_reference_set_digest, transition_kind(admitted|reference_attached|product_staged|reference_released|reclaiming|reclaimed), transition_evidence_digest, previous_revision NULL, revision_digest, committed_at_ms` | `PK(workspace_id,workspace_revision)`；append-only且从1连续；`workspace_id`到`libra_workspaces`的普通父FK保持immediate，配合上述固定aggregate-first插入顺序消除首次建立循环；current Workspace row由最高revision重建，set digest来自每个handle最高Reference revision |
| `libra_workspace_material_refs` | `workspace_id FK, libra_run_id FK, reference_id, material_handle_id, material_key, workspace_handle_schema_ref, workspace_handle_json, workspace_handle_digest, reference_revision, reference_state(working|product_staging|released), episode_claims_schema_ref, episode_claims_json, episode_scope_digest, product_verification_schema_ref NULL, product_verification_id NULL, product_verification_json NULL, product_verification_digest NULL, previous_reference_revision NULL, committed_workspace_revision, reference_digest, committed_at_ms` | `PK(reference_id,reference_revision)`及`UNIQUE(workspace_id,material_handle_id,reference_revision)`；typed Handle JSON固定`WorkspaceMaterialHandle@1`且≤`4 KiB`，Episode claims JSON固定`LibraWorkspaceEpisodeClaims@1`且≤`16 KiB`，完整保存`0..32`项本节typed Episode claim并重算同一scope digest；Handle必须与`fx_workspace_materials.handle_schema_ref/json/digest/fence_digest`及Decision逐字节一致后冻结，允许Foundation material后来reclaimed仍重建历史Reference。Verification四列在working全NULL；product_staging全非NULL且schema固定role-aware `WorkspaceProductVerificationSnapshot@1`、canonical JSON≤`128 KiB`、ID/digest逐字节匹配；released逐字节沿用前一revision四列：由working释放时全NULL，由product_staging释放时全非NULL，不能清空或替换。每次Reference transition与同一事务Workspace revision一一绑定，历史Workspace revision N按`committed_workspace_revision<=N`选择每个stream最高Reference revision重建set；每个reference stream从1连续、append-only，最高revision是current；同一stream后续revision的Handle、materialKey与Episode claims逐字节沿用；同一Workspace最多1024个current non-released Reference且同一materialKey至多一个，防止Cleanup无法形成完整Scope或hard-link Handle对同一Control重复release；released不得再回到前态；同一handle不能同时存在两个current role；只引用同Workspace的`fx_workspace_materials`，Artifact分支还必须精确命中`fx_artifact_registry`中的immutable Handle |
...
| `arca_acceptance_attempts` | `acceptance_attempt_id PK, offer_id, on_deck_package_id, package_digest, shelf_id FK, standard_revision, placement_revision, state(active|accepted|rejected), created_at_ms, finished_at_ms NULL` | 同一Offer至多一个Attempt且相同`package_digest+standard+placement`至多一个active attempt；accepted/rejected只允许从active以Decision事务CAS一次；Supporting Work通过Foundation反向关联；`INDEX(state,created_at_ms)` |
...
| `perception_resolution_revisions` | `resolution_id PK, query_contract, query_schema_ref, query_input_digest, fact_kind, revision, record_set_digest, rule_revision, rule_digest, result_kind, winning_perception_id NULL, reason_code NULL, result_schema_ref, result_json, result_digest, resolved_at_ms` | `UNIQUE(query_contract,query_input_digest,revision,resolution_id)`供head精确复合FK；`winning_perception_id`显式FK；found必须有winner且reason为NULL，not_found必须无winner并具有`no_matching_record|requested_fact_absent|strongest_value_conflict`之一；`result_json`保存完整typed `PerceptionResolutionRevision`且≤`16 KiB`，digest使用统一JCS basis；current pointer/index按query digest |
| `perception_resolution_heads` | `query_contract, query_input_digest, current_resolution_id, current_revision, updated_at_ms` | `PK(query_contract,query_input_digest)`；四列显式复合FK引用同一`perception_resolution_revisions(query_contract,query_input_digest,revision,resolution_id)`；head CAS与新revision/typed result在同一Resolution Commit事务；Resolution Facade只读head，不在热路径计算`MAX(revision)` |
| `people_persons` | `person_id PK, status(active|merged), current_revision, current_preference_revision NULL, current_reference_revision NULL, current_reference_projection_revision, current_reference_projection_digest, created_at_ms, terminal_at_ms` | `current_revision`显式复合FK引用`people_person_revisions`；nullable Preference/Reference pointer分别显式复合FK引用对应revision表；`current_reference_revision IS NULL`表示尚无Reference revision，Commit逻辑expected `0`映射为“期望不存在”而不是存储revision 0；两个Projection checkpoint字段均`NOT NULL`，`CHECK(current_reference_projection_revision>=1)`且digest为64位lowercase hex；Person成立时以revision `1`和无Reference语义payload的digest初始化；与child表的循环FK均`DEFERRABLE INITIALLY DEFERRED`；`INDEX(status,person_id)` |
| `people_person_revisions` | `person_id FK, revision, person_status(active|merged), canonical_name, merged_into_person_id NULL, origin_kind(direct|candidate), origin_decision_id, origin_decision_digest, origin_candidate_kind NULL, origin_candidate_id NULL, origin_candidate_revision NULL, origin_candidate_payload_digest NULL, fact_digest, committed_at_ms` | `PK(person_id,revision)`；immutable；direct Registration保存完整Direct Decision ID/digest且Candidate列全NULL；Candidate acceptance保存Acceptance Decision ID/digest及精确Candidate revision/payload；CHECK禁止两种来源混合；Person是全局Registry，不持久化媒体`content_scope` |
...
| `people_reference_assets` | `reference_asset_id PK, person_id FK, artifact_handle_id, artifact_digest, state(active|released), created_reference_revision, released_reference_revision NULL, created_at_ms, released_at_ms NULL` | stable业务ID由`ReferenceAssetCoordinator`在Plan前分配；`artifact_handle_id/artifact_digest`必须分别等于Decision中`artifactHandle.artifactHandleId/digestHex`；`UNIQUE(person_id,artifact_digest)`；created/released revision都显式引用同Person的`people_reference_revisions`；released不可回到active |
| `people_reference_faces` | `reference_face_id PK, person_id FK, reference_asset_id FK, embedding_handle_id, embedding_digest, model_ref, state(active|released), created_reference_revision, released_reference_revision NULL, created_at_ms, released_at_ms NULL` | `embedding_handle_id/embedding_digest/model_ref`必须分别等于Decision中`faceEmbeddingSetHandle.artifactHandleId/digestHex/modelRef`；每个active Asset恰好一个active派生Face；Face与Asset同事务建立/释放；`UNIQUE(reference_asset_id)`及`UNIQUE(person_id,embedding_handle_id,model_ref)`；用户API不得按Face ID修改 |
| `people_reference_revisions` | `person_id FK, revision, operation_kind(add_image|release_image), reference_asset_id, reference_face_id, active_asset_set_digest, active_face_set_digest, reference_set_digest, decision_digest, fact_digest, committed_at_ms` | `PK(person_id,revision)`；immutable；`people_persons.current_reference_revision`显式FK指向current row；Reference commit以expected reference revision CAS并保存完整active set digest，不通过扫描时间猜current；三个set digest及`fact_digest`必须严格使用`PersonReferenceRevision` nominal合同声明的唯一basis |
...
| `platform_compute_devices` | `device_id PK, device_kind(software_cpu|intel_qsv|nvidia_nvenc|amd_vaapi|remote_worker), stable_device_key, current_probe_revision, enabled, state(ready|unavailable|faulted), updated_at_ms` | `UNIQUE(stable_device_key)`；`deviceClass=device_kind`为唯一公开映射；只有current probe验证能力且state=ready才可enabled；`INDEX(enabled,state,device_id)` |
| `platform_compute_device_probes` | `device_id FK, revision, capability_schema_ref, capability_json, capability_digest, probe_result(passed|failed), probed_at_ms` | `PK(device_id,revision)`；Capability JSON上限`16 KiB`且至少关系化声明`supportedVideoCodecs[]/supportedRateControlModes[]`的closed typed payload；actual probe Evidence immutable，只有passed可成为ready Snapshot |
...
| `shared.material.bounded_fingerprint.compute@1` | `PhysicalMaterialReadHandle + BoundedFingerprintProfile → BoundedContentFingerprintEvidence` | `pure_observation` |
...
| `shared.artifact.manifest.verify@1` | `ArtifactHandle[1..64] + ArtifactRequirement → ArtifactManifestVerification` | `pure_observation` |
| `shared.integration.availability.observe@1` | `IntegrationHandle → IntegrationAvailabilityEvidence` | `pure_observation` |
| `shared.face.embedding.compute@1` | `ArtifactHandle[1..16] + FaceModelRef → FaceEmbeddingSetHandle` | `workspace_write` |
| `shared.face.cluster.compute@1` | `FaceEmbeddingSetHandle + ClusterParameters → FaceClusterSetHandle` | `workspace_write` |
| `shared.face.reference.match@1` | `FaceClusterSetHandle + PersonReferenceProjection[0..256] → PersonMatchEvidence` | `pure_observation` |
...
`shared.face.reference.match@1`的Projection输入是People Facade发布的完整有界集合，不是Executor对People Store的
查询权。集合按`personId`的UTF-8 bytes排序、ID唯一且允许为空；空集合仍必须执行真实Match Event并得到
`matches=[]`及覆盖全部Cluster的`unmatchedClusterIds`，不能伪造Person、跳过Event或把“没有输入Person”直接当成
Media-Cast Fact。非空集合的每个match只能引用集合内Person及其精确Projection revision/digest。

`FaceModelRef.mode=single_reference_face`时input必须恰好包含一张图片，Result必须证明
`detectedFaceCount=1`、`vectorCount=1`且`sourceArtifactSetDigest`绑定该输入Artifact。零张或多张Face是稳定
contract failure，不得选择“最大的人脸”、按顺序猜测或把多个Embedding交给Reference commit。

#### 8.6.4 Procurement Capability

| Capability ref | Input → Output | Effect Class |
| --- | --- | --- |
| `procurement.field.observation.page.commit@1` | `FieldAccessHandle + FieldObservationPageRequest → ObservationPageCommitResult` | `domain_fact_commit` |
...
| `procurement.triage.bdmv.assess@1` | `BdmvAssessmentInput → BdmvAssessmentEvidence` | `pure_observation` |
...
Coordinator必须预分配`procurementRunId`并装配完整`ProcurementRunExecutionBasis@1`；Capability的
`SelectedFieldMaterialSet`必须与该Basis内嵌Set逐字段相等。Handle固定
`ownerDomain=procurement,processType=procurement_run,processId=procurementRunId`，`basisRef/basisDigest`绑定完整
Run Basis，`controlScopeDigest`绑定`selectionDigest`，`expectedControlRevisions`与Set成员按materialKey一一对应且
同为`1..256`项。Control Authority的目标Owner固定为`material_field/fieldId`：action为`acquire`才新增Control
revision，`assert_same_field`只CAS确认既有revision/digest且不制造假revision。任一Run/Selection/Basis或Control
participant失败使整个Admission rollback；Capability不得单独成功后由另一个事务补建Run。

`8.3.6`面向普通调用者的Control Query仍保持每批最多500项；Admission Input Assembler可以用多个有界只读批次
组装最多256项snapshot，但不能据此宣称提交成功。最终Foundation participant必须在同一个SQLite事务中使用
relationized values/temp relation一次重读并CAS全部`1..256`项；任一成员stale或冲突时整笔rollback，禁止把
...
Shared probe Executor可以通过其既有typed media-probe port执行一次有界只读观察；四个
...
Manifest构造证明；当Structure Unit使用`UnitScopeReference`时，Draft仍必须携带由Candidate Context
重建并按materialKey排序的最终候选成员明细（只包含该Candidate的primary/structural成员，不包含Scope中
未选中的物理成员）；最终`PrimaryInputManifest`只能由`procurement.candidate.publish@1`与Candidate Package、
成员关系、Reservation、Offer和Outbox同事务发布，避免pure Capability伪造业务发布时间或旁读成员。

`procurement.field.observation.page.commit@1`的正式Request仍冻结完整
`MaterialFieldProfileHintSnapshot@1`，但Event内部同时完成当前页必要的list/stat/bounded fingerprint与事实提交。
一个Page最多256项、物理读取最多64 MiB；两个上限任一先达到即产生当前页。Page只产生compact
`ObservationPageCommitResult@1`/`ObservationPageCommitReceipt@1`，不把Material snapshot写入Result或Evidence。
Event申请完整`volume_read:<mountScopeId> + sqlite_write` Permit，且所有Entry、Page header、Material current row、
Field head、typed Result、commit marker必须在同一Canonical Transaction中全有或全无。
...
二者均不变则保持，任一变化则加1。这样Entry提供完整可提交的Binding reality，且单一Event不需要在两个
Capability之间传输完整文件数组。

Commit前必须同时验证Field仍active、`page.accessRevision`仍等于Field current Access revision、
`page.profileHintSnapshot`逐字节命中Field current Profile Hint pointer及精确revision row、Field current
Observation pointer等于expected revision，以及Work/page/cursor连续；同一Work已有Page时还必须命中其冻结
Snapshot。第一次提交将SQL NULL pointer映射为逻辑
expected `0`；任何后续page或后续Work都使用真实正整数revision。成功事务按materialKey upsert当前Field Material、
追加不可变Observation revision及`proc_field_observation_entries`明细、切换Field head，并在同一
`fx_event_result_bindings`中写入compact Receipt/Result，再写入同一result digest与commit marker；相同
...
Observation detail rows are the canonical historical payload. Page Result/Receipt only proves the row-set digest and
cursor transition; any Projection, audit or Triage reader must rebuild details from the pinned Observation revision and
must never parse a historical Page JSON to recover file facts.
...
P7-04是`MaterialFieldManager`拥有的Application reconcile，不是Catalog Capability，也不建立新的Business
Process或Object。每批最多100项，并按以下固定阶段形成完整输入：

~~~text
读取current Field status / Access / terminal Observation head / Extraction Policy
→ 读取Material current Binding、Reality及其本轮coverage
→ 读取active/waiting Run Selection与尚未Handoff终结的Candidate Delivery Reservation snapshot
→ 通过MaterialControlAuthority批量读取versioned current Control snapshot
→ 纯确定性评估ExtractionPolicy与Eligibility
→ 提交ExtractionEligibilityDecisionBatch
~~~

Input Assembler可以读取Procurement自己的Repository和Foundation公开Control Query，但不得读取Arca Store、
Libra Store、历史失败Run作为隐藏规则或任何旧Runtime。纯Evaluator不能读Repository、Provider或文件系统；它只
消费完整typed snapshot。Decision必须内联绑定Field状态、Access digest、terminal Observation Work/revision、
Material Binding/reality/相对Field root的位置/size/extension、当前Policy revision、Selection snapshot和Control
snapshot，不得用opaque digest代替Policy评估所需字段。

当输入完整时，唯一Decision顺序为：

1. Field不是active：`ineligible(field_inactive)`；
2. Material未出现在current terminal Observation Work：`ineligible(not_observed_in_current_terminal_work)`；
3. Control投影为`production|finished_goods`：`ineligible(outside_procurement_region)`；
4. 存在`run_selection|candidate_delivery` guard：`ineligible(selection_conflict)`；
5. Control投影为`procurement`但Owner Scope不是当前`material_field/fieldId`，或其他当前事实使Run admission无法
   原子取得/确认Control：`ineligible(control_not_acquirable)`；同一Field保留的Procurement Control在无guard时是
   合法可复用Control，不构成该失败；
6. `excludedMaterialKeys`命中：`ineligible(policy_material_excluded)`；
7. `excludedDirectories`命中：`ineligible(policy_directory_excluded)`；
8. 非空`includedDirectories`未命中：`ineligible(policy_directory_not_included)`；
9. 非空`allowedExtensions`未命中：`ineligible(policy_extension_not_allowed)`；
10. `sizeBytes < minimumSizeBytes`：`ineligible(policy_size_below_minimum)`；
11. 其余完整basis：`eligible(eligible)`。

上述顺序同时是多项不满足时的唯一`reasonCode`优先级。正常P7-04 Decision不包含“重复开采抑制”输入；失败
Run、旧Candidate、Arca Off-deck Suppression和已释放的历史Control均不得改变结果。Control Query返回typed
unavailable、current Policy不能验证或当前Observation/Access不构成完整basis时，不得猜测eligible/ineligible；
有效Projection为`unknown(control_projection_unavailable|policy_unavailable_or_invalid|
observation_pending_reconcile|observation_work_incomplete|access_revision_changed)`并触发系统诊断。其中只有可以在
同一事务中重验的typed unavailable snapshot才允许把unknown写入current row；数据库不可用或basis无法重读时
保持原row不变，但所有Query必须按stale-basis规则对外投影unknown。

`Field Eligibility Reconcile Commit`在一个SQLite Unit of Work内重新读取并逐项验证Decision绑定的current
Field/Access/terminal Observation head、Policy head、Material Binding/reality/eligibility revision、Selection set及
Foundation Control revision/digest。任一不一致的Material保持原row不变并进入`staleMaterialKeys[]`；不能让旧
Decision覆盖新Reality或新Control。Basis完全相同返回`noOpMaterialKeys[]`；Basis变化时将对应Material的
`eligibility_revision`加1，并原子写Decision、reason、Observation/Policy/Selection/Control basis和reconcile时间。
一个Batch的全部`applied[]`更新要么一起提交，要么一起回滚。

该事务只维护可从current事实重建的Procurement decision fact，不是`domain_fact_commit` Event：它不写
`fx_event_result_bindings`、commit marker或Outbox。`expectedEligibilityRevision + basisDigest`就是幂等与并发边界；
崩溃发生在事务前等于未处理，事务后以current row为准，启动恢复和周期reconcile会重新计算尚未收敛的Material。
Signal只负责唤醒，不是正确性来源。

一轮新Observation Work尚未提交第一页时，上一轮terminal basis仍可使用；第一张non-terminal page提交并切换
Field head后，旧Eligibility不需要mass rewrite，但因head不再terminal而立即成为effective `unknown`。新Work terminal
page提交后，P7-04按批重算：本轮出现的Material使用新Reality，未出现的历史Material保留row和Binding历史但写为
`ineligible(not_observed_in_current_terminal_work)`。Field current Access revision变化时，任何旧Access basis同样
立即成为effective `unknown(access_revision_changed)`，直到该Access下的新terminal observation与reconcile完成。

P7-05 Run Admission由`ProcurementRunCoordinator`装配单一SQLite Unit of Work，不建立第二个Process Root：

~~~text
预分配procurementRunId
→ 组装完整ProcurementRunExecutionBasis@1与Commit Handle
→ Procurement participant从注入Registry解析active Triage Rule并重验Field/Profile Hint/Access/Observation/Policy/Rule、逐Material Binding/Eligibility/Reality与guard唯一性
→ Foundation participant逐Material CAS acquire或assert_same_field
→ 写Run、relationized Basis、run_selection、typed Receipt和commit marker
→ transaction commit
~~~

Procurement participant只能读取/写入`proc_*`，Foundation participant只能读取/写入`fx_material_control*`和
commit/result基础表；Coordinator通过typed Basis和Handle传值，不把任一Repository交给另一方。Run row、全部
Basis members、全部Control变化/确认、Receipt和marker必须全有或全无。Admission固定`hasOutbox=false`；成功Run由
Procurement Automation的current Run scan继续，Signal只可加速。相同`procurementRunId + basisDigest +
selectionDigest`及commit idempotency key重放返回原`ProcurementControlReceipt`；相同ID异digest稳定冲突。事务前
崩溃没有Run或Control变化，事务后崩溃由marker恢复原Result，不重新取得Control或创建第二个Run。

完整Run Basis不保存为热路径大JSON。`proc_procurement_runs`冻结Field/Profile Hint/Policy/Triage authority头，
`proc_run_materials`冻结逐项snapshot；两者连同明确引用的immutable Field Access、Observation Page、Policy
revision、Profile Hint revision与Triage Rule Registry entry按
`ProcurementRunExecutionBasis@1`稳定排序重建。Basis列在Admission后immutable，Selection lifecycle列不进入
`basisDigest`。所有Triage `MaterialFieldContext`只能复制该Basis中的Profile Hint Snapshot；Candidate Publisher
只能消费该Basis，不得旁读最新Field current row、调用者Hint或默认`mixed`冒充原始Triage输入。
...
保存，固定`hasOutbox=false`。

Retry Intent Create先重读失败sealed Run、原Basis和所有`released+triage_failed`成员，并从Procurement Registry取得
current active Triage Rule，再冻结`ProcurementRetryAdmissionHead`、`ProcurementRetryIntent@1`及relationized
member precondition；Intent、typed Receipt、marker与Procurement-internal Outbox同事务成立。Run Creator消费时以
expected Intent state/digest CAS并形成actual head和全部`ProcurementRetryConsumeMemberSnapshot`：全部匹配时复用
上述Admission Unit of Work，在同一事务中写`consumed + linked new Run + new Basis/Selection/Control + typed
Result/marker + consume snapshots`；任一不匹配按closed precedence写全部snapshot、`stale`终态和stale
Result/marker，不创建Run或改变Control。`UNIQUE(retry_intent_id)`、commit marker与Intent state CAS共同保证一个
Intent最多建立一个Run；失败的新Run不自动创建新Intent。
...
| `libra.intake.rejection.commit@1` | `IntakeRejectionDecision + DomainFactCommitHandle → IntakeRejectionReceipt` | `domain_fact_commit` |
...
| Capability ref | Input → Output | Effect Class |
| --- | --- | --- |
| `libra.media.frames.extract@1` | `PhysicalMaterialReadHandle\|WorkspaceMaterialHandle + SamplingPlan + WorkspaceArtifactOutputTarget → FrameArtifactSet` | `workspace_write` |
| `libra.western.analysis.request@1` | `FrameArtifactSet + FaceEmbeddingSetHandle + FaceClusterSetHandle + AnalysisSpec + WorkspaceArtifactOutputTarget → ArtifactHandle` | `workspace_write` |
| `libra.western.analysis.observe@1` | `FrameArtifactSet + FaceEmbeddingSetHandle + FaceClusterSetHandle + AnalysisSpec + ArtifactHandle → WesternAnalysisResult` | `pure_observation` |
...
Western Adult Beta的唯一正式分析链固定为service-local Helix Capability，不存在Worker或External Provider
替代分支：

~~~text
Run/Workspace内的正式Material Handle
  → libra.media.frames.extract@1
  → FrameArtifactSet（一个immutable composite Frame-set Artifact Handle）
  → shared.face.embedding.compute@1
  → FaceEmbeddingSetHandle
  → shared.face.cluster.compute@1
  → FaceClusterSetHandle
       ├─ libra.western.analysis.request@1
       │    → ArtifactHandle(kind=western_analysis)
       │    → libra.western.analysis.observe@1
       │    → WesternAnalysisResult
       └─ shared.face.reference.match@1
            + PersonReferenceProjection[0..256]
            → PersonMatchEvidence
~~~

上述Capability全部在clean ShelfDeck service Composition Root内通过Node.js注入的有界media/ONNX library与
Artifact/effect ports执行；不调用`shared.worker.asset.register|upload@1`，不产生或消费
`WorkerAssetReceipt|WorkerUploadReceipt|ExternalJobReceipt`，也没有Mirex、Python、FastAPI、Ollama、独立Face
service或调用者Result fixture的运行资格。`shared.face.*`继续是Owner-neutral计算：只写Foundation
Artifact/Effect事实，不写Libra或People Store；People只通过正式Facade提供上述Projection集合。

Capability selector由完整typed value而非括号别名决定：`frames.extract`只接受
`WorkspaceArtifactOutputTarget.outputKind=frame_set`，其`sourceInputDigest=SHA-256(JCS({sourceHandle,
samplingPlan}))`，Result中的`frameSetArtifactHandle.artifactKind=western_frame_set`；Embedding节点把该单一
composite Handle作为其`ArtifactHandle[1..16]`输入并同时冻结完整Frame Result ref，不把内部frame成员展开进Plan；
`western.analysis.request`只接受`outputKind=western_analysis`，其
`sourceInputDigest=SHA-256(JCS({frameArtifactSet,faceEmbeddingSetHandle,faceClusterSetHandle,analysisSpec}))`且
Result `ArtifactHandle.artifactKind=western_analysis`；`western.analysis.observe`只接受同一Request Result完整
Handle并逐字节匹配上述input/spec/registry provenance。机器Capability schema统一引用
`WorkspaceArtifactOutputTarget@1`和`ArtifactHandle@1`，semantic validator再执行这些closed variant约束，不得
物化`WorkspaceArtifactOutputTargetFrameSet`、`WorkspaceArtifactOutputTargetWesternAnalysis`、
`ArtifactHandleWesternAnalysis`等影子类型或兼容schema。

Western Analysis使用有限、phase-specific Supporting Work链；每个phase只允许一张immutable one-node Plan，后一个
Work只能在前一个durable typed Result成立后由Coordinator按固定顺序admit，不得动态修改已存在Plan或在Executor内
扩Graph。顺序固定为`frames → embedding → cluster → analysis_request → analysis_observe`，并从同一cluster
Result建立`reference_match`；`normalize`只能依赖已成功的observe Result集合。每个下游Plan的input binding必须
同时保存完整typed named input及其上游`{workId,attemptId,planId,eventId,resultId,capabilityRef,
resultSchemaRef,resultDigest}`引用，逐字节命中同Run、同Execution Basis的durable Event Result；禁止只保存Handle
ID、current Artifact row或调用者cache；完整Plan binding的JCS UTF-8 bytes必须≤`16 KiB`，超限稳定拒绝且不得
截断、拆成未声明side input或把成员改存caller cache。AnalysisSpec与各Output Target都在其phase Work admission时从已提交上游
Result构造，因此不需要预知未来Artifact content digest，也不能事后修改原Plan。Frames、Embedding、Cluster和
Analysis Request都是Effect Journal保护的service-local workspace/artifact
效果：effect identity由Plan Node、完整输入digest与Output Target digest唯一确定，物理bytes存在但Result未提交时
从相同journal/Artifact Registry恢复，Result已提交但Event未success时只完成原Event，不重复抽帧、推理、聚类或
写Artifact。Analysis Observe、Reference Match和Normalize是pure observation；restart只复用显式Result refs，
不得重读Worker/Provider、扫描latest Result或把新People Projection替换进旧Plan。

`PersonReferenceProjection[]`允许空集合，但`PersonMatchEvidence(matches=[])`只有在Frames、Embedding、Cluster及
真实Reference Match Event均成功、Cluster set完整且所有Cluster进入`unmatchedClusterIds`时才是正式no-match
Evidence。非空matches必须逐项命中Plan冻结Projection集合；后续Media-Cast仍只消费该Result形成
`WesternMediaCastBasisSnapshot@1`。`libra.media_cast.resolve@1`必须复用Match Plan冻结的同一Projection集合并重算
相同`referenceProjectionSetDigest`；`no_matches`时集合可为空且只形成空Draft，非空match时每个relation的
personId/revision/projection digest必须命中集合唯一成员。它不会使Shared Face Runtime或Libra获得People事实写权。
...
逐字节等于同次输入Set且Selection无损重建该Set；Western Product Metadata要求Draft的
`westernAnalysisVariantDigest`等于Basis内先于Normalize存在的Variant，Western Media-Cast要求Draft的
`westernMatchBasisDigest`等于Match Basis。两者均禁止携带Metadata Observation Set；最终包含Normalize Result的
Western Product Metadata `sourceBasisDigest`只在Commit/Fact形成，不能反写进产生它的Draft造成自引用。
`product_metadata` variant还要求Manifest内每项Artifact Handle可由`fx_artifact_registry`验证、Requirement逐字节命中
同一Draft、显式`verificationResultRef`命中同Run/Basis的passed `ArtifactManifestVerification@1`，且可选
`mediaCastFactRef`命中同一Run immutable `MediaCastFact`。完整Manifest随Fact Owner row冻结；任何不一致都在提交前
拒绝，不能由Executor修补Draft、信任调用者Evidence pair或扫描Foundation Result。

#### 8.6.7 Libra Media Production Capability

| Capability ref | Input → Output | Effect Class |
| --- | --- | --- |
| `libra.transcode.input.verify@1` | `PhysicalMaterialReadHandle\|WorkspaceMaterialHandle + MediaProbeEvidence + EncodeIntent + MediaExecutionDeviceSnapshot → TranscodeInputVerification` | `pure_observation` |
| `libra.media.remux@1` | `Material handle + RemuxIntent + WorkspaceMediaOutputTarget → WorkspaceMediaHandle` | `workspace_write` |
| `libra.media.transcode@1` | `Material handle + EncodeIntent + MediaExecutionDeviceSnapshot + TranscodeInputVerification + WorkspaceMediaOutputTarget → WorkspaceMediaHandle` | `workspace_write` |
| `libra.product_media.verify@1` | `ProductMediaCandidateInput → ProductMediaVerification` | `pure_observation` |
| `libra.product_output.select@1` | `ProductOutputSelectionInput → SelectedProductOutput` | `pure_observation` |
| `libra.product.conformance.verify@1` | `ProductConformanceInputSnapshot → ProductConformanceEvidence` | `pure_observation` |
| `libra.product_package.publish@1` | `LibraDeliverablePromotionDecision + ResponsibilityControlCommitHandle → OnDeckProductPackageCommitReceipt` | `responsibility_control_commit` |
| `libra.workspace.material.reclaim@1` | `WorkspaceMaterialHandle + WorkspaceCleanupEffectIntent → ReclamationReceipt` | `workspace_write` |
| `libra.workspace.cleanup.commit@1` | `WorkspaceCleanupCommitDecision + ResponsibilityControlCommitHandle? → WorkspaceCleanupCommitReceipt` | `responsibility_control_commit` |

`libra.media.transcode`一次Event只执行Plan已经声明的一种EncodeIntent。Planner在Plan中冻结完整输入Probe、
Platform已验证Device Snapshot及Workspace Output Target；Executor只能在该Device/Intent/Target内选择bounded
rate-control和FFmpeg命令细节。它不能读取current Device/Workspace、隐藏replan、失败后换Device、改为External
Upgrade、追加Remux、提交Product或修改Acceptance Spec。`libra.media.remux`遵守同一Target合同，但不接受或选择
Compute Device。两个workspace-write Capability都只能由Effect Journal以Target stable identity恢复相同效果。

`libra.product_output.select`不把调用数组顺序当成Policy：Planner必须把Acceptance Spec派生的显式rank冻结进
`ProductOutputSelectionCriteria@1`，Capability只按closed passed-filter、rank和verification ID tie-break执行。
因此多个合法产品之间的业务偏好仍来自Acceptance Spec/Planner，不被Executor硬编码；无passed候选返回typed
`not_selected`，不得“选第一个”或自行降低Requirement。`libra.product.conformance.verify`只消费完整bounded
Product/Spec Snapshot，不扫描latest Fact、current Spec或任何Owner Store。
...
`Verified care inventory change`允许`changeKind=person_relation_correction`：它必须携带previous inventory revision、
完整new relation snapshot、确定性Evidence与unchanged material/control set digest；此分支不执行Material Effect，
但仍以新Inventory revision和Receipt原子提交，不能原地UPDATE历史relation或绕过Aftercare Basis。
...
Resolution另外使用三个精确named input：

- `PerceptionResolutionQuery`：冻结请求fact kind与当前Query Identity Evidence；
- `PerceptionResolutionRecordSet`：冻结Owner Application按当前Rule取得的完整候选Record及相关Record Relation；
- `PerceptionResolutionRuleSnapshot`：冻结本次candidate retrieval、Anchor匹配、strength、冲突与duplicate proof的
  可执行系统规则；只传revision/digest而没有规则内容不满足合同。
...
| `perception.dedup.resolve@1` | `PerceptionResolutionQuery + PerceptionResolutionRecordSet + PerceptionResolutionRuleSnapshot → PerceptionResolutionDraft` | `pure_observation` |
...
lineage evidence。Resolution Executor不得读取Perception Repository、Foundation Event Result、Provider或缓存；
只有`PerceptionResolutionInputAssembler`可以在Perception Application内读取本域Repository并形成完整、
digest-bound的Record Set。Perception Query由public Resolution Facade返回已提交Resolution；相同三重digest
不需要为每次消费创建Event，变化后必须先提交新revision，HTTP GET仍保持无副作用。

#### 8.6.14 People Management Capability

| Capability ref | Input → Output | Effect Class |
| --- | --- | --- |
| `people.registration_evidence.observe@1` | `(Provider person hint + IntegrationHandle) | OnDeckPersonEvidenceProjectionItem → PersonRegistrationEvidence` | `pure_observation` |
| `people.reference_asset.import@1` | `External/user asset handle + People Workspace → ArtifactHandle` | `workspace_write` |
| `people.reference_fact.commit@1` | `PeopleReferenceMaintenanceDecision + DomainFactCommitHandle → PersonReferenceRevision` | `domain_fact_commit` |
...
Direct Person Registration由`PersonRegistrationCoordinator`消费`DirectPersonRegistrationDecision`并运行People
普通Domain transaction，不伪造Candidate或`people.person.commit`Event。Reference add/release则始终通过
`ReferenceAssetCoordinator`组织既有import、embedding与reference commit。`add_image` Decision必须同时绑定
一个稳定`referenceAssetId`、一个稳定`referenceFaceId`、精确Artifact Handle、`vectorCount=1`的Embedding
Handle与model ref；`release_image`必须绑定当前active Asset、它唯一派生Face及expected reference revision。
CommitParticipant不得从Artifact Registry、Workspace、Foundation Result或People其他Store补读这些字段。

Merge后Reference Fact仍归原personId历史保存。`PersonReferenceQueryFacade`查询target Person时可以按正式Merge
correlation展开target及全部merged-source的当前active Reference，Projection digest必须包含每个贡献Person的
personId/currentReferenceRevision/referenceSetDigest；展开不复制Asset/Face、不改变其Owner。merged-source
contribution在Admin Projection中标记为`inheritedReadOnly=true`，普通release命令只允许`personId`等于Asset真实
ownerPersonId的target-owned Reference；删除target图片不得隐式释放source图片。

Reference Projection checkpoint固定保存在`people_persons.current_reference_projection_revision`和
`current_reference_projection_digest`，不是Foundation Event Result、UI `read_*`缓存或按查询临时生成的计数。
Person首次成立时checkpoint revision固定为`1`；其basis为该Person首revision、JSON `null`的
`currentReferenceRevision`和空`contributions`。任何People Owner事务只要改变某个可查询Person的Projection
semantic basis，就必须在同一事务内计算新digest：digest相同时保留revision，digest不同时以CAS恰好递增`1`并
替换digest。Registration初始化自己的checkpoint；Reference Commit更新真实Owner及受Merge correlation影响的
query target；Merge Commit更新所有受该新correlation影响的Person checkpoint。不得在异步Read-model中另起
一套revision。

进程重启或Projection payload缓存重建时，`PersonReferenceQueryFacade`从People Canonical Facts、current
Reference pointers和正式Merge correlations重新组装同一semantic basis；重算digest必须等于持久checkpoint，
随后复用原revision。重建不得把revision重置为`1`、按`MAX(reference_revision)`猜测、按启动次数递增或由GET
补写。现有Person缺失checkpoint或重算不一致属于`PEOPLE_REFERENCE_PROJECTION_INVARIANT_VIOLATION`，必须作为
People/Projection系统故障暴露；clean schema不允许lazy default或静默修复。
...
| --- | --- |
| `PhysicalMaterialIdentity@2` | `materialKey,mountScopeId,inode,sizeBytes,fingerprintAlgorithm=middle-256k-sha256,fingerprintVersion=1,contentFingerprint`；`inode`为无符号十进制字符串；`materialKey=sha256(JCS({schema:"physical-material-identity@2",mountScopeId,inode,sizeBytes,fingerprintAlgorithm:"middle-256k-sha256",fingerprintVersion:1,contentFingerprint}))`的lowercase hex |
| `RelatedMaterialReference` | `referenceId,primaryMaterialKey,role(nfo|poster|fanart|subtitle|external_audio|chapter|sidecar),identity(PhysicalMaterialIdentity@2),endpointId,location,fingerprintAlgorithm,fingerprintVersion,contentFingerprint,associationEvidenceDigest,referenceDigest`；三项fingerprint字段逐字节等于identity；`referenceId=SHA-256(JCS({schema:"procurement.related-material-reference-id@1",primaryMaterialKey,role,relatedMaterialKey:identity.materialKey,endpointId,location}))`；`referenceDigest=SHA-256(JCS(完整value excluding referenceDigest))`；同一数组最多256项，按referenceId UTF-8 bytes升序且ID唯一；它是immutable Package fact，不形成独立Observation membership或Material Control |
...
| `PlatformComputeDeviceQuery` / `PlatformComputeDeviceReadResult` / `MediaExecutionDeviceSnapshot` | Query固定为`{deviceId,expectedProbeRevision?,expectedCapabilityDigest?,queryDigest}`，`queryDigest=SHA-256(JCS(完整Query excluding queryDigest))`。Snapshot固定为`{deviceId,deviceClass(software_cpu|intel_qsv|nvidia_nvenc|amd_vaapi|remote_worker),probeRevision,capabilitySchemaRef,capabilityPayload,capabilityDigest,enabled(true),state(ready),workerRef?,snapshotDigest}`；`deviceClass`逐字节复制Owner row的closed `device_kind`；`capabilityPayload`是`platform_compute_device_probes.capability_json`的完整closed typed value并至少声明`supportedVideoCodecs[]/supportedRateControlModes[]`，其JCS digest必须等于capabilityDigest；`workerRef`只允许remote_worker且冻结Worker ID/revision/capability digest，不含secret；`snapshotDigest=SHA-256(JCS(完整Snapshot excluding snapshotDigest))`。Read Result为`{queryDigest,resultKind(found|not_found|stale|unavailable|integrity_error),snapshot?,reasonCode?,resultDigest}`，found恰含Snapshot；reason依次只允许`device_not_found|device_probe_changed|device_disabled|device_not_ready|device_probe_integrity_failure`；`resultDigest=SHA-256(JCS(完整Result excluding resultDigest))`。Port从Platform Owner rows形成Result，完整值≤`16 KiB`；它不发Permit、不决定Device、不暴露driver/config/secret |
| `EncodeIntent` | exact machine type固定为`{intentId,revision,schemaRef:"EncodeIntent@1",libraRunId,sourceHandleDigest,mediaRequirementDigest,outputContainer:"matroska",outputExtension:"mkv",video{codec:"hevc",rateControlMode(target_size|quality_bound),targetVideoBitrateBps?,qualityBound?,preserveRaster:true,forbidUpscale:true},audio{mode:"copy"},subtitle{mode:"copy"},deviceClass(software_cpu|intel_qsv|nvidia_nvenc|amd_vaapi|remote_worker),intentDigest}`；`target_size`恰含positive `targetVideoBitrateBps`且`qualityBound=NULL`，`quality_bound`恰含bounded `qualityBound`且bitrate为NULL；`intentId=SHA-256(JCS({schema:"libra.media-production-intent-id@1",semanticIntent:完整value excluding intentId and intentDigest}))`，`intentDigest=SHA-256(JCS(完整value excluding intentDigest))`；完整值≤`16 KiB`。Planner只能从Acceptance Spec、Probe Evidence与正式Device Snapshot编译；Executor不得改写Intent、选择Device或补第二种生产动作 |
...
| `WorkspaceMediaOutputTarget` | `targetId,libraRunId,executionBasisDigest,workspaceId,expectedWorkspaceRevision,expectedWorkspaceStateDigest,rootSnapshot(PlatformWorkspaceRootSnapshot),workspaceScopeDigest,targetRelativePath,outputRole(product_media),productionIntentDigest,effectScopeDigest,targetDigest`；relative path必须是Workspace-root-relative规范路径、不得为`.`/`..`/absolute/symlink escape，并由Planner唯一分配；`targetId=SHA-256(JCS({schema:"libra.workspace-media-output-target-id@1",workspaceId,targetRelativePath,productionIntentDigest}))`；`effectScopeDigest=SHA-256(JCS({schema:"libra.workspace-media-output-effect-scope@1",targetId,libraRunId,executionBasisDigest,workspaceId,expectedWorkspaceRevision,expectedWorkspaceStateDigest,rootSnapshotDigest:rootSnapshot.snapshotDigest,workspaceScopeDigest}))`；`targetDigest=SHA-256(JCS(完整value excluding targetDigest))`，完整值≤`16 KiB`。Target由LibraProductionCoordinator从current Workspace revision及其已冻结Platform Root Snapshot形成并进入Plan；CapabilityExecutionContext、Executor、FFmpeg/Worker adapter都不能选择或修改target path |
| `WorkspaceArtifactOutputTarget` | exact machine type固定为`{targetId,libraRunId,executionBasisDigest,workspaceId,expectedWorkspaceRevision,expectedWorkspaceStateDigest,rootSnapshot(PlatformWorkspaceRootSnapshot),workspaceScopeDigest,targetRelativePath,outputKind(frame_set\|western_analysis),sourceInputDigest,effectScopeDigest,targetDigest}`；`frame_set`的targetRelativePath是该Event唯一有界frame-set root，`western_analysis`是唯一result artifact file；二者均须为Workspace-root-relative规范路径且不得`.`/`..`/absolute/drive/backslash/symlink escape。`targetId=SHA-256(JCS({schema:"libra.workspace-artifact-output-target-id@1",workspaceId,targetRelativePath,outputKind,sourceInputDigest}))`；`effectScopeDigest=SHA-256(JCS({schema:"libra.workspace-artifact-output-effect-scope@1",targetId,libraRunId,executionBasisDigest,workspaceId,expectedWorkspaceRevision,expectedWorkspaceStateDigest,rootSnapshotDigest:rootSnapshot.snapshotDigest,workspaceScopeDigest,outputKind,sourceInputDigest}))`；`targetDigest=SHA-256(JCS(完整value excluding targetDigest))`，完整值≤`16 KiB`。Target只由Libra Production Planner从同一Run的immutable Workspace revision/Root Snapshot和上游typed input形成；Executor、ONNX/media library或ExecutionContext不得选路径、扩大root或换output kind |
| `SamplingPlan` | exact machine type固定为`{contractId,revision,schemaRef:"SamplingPlan@1",intervalMs,maxFrames,frameProfileDigest,typedParameters[],digest}`；`intervalMs=1..86400000`、`maxFrames=1..1024`。typed parameters最多64项，按parameter的UTF-8 bytes排序且name唯一，每项为`{parameter,valueType(string\|integer\|number\|boolean),value,valueDigest}`并要求value的JSON type命中valueType、`valueDigest=SHA-256(JCS({parameter,valueType,value}))`；`digest=SHA-256(JCS(完整value excluding digest))`且完整值≤`16 KiB`。Planner从Acceptance Spec/Western system profile与source duration编译并冻结，Executor不得自行增加帧、改变间隔或以current配置覆盖 |
| `FaceModelRef` | exact machine type固定为`{contractId,revision,schemaRef:"FaceModelRef@1",mode(western_frame_set\|single_reference_face),modelId,modelRevision,modelDigest,runtimeKind(onnx),inputContractDigest,outputContractDigest,licenseDigest,typedParameters[],digest}`；typed parameters使用SamplingPlan同一closed scalar item、排序与digest规则且最多64项；`digest=SHA-256(JCS(完整value excluding digest))`，完整值≤`16 KiB`。Model Pack loader只可按完整tuple装载本地已验证Model bytes；路径、GPU设备、session对象和下载行为不进入Ref。`single_reference_face`继续服从8.6.3唯一Face约束；Western chain固定`western_frame_set` |
| `ClusterParameters` | exact machine type固定为`{parameterSetId,revision,schemaRef:"ClusterParameters@1",modelRefDigest,distanceMetric(cosine),distanceThreshold,minClusterSize,typedParameters[],digest}`；threshold为`0..1`有限数，minClusterSize为positive整数，typed parameters使用上述closed规则且最多64项；`digest=SHA-256(JCS(完整value excluding digest))`，完整值≤`16 KiB`。同一Plan的`modelRefDigest`必须逐字节等于Embedding使用的FaceModelRef digest；Executor不得从current配置、设备或结果分布改阈值 |
| `AnalysisSpec` | exact machine type固定为`{specId,revision,schemaRef:"AnalysisSpec@1",libraRunId,runExecutionBasisDigest,contentProfile(western_adult),sourceMaterialDigest,frameArtifactSetDigest,faceModelRefDigest,clusterParameterDigest,analysisVariantRef,outputContractRef("helix://contracts/types/WesternAnalysisResult/v1"),outputContractDigest,typedParameters[],specDigest}`；typed parameters使用上述closed规则且最多64项；`analysisVariantRef`为版本化analysis profile/algorithm variant key，不是Provider/Worker job ID，也不等于或依赖事后由Result集合形成的`WesternAnalysisVariant.variantId`；`specId=SHA-256(JCS({schema:"libra.western-analysis-spec-id@1",libraRunId,runExecutionBasisDigest,sourceMaterialDigest,frameArtifactSetDigest,faceModelRefDigest,clusterParameterDigest,analysisVariantRef,outputContractDigest}))`，`specDigest=SHA-256(JCS(完整value excluding specDigest))`且完整值≤`16 KiB`。Request/Observe必须使用同一完整Spec；任何上游Handle或Model/Cluster fence漂移稳定拒绝，不允许current model/config fallback |
| `MediaRequirement` | exact machine type固定为`{requirementId,revision,schemaRef:"MediaRequirement@1",acceptanceSpecId,acceptanceSpecRecordDigest,contentProfile(movie|series|jav|western_adult),structureKind(single|season),mandatoryMedia{mediaForm(any|stream_file),videoCodec(any|hevc),container(any|matroska),fileExtension(any|mkv),minimumRasterClass(none|4k),acceptedPrimaryAudioClasses[eac3_atmos|truehd|truehd_atmos|dts_hd_ma|dts_x],forbidSystemUpscaleFor4k},space{unit(product|episode),maxSizeGiB?,maxSizeBytes?},requirementDigest}`；`revision=AcceptanceSpec.specRevision`，其余字段逐字节复制同一immutable Spec；数组按UTF-8 bytes排序且唯一，无空间上限时两个max字段同时NULL，有上限时bytes按8.6.18精确换算；不接受`videoRequirementDigest|audioRequirementDigest|subtitleRequirementDigest|typedParameters`或其他opaque替代。`requirementId=SHA-256(JCS({schema:"libra.media-requirement-id@1",acceptanceSpecId,revision,mandatoryMedia,space}))`，`requirementDigest=SHA-256(JCS(完整value excluding requirementDigest))`，完整值≤`16 KiB` |
| `ProductMediaCandidateInput` | closed union公共字段`candidateId,candidateNodeId,candidateBasisDigest,libraRunId,mediaRequirement(MediaRequirement),candidateKind(direct_input|workspace_output),inputDigest`。direct_input恰含`sourceMaterialHandle(PhysicalMaterialReadHandle),sourceProbeEvidence(MediaProbeEvidence)`且`candidateBasisDigest=SHA-256(JCS(sourceMaterialHandle))`；workspace_output恰含`workspaceMediaHandle(WorkspaceMediaHandle),sourceProbeEvidence,outputProbeEvidence`且`candidateBasisDigest=SHA-256(JCS({schema:"libra.workspace-product-candidate-basis@1",outputTargetId:workspaceMediaHandle.outputTargetId,outputTargetDigest:workspaceMediaHandle.outputTargetDigest,productionIntentDigest:workspaceMediaHandle.productionIntentDigest}))`。`candidateNodeId`是immutable Plan中产生/声明该候选的Node ID；direct input也由Plan显式Node声明，不伪造Event ID。`candidateId=SHA-256(JCS({schema:"libra.product-media-candidate-id@1",candidateNodeId,candidateKind,candidateBasisDigest,mediaRequirementDigest:mediaRequirement.requirementDigest}))`，因此Workspace实际content hash在运行后产生也不改变Plan已冻结的Candidate identity；`inputDigest=SHA-256(JCS(完整value excluding inputDigest))`，完整值≤`64 KiB` |
| `ProductOutputSelectionCriteria` / `ProductOutputSelectionInput` | Criteria为`{criteriaId,libraRunId,acceptanceSpecId,acceptanceSpecRecordDigest,mediaRequirementDigest,rankedCandidates[1..32]{rank,candidateId,candidateNodeId},tieBreak(verification_id_utf8),criteriaDigest}`；rank为1..N连续且candidateId/candidateNodeId各自唯一，按rank排序；这些ID由immutable Plan与Candidate input确定，不依赖运行后Result。`criteriaId=SHA-256(JCS({schema:"libra.product-output-selection-criteria-id@1",libraRunId,acceptanceSpecId,mediaRequirementDigest,rankedCandidates}))`，`criteriaDigest=SHA-256(JCS(完整Criteria excluding criteriaDigest))`。Input为`{criteria,candidates[1..32 ProductMediaVerification],candidateSetDigest,inputDigest}`；每个rank ref恰好匹配一项candidate的ID/Node ID，反之亦然；candidate按verificationId UTF-8 bytes排序，`candidateSetDigest=SHA-256(JCS({schema:"libra.product-media-verification-set@1",items:candidates}))`，`inputDigest=SHA-256(JCS(完整value excluding inputDigest))`，完整值≤`512 KiB`。显式rank由Production Planner按同一Acceptance Spec形成，不从调用数组位置推断 |
| `ResolvedProviderIdentity` | exact machine type固定为`{provider(tmdb|jav|internal),namespace(tmdb_movie|tmdb_series|jav_code|internal_identity),providerKey,seasonNumber?,identityAnchorDigest}`；namespace与provider唯一映射，只有`tmdb_series`恰含positive `seasonNumber`，其余variant该字段为NULL；`providerKey`是trim后非空规范字符串，JAV使用规范番号；`identityAnchorDigest=SHA-256(JCS(完整value excluding identityAnchorDigest))`。集合按`provider,namespace,providerKey,seasonNumberOrZero`的UTF-8 bytes/整数升序且tuple唯一，不允许generic snapshot或仅digest替代 |
| `ProductConformanceFactSnapshot` | closed union公共字段`productFactId,factKind,factRevision,schemaRef,factValue,factDigest,evidenceDigest,referenceDigest`；只允许三种variant：`resolved_identity + ResolvedProductIdentity@1`、`product_metadata + ProductMetadataFact@1`、`media_cast + MediaCastFact@1`，`schemaRef/factValue`必须匹配variant且`factDigest`由完整typed value重算；`referenceDigest=SHA-256(JCS(完整value excluding referenceDigest))`。集合必须恰含一项resolved identity、至多一项product metadata和至多一项media cast；后两项缺失是可评估的业务unmet，不是snapshot integrity failure；不得退化为`{objectId,revision,schemaRef,digest}`或`{valueDigest}` |
| `ArtifactConformanceVerificationSnapshot` | exact machine type固定为`{ordinal,verifiedManifestItem,artifactManifestItem?,verificationResultRef,verificationValue(ArtifactManifestVerification@1),snapshotDigest}`；`verifiedManifestItem`逐字节复制`VerifiedArtifactManifest@1`单项，`verificationResultRef`逐字节等于该单项ref，`verificationValue`是该ref指向的完整typed Result且Foundation完整Result digest命中ref；Result必须passed并含同一Handle/revision/digest及完整同一`ArtifactRequirement@1`。若同一Artifact进入最终Product，`artifactManifestItem`逐字节匹配Handle/kind/revision/digest/requirement并给出`materializationState`，否则为NULL；`snapshotDigest=SHA-256(JCS(完整value excluding snapshotDigest))`。数组按Manifest ordinal排序、恰覆盖Verified Manifest全部items、允许同一Result被多项复用；sidecar的passed Requirement固定证明renderable，poster/fanart的passed Requirement固定证明decodable，不再从摘要猜布尔值 |
| `ProductInventoryConformanceSnapshot` | exact machine type固定为`{productStructureSnapshot(ProductStructureSnapshot@1),productMaterialManifest(ProductionMaterialManifest@1),artifactManifest(ArtifactManifest@1),inventoryDigest}`；三个对象必须同一Run/Product revision且完整typed值可重算各自digest。`requireDomainBinding`只接受每个Material均具完整`libra_material_binding|workspace_material_reference`、对应location/Handle及binding Evidence；`requireChecksum`要求`physicalIdentity.fingerprintAlgorithm=middle-256k-sha256`且direct input命中Identity、workspace output还须命中Handle digest；Artifact materialization要求所需kind的Artifact Manifest item为`included_product`并存在同kind/digest Product Material member。`layoutModel=single`要求structure single、恰一primary且无Episode Claim；`season_episode`要求structure season、每个Product Scope Episode至少由一项primary Claim覆盖、所有Claim属于同一season scope且无scope外Episode；它验证logical product layout，不预判Arca最终目录路径。`inventoryDigest=SHA-256(JCS(完整value excluding inventoryDigest))`；不得用binding digest数组代替完整Manifest |
| `ProductConformanceInputSnapshot` | exact machine type固定为`{snapshotId,libraRunId,runExecutionBasisDigest,acceptanceSpec(AcceptanceSpec@1),resolvedIdentitySnapshot(ProductConformanceFactSnapshot@1 resolved_identity),productStructureSnapshot(ProductStructureSnapshot@1),productFactSnapshots[1..3 ProductConformanceFactSnapshot@1],verifiedArtifactManifest(VerifiedArtifactManifest@1),artifactVerificationSnapshots[0..256 ArtifactConformanceVerificationSnapshot@1],inventorySnapshot(ProductInventoryConformanceSnapshot@1),selectedProducts[1..32]{selectedProduct(SelectedProductOutput@1),verification(ProductMediaVerification@1),workspaceHandleDigest?},productFactSetDigest,artifactVerificationSetDigest,selectedProductSetDigest,productSnapshotDigest,snapshotDigest}`。Fact按`factKind,productFactId,factRevision`，Artifact Verification按ordinal，selected product按`selectedHandleId,selectedVerificationId`排序且tuple唯一；resolved snapshot必须与Fact集合唯一resolved item逐字节相等。每项selected Product必须`result=selected`并引用同一passed verification；workspaceHandleDigest只允许workspace_output且命中Product Staging Reference，direct_input为NULL且命中Run immutable Input Material。Acceptance Spec固定为Run Basis revision。三个set digest分别使用schema`libra.product-conformance-fact-set@1`、`libra.product-conformance-artifact-verification-set@1`、`libra.product-conformance-selected-product-set@1`覆盖对应完整数组；`productSnapshotDigest=SHA-256(JCS({schema:"libra.product-conformance-product-snapshot@1",resolvedIdentityReferenceDigest:resolvedIdentitySnapshot.referenceDigest,productStructureDigest:productStructureSnapshot.productStructureDigest,productFactSetDigest,verifiedArtifactManifestDigest:verifiedArtifactManifest.manifestDigest,artifactVerificationSetDigest,inventoryDigest:inventorySnapshot.inventoryDigest,selectedProductSetDigest}))`；`snapshotId=SHA-256(JCS({schema:"libra.product-conformance-input-id@1",libraRunId,runExecutionBasisDigest,acceptanceSpecRecordDigest:acceptanceSpec.recordDigest,productSnapshotDigest}))`，`snapshotDigest=SHA-256(JCS(完整value excluding snapshotDigest))`；完整值≤`8 MiB`并通过Plan typed binding refs装配，不进入Event Result/Outbox。所有nested schema必须`additionalProperties:false`并使用上述exact `$ref/oneOf`，禁止generic bounded snapshot、`valueDigest`或opaque object ref降级 |
...
| `MaterialFieldRegistrationCommand` | `{fieldId,name,policy(ExtractionPolicy@1 revision=1),access{fieldId,revision(1),endpointId,rootLocation,mountScopeId,mountScopeRevision,accessSchemaRef,accessDigest},contentProfileHint?(mixed|movie|series|jav|western_adult),idempotencyKey}`；closed root/nested shapes且两个fieldId一致；Host在Command `requestDigest`计算前把缺失Hint规范化为`mixed`，所以Owner participant只接收必有的规范化枚举；Policy/Access/Profile三者均以revision 1建立，禁止NULL Hint、额外字段或由Session/Execution Context补值 |
| `MaterialFieldProfileHintSnapshot` | 正式`schemaRef=helix://contracts/application-types/MaterialFieldProfileHintSnapshot/v1`；`{fieldId,revision,contentProfileHint(mixed|movie|series|jav|western_adult),hintDigest}`；`revision`为从1开始的positive safe integer；`hintDigest=SHA-256(JCS({schema:"procurement.material-field-profile-hint@1",fieldId,revision,contentProfileHint}))`；完整value必须命中一条immutable `proc_field_profile_hint_revisions` Owner row，禁止NULL、opaque profile ref、调用者默认、当前Field旁读或把Hint升级为Canonical Identity |
| `MaterialFieldProfileHintRevisionCommand` | `{operation(revise_profile_hint),fieldId,expectedProfileHintRevision,newContentProfileHint(mixed|movie|series|jav|western_adult),idempotencyKey,requestDigest}`；URL/body fieldId必须相等，actor只来自已验证Admin session并进入Audit而不由body声明；`requestDigest=SHA-256(JCS({schema:"procurement.material-field-profile-hint-revision-command@1",operation,fieldId,expectedProfileHintRevision,newContentProfileHint}))`由Host计算并由Owner重算，idempotencyKey作为receipt/dedup identity而不进入payload digest；只允许expected current CAS，same value是no-op，不能跳revision或修改历史row |
| `MaterialFieldAdminProjection` | `{fieldId,name,status,extractionPolicyId,extractionPolicyRevision,currentAccessRevision,currentProfileHintSnapshot(MaterialFieldProfileHintSnapshot),currentObservationRevision?,createdAtMs,updatedAtMs,policy(ExtractionPolicy@1),access{fieldId,revision,endpointId,rootLocation,mountScopeId,mountScopeRevision,accessSchemaRef,accessDigest,effectiveAtMs},projectionDigest}`；policy/access必须逐字节命中current pointers且两个fieldId一致；GET list/exact read必须返回完整current Profile Snapshot及既有Policy/Access projection，不能为增加Hint而缩水旧字段；`projectionDigest=SHA-256(JCS(完整value excluding projectionDigest))`；不内联历史revision列表 |
| `ProcurementMaterialFieldAdminResult` | `{materialField(MaterialFieldAdminProjection),commandReceiptId,resultDigest}`；register、revise access、revise profile hint、publish policy与deregister共用该application Result；`resultDigest=SHA-256(JCS(完整value excluding resultDigest))`；transport可在该typed value之外标记`replayed`，不得把它纳入持久Result；同key同规范化Command跨重启返回逐字节相同typed value，同key异payload冲突且不得建立第二Field/revision |
| `FieldObservationPageRequest` | `fieldObservationWorkId,observationId,pageOrdinal,expectedObservationRevision,cursorIn,pageBudget,profileHintSnapshot(MaterialFieldProfileHintSnapshot),requestDigest`；ID由Coordinator在Plan前分配并随重放保持；`fieldObservationWorkId`在整轮分页内不变，`observationId`每页唯一且只在该页重试中复用；同一Work全部Request的Profile Snapshot逐字节相同；pageOrdinal从0连续递增，pageBudget为`1..256`，单页物理读取上限为`64 MiB`；`requestDigest=SHA-256(JCS(Request excluding requestDigest))`；首次从未观察Field才允许`expectedObservationRevision=0`，新一轮Work第一页不重置Field revision |
| `FieldMaterialObservationSnapshot` | `materialObservationId,observationId,fieldId,accessRevision,accessDigest,fieldAccessHandleId,endpointId,mountScopeRevision,identity(PhysicalMaterialIdentity),location,sizeBytes,mtimeNs,ctimeNs,fingerprintVerifiedAtMs,observedAtMs,containmentDigest,realityDigest,provenanceDigest,snapshotDigest`；`sizeBytes`为不超过`2^53-1`的non-negative integer，`mtimeNs/ctimeNs`为`0..9223372036854775807`的十进制字符串并在SQLite持久化边界无损转为signed 64-bit INTEGER，两个`*AtMs`为non-negative safe integer；`materialObservationId=SHA-256(JCS({schema:"procurement.field-material-observation-id@1",observationId,materialKey:identity.materialKey}))`；`realityDigest=SHA-256(JCS({schema:"procurement.field-material-reality@1",identity,endpointId,location,sizeBytes,mtimeNs,ctimeNs}))`；`provenanceDigest=SHA-256(JCS({schema:"procurement.field-material-provenance@1",fieldId,accessRevision,accessDigest,fieldAccessHandleId,mountScopeRevision,containmentDigest,fingerprintVerifiedAtMs,observedAtMs}))`；`snapshotDigest=SHA-256(JCS(完整Snapshot excluding snapshotDigest))`；单项≤`4 KiB`，禁止opaque object ref替代完整snapshot |
...
| `MaterialControlProjectionSnapshot` | `materialKey,resultKind(available|unavailable),controlRevision?,controlState?(uncontrolled|controlled),ownerDomain?,ownerScopeType?,ownerScopeId?,regionProjection?(uncontrolled|procurement|production|finished_goods),failureCode?,evidenceDigest,projectionDigest`；available时revision 0只表示Query成功且从未存在row，controlled必须完整owner并按Foundation current row映射；unavailable时只允许稳定failure/evidence且禁止猜测revision/state/owner/Region；`evidenceDigest=SHA-256(JCS({schema:"foundation.material-control-evidence@1",materialKey,resultKind,controlRevision?,controlState?,ownerDomain?,ownerScopeType?,ownerScopeId?,failureCode?}))`，`projectionDigest=SHA-256(JCS(完整value excluding projectionDigest))` |
| `MaterialSelectionSnapshot` | `materialKey,activeGuards[{guardKind(run_selection|candidate_delivery),procurementRunId,runState,candidatePackageId?,packageDigest?,selectionRole,bindingRevision}],hasConflict,selectionBasisDigest`；`run_selection`只允许active/waiting Run且不得含Candidate字段，`candidate_delivery`允许来源Run已sealed并必须含精确Candidate/package digest，持续到Handoff A Accepted/Rejected终态；数组按guardKind/runId/candidatePackageId/role排序；`hasConflict=(activeGuards.length>0)`，digest覆盖完整value excluding itself |
...
| `SelectedFieldMaterialSet` | `procurementRunId,fieldId,members[{ordinal,materialKey,selectionRole(triage_input),physicalIdentity{mountScopeId,inode,sizeBytes,fingerprintAlgorithm(middle-256k-sha256),fingerprintVersion(1),contentFingerprint},sizeBytes,bindingRevision,eligibilityRevision,eligibilityBasisDigest,lastSnapshotDigest,lastObservationId,endpointId,location,realityDigest,provenanceDigest,controlSnapshot(MaterialControlProjectionSnapshot),admissionControlAction(acquire|assert_same_field),basisMemberDigest}],selectionDigest`；成员`1..256`项、ordinal从0连续且materialKey唯一，按materialKey的UTF-8 bytes升序后赋ordinal；每项Physical Identity/size必须由Run Admission同事务重读current `proc_field_materials`形成，满足`PhysicalMaterialIdentity@2`的materialKey公式且size为non-negative safe integer；每个`basisMemberDigest=SHA-256(JCS(完整member excluding basisMemberDigest))`，`selectionDigest=SHA-256(JCS({schema:"procurement.selected-field-material-set@1",procurementRunId,fieldId,members}))`；Control Action为acquire时snapshot必须uncontrolled，为assert时必须由同一`material_field/fieldId`控制；禁止opaque material key数组替代完整member snapshot |
| `BdmvAssessmentInput` | `runId,bdmvGroupKey,scopeDigest,memberSetDigest,accessRevision,profileHint`；只携带冻结Run/Observation scope引用和digest，不携带完整成员数组、路径、Store或Runtime authority；`profileHint`为`movie|series|jav|western_adult|mixed`且由Run Execution Basis冻结 |
| `TriageMaterialProbeBatch` | `procurementRunId,runBasisDigest,selectionDigest,batchOrdinal,members[]`；每批`1..100`个逻辑项，普通项为`material`并携带`readHandle + mediaProbe`，BDMV项为`bdmv_container`并携带`scopeDigest + bdmvAssessment`；按selectionOrdinal连续升序，全部Batch精确覆盖Selection的逻辑项一次；BDMV容器内部物理成员不得各自出现为Probe项；`memberDigest=SHA-256(JCS(完整member excluding memberDigest))`，`batchDigest=SHA-256(JCS(完整value excluding batchDigest))`；不得用result ID、数组位置或Store旁读代替映射 |
| `MaterialFieldContext` | `fieldId,accessRevision,accessDigest,profileHintSnapshot(MaterialFieldProfileHintSnapshot),memberContexts[{selectionOrdinal,materialKey,fieldRelativeLocation,baseName,extension,parentSegments[],layoutEvidenceRefs[{evidenceId,payloadDigest,boundedScopeDigest}]}],contextDigest`；Profile Snapshot逐字节复制本Run Execution Basis且fieldId一致，Executor不得读current Field或接收caller override；成员按selectionOrdinal完整覆盖Selection，location/baseName/extension/parentSegments均从冻结Binding与Shared Layout Evidence确定，Linux大小写敏感且不得归一化成另一个路径；Layout ref必须来自本Run输入Handle授权的scope；`contextDigest=SHA-256(JCS(完整value excluding contextDigest))`，禁止opaque fieldRefs/context object |
...
| `TriageStructureInspectionInput` | `selectedFieldMaterialSet(SelectedFieldMaterialSet),probeBatches[TriageMaterialProbeBatch],bdmvAssessments[BdmvAssessmentEvidence + UnitScopeReference],playabilityPages[PlayabilityEvidence],materialFieldContext(MaterialFieldContext),observationScopeProjection{projectionRevision,scopeDigest,entriesDigest,entryCount,entries[...]},pageRequest(TriageStructurePageRequest),inputDigest`；Selection与全部普通Probe、BDMV Assessment、Playability和Context必须具有相同run/basis/selection digest并完整覆盖Selection；BDMV Assessment只以`UnitScopeReference`和durable evidence进入，不携带1024成员数组；Observation Scope Projection最多4096项，来自本Run冻结Observation revision；完整值通过Plan typed Result binding装配，Executor不得从Procurement或Foundation Store补读；`inputDigest=SHA-256(JCS({schema:"procurement.triage-structure-input@1",selectionDigest,probeBatchDigests[],bdmvAssessmentPayloadDigests[],playabilityPayloadDigests[],contextDigest,observationScopeProjectionDigest,pageRequest}))` |
| `TriageIdentityMetadata` | `claimedTitle,claimedYear?,seasonClaim?,javCode?,contentProfileHint,sourceHints[{hintKind,hintValue,evidenceDigest}],metadataDigest`；`contentProfileHint`必须等于本Run Profile Snapshot值；sourceHints必须恰含一项`field_content_profile_hint`，固定`hintValue=contentProfileHint,evidenceDigest=profileHintSnapshot.hintDigest`，不得省略或使用Run/Observation/current Field的其他摘要；`seasonClaim`为`{claimKind(explicit_number|provisional_group),seasonNumber?,provisionalGroupKey?,claimDigest}`且两分支互斥；source hint只允许`field_content_profile_hint|filename_title|directory_title|filename_year|directory_year|filename_season|directory_season|filename_episode|jav_code|disc_structure|temporary_label`，按`hintKind,hintValue,evidenceDigest`排序；`metadataDigest=SHA-256(JCS(完整value excluding metadataDigest))`，所有值只是可纠正Evidence，不是Provider或Canonical Identity |
| `SeasonContinuityClaim` | `claimKind(provider_season_identity|triage_grouping_lineage),claimNamespace,claimKey,claimDigest,evidenceDigest`；这是Level 3、Procurement Candidate relation与Libra Subject relation共用的唯一nominal value，不存在别名或转换层；`claimDigest=SHA-256(JCS({schema:"season-continuity-claim@1",claimKind,claimNamespace,claimKey}))`；数组最多64项，按`claimKind,claimNamespace,claimKey`的UTF-8 bytes升序且tuple唯一；`seasonContinuityClaimSetDigest=SHA-256(JCS({schema:"season-continuity-claim-set@1",items:排序后的完整SeasonContinuityClaim数组}))`，空数组也按同一公式形成确定digest |
| `TriageUnitSnapshot` | `unitId,mediaType(single|group),contentProfile(movie|series|jav|western_adult),structureKind(single|season),displayIdentity,identityMetadata(TriageIdentityMetadata),seasonContinuityClaims[SeasonContinuityClaim],seasonContinuityClaimSetDigest,members[...]? ,memberScope?{scopeKind(bdmv_container),procurementRunId,bdmvGroupKey,scopeDigest,memberSetDigest,memberCount,topologyDigest,selectedPayloadSetDigest},relatedScope{scopeKind(ordinary_parent|bdmv_external_parent),parentRelativeLocation,stemKey,observationProjectionRevision,relatedRuleRevision,scopeDigest},materialInputForm(stream_file|bdmv|dvd|iso),unitDigest`；普通Unit使用完整members；BDMV Movie Unit使用memberScope且禁止在Structure Result中内联完整成员数组，Candidate Context根据scope reference重建成员；members/memberScope二选一；unitId覆盖scope、topology、member set和selected payload digest，unitDigest覆盖完整Unit；单标题BDMV仍只形成一个Movie Unit，内部STREAM/PLAYLIST/CLIPINF/index/MovieObject不能独立成为Candidate或Related；完整Unit的JCS bytes必须≤`64 KiB` |
...
| `ProcurementCandidateAcceptanceClosureResult` | `offerId,candidatePackageId,packageRevision,packageDigest,acceptanceBasisDigest,terminalDeliveryState(accepted),transferredMaterialCount,transferredMaterialSetDigest,handoffReceiptDigest,closureDigest`；transferred成员恰好是该Candidate全部非空Reservation，按materialKey UTF-8 bytes升序；`transferredMaterialSetDigest=SHA-256(JCS({schema:"procurement.handoff-a-accepted-transferred-material-set@1",items:[{materialKey,terminalDisposition:"handoff_accepted",terminalEvidenceDigest:handoffReceiptDigest}]}))`；`closureDigest=SHA-256(JCS(完整value excluding closureDigest))`；它是Procurement application transaction result而非新增Capability Result family，必须可由Delivery与Run member terminal rows重建 |
...
| `ProcurementRunExecutionBasis` | `procurementRunId,fieldId,fieldStatus(active),fieldAccess{revision,digest},profileHintSnapshot(MaterialFieldProfileHintSnapshot),terminalObservation{revision,fieldObservationWorkId,profileHintSnapshot(MaterialFieldProfileHintSnapshot)},extractionPolicy{policyId,revision,digest},triageRule(ProcurementTriageRuleSnapshot),sourceRetryIntentId?,selectedFieldMaterialSet(SelectedFieldMaterialSet),basisDigest`；两份Profile Snapshot必须逐字节相等且命中Admission时current Hint revision；嵌套Set的run/field必须一致；`basisDigest=SHA-256(JCS(完整value excluding basisDigest))`；普通Admission只能使用Registry当前active Snapshot，Retry consume只能使用Intent冻结且仍为active的Snapshot；Run成立后按精确historical entry恢复，不跟随Profile/Rule active pointer；它由`proc_procurement_runs + proc_run_materials`、精确immutable Field/Profile Hint/Policy/Observation引用及Triage Registry entry关系化/引用式恢复，不能用Foundation Work、Event input、调用者值或最新Field current row代替；重建值与原digest不一致是系统故障 |
...
| `ProcurementRetryAdmissionHead` | `fieldId,fieldStatus(active|deregistered),profileHintSnapshot(MaterialFieldProfileHintSnapshot),fieldAccess{revision,digest},terminalObservation{resultKind(available|unavailable),revision?,fieldObservationWorkId?,profileHintSnapshot?},extractionPolicy{policyId,revision,digest},triageRule(ProcurementTriageRuleSnapshot),headDigest`；available时要求完整terminal pair及逐字节相同的Profile Snapshot，unavailable时禁止Observation pair/snapshot；`headDigest=SHA-256(JCS(完整value excluding headDigest))`；Intent创建要求`fieldStatus=active`、Profile Snapshot命中current Field、terminal available且Rule为Registry active entry，消费时形成同schema actual head并逐字段比较；完整value≤`16 KiB` |
| `ProcurementRetryConsumeMemberSnapshot` | `retryIntentId,ordinal,materialKey,currentAdmissionHeadDigest,materialState(present|missing),currentBindingRevision?,currentEligibilityRevision?,currentEligibilityState?(unknown|eligible|ineligible),currentEligibilityBasisDigest?,currentSelection{hasConflict,selectionBasisDigest},currentControlSnapshot(MaterialControlProjectionSnapshot),consumeOutcome(matched|stale),staleReasonCode?,snapshotDigest`；present要求完整Binding/Eligibility，missing禁止这些字段；matched要求无reason，stale要求一个closed primary reason；`snapshotDigest=SHA-256(JCS(完整value excluding snapshotDigest))`；完整value≤`4 KiB` |
| `ProcurementRetryIntent` | `retryIntentId,fieldId,failedRunId,failedRunBasisDigest,retryAdmissionHead(ProcurementRetryAdmissionHead),members[{ordinal,materialKey,failedRunMaterialDigest,expectedBindingRevision,expectedEligibilityRevision,expectedEligibilityBasisDigest,expectedSelectionBasisDigest,expectedSelectionHasConflict(false),expectedControlSnapshot(MaterialControlProjectionSnapshot),memberPreconditionDigest}],retryScopeDigest,preconditionSetDigest,actorId,idempotencyKey,intentDigest`；members为failed Run中`released+triage_failed`成员的`1..256`项稳定排序快照；全部digest使用下述唯一formula；`intentDigest=SHA-256(JCS(完整value excluding intentDigest))` |
| `ProcurementRetryIntentReceipt` | `ReceiptEnvelope + retryIntentId + fieldId + failedRunId + intentDigest + retryScopeDigest + preconditionSetDigest + intentState(open)`；Receipt固定`receiptKind=procurement_retry_intent_created,ownerDomain=procurement,scopeType=procurement_retry_intent,scopeId=retryIntentId,scopeDigest=intentDigest`；它只证明Intent Create事务成立，消费终态由`ProcurementRetryAdmissionResult`表达 |
| `ProcurementRetryIntentAvailableMessage` | `messageKind(procurement_retry_intent_available),retryIntentId,fieldId,failedRunId,intentStateRevision(1),intentDigest,retryScopeDigest`；完整payload≤`16 KiB`，producer/唯一consumer均为Procurement，`dedupKey=procurement_retry_intent_available:{retryIntentId}:1`；只唤醒Run Creator，不授权建立与Intent不一致的Run |
| `ProcurementRetryAdmissionResult` | `ReceiptEnvelope + retryIntentId + intentDigest + terminalIntentState(consumed|stale) + resultKind(created|stale) + createdControlReceipt? + staleMaterialCount? + staleMaterialSetDigest? + staleReasonCodes[]`；Receipt固定`receiptKind=procurement_retry_admission,ownerDomain=procurement,scopeType=procurement_retry_intent,scopeId=retryIntentId,scopeDigest=intentDigest`；created必须对应consumed、内联一份合法`ProcurementControlReceipt`且不得有stale字段；stale必须对应stale state、不得有Control Receipt、count为`1..retryMemberCount`且reason数组非空；reason只使用下述closed set并按固定precedence去重排序；完整typed value≤`64 KiB`且不内联stale key列表 |
...
| `DomainFactCommitHandle` | `handleId, ownerDomain, aggregateType, aggregateId, factType, factSchemaRef, expectedRevision, payloadDigest, resultSchemaRef, commitIdempotencyKey, eventFenceDigest`；payloadDigest绑定合同声明的`commitPayload`（全部typed input/parameter排除本Handle及Runtime context），resultSchemaRef绑定重放必须恢复的typed result |
| `ResponsibilityControlCommitHandle` | `handleId, operationKind(acquire|transfer|release|replace_control_set), ownerDomain, processType, processId, basisRef, basisDigest, canonicalFactSetDigest, bindingSetDigest, controlScopeDigest, expectedControlRevisions[{materialKey,revision}], receiptContract, eventFenceDigest, receivingDomain?, transferPoint?`；revision为non-negative integer，数组按materialKey UTF-8 bytes升序、key唯一且最多1024项；只有跨Domain transfer要求`receivingDomain+transferPoint`，Domain内promotion或Inventory revision仍使用相同CAS/atomic participant但不伪造Handoff；具体合同可以要求non-empty，Procurement Run Admission固定与Selection同为`1..256`项 |
...
| `FaceEmbeddingSetHandle` | `artifactHandleId,artifactHandle(ArtifactHandle@1),computationMode(western_frame_set\|single_reference_face),libraRunId NULL,workspaceId NULL,faceModelRefDigest,sourceArtifactSetDigest,detectedFaceCount,vectorCount,dimension,embeddingDigest,handleDigest`；nested Handle固定`artifactKind=face_embedding_set`且顶层ID逐字节等于nested ID，Handle/provenance/referenceRevision/digest逐字节命中Artifact Registry。Western variant要求libraRunId/workspaceId非NULL、owner scope固定同一`libra_run/libraRunId`且provenance命中该Workspace，Embedding Plan的Artifact input恰为`FrameArtifactSet.frameSetArtifactHandle`且`sourceArtifactSetDigest=FrameArtifactSet.manifestDigest`；single-reference variant要求两字段为NULL且owner scope固定正式People maintenance scope。`embeddingDigest=artifactHandle.digestHex`，`handleDigest=SHA-256(JCS(完整value excluding handleDigest))`；向后续节点传完整handle而非向量数组。single-reference模式要求两个count都恰为1，Western模式要求`vectorCount=detectedFaceCount`并允许0 |
| `FaceClusterSetHandle` | `artifactHandleId,artifactHandle(ArtifactHandle@1),libraRunId,workspaceId,faceModelRefDigest,clusterParameterDigest,sourceEmbeddingDigest,clusterCount,clusterDigest,handleDigest`；nested Handle固定`artifactKind=face_cluster_set`、owner scope固定同一`libra_run/libraRunId`，顶层ID与nested ID相等且Registry/provenance/fence逐字节匹配；`clusterDigest=artifactHandle.digestHex`，`sourceEmbeddingDigest=FaceEmbeddingSetHandle.embeddingDigest`，`handleDigest=SHA-256(JCS(完整value excluding handleDigest))`；Cluster成员大payload只存在Artifact bytes，不内联进Handle |
...
| `WesternProductMetadataBasisSnapshot` | `basisId,basisKind(western_analysis),libraRunId,runExecutionBasisDigest,resolvedIdentityDigest,analysisVariantDigest,analysisRefs[1..16]{workId,attemptId,planId,eventId,resultId,resultDigest,inputBindingDigest,analysisArtifactHandleId,analysisArtifactDigest,evidenceId,evidenceDigest},normalizeRef{workId,attemptId,planId,eventId,resultId,resultDigest,inputBindingDigest,analysisVariantId,productMetadataDraftDigest},sourceRefsDigest,basisDigest`；analysisRefs逐项对应上述Variant，所有analysisRefs/normalizeRef的`resultDigest`均为对应Foundation完整typed Result storage digest并逐字节命中`fx_event_result_bindings.result_digest`；`analysisArtifactHandleId/analysisArtifactDigest`逐字节复制展开Result的`resultArtifactHandle.artifactHandleId/digestHex`并命中Artifact Registry，不是Worker/Provider receipt。Analysis Result内部领域`resultDigest`只在展开typed Result时独立验证，不进入外层ref。`evidenceId/evidenceDigest`继续逐字节命中Analysis Result的Evidence Envelope，不受两层Result digest分工影响。normalize固定`libra.western.metadata.normalize@1 → ProductMetadataDraft@1`且Plan input binding逐字节引用同一Variant；Commit Draft必须等于normalize Result。先构造`canonicalSourceRefs[2..17]`：每项固定为`{ordinal,capabilityRef,resultSchemaRef,workId,attemptId,planId,eventId,resultId,resultDigest,sourceRef,sourceOrder,evidenceId,evidenceDigest,inputBindingDigest}`；按Variant既定顺序放入Analysis refs（capability/schema固定Western observe/Result，`sourceRef=analysisArtifactHandleId`，Evidence非NULL，连续ordinal/sourceOrder=`0..N-1`），最后放入Normalize ref（capability/schema固定Western normalize/Draft，`sourceRef=analysisVariantId`，Evidence为NULL，ordinal/sourceOrder=`N`）。`sourceRefsDigest=SHA-256(JCS({schema:"libra.western-product-metadata-source-refs@1",items:canonicalSourceRefs}))`，`basisId=SHA-256(JCS({schema:"libra.western-product-metadata-basis-id@1",libraRunId,runExecutionBasisDigest,sourceRefsDigest}))`，`basisDigest=SHA-256(JCS(完整value excluding basisDigest))`；完整value≤`64 KiB` |
| `WesternMediaCastBasisSnapshot` | `basisId,basisKind(western_match),libraRunId,runExecutionBasisDigest,resolvedIdentityDigest,matchRef{workId,attemptId,planId,eventId,resultId,resultDigest,inputBindingDigest,evidenceId,evidenceDigest,personMatchEvidenceDigest},referenceProjectionSetDigest,matchState(matches_found|no_matches),basisDigest`；Ref固定同Run/Basis的`shared.face.reference.match@1 → PersonMatchEvidence@1`，Evidence字段逐字节等于Result的Evidence Envelope，`referenceProjectionSetDigest`逐字节等于展开Result同名字段及Media Cast Resolve Plan冻结Projection集合重算值；`matches_found`要求matches非空且Draft逐项解释全部match，`no_matches`要求`matches=[]`并只允许Draft `relations=[]`；`basisId=SHA-256(JCS({schema:"libra.western-media-cast-basis-id@1",libraRunId,runExecutionBasisDigest,matchResultId:matchRef.resultId,matchResultDigest:matchRef.resultDigest,referenceProjectionSetDigest}))`，canonical object恰含这六个顶层property；`basisDigest=SHA-256(JCS(完整value excluding basisDigest))`；完整value≤`16 KiB` |
| `LibraProductMetadataSourceBasis` / `LibraMediaCastSourceBasis` | Product Metadata为closed union：`metadata_observation{selection,observationSet,sourceBasisDigest=selection.selectionDigest}`或`western_analysis{westernBasis,sourceBasisDigest=westernBasis.basisDigest}`；Media Cast为closed union：`metadata_observation{selection,observationSet,sourceBasisDigest=selection.selectionDigest}`或`western_match{westernBasis,sourceBasisDigest=westernBasis.basisDigest}`。discriminator与payload必须一致，禁止空对象、双variant或隐式fallback；完整union分别受其成员上限约束 |
| `ShelfRoutingProjectionQuery` / `ShelfRoutingTargetReadResult` / `ShelfRoutingTargetProjection` | Query为`queryContract(arca.shelf-routing-projection@1),shelfIds[],queryDigest`，Shelf ID为`1..64`个、去重并按调用方Routing rank顺序，`queryDigest=SHA-256(JCS(完整Query excluding queryDigest))`。每项Projection为`shelfId,status(active|deregistering|deregistered),routingProjectionRevision,currentStandardRevision,currentStandardDigest,projectionDigest`；`projectionDigest=SHA-256(JCS({schema:"arca.shelf-routing-target-projection@1",shelfId,status,routingProjectionRevision,currentStandardRevision,currentStandardDigest}))`；current Standard不存在时revision/digest均为NULL且Shelf不能成为resolved target。Read Result为`queryDigest,items[{shelfId,resultKind(found|not_found|unavailable),projection?,reasonCode?,itemDigest}],resultDigest`，按Query输入一一同序且不得省略；found恰含Projection且无reason，not_found只允许`shelf_not_found`，unavailable只允许`arca_projection_unavailable|projection_integrity_failure`；各digest排除自身后覆盖完整typed value |
| `ShelfStandard` / `ShelfStandardProjection` | `ShelfStandard@1`为`shelfId,standardRevision,ruleTemplateId,ruleTemplateRevision,profileRuleSets[],standardDigest`；Profile为`1..4`项，按`movie|series|jav|western_adult`UTF-8 bytes排序且唯一，每项为`contentProfile,decisionInputKinds[](Beta仅rating),baseRequirements(AcceptanceRequirementSet),decisionBranches[{conditionKind(no_rating|rating_equals),rating?,requirements(AcceptanceRequirementSet)}],profileRuleSetDigest`。未声明rating时Branches必须为空并直接使用base；声明rating时恰有一项no_rating和rating 1..5各一项，按no_rating后rating升序，分支携带完整六类Requirement而不是partial merge；`profileRuleSetDigest=SHA-256(JCS(完整Profile excluding profileRuleSetDigest))`，`standardDigest=SHA-256(JCS(完整Standard excluding standardDigest))`。Projection为`shelfId,status(active),routingProjectionRevision,projectionDigest,standard(ShelfStandard),projectionResultDigest`，全部pair必须与`ShelfRoutingTargetProjection@1`逐字节相等，`projectionResultDigest=SHA-256(JCS(完整Projection excluding projectionResultDigest))`；完整value≤`64 KiB` |
| `ShelfStandardProjectionQuery` / `ShelfStandardProjectionReadResult` | Query为`queryContract(arca.shelf-standard-projection@1),shelfId,expectedRoutingProjectionRevision,expectedStandardRevision,expectedStandardDigest,queryDigest`；Result为`queryDigest,resultKind(found|not_found|stale|unavailable),projection?,reasonCode?,resultDigest`。found恰含一份匹配全部expected pair的Projection；not_found只允许`shelf_not_found`，stale只允许`shelf_not_active|routing_projection_changed|standard_changed`，unavailable只允许`arca_projection_unavailable|projection_integrity_failure`；`resultDigest=SHA-256(JCS(完整Result excluding resultDigest))`，Query无副作用 |
| `RoutingMatchExpression` | `RoutingMatchExpression@1`只允许`always|predicate|all|any|not`节点；predicate为`factKind(content_profile|structure_kind|material_field|release_year|region|genre|resolved_provider_identity),operator(eq|one_of|gte|lte|exists),expectedValue`，类型必须与fact kind匹配；树深度≤4、总节点≤64，`all/any`子节点`1..16`且按各子表达式digest的UTF-8 bytes排序，`not`恰一子节点；三值求值固定为`true|false|unknown`，缺失/冲突输入只能产生unknown，禁止任意JavaScript、Provider调用或隐式fallback；一次性选Shelf是独立`ManualShelfSelectionIntent@1`，永远不进入长期Rule AST |
| `FieldRoutingPolicySnapshot` | `routingPolicyId,revision,fieldId,mode(direct|sorting),targets[{shelfId,rank,matchExpression(RoutingMatchExpression),matchRuleDigest}],policyDigest`；target为`1..64`项、rank从1连续且唯一并按rank排序；direct恰一项且expression为always，sorting逐项使用closed AST；`matchRuleDigest=SHA-256(JCS(matchExpression))`，`policyDigest=SHA-256(JCS(完整Snapshot excluding policyDigest))`，必须逐项重建既有Policy/target rows |
| `ManualShelfSelectionIntent` / `RoutingAuthoritySnapshot` | Intent为`subjectId,targetShelfId,expectedDecisionHead{revision,digest},actorId,idempotencyKey,requestDigest`，`requestDigest=SHA-256(JCS(完整Intent excluding requestDigest))`；它只表达当前Subject一次性选Shelf，不改写Field Policy。Authority是union：policy分支为`authorityKind(policy),policy(FieldRoutingPolicySnapshot),authorityDigest`；manual分支为`authorityKind(manual_selection),manualIntent(ManualShelfSelectionIntent),authorityDigest`；`authorityDigest=SHA-256(JCS(完整variant excluding authorityDigest))`。manual只评估这一座active Shelf；Command Receipt与routing Decision Basis同事务成立，Routing Decision只验证并引用该Receipt，不建立长期Policy或fallback |
| `SubjectDecisionSnapshot` | `subjectId,status(active),intakeRevision,structureKind(single|season),contentProfile(movie|series|jav|western_adult),routingAnchorIntakeDecisionId,routingProvenance{candidatePackageId,sourceFieldId,sourceFieldAccessRevision,sourceFieldContextDigest,candidateIdentityClaimDigest},currentIdentityRevision?,currentIdentityDigest?,continuitySetDigest,episodeScopeDigest,snapshotDigest`；routingProvenance恰从Subject不可变anchor指向的Accepted Intake Decision恢复；extension不得换成最新Field或合并多个Field Policy。identity revision/digest同时存在或同时NULL；`snapshotDigest=SHA-256(JCS(完整Snapshot excluding snapshotDigest))`，不得回读Procurement或Candidate Store补值 |
| `SubjectDecisionHeadSnapshot` | `subjectId,headState(absent|present),headRevision,headDigest,currentRoutingDecisionId,currentDecisionBasisId,currentAcceptanceSpecId,snapshotDigest`。`absent`只允许`headRevision=0`且headDigest/三个pointer全部NULL，表示Subject已成立但从未提交Decision Basis；`present`要求revision为正整数且current Basis非NULL，current Routing非NULL时Basis也必须非NULL，current Spec非NULL时Routing/Basis都必须非NULL；`headDigest=SHA-256(JCS({schema:"libra.subject-decision-head@1",subjectId,headRevision,currentRoutingDecisionId,currentDecisionBasisId,currentAcceptanceSpecId}))`必须与Owner current row逐字节相等。`snapshotDigest=SHA-256(JCS({schema:"libra.subject-decision-head-snapshot@1",subjectId,headState,headRevision,headDigest,currentRoutingDecisionId,currentDecisionBasisId,currentAcceptanceSpecId}))`；它是Basis提交前的immutable typed input，不随current Head后续CAS变化 |
| `RoutingDecisionFact` | tagged union：`content_profile{value}`、`structure_kind{value}`、`material_field{fieldId}`、`release_year{year(1870..3000)}`、`region{countryCodes[]}`、`genre{genreCodes[]}`、`resolved_provider_identity{provider,namespace,providerKey,identityRevision,identityDigest}`；公共字段为`factKind,sourceObjectId,sourceRevision,schemaRef,factDigest`，数组值去重并按UTF-8 bytes排序；`factDigest=SHA-256(JCS(完整typed fact excluding factDigest))`。Beta Routing vocabulary只含这些variant；一次性选Shelf使用独立`ManualShelfSelectionIntent@1`，不伪装成Canonical Fact；新增条件必须升级Policy/Fact schema，不能用opaque JSON扩展 |
| `ProductScopeSnapshot` | `subjectId,scopeKind(single|episode_manifest),subjectIntakeRevision,episodeKeys[],scopeDigest`；single要求Subject structure=single且episodeKeys为空；episode_manifest要求season且为Subject current accepted Episode scope的`1..1024`项非空子集，去重并按episodeKey UTF-8 bytes升序；`scopeDigest=SHA-256(JCS({schema:"libra.product-scope@1",subjectId,scopeKind,subjectIntakeRevision,episodeKeys}))`。它只是Spec/未来Run的确定交付范围，不创建Run或锁定执行资源；更大Season按不重叠scope分批形成Spec/Run，不扩大单次事务边界 |
| `DecisionInputSet` | `decisionInputSetId,basisKind(routing|acceptance_spec),subjectSnapshot(SubjectDecisionSnapshot),expectedDecisionHead(SubjectDecisionHeadSnapshot),readiness{result(ready|unresolved),reasonCode?},routingAuthoritySnapshot?,shelfRoutingTargets[],routingDecision?,shelfStandardProjection?,productScope?,decisionFacts[],queryResults[VersionedQueryResult],queryResultSetDigest,routingInputDigest?,specInputDigest?,inputSetDigest`。两个Snapshot必须具有相同subjectId；顶层`shelfRoutingTargets/decisionFacts/queryResults`各≤128项并按正式ID/digest稳定排序；Product Scope的Episode上限独立遵守`ProductScopeSnapshot@1`的1024项合同。Readiness unresolved closed reason为`routing_policy_unavailable|subject_provenance_incomplete|required_input_missing|required_input_conflicting|required_input_unavailable|shelf_projection_unavailable|routing_not_resolved|target_shelf_not_active|shelf_standard_unavailable|profile_rule_not_found|product_scope_unavailable`。routing ready必须携带Authority及对应Target Projection：policy按rank携带全部候选和声明Facts，manual恰携带选中Shelf且不需要Policy；两者都不得携带Standard/Product Scope。unresolved只可省略其reason精确指向的输入。`routingInputDigest=SHA-256(JCS({schema:"libra.routing-input@1",subjectSnapshot,routingAuthoritySnapshotOrNull:routingAuthoritySnapshot??null,shelfRoutingTargets,decisionFacts,queryResults}))`。acceptance_spec ready必须携带resolved Routing Decision、匹配Target的active Standard Projection、Product Scope及Standard声明的Facts/Query；unresolved同样只可省略reason指向的输入。`specInputDigest=SHA-256(JCS({schema:"libra.spec-input@1",subjectSnapshot,routingDecisionOrNull:routingDecision??null,shelfStandardProjectionOrNull:shelfStandardProjection??null,productScopeOrNull:productScope??null,decisionFacts,queryResults}))`。两个branch digest在unresolved时仍以显式null形成，不得缺省字段。`queryResultSetDigest=SHA-256(JCS({schema:"libra.decision-query-result-set@1",items:[{providerDomain,queryContract,queryVersion,inputDigest,resultKind,resultRevision,resultDigest,expiresAtMs}]排序}))`；`inputSetDigest=SHA-256(JCS(完整value excluding decisionInputSetId and inputSetDigest))`，因此完整pre-CAS Head Snapshot直接受其保护；`decisionInputSetId=SHA-256(JCS({schema:"libra.decision-input-set-id@1",basisKind,subjectId:subjectSnapshot.subjectId,inputSetDigest}))`；完整JCS≤`1 MiB` |
| `RoutingAssessmentEvidence` | `routingAssessmentId,subjectId,decisionBasisId,routingInputDigest,routingAuthorityKind(policy|manual_selection),routingPolicyId?,routingPolicyRevision?,manualSelectionDigest?,evaluatedTargets[{rank,shelfId,shelfProjectionRevision,shelfProjectionDigest,matchRuleDigest?,evaluation(true|false|unknown),inputFactSetDigest,evaluationDigest}],result(resolved|unresolved),targetShelfId?,unresolvedReasonCode?,shelfPrioritySetDigest,assessmentDigest`；policy分支的evaluatedTargets必须是rank 1连续前缀，第一项true或unknown后停止，全部false才允许`no_matching_shelf`；manual分支恰一项rank=1、无matchRule，Shelf active时true，否则unresolved。unresolved reason closed set为`higher_priority_rule_unknown|no_matching_shelf|manual_target_invalid|target_shelf_inactive`；`shelfPrioritySetDigest=SHA-256(JCS({schema:"libra.shelf-priority-set@1",authorityKind:routingAuthorityKind,items:[{rank,shelfId,matchRuleDigestOrNull:matchRuleDigest??null,shelfProjectionRevision,shelfProjectionDigest}]}))`；`routingAssessmentId=SHA-256(JCS({schema:"libra.routing-assessment-id@1",subjectId,decisionBasisId,routingInputDigest}))`，每项/整体digest都排除自身后覆盖完整typed value |
| `RoutingDecision` | `routingDecisionId,subjectId,decisionRevision,assessmentId,decisionBasisId,routingAuthorityKind(policy|manual_selection),routingPolicyId?,routingPolicyRevision?,manualSelectionDigest?,routingInputDigest,shelfPrioritySetDigest,result(resolved|unresolved),targetShelfId?,unresolvedReasonCode?,decisionDigest`；authority variant与Assessment逐项相同；resolved恰含target且无reason，unresolved相反并沿用Assessment closed reason；`routingDecisionId=SHA-256(JCS({schema:"libra.routing-decision-id@1",assessmentId}))`，`decisionDigest=SHA-256(JCS(完整value excluding decisionDigest))`；不包含Acceptance Spec、Run或Workspace |
| `AcceptanceRequirementSet` | 六个mandatory对象：`identity{identityKind(tmdb_movie|tmdb_series_season|jav_code|internal_identity),requiredProvider?(tmdb),requireSeasonNumber}`；`structure{structureKind(single|season),primaryModel(single_primary|episode_primary),requireOnePrimaryPerEpisode}`；`metadata{requiredFieldCodes[],requiredArtifactKinds[],requireRenderableSidecar,requireDecodableImages}`，field closed set为`tmdb_movie_id|tmdb_series_id|title|series_title|year_or_release_date|release_date|plot|genre|actor|director|season_number|episode_number|episode_title|episode_plot|jav_code|studio|internal_identity`，Artifact closed set为`nfo|poster|fanart`；`mandatoryMedia{mediaForm(any|stream_file),videoCodec(any|hevc),container(any|matroska),fileExtension(any|mkv),minimumRasterClass(none|4k),acceptedPrimaryAudioClasses[],forbidSystemUpscaleFor4k}`，Audio closed set为`eac3_atmos|truehd|truehd_atmos|dts_hd_ma|dts_x`；`space{unit(product|episode),maxSizeGiB?,maxSizeBytes?}`；`inventory{requireDomainBinding,requireChecksum,requiredMaterializedArtifactKinds[],layoutModel(single|season_episode)}`。数组去重按UTF-8 bytes排序；无空间上限时两个max字段同时NULL，有上限时`maxSizeGiB`为positive decimal且`maxSizeBytes=maxSizeGiB×1073741824`为精确整数；Requirement不能携带Capability、Provider选择、设备、Task或Flow参数 |
| `AcceptanceSpecDraft` / `AcceptanceSpec` | Draft为`DraftEnvelope + subjectId,targetShelfId,contentProfile(movie|series|jav|western_adult),structureKind(single|season),productScope(ProductScopeSnapshot),shelfRoutingProjectionRevision,shelfProjectionDigest,shelfStandardRevision,shelfStandardDigest,decisionBasisId,decisionBasisDigest,requirements(AcceptanceRequirementSet),specDigest`；`series`唯一对应season，其余Beta profile对应single，禁止把season写入contentProfile。`specDigest=SHA-256(JCS({schema:"libra.acceptance-spec-semantic@1",targetShelfId,contentProfile,structureKind,productScope:{scopeKind,episodeKeys},requirements}))`，因此Policy/query revision变化但产品要求相同仍保持同semantic digest；`DraftEnvelope.draftDigest=SHA-256(JCS(完整Draft excluding DraftEnvelope.draftDigest))`。Published Spec追加`acceptanceSpecId,specRevision,recordDigest,publishedAtMs`；`recordDigest=SHA-256(JCS({schema:"libra.acceptance-spec-record@1",specRevision,draft:完整Draft}))`，明确不包含`acceptanceSpecId/recordDigest/publishedAtMs`；`acceptanceSpecId=SHA-256(JCS({schema:"libra.acceptance-spec-id@1",subjectId,specRevision,decisionBasisId,recordDigest}))`，不存在自引用。完整Spec≤`64 KiB`，六类Requirement不得缺项 |
| `RoutingDecisionCommitResult` / `AcceptanceSpecPublicationResult` | Routing Result为`subjectId,routingDecisionId,decisionRevision,assessmentId,decisionBasisId,result,targetShelfId?,unresolvedReasonCode?,committedHeadRevision,decisionDigest,resultDigest`；Spec Result为`subjectId,acceptanceSpecId,specRevision,decisionBasisId,productScopeDigest,specDigest,recordDigest,committedHeadRevision,resultDigest`；各自`resultDigest=SHA-256(JCS(完整value excluding resultDigest))`并可由Owner immutable rows加current Decision head重建。二者是application transaction result，不是Catalog Capability Result family |
| `DirectPersonRegistrationDecision` | `decisionId,newPersonId,canonicalName,aliases[{aliasDisplay,aliasNormalized,provenanceDigest}],providerIdentities[{provider,namespace,providerKey,provenanceDigest}],actorId,decisionDigest`；完整value≤`16 KiB`，`decisionDigest=SHA-256(JCS(Decision excluding decisionDigest))`；不含Reference、Candidate或Media-Cast字段 |
| `PeopleReferenceMaintenanceDecision` | 公共字段`decisionId,operationKind(add_image|release_image),personId,expectedPersonRevision,expectedReferenceRevision,actorId,decisionDigest`；首个add的逻辑expected revision为`0`并要求storage pointer为NULL，提交后建立revision 1；后续操作CAS真实正整数revision；`add_image`追加`referenceAssetId,referenceFaceId,artifactHandle,artifactDigest,faceEmbeddingSetHandle,modelRef,initialState(active)`且要求handle input digest一致、`detectedFaceCount=vectorCount=1`；`release_image`追加`referenceAssetId,referenceFaceId,expectedAssetState(active),expectedFaceState(active),terminalState(released),artifactDigest,embeddingDigest,modelRef`；digest排除自身后覆盖完整Decision |
| `PersonReferenceProjection` | `projectionContract,projectionRevision,personId,personRevision,currentReferenceRevision(null|positive),contributions[{ownerPersonId,ownerReferenceRevision,referenceSetDigest,inheritedReadOnly,activeAssets[{referenceAssetId,artifactHandleId,artifactDigest}],activeFaces[{referenceFaceId,referenceAssetId,embeddingHandleId,embeddingDigest,modelRef}]}],projectionDigest`；`currentReferenceRevision`只表示查询Person自己的local Reference pointer，从未形成local Reference revision时为`null`；此时无Merge contribution才有`contributions=[]`，正式merged-source贡献仍可出现；有local current revision但active集合为空时仍保留owner自己的Contribution；Contribution按`ownerPersonId`、内部数组按各自ID的UTF-8 bytes升序，禁止locale/case-fold排序；`projectionDigest=SHA-256(JCS({projectionContract,personId,personRevision,currentReferenceRevision,contributions}))`，明确排除`projectionRevision`和`projectionDigest`；`projectionRevision`使用People Store持久checkpoint，Person成立时为`1`，只有该semantic digest变化才CAS加`1`；Merge只展开，不复制或转移Reference，source contribution为只读 |
| `OnDeckPersonEvidenceProjectionItem` | `projectionItemId,shelfEntryId,inventoryRevision,relationId,relationDigest,displayName,displayNameNormalized,role,providerIdentities[],originEvidenceDigest,nfoObservationDigest,projectionRevision,projectionItemDigest`；只来自Arca当前Inventory person relation，完整value≤`16 KiB` |
| `OnDeckPersonEvidenceProjection` | `projectionContract,projectionRevision,cursorIn,cursorOut,items[],hasMore,pageDigest`；items最多100项并按`shelfEntryId,inventoryRevision,relationId`稳定排序；是跨Domain只读Projection，不是Business Handoff或People Fact |
| `PerceptionResolutionQuery` | `queryContract,queryVersion,querySchemaRef,factKind(rating|watched),identityEvidence[{anchorKind,anchorValue,confidenceClass,evidenceDigest}],queryInputDigest`；Identity Evidence最多16项，完整value≤`16 KiB`，`queryInputDigest=SHA-256(JCS(Query excluding queryInputDigest))`并与CanonicalQueryHandle typedInput一致；`@1`只支持这两个kind，新增kind必须升级schema/contract version |
| `PerceptionResolutionRecordSet` | `queryInputDigest,records[{perceptionId,recordKind,sourceKind,sourceRecordKey,sourceRecordRevision,recordDigest,facts{rating?,watchedState?},observedTitle,observedAtMs,identityAnchors[{anchorKind,anchorValue,confidenceClass,evidenceDigest}],provenanceRef,provenanceDigest}],relations[{relationId,relationKind(duplicate_of|supersedes|retracts),sourcePerceptionId,targetPerceptionId,ruleRevision,evidenceDigest}],recordSetDigest`；每个Record最多16个Anchor，records最多256项、relations最多1024项、完整canonical value≤`512 KiB`；所有Record/Relation必须可由Perception immutable facts精确验证；候选数不得超过Rule Snapshot的`maxCandidateRecords`，`recordSetDigest`排除自身后覆盖完整canonical value |
...
Handoff A `IntakeStructuredRejection@1`只允许以下closed reason code，并按此precedence排序：

1. `candidate_contract_invalid`：完整Delivery Snapshot可重建，但Candidate/Manifest/Offer字段、digest或成员闭包验证失败；
2. `candidate_material_identity_changed`：Primary Material当前Identity/Fence与Delivery冻结值不一致；
3. `candidate_material_unavailable`：正式Read Handle无法取得对应Primary Material；
4. `candidate_material_unreadable`：Material存在但有界读取/探测无法完成Intake验证；
5. `candidate_control_scope_unavailable`：精确Candidate member的current Control已不满足从Procurement转给Libra的前置条件。

每个reason必须引用对应`CandidateContractVerification@1`、`IntakeMaterialVerification@1`或
`MaterialControlProjectionSnapshot@1`的正式Evidence ID/digest。数组只包含真实发生的code，按precedence去重；第一项
就是`primaryRejectionCode`。上述reason是Handoff合同、Material reality或Control前置条件失败；Subject continuity的
none/multiple/Episode overlap不是rejection reason，仍按`SubjectContinuityResolutionDecision@1`建立new Subject。
...
3. **Profile与Unit**：显式Hint按Rule mapping直接决定Profile/mediaType；`mixed`先检测Series Episode token，再
   ...
   `BDMV/MovieObject.bdmv`、至少一个`PLAYLIST/*.mpls`和`STREAM/*.m2ts`且Probe形成可审计主标题选择才形成
   Movie Unit；DVD要求`VIDEO_TS/VIDEO_TS.IFO`、至少一个匹配VTS IFO/VOB；ISO要求Probe明确`discTopology=single_title`。
   多标题本身不再阻塞；真实歧义、结构缺失或不同Unit成员重叠仍not_ready。
4. **Related association**：只从Shared Layout Evidence中选择有完整Physical Identity、SHA-256、Endpoint与
   ...
   多个候选可以分别作为只读Reference，不推断winner，缺失或歧义不阻止Candidate。BDMV根目录内的Playlist、
   Clipinfo、Stream、index和MovieObject只属于该BDMV Unit的结构成员，禁止进入Related Reference；普通电影的
   sidecar只可来自已观察的外部电影目录Layout Evidence。
5. **Structure Result**：`resolved` page中的Unit按`unitId` UTF-8 bytes排序，`unassignedMaterials[]`按materialKey
   ...
   conflicting_season_claim|episode_claim_unresolved|disc_structure_incomplete|disc_non_primary_title|
   disc_multi_title_unsupported|triage_unit_contract_too_large|structure_ambiguous`；Movie多Playlist本身不再进入
   `disc_multi_title_unsupported`，只有无法按确定性规则选出主标题时才稳定not_ready；同一Material必须且只能出现在所有page的一个Unit或一项
...
~~~text
candidateReservationSetDigest = SHA-256(JCS({
  schema: "procurement.candidate-reservation-set@1",
  items: [{materialKey, candidatePackageId, packageDigest}]
}))

releasedMaterialSetDigest = SHA-256(JCS({
  schema: "procurement.run-released-material-set@1",
  items: [{materialKey, disposition, evidenceDigest}]
}))

sealEvidenceDigest = SHA-256(JCS({
  schema: "procurement.run-seal-evidence@1",
  procurementRunId,
  sealedStateRevision,
  runBasisDigest,
  sealDecisionDigest,
  sealOutcome,
  candidateReservationCount,
  candidateReservationSetDigest,
  releasedMaterialCount,
  releasedMaterialSetDigest
}))
~~~

公式中的`sealDecisionDigest`固定等于`ProcurementRunSealDecision.decisionDigest`。
`candidateReservationCount`是展开全部published Candidate Manifest后的Material数，不是Candidate数；每个展开项
必须与`proc_run_materials.candidate_package_id`精确相等，Seal时其guard可以仍为`candidate_delivery`，也可以已因
Handoff A终态变为`transferred|released`；digest表达本Run发布过的Candidate member set，不伪称终态后仍有active
Reservation。`releasedMaterialCount`是Seal本次从
`run_selection`释放的成员数；每项`evidenceDigest`必须同时等于Decision member和
`proc_run_materials.terminal_evidence_digest`。两组count、digest和Run head的`seal_evidence_digest`在同一Seal
事务写入；后续Handoff状态变化不修改本次Seal digest。

Procurement Retry的digest与重放basis固定如下。`failedRunMaterialDigest`只从failed Run的durable head/member row
重建；`retryAdmissionHeadDigest`固定等于`ProcurementRetryIntent.retryAdmissionHead.headDigest`；Retry Creator不得
从最新Field Material或调用者payload补值：

~~~text
failedRunMaterialDigest = SHA-256(JCS({
...
memberPreconditionDigest = SHA-256(JCS({
...
retryScopeDigest = SHA-256(JCS({
  schema: "procurement.retry-scope@1",
  failedRunId,
  failedRunBasisDigest,
  items: [{ordinal, materialKey, failedRunMaterialDigest}]
}))

preconditionSetDigest = SHA-256(JCS({
  schema: "procurement.retry-precondition-set@1",
  retryAdmissionHeadDigest,
  items: [{ordinal, materialKey, memberPreconditionDigest}]
}))

staleMaterialSetDigest = SHA-256(JCS({
  schema: "procurement.retry-stale-material-set@1",
  items: [{ordinal, materialKey, consumeSnapshotDigest, staleReasonCode}]
}))
~~~

`retryScopeDigest`与`preconditionSetDigest`中的完整member数组按`materialKey` UTF-8 bytes升序并使用连续
`ordinal`；`staleMaterialSetDigest`只保留stale子集及其原始ordinal。三者都必须与Intent relation rows精确对应。
`ProcurementRetryConsumeMemberSnapshot.snapshotDigest`就是公式中的`consumeSnapshotDigest`。Intent terminal时，
实际`ProcurementRetryAdmissionHead`、全部member consume snapshot、Intent state/new Run link、typed Result和marker
同事务成立；current row之后再次变化不影响这份immutable consume Evidence。

Retry stale primary reason采用以下closed set与固定precedence；每个stale member只保存第一个成立的code：

1. `field_status_changed`；
2. `field_profile_hint_changed`；
3. `field_access_changed`；
4. `terminal_observation_changed`；
5. `extraction_policy_changed`；
6. `triage_rule_changed`；
7. `material_not_present`；
8. `material_binding_changed`；
9. `material_not_eligible`；
10. `material_eligibility_changed`；
11. `material_guard_conflict`；
12. `material_control_unavailable`；
13. `material_control_not_acquirable`；
14. `material_control_changed`。

前六项比较Intent与consume的`ProcurementRetryAdmissionHead`精确字段；Profile比较完整
`MaterialFieldProfileHintSnapshot@1`，其中Triage比较
`ruleRef/revision/ruleSchemaRef/ruleDigest/authorityDigest`并要求consume时仍是Registry active entry。其余项依次比较
material存在性、Binding revision、Eligibility state、Eligibility revision/basis、Selection conflict、Control Query
可用性、是否为`uncontrolled`或同一`material_field/fieldId`可assert、最后比较完整Control `projectionDigest`。
任一member stale则整个Intent进入`stale`且不建Run；head stale作用于全部member。`staleReasonCodes[]`只收集各
stale member的primary code，按上述precedence去重排序；`primary_stale_reason_code=staleReasonCodes[0]`。created
结果要求全部member为matched；stale结果的count/digest/reasons必须可由consume rows唯一重建。

`ProcurementRunSealReceipt`、`ProcurementRetryIntentReceipt`与`ProcurementRetryAdmissionResult`是Procurement
Application Transaction的typed command/result DTO，不是新增Catalog Capability output family；它们必须生成正式
JSON Schema并参与transaction/replay fixture；该Procurement闭合本身不改变当时96个Catalog Result family计数。
`PBF-22`的`MaterialFieldRegistrationCommand`、`MaterialFieldProfileHintSnapshot`、
`MaterialFieldProfileHintRevisionCommand`、`MaterialFieldAdminProjection`与
`ProcurementMaterialFieldAdminResult`同样必须进入正式application contract registry；
`FieldObservationPageRequest`、`FieldObservationPage`、`ObservationCommitResult`、
`MaterialFieldContext`、`ProcurementRunExecutionBasis`、`ProcurementRetryAdmissionHead`及相关Plan binding
schema必须原位传播完整Snapshot `$ref`。Materializer对任一missing/unresolved ref、仍只接受裸
`contentProfileHint`的旧shape、nullable/opaque Snapshot或dual schema path必须fail closed；不因此新增Catalog
Capability或Result family。
...
建立的accepted Deliverable。该Triage修正本身保持112项Capability与当时96个Catalog Result family不变。

`PBF-11-R2-R1`把Handoff A富拒绝输出正式收窄为`IntakeRejectionReceipt@1`，Handoff B继续使用可由Arca
Owner rows重建的`RejectionReceipt@1`；因此当前Catalog仍为112项Capability，但unique Result family由96增加为97。
`PBF-11-R2-R2`只闭合Procurement Handoff A terminal consumption的row lifecycle：
`proc_candidate_deliveries`仅允许一次`open → accepted|rejected`CAS，`proc_run_materials`仅允许已列出的Reservation
transition；Accepted/Rejected application transaction result都从terminal Owner rows重建，不新增Catalog Result family。
`PBF-11-R3`只闭合Handoff A Accepted Receipt的Control revision set：唯一JCS formula绑定Payload、Commit Handle、
expected/committed revision与两端scope；historical Control revision可以在current owner后来变化后重建相同Receipt，
不新增Catalog Result family或持久表。
`PBF-12`只把Level 5已经Accepted的Routing/Decision Preparation/Acceptance Spec语义物化为formal DTO、三项
Canonical Transaction与既有Owner row连续性，并把Canonical `contentProfile=series`传播到Spec Schema；新增的
Routing/Spec application results不是Catalog Capability output，故Capability仍为112、Catalog Result family仍为97，
关系表仍为169。Canonical Transaction由35项增至38项。
`PBF-12-R1`只补齐Decision Basis的历史pre-CAS Head input连续性：新增formal
`SubjectDecisionHeadSnapshot@1`，把它作为每份Basis恰一条既有Libra input relation保存，并在Basis row/Result冻结
revision与snapshot digest。它不新增表、Capability、Result family、Canonical Transaction、Owner、Store或Handoff；
实现DDL把logical expected revision 0映射为non-negative列约束属于同一合同的机器传播。
`PBF-13`只把Level 3/6已经Accepted的Libra Run、Workspace、Product Package、Discard与Reclamation责任链补成
Owner-row可恢复合同：新增Run admission/state history、Production Material N:M Episode快照、Workspace revision及
Package Material Episode/Fact/Artifact membership七张Libra关系表，五项Application transaction与完整typed DTO；不新增Domain、
Owner、Store、Business Object、Handoff或Catalog Capability。关系表由169增至176，Canonical Transaction由38增至43，
Catalog仍为112项Capability/97个Result family。
`PBF-13-R2`只修正同一Run Admission/Promotion链的机器连续性：initial expected admission head在历史Run revision
中合法保存logical 0，实际admission head及committed head仍从1开始；`package_revision_head`固定为non-negative
SQLite INTEGER、Run建立时为0并由Promotion执行数值CAS+1；每个Material的`outputRequirementDigest`固定从同一
immutable Acceptance Spec、manifest/material role与Episode application scope派生，不接受caller opaque值。它不新增
表、事务、Owner、Capability或业务状态；新增的typed requirement binding只是既有Manifest字段的正式来源合同。
`PBF-13-R3`只闭合Run Input Physical Identity/size的跨Handoff连续性：Procurement Run Admission从同事务current
Field Material冻结完整`PhysicalMaterialIdentity@2 + sizeBytes`到Selected Set与immutable Run member；Candidate
Publication把同一值复制到final Manifest及Candidate Primary relation；`CandidateDeliverySnapshot@1`从两组immutable
Owner rows交叉验证后输出；Handoff A通过`LibraBindingDraft@1`逐项写入Libra Binding；Libra Run Admission只从该
...
`PBF-13-R5-R1`只修正Workspace aggregate/current revision首次建立的SQLite循环FK：current-head复合FK延迟到
transaction commit验证，child→aggregate父FK保持immediate并固定aggregate-first插入；没有nullable、sentinel或计数变化。
`PBF-13-R5-R2`只把immutable Run Manifest/member传播进Workspace Admission exact read set，使空间需求在同一事务
重算并验证；Episode Claim明确排除，write set与全部计数不变。
`PBF-13-R5-R3`闭合Workspace Material Handle与Product Verification连续性：Foundation既有material row冻结完整
Handle，Libra既有Reference row冻结完整typed Verification，Decision ID使用显式JCS keys；不新增表、能力或事务。
...
`libra_intake_decisions.decision_digest`按`decision_kind`唯一映射：`accepted_resolution`逐字节等于正式
`SubjectContinuityResolutionDecision@1.decisionDigest`，`rejected_acceptance`逐字节等于正式
`IntakeRejectionDecision@1.decisionDigest`。Accepted的match witness与overlap relation必须能按同一排序和公式重建
continuity Decision中的两个aggregate digest；Rejected的`libra_intake_rejection_reason_evidence`必须能按closed
precedence重建完整Reason/Evidence数组及`rejectionReasonSetDigest/rejectionDigest`。`libra_handoff_a_receipts`也按
outcome唯一映射：accepted重建`SubjectAndTransferReceipt@1`并以`accepted_payload_digest`作为scopeDigest，rejected
重建`IntakeRejectionReceipt@1`并以`IntakeRejectionDecision.decisionDigest`作为scopeDigest。Accepted Receipt中的Subject
continuity/Episode digest使用Receipt冻结值，禁止旁读后来扩充的Subject current row；其Control set使用同一commit
marker的transfer revisions及每项previous revision重建expected/committed projection与唯一digest，禁止读取后来
current Control owner或把Receipt中的摘要当成不可验证真值；Rejected Receipt不允许补写
Subject、Continuity head、Binding或Control字段。任何row能写入但对应typed Decision/Receipt无法重建，typed payload
存在而relation row缺失，或两种variant列同时出现，均为Canonical Transaction integrity failure，不能用最新状态、
Foundation Event Result或另一Domain Store补齐。

Rejected variant固定`libra_intake_decisions.rejection_schema_ref=IntakeStructuredRejection@1`，且
`decided_at_ms=structuredRejection.decidedAtMs`；Reason ordinal从0连续，Evidence ordinal在各Reason内从0连续。
Accepted variant的`rejection_schema_ref`及全部rejection列必须为NULL。SQLite必须以variant `CHECK`和deferred FK
表达这些条件，不能只依赖Application约定。
...
`FieldObservationPage`、`ObservationCommitResult`和`LayoutEvidence`的旧registry行仅保留作历史语义追溯，
不属于active Catalog；active Observation合同是`ObservationPageCommitResult@1`与紧凑
`ObservationPageCommitReceipt@1`，Layout是Observation entry Projection，不是Capability或Result。
...
| --- | --- |
| `ObservationPageCommitReceipt` | `ReceiptEnvelope + observationId + fieldObservationWorkId + fieldId + committedObservationRevision + pageOrdinal + entryCount + pageDigest + entrySetDigest + nextCursor + hasMore + receiptDigest`；只证明Observation entry rows已在同一事务成立，不内联文件明细；完整value≤`16 KiB` |
| `FilesystemIdentityEvidence` | `EvidenceEnvelope + identity + endpointId + location + statSizeBytes + statMtimeMs` |
| `BoundedContentFingerprintEvidence` | `EvidenceEnvelope + identity + fingerprintProfileRef + sampleOffset + sampleLength + bytesSampled`；profile固定`middle-256k-sha256@1`且`bytesSampled=sampleLength≤262144` |
| `MediaProbeEvidence` | `EvidenceEnvelope + sourceHandleDigest + resultKind(probed|not_media) + reasonCode? + container? + durationMs? + sizeBytes + videoStreams[] + audioStreams[] + subtitleStreams[] + discTopology?{discKind(bdmv|dvd|iso),titleCount,singleTitleEvidenceDigest,topologyVersion,topologyDigest,selectedPlaylist{relativeLocation,durationMs,clipIds[]},members[{relativeLocation,role,clipId?}]}`；`probed`要求container/duration/stream数组并禁止reason，`not_media`只允许`probe_not_media`且stream数组为空；每个video stream至少含`streamIndex,dispositionDefault,codec,codedWidth,codedHeight,sampleAspectRatio,rotation,displayWidth,displayHeight,longEdge,shortEdge`；每个audio stream至少含`streamIndex,dispositionDefault,codec,profile,channels,channelLayout,formatTags[],normalizedAudioClass(eac3_atmos|truehd|truehd_atmos|dts_hd_ma|dts_x|other)`。Audio mapping按ASCII case-fold后的codec/profile/tag固定：E-AC-3且JOC/Atmos tag→eac3_atmos；TrueHD且Atmos tag→truehd_atmos；其余TrueHD→truehd；DTS且DTS:X tag→dts_x；其余DTS-HD MA profile→dts_hd_ma；不能精确证明→other，precedence按上述顺序。所有stream按streamIndex升序且index唯一，数组有数量上限；Primary Video与Primary Audio分别取本类全部`dispositionDefault=true`项，若为空则取该类最低streamIndex一项，仍为空表示该类不存在；Product Requirement只评估Primary集合，不能因任意附加轨满足而通过。BDMV `discTopology`只由Media Probe Event产生，Playlist读取有界且不读取M2TS payload；Structure不得回读物理文件。FFprobe原始stdout/stderr有独立bounded transport limit（分别256 KiB/16 KiB），typed MediaProbeEvidence仍≤64 KiB；transport overflow、JSON解析或typed shape失败属于Capability contract failure，不得伪装为`probe_not_media`。`payloadDigest=SHA-256(JCS(完整domain payload excluding EvidenceEnvelope.payloadDigest))`；4K-class只使用归一化display raster |
| `LayoutEvidence` | `EvidenceEnvelope + sourceHandleDigest + boundedScopeDigest + entries[{entryOrdinal,entryKind(file|directory),relativeLocation,baseName,extension?,identity?,endpointId,location,sizeBytes?,mtimeNs?,entryDigest}] + entriesDigest + layoutDigest`；entries按relativeLocation UTF-8 bytes排序并从0赋ordinal，file只有在完整`PhysicalMaterialIdentity@2`存在时才可进入Related Reference；Related不另行计算或携带全文件checksum；`entriesDigest=SHA-256(JCS({schema:"shared.layout-entries@1",items:entries}))`，`layoutDigest=SHA-256(JCS({schema:"shared.layout-evidence@1",sourceHandleDigest,boundedScopeDigest,entriesDigest}))`，完整Evidence≤`64 KiB`且不得返回scope外entry |
| `ManifestVerification` / `ArtifactManifestVerification` | 普通Manifest版为`VerificationEnvelope + manifestDigest + contractRef`。Artifact版为`VerificationEnvelope + manifestDigest + contractRef + requirement(ArtifactRequirement@1) + verifiedArtifacts[1..64]{ordinal,artifactHandleId,artifactKind,artifactRevision,artifactDigest,referenceDigest} + artifactDigests[] + verificationDigest`；verified items按`artifactKind,artifactHandleId,artifactRevision` UTF-8 bytes排序、ordinal从0连续并精确覆盖Capability输入Handle set，每项必须与输入Handle及当时`fx_artifact_registry` active row的kind/`reference_revision`/digest逐字节一致；`referenceDigest=SHA-256(JCS(完整item excluding referenceDigest))`，`artifactDigests`由同序items的artifactDigest投影；`manifestDigest=SHA-256(JCS({schema:"shared.artifact-verification-input-manifest@1",requirementDigest:requirement.requirementDigest,items:verifiedArtifacts}))`，`VerificationEnvelope.basisDigest=SHA-256(JCS({schema:"shared.artifact-manifest-verification-basis@1",manifestDigest,requirementDigest:requirement.requirementDigest}))`，`verificationId=SHA-256(JCS({schema:"shared.artifact-manifest-verification-id@1",manifestDigest,requirementDigest:requirement.requirementDigest,basisDigest:VerificationEnvelope.basisDigest}))`，`verificationDigest=SHA-256(JCS(完整typed value excluding verificationDigest))`是领域内Evidence digest；Foundation `bindingResultDigest=SHA-256(JCS(包含verificationDigest的完整typed Result))`且`fx_event_result_bindings.result_digest=bindingResultDigest`。只有`result=passed`可进入Verified Artifact Manifest；单个Result同时满足`1..64`项与完整UTF-8 JCS bytes≤`65,536`双界，更多Artifact由多个独立验证Event覆盖 |
| `IntegrationAvailabilityEvidence` | `EvidenceEnvelope + integrationId + configRevision + availabilityState + latencyMs?` |
| `PersonMatchEvidence` | `EvidenceEnvelope + clusterSetDigest + referenceProjectionSetDigest + matches[0..1024]{clusterId,personId,personRevision,projectionRevision,projectionDigest,confidenceClass,evidenceDigest} + unmatchedClusterIds[0..1024]`；Projection input为`0..256 PersonReferenceProjection@1`，按personId UTF-8 bytes排序且ID唯一；`referenceProjectionSetDigest=SHA-256(JCS({schema:"people.person-reference-projection-set@1",items:[{personId,personRevision,projectionRevision,projectionDigest}]}))`。matches按`clusterId,personId`排序且tuple唯一，每项Person/revision/digest逐字节命中输入Projection；同一Cluster最多一项match，matches与unmatchedClusterIds对Cluster set形成无重叠完整分区。输入Projection为空时必须`matches=[]`且全部Cluster进入unmatched；`basisDigest=SHA-256(JCS({schema:"shared.face-reference-match-basis@1",clusterSetDigest,referenceProjectionSetDigest}))`，`payloadDigest=SHA-256(JCS(完整domain payload excluding EvidenceEnvelope.payloadDigest))`，完整value≤`64 KiB` |
| `FieldObservationPage` | `EvidenceEnvelope + fieldObservationWorkId + observationId + fieldId + accessRevision + profileHintSnapshot(MaterialFieldProfileHintSnapshot) + pageOrdinal + expectedObservationRevision + cursorIn + cursorOut + materialObservations[FieldMaterialObservationSnapshot] + pageDigest + hasMore`；`evidenceId=observationId,evidenceKind=field_observation_page,payloadDigest=pageDigest`；Profile Snapshot必须逐字节等于Request值；`basisDigest=SHA-256(JCS({schema:"procurement.field-observation-basis@1",fieldAccessHandle:完整FieldAccessHandle,pageRequest:完整FieldObservationPageRequest}))`，从而空页也绑定精确Access/Profile Hint/containment与Request；snapshots按`identity.materialKey`的UTF-8 bytes升序且key唯一，并逐项匹配同一observation/field/access；每项endpoint、identity.mountScopeId、mountScopeRevision与containment provenance必须匹配输入`FieldAccessHandle`且location位于其root containment；`pageDigest=SHA-256(JCS({schema:"procurement.field-observation-page@1",producerRef,basisDigest,observedAtMs,fieldObservationWorkId,observationId,fieldId,accessRevision,profileHintSnapshot,pageOrdinal,expectedObservationRevision,cursorIn,cursorOut,materialObservations,hasMore}))`；`hasMore=true`要求bounded非空`cursorOut`且不同于`cursorIn`，terminal page固定`hasMore=false,cursorOut=null`；最多100项且完整typed value的UTF-8 JCS bytes≤`65,536`，该完整value是Field Observation Commit Event唯一的durable `evidence_json` payload；`fx_event_result_bindings.evidence_digest=SHA-256(JCS(完整typed value))`，它与命名basis的`pageDigest`是两个明确不同的digest |
| `ObservationCommitResult` | `DomainFactEnvelope + observationId + fieldObservationWorkId + fieldId + accessRevision + profileHintSnapshot(MaterialFieldProfileHintSnapshot) + pageOrdinal + committedObservationRevision(=revision) + pageDigest + acceptedMaterials[{materialKey,bindingRevision,changeKind(inserted|refreshed|rebound),realityDigest,snapshotDigest}] + acceptedMaterialSetDigest + nextCursor + hasMore`；`factId=observationId,aggregateType=material_field_observation,aggregateId=fieldId,revision=committedObservationRevision`；Profile Snapshot逐字节复制已提交Page；acceptedMaterials按materialKey的UTF-8 bytes升序；`acceptedMaterialSetDigest=SHA-256(JCS({schema:"procurement.field-observation-accepted-materials@1",items:acceptedMaterials}))`；`factDigest=SHA-256(JCS({schema:"procurement.field-observation-revision@1",fieldId,committedObservationRevision,observationId,fieldObservationWorkId,accessRevision,profileHintSnapshot,pageOrdinal,pageDigest,acceptedMaterialSetDigest,nextCursor,hasMore}))`；terminal page的`nextCursor=null`；完整typed value的UTF-8 JCS bytes≤`65,536`且内部不含`resultDigest`，`proc_field_observations.result_digest`必须等于`fx_event_result_bindings.result_digest=SHA-256(JCS(完整typed Result value))`，commit marker重放返回原Result而不重算revision |
| `ProcurementControlReceipt` | `ReceiptEnvelope + procurementRunId + fieldId + runBasisDigest + selectedMaterialCount + selectedMaterialSetDigest + acquiredMaterialCount + assertedMaterialCount + controlRevisionSetDigest`；Receipt固定`receiptKind=procurement_run_admission,ownerDomain=procurement,scopeType=procurement_run,scopeId=procurementRunId,scopeDigest=runBasisDigest`，Control owner scope仍是`material_field/fieldId`；两个count之和必须等于`selectedMaterialCount(1..256)`；不内联material key数组，精确成员和每项admitted revision/digest由relationized `proc_run_materials`与`fx_material_control_revisions`恢复；`controlRevisionSetDigest=SHA-256(JCS({schema:"procurement.admitted-control-set@1",items:[{materialKey,admittedControlRevision,admittedControlProjectionDigest}]按materialKey排序}))`；完整typed value≤`64 KiB` |
...
| `BdmvAssessmentEvidence` | `EvidenceEnvelope + runId + bdmvGroupKey + scopeDigest + memberSetDigest + resultKind(resolved|not_ready) + reasonCode? + discKind + titleCount + selectedPlaylist? + selectedClipIds[] + topologyDigest + selectedPayloadSetDigest + memberCount + mediaSummary{probeState,durationMs,videoClasses[],audioClasses[],subtitleClasses[]} + evidenceDigest`；完整成员集合由冻结Observation scope与digest重建，Evidence不内联1024成员或完整ffprobe stream数组；transport/schema/stat fence失败是技术失败，不得伪装为`probe_not_media`；完整typed value≤`64 KiB` |
| `TriageStructureEvidence` | `EvidenceEnvelope + procurementRunId + runBasisDigest + selectionDigest + triageRuleAuthorityDigest + materialFieldContextDigest + pageRequestDigest + pageOrdinal + cursorIn + cursorOut + resultKind(resolved|not_ready) + units[TriageUnitSnapshot] + unassignedMaterials[{materialKey,reasonCode,evidenceDigest}] + unitSetDigest + unassignedSetDigest`；units按unitId、unassigned按materialKey排序且跨page共同精确分区完整Selection；BDMV Unit使用紧凑`UnitScopeReference`而不内联完整成员数组；`unitSetDigest=SHA-256(JCS({schema:"procurement.triage-unit-set-page@1",items:units}))`，`unassignedSetDigest=SHA-256(JCS({schema:"procurement.triage-unassigned-set-page@1",items:unassignedMaterials}))`，Evidence payload digest覆盖全部字段并排除自身；terminal page的cursorOut为NULL，完整Evidence≤`64 KiB` |
...
| `PrimaryInputManifestDraft` | `DraftEnvelope + preallocatedManifestId + procurementRunId + runBasisDigest + structureEvidencePayloadDigest + unitId + structureKind(single|season) + memberCount + members[{ordinal从0连续,materialKey,role,physicalIdentity,sizeBytes,bindingRevision,admittedControlRevision,admittedControlProjectionDigest,episodeClaims[]}] + membersDigest + memberSourceDigest + manifestDraftDigest`；`membersDigest=SHA-256(JCS({schema:"procurement.primary-input-manifest-members@1",items:members}))`；当Unit是`memberScope`时，members由Candidate Context从冻结Run/Observation/BDMV Evidence重建，只包含选定primary/structural成员，不把完整Scope成员集塞进Plan或Structure Result；每项Identity/size必须按materialKey从`TriageManifestBuildInput.selectedFieldMaterialSet`精确取得并与Unit member一致，不允许由文件名、current Field或Handle反推；`memberSourceDigest=TriageUnitSnapshot.unitDigest`，`manifestDraftDigest=SHA-256(JCS(完整domain payload excluding manifestDraftDigest))`且等于DraftEnvelope.draftDigest；它不含publishedAtMs、不写Domain Store |
| `PrimaryInputManifest` | `ManifestEnvelope + structureKind(single|season) + members[{ordinal,materialKey,role(primary_payload|structural_dependency),physicalIdentity{mountScopeId,inode,sizeBytes,fingerprintAlgorithm(middle-256k-sha256),fingerprintVersion(1),contentFingerprint},sizeBytes,bindingRevision,admittedControlRevision,admittedControlProjectionDigest,episodeClaims[{episodeKey,seasonClaimDigest,claimDigest}],memberDigest}]`；最终Manifest只由Candidate Publication commit从同一`CandidateDraft.structureEvidence.unit`、`PrimaryInputManifestDraft`和immutable Run Basis原子建立；members为Run Selection非空子集，普通Candidate最多256项且BDMV Unit最多4096项，按materialKey UTF-8 bytes排序后从0赋ordinal，materialKey唯一；Identity/size逐项等于Run member并满足`PhysicalMaterialIdentity@2`；每个`memberDigest=SHA-256(JCS(单项excluding memberDigest))`，memberCount/membersDigest/manifestDigest必须与完整稳定值匹配 |
| `CandidatePackage` | `ManifestEnvelope + candidatePackageId + packageRevision + procurementRunId + runBasisDigest + triageRule{ruleRef,revision,authorityDigest} + materialFieldContextRef{fieldId,accessRevision,contextDigest} + mediaType + contentProfile + materialInputForm(stream_file|bdmv|dvd|iso) + displayIdentity + identityMetadata + identityClaim + structureEvidenceRef{evidenceId,payloadDigest,unitId,unitDigest} + seasonContinuityClaims[SeasonContinuityClaim] + seasonContinuityClaimSetDigest + primaryInputManifestRef{manifestId,manifestDigest,memberCount} + relatedReferences[RelatedMaterialReference] + relatedReferenceSetDigest + memberControlEvidenceSetDigest + packageDigest`；Package/Manifest、Field Context、Identity/Structure/Profile、Control Evidence和Related必须逐项来自完整CandidateDraft；Related完整值必须与`proc_candidate_related_references`逐列一一相等并按referenceId排序，不能用checksum摘要替代Physical Identity；continuity数组有界、允许空，只接受`provider_season_identity|triage_grouping_lineage`且set digest按8.6.18重算；`packageDigest=SHA-256(JCS(完整value excluding ManifestEnvelope.manifestDigest and packageDigest))`且ManifestEnvelope.manifestDigest=packageDigest；`offerId|acceptanceBasisDigest`属于同事务建立的Handoff Offer，`subjectId`属于Libra Acceptance结果，三者均不得写入或要求于`CandidatePackage@1` |
| `CandidateContractVerification` / `IntakeMaterialVerification` | Candidate版为`VerificationEnvelope + offerId + candidatePackageId + packageRevision + packageDigest + acceptanceBasisDigest + primaryInputManifestDigest + candidateDeliverySnapshotDigest`；Material版为`VerificationEnvelope + candidatePackageId + packageDigest + candidateDeliverySnapshotDigest + verifiedMaterials[{materialKey,bindingRevision,locationEvidenceDigest,readHandleDigest,verificationDigest}] + verifiedMaterialSetDigest`，按materialKey排序并恰好覆盖全部Delivery member；每个`PhysicalMaterialReadHandle.identity/expectedSizeBytes/endpoint/location/bindingRevision`必须逐项等于对应`CandidatePrimaryMaterialDelivery@1`，否则不得passed；`verifiedMaterialSetDigest=SHA-256(JCS({schema:"libra.intake-verified-material-set@1",candidateDeliverySnapshotDigest,items:verifiedMaterials}))`；只有`result=passed`可进入Accepted payload，Executor不能只返回key或丢弃Identity/size/Location/Handle continuity |
| `LibraBindingDraft` | `DraftEnvelope + subjectRef + candidateDeliverySnapshotDigest + bindings[{materialKey,role,physicalIdentity{mountScopeId,inode,sizeBytes,fingerprintAlgorithm(middle-256k-sha256),fingerprintVersion(1),contentFingerprint},sizeBytes,endpointId,location,bindingRevision(1),locationEvidenceDigest,episodeClaims[{episodeKey,seasonClaimDigest,claimDigest}],bindingDigest}] + bindingSetDigest`；每个Delivery member恰好一条Binding，按materialKey排序，Physical Identity/size/endpoint/location必须逐字节复制对应`CandidatePrimaryMaterialDelivery@1`并满足materialKey公式，不能从current Field、Provider或Foundation Result补值；Episode Claims完整复制同Manifest member并按episodeKey排序，不能取第一项、拼字符串或复制Binding row；`locationEvidenceDigest=CandidatePrimaryMaterialDelivery.deliveryMemberDigest`，`bindingDigest=SHA-256(JCS(单项excluding bindingDigest))`，`bindingSetDigest=SHA-256(JCS({schema:"libra.binding-set@1",subjectRef,candidateDeliverySnapshotDigest,items:bindings}))`且等于DraftEnvelope.draftDigest |
| `IntakeRejectionReceipt` | `ReceiptEnvelope + intakeDecisionId + handoffKind(procurement_to_libra) + offerId + deliverableId + deliverableRevision + deliverableDigest + rejectionId + primaryRejectionCode + rejectionReasonSetDigest + rejectionDigest + receiptDigest`；固定`receiptKind=handoff_a_rejected,ownerDomain=libra,scopeType=intake_decision,scopeId=intakeDecisionId,scopeDigest=IntakeRejectionDecision.decisionDigest`；`receiptId=SHA-256(JCS({schema:"handoff-a-rejection-receipt-id@1",offerId,deliverableId,rejectionId}))`；`receiptDigest=SHA-256(JCS(完整value excluding receiptDigest))`，完整value≤`64 KiB` |
| `RejectionReceipt` | Handoff B rejected receipt：`ReceiptEnvelope + acceptanceDecisionId + handoffKind(libra_to_arca) + offerId + deliverableId + rejectionCode + acceptanceEvidenceSetDigest + rejectionDigest + receiptDigest`；固定`receiptKind=handoff_b_rejected,ownerDomain=arca,scopeType=acceptance_decision,scopeId=acceptanceDecisionId,scopeDigest=ArcaAcceptanceRejectionDecision.decisionDigest`，且`deliverableId=onDeckPackageId`、`offerId`逐字节等于Decision；`receiptId=SHA-256(JCS({schema:"handoff-b-rejection-receipt-id@1",acceptanceDecisionId}))`；`receiptDigest=SHA-256(JCS(完整value excluding receiptDigest))`，完整value≤`16 KiB` |
| `SubjectAndTransferReceipt` | `ReceiptEnvelope + intakeDecisionId + offerId + candidatePackageId + packageRevision + packageDigest + candidateDeliverySnapshotDigest + subjectId + subjectIntakeRevision + subjectContinuityHeadRevision + subjectContinuitySetDigest + subjectEpisodeScopeDigest + libraBindingSetDigest + controlRevisionSetDigest + receiptDigest`；固定`receiptKind=handoff_a_accepted,ownerDomain=libra,scopeType=intake_decision,scopeId=intakeDecisionId,scopeDigest=AcceptedIntakePayload.payloadDigest`。Control set成员必须与`AcceptedIntakePayload.controlTransferScope.items`及`ResponsibilityControlCommitHandle.expectedControlRevisions`精确相等，为`1..256`项、materialKey唯一并按materialKey UTF-8 bytes升序；每项固定为`{materialKey,expectedControlRevision,expectedControlProjectionDigest,committedControlRevision,committedControlProjectionDigest,fromOwnerDomain:"procurement",fromOwnerScopeType:"material_field",fromOwnerScopeId,toOwnerDomain:"libra",toOwnerScopeType:"subject",toOwnerScopeId:subjectId}`，`committedControlRevision=expectedControlRevision+1`。expected projection由`fx_material_control_revisions(materialKey,expectedControlRevision)`的historical post-state按8.6.18重建且digest逐字节等于Payload；committed projection在提交时必须由post-CAS `fx_material_controls` terminal row重算，并与同一Handoff marker的新transfer revision按相同规则重建的historical post-state逐字节相等；revision row的from/to scope必须逐项等于该item且`basis_digest=AcceptedIntakePayload.payloadDigest`。从items抽取`fieldId=fromOwnerScopeId`及原Payload的expected/from/to字段，必须按`AcceptedIntakePayload`公式重算出同一`controlScopeDigest`。`controlRevisionSetDigest=SHA-256(JCS({schema:"libra.handoff-a-transferred-control-set@1",intakeDecisionId,subjectId,controlScopeDigest,items}))`；`receiptDigest=SHA-256(JCS(完整value excluding receiptDigest))`，完整value≤`64 KiB`。历史重建先以`fx_commit_markers(owner_domain=libra,scope_type=intake_decision,scope_id=intakeDecisionId,result_schema_ref=SubjectAndTransferReceipt@1,result_digest=receiptDigest)`唯一定位marker，再由其全部transfer revision、各自previous revision和Libra Receipt/Binding rows重算Control scope/set；不得依赖后来current Control owner、Foundation Event Result JSON或调用方缓存 |
...
| `FrameArtifactSet` | `ManifestEnvelope + libraRunId + workspaceId + sourceMaterialDigest + samplingPlanDigest + outputTargetDigest + frameCount + frameMemberSetDigest + frameSetArtifactHandle(ArtifactHandle@1)`；Handle固定`artifactKind=western_frame_set,ownerDomain=libra,ownerScope={scopeType:"libra_run",scopeId:libraRunId}`并命中同Workspace的active `fx_artifact_registry` row，provenance逐字节绑定本次frames Event/Output Target。该Handle指向一个immutable composite Artifact，其内部closed frame index恰含`frameCount=1..1024`项并按frame ordinal排序，逐项保存timestamp、member content digest与bounded member locator；`frameMemberSetDigest`逐字节等于该index按`SHA-256(JCS({schema:"libra.western-frame-member-set@1",items}))`重算值，Handle的content digest覆盖完整index及全部frame bytes。`memberCount=1`且`membersDigest=SHA-256(JCS({schema:"libra.western-frame-artifact-set@1",items:[frameSetArtifactHandle]}))`；`manifestId=SHA-256(JCS({schema:"libra.western-frame-artifact-set-id@1",libraRunId,sourceMaterialDigest,samplingPlanDigest,outputTargetDigest}))`，`manifestDigest=SHA-256(JCS(完整value excluding ManifestEnvelope.manifestDigest and publishedAtMs))`；完整typed Result≤`16 KiB`。只保存Handle ID、路径数组或把全部frame Handles内联进Result/Plan都不是合法值 |
| `WesternAnalysisResult` | `EvidenceEnvelope + libraRunId + runExecutionBasisDigest + frameArtifactSetDigest + embeddingSetDigest + clusterSetDigest + analysisSpecDigest + analysisVariantRef + resultArtifactHandle(ArtifactHandle@1) + resultDigest`；Handle固定`artifactKind=western_analysis,ownerDomain=libra,ownerScope={scopeType:"libra_run",scopeId:libraRunId}`，逐字节等于`libra.western.analysis.request@1`的durable Result及active Artifact Registry row，provenance绑定同Plan Request Event、Analysis Spec与Output Target。`frameArtifactSetDigest=FrameArtifactSet.manifestDigest`、`embeddingSetDigest=FaceEmbeddingSetHandle.handleDigest`、`clusterSetDigest=FaceClusterSetHandle.handleDigest`、`analysisSpecDigest=AnalysisSpec.specDigest`并逐字节命中Observe Plan完整named inputs；`EvidenceEnvelope.basisDigest=SHA-256(JCS({schema:"libra.western-analysis-observation-basis@1",libraRunId,runExecutionBasisDigest,frameArtifactSetDigest,embeddingSetDigest,clusterSetDigest,analysisSpecDigest,resultArtifactHandleDigest:SHA-256(JCS(resultArtifactHandle))}))`。`resultDigest=SHA-256(JCS(完整typed value excluding resultDigest))`是领域内摘要；Foundation Event binding的storage digest固定为`SHA-256(JCS(包含resultDigest的完整typed Result))`，不得与内部`resultDigest`混用；完整value≤`16 KiB`，分析大结果只能经该immutable Artifact Handle引用 |
...
| `ProductMetadataDraft` | `DraftEnvelope + resolvedIdentityDigest + sourceBasisKind(metadata_observation|western_analysis) + metadataObservationSetDigest NULL + westernAnalysisVariantDigest NULL + fieldProvenance[{fieldPath,sourceKind(related_nfo|provider|western_analysis),sourceRef,evidenceDigest}] + descriptiveFacts + providerIdentities[] + artifactRequirements[0..256 ArtifactRequirement@1]`；Product Metadata Draft不携带Media Cast Draft/Fact引用，两个Fact的生产时序保持解耦，关联只由Product Metadata Commit的显式`mediaCastFactRef`输入建立。Artifact Requirements按正式type排序且完整进入Draft digest；Observation variant只允许`metadataObservationSetDigest`非NULL，Western variant只允许`westernAnalysisVariantDigest`非NULL且providerIdentities不得伪造外部Provider；该Variant digest在Normalize前已由Analysis Results形成，不包含当前Draft/Normalize Result，禁止用最终Western Basis digest替代造成自引用；Movie/Series相同有效字段按Related NFO优先、TMDB补缺，JAV/Western按`6.4.7.1`唯一来源规则；`DraftEnvelope.draftDigest=SHA-256(JCS(完整Draft excluding DraftEnvelope.draftDigest))`且完整value≤`64 KiB` |
| `MediaCastDraft` | `DraftEnvelope + subjectId + sourceBasisKind(metadata_observation|western_match) + metadataObservationSetDigest NULL + westernMatchBasisDigest NULL + relations[{relationId,personId?,displayName,displayNameNormalized,role,source,providerIdentities[],originEvidenceDigest,confidenceClass,relationDigest}]`；两个source-specific digest恰一非NULL；`personId`允许NULL，关系按`role,displayNameNormalized,relationId`的UTF-8 bytes稳定排序；单项`relationDigest=SHA-256(JCS(完整relation excluding relationDigest))`，Draft digest排除自身后覆盖完整value；`western_match`的no_matches basis合法要求`relations=[]` |
| `VerifiedArtifactManifest` | `manifestId,libraRunId,items[0..256]{ordinal,artifactHandleId,artifactKind,artifactRevision,artifactDigest,requirementId,requirementRevision,requirementSchemaRef,requirementDigest,verificationEvidenceId,verificationEvidenceDigest,verificationResultRef{workId,attemptId,planId,eventId,resultId,capabilityRef,resultSchemaRef,resultDigest,inputBindingDigest},referenceDigest},artifactSetDigest,manifestDigest`；items按`artifactKind,artifactHandleId,artifactRevision`的UTF-8 bytes排序且ordinal从0连续。每项Requirement四元组必须逐字节命中同一Commit的`ProductMetadataDraft.artifactRequirements`唯一成员；Result ref固定同Run/Execution Basis的`shared.artifact.manifest.verify@1 → ArtifactManifestVerification@1`，Plan input必须显式包含同一完整Handle与Requirement，typed Result必须`passed`并含逐字节相同verified item，`verificationEvidenceId=Result.verificationId`、`verificationEvidenceDigest=Result.verificationDigest`，而`verificationResultRef.resultDigest`必须等于`SHA-256(JCS(包含verificationDigest的完整typed Result))`并逐字节命中`fx_event_result_bindings.result_digest`。同一Result最多验证64项且可被多个Manifest item显式重复引用；Commit按resultId去重读取且不得扫描，0..256项Manifest可由多个Result覆盖。Registry active row的handle/kind/`reference_revision`/digest、Result、Requirement及Manifest item五方必须一致；`referenceDigest=SHA-256(JCS(完整item excluding referenceDigest))`，`artifactSetDigest=SHA-256(JCS({schema:"libra.verified-artifact-set@1",items}))`，`manifestId=SHA-256(JCS({schema:"libra.verified-artifact-manifest-id@1",libraRunId,artifactSetDigest}))`，`manifestDigest=SHA-256(JCS(完整value excluding manifestDigest))`；只携带Handle/Requirement/Result refs，不内联二进制，完整value≤`256 KiB` |
| `MediaCastFact` | `DomainFactEnvelope + subjectId + sourceBasisKind(metadata_observation|western_match) + sourceBasisDigest + relations[]完整沿用MediaCastDraft relation schema + relationsDigest + relationCount`；`relationsDigest=SHA-256(JCS({schema:"libra.media-cast-relations@1",relations}))`，`DomainFactEnvelope.factDigest=SHA-256(JCS({schema:"libra.media-cast-fact@1",subjectId,sourceBasisKind,sourceBasisDigest,relations,relationsDigest,relationCount}))`；Western no_matches允许`relationCount=0,relations=[]`，其他情况下不得凭空丢关系；不是digest-only摘要，必须可直接进入On-deck Product Package而无需旁读People或Provider |
| `ProductMetadataFact` | `DomainFactEnvelope + subjectId + resolvedIdentityDigest + sourceBasisKind(metadata_observation|western_analysis) + sourceBasisDigest + metadataObservationSetDigest NULL + westernAnalysisVariantDigest NULL + fieldProvenance[{fieldPath,sourceKind,sourceRef,evidenceDigest}] + descriptiveFacts + providerIdentities[] + mediaCastFactRef?{productFactId,factRevision,factDigest} + verifiedArtifactManifestDigest + productMetadataDigest`；两个source-specific digest恰一非NULL并与Draft及对应Source Basis成员逐字节一致；Western的最终`sourceBasisDigest`覆盖Analysis+Normalize，`westernAnalysisVariantDigest`只覆盖先行Analysis集合，两者不得混用；除Draft-only envelope外逐项来自已验证`ProductMetadataDraft`及正式Media Cast/Artifact commit结果，完整JCS≤`64 KiB`；`productMetadataDigest=SHA-256(JCS({schema:"libra.product-metadata@1",subjectId,resolvedIdentityDigest,sourceBasisKind,sourceBasisDigest,metadataObservationSetDigest,westernAnalysisVariantDigest,fieldProvenance,descriptiveFacts,providerIdentities,mediaCastFactRef,verifiedArtifactManifestDigest}))`且`DomainFactEnvelope.factDigest=productMetadataDigest`，不能只保存摘要而丢弃Arca/播放器需要的描述字段 |
| `LibraMediaCastCommitPayload` / `LibraProductMetadataCommitPayload` | Media Cast payload固定为`{schema:"libra.media-cast-fact-commit-payload@1",sourceBasis,mediaCastDraft}`；Product Metadata payload固定为`{schema:"libra.product-metadata-fact-commit-payload@1",sourceBasis,productMetadataDraft,verifiedArtifactManifest,mediaCastFactRef}`，其中`mediaCastFactRef`为`NULL`或完整`{productFactId,factRevision,factDigest}`。`sourceBasis`必须是对应closed union的完整typed value；`DomainFactCommitHandle.payloadDigest=SHA-256(JCS(对应完整payload))`，Handle本身不进入digest。Observation Draft匹配Set digest；Western Media-Cast Draft匹配Match Basis digest；Western Product Metadata Draft匹配Basis中的`analysisVariantDigest`并逐字节等于Normalize Result，最终Basis digest不得要求出现在Draft。Product variant的Manifest run必须等于当前Run。`mediaCastFactRef=NULL`时Result该字段必须为NULL且Commit不得扫描Media Cast Fact；非NULL时按`productFactId`精确读取`libra_product_fact_revisions`，要求同一`libra_run_id`、`fact_kind=media_cast`、`fact_revision/fact_digest`逐字节匹配，重新解析并验证完整`MediaCastFact@1`后原样构造Result三元组；禁止latest/current fallback或把ID字符串猜成完整ref |
| `LibraProductFactCommitIdentity` | 对两个variant：`factKind=media_cast|product_metadata`；`aggregateType=libra_product_fact`；`aggregateId=SHA-256(JCS({schema:"libra.product-fact-aggregate-id@1",libraRunId,factKind}))`；`factRevision=expectedRevision+1`；`productFactId=SHA-256(JCS({schema:"libra.product-fact-id@1",libraRunId,factKind,factRevision}))`。Handle固定`ownerDomain=libra,aggregateType,aggregateId,factType=factKind,factSchemaRef/resultSchemaRef=MediaCastFact@1|ProductMetadataFact@1,expectedRevision,payloadDigest,eventFenceDigest`；`commitIdempotencyKey=SHA-256(JCS({schema:"libra.product-fact-commit-key@1",aggregateId,expectedRevision,payloadDigest,eventFenceDigest}))`，`handleId=SHA-256(JCS({schema:"libra.product-fact-commit-handle-id@1",aggregateId,factKind,expectedRevision,payloadDigest,resultSchemaRef,eventFenceDigest}))`；`commit_marker=commitIdempotencyKey` |
| `LibraProductFactEvidence` | `evidenceDigest=SHA-256(JCS({schema:"libra.product-fact-evidence@1",libraRunId,factKind,factRevision,sourceBasisKind,sourceBasisDigest,commitPayloadDigest,eventFenceDigest}))`。`fact_json`保存完整typed Result；`fact_digest=DomainFactEnvelope.factDigest`；`result_digest=SHA-256(JCS(完整typed Result))`且逐字节等于同marker的Event Result。Media Cast Result逐项复制Draft relation并以当前Run的subjectId校验；Product Metadata Result逐项复制Draft、同Run Media Cast Fact引用与Manifest digest，Owner row额外冻结完整Verified Artifact Manifest。Owner row、Source refs、Manifest内显式Artifact Verification Result refs、marker与Result足以历史重建完整Commit Payload；不得回读Provider、People、later Foundation Result或扫描无关Result补字段 |
| `TranscodeInputVerification` | `VerificationEnvelope + sourceHandleDigest + encodeIntentDigest + probeEvidenceId + probeEvidenceDigest + deviceId + deviceSnapshotDigest + selectedDeviceClass + reasonCodes[]`；selectedDeviceClass是单一scalar并逐字节等于Intent.deviceClass和Device Snapshot.deviceClass，不是数组。Probe必须`probed`且`sourceHandleDigest`等于完整输入Handle JCS digest；Device Snapshot必须enabled/ready、capabilityPayload digest匹配且声明支持Intent codec/rate-control。closed reason按`source_handle_mismatch|source_not_media|probe_integrity_failure|device_class_mismatch|encode_intent_unsupported|input_fence_mismatch`顺序去重；空数组时result=passed，否则failed。Snapshot形成后Platform current revision变化不伪装为本Result reason，而由transcode Effect pre-fence产生`fence_rejected`。`basisDigest=SHA-256(JCS({schema:"libra.transcode-input-verification-basis@1",sourceHandleDigest,encodeIntentDigest,probeEvidenceDigest,deviceSnapshotDigest}))`，`verificationId=SHA-256(JCS({schema:"libra.transcode-input-verification-id@1",basisDigest}))`，Evidence/Result digest均按各自Envelope排除自身字段的统一公式；不选择Device、不生成后续Flow，完整value≤`16 KiB` |
| `WorkspaceMediaHandle` | `workspaceMediaHandleId,sourceMaterialHandleDigest,workspaceMaterialHandle(WorkspaceMaterialHandle),workspaceMaterialHandleDigest,outputTargetId,outputTargetDigest,producingEventId,productionIntentKind(remux|encode),productionIntentDigest,executionDeviceRef?{deviceId,deviceClass,deviceSnapshotDigest},effectReceiptRef{effectId,effectReceiptId,effectReceiptDigest},resultDigest`；只有一个Foundation Material Handle，`workspaceMaterialHandleId`永远是嵌套`workspaceMaterialHandle.handleId`，不再存在第二个隐式Handle ID。Remux禁止executionDeviceRef；Encode必须逐字节复制正式Device Snapshot三元组。`sourceMaterialHandleDigest`逐字节等于Effect input完整Handle JCS digest并受Effect intent保护。Effect必须逐项验证Target Workspace/root/relativePath/fence并在相同idempotency key重放时从Effect Journal + `fx_workspace_materials`返回相同Handle；`workspaceMaterialHandleDigest=SHA-256(JCS(workspaceMaterialHandle))`，`workspaceMediaHandleId=SHA-256(JCS({schema:"libra.workspace-media-handle-id@1",sourceMaterialHandleDigest,workspaceMaterialHandleId:workspaceMaterialHandle.handleId,outputTargetId,producingEventId,productionIntentDigest,executionDeviceRefOrNull:executionDeviceRef??null}))`，`resultDigest=SHA-256(JCS(完整value excluding resultDigest))`。Result不内含Probe；输出Probe由后续`shared.material.media.probe@1`显式产生，完整value≤`16 KiB` |
| `ProductMediaVerification` | `VerificationEnvelope + candidateId + candidateNodeId + candidateBasisDigest + candidateKind(direct_input|workspace_output) + libraRunId + producingEventId? + productMaterialHandleId + productMaterialHandleDigest + productMaterialFenceDigest + workspaceMediaHandleId? + mediaRequirementId + mediaRequirementDigest + sourceProbeEvidenceId + sourceProbeEvidenceDigest + outputProbeEvidenceId + outputProbeEvidenceDigest + qualitySummary{videoCodec,container,fileExtension,displayRasterClass,primaryAudioClasses[],sourceDisplayRasterClass,systemUpscaleDetected} + spaceSummary{unit,actualSizeBytes,maxSizeBytes?,withinLimit} + reasonCodes[]`。Candidate三元组逐字节复制Input并重算其Plan-known公式。direct_input禁止producingEventId/workspaceMediaHandleId，product三元组来自完整Physical Handle，output Probe与source Probe为同一Evidence；workspace_output要求producingEventId/workspaceMediaHandleId并从嵌套Workspace Handle形成product三元组，Output Probe的`sourceHandleDigest`必须等于该完整Handle JCS digest，source Probe必须等于`WorkspaceMediaHandle.sourceMaterialHandleDigest`。Requirement逐字节携带Acceptance Spec完整mandatoryMedia/space。closed evaluation依次为`output_handle_mismatch|output_not_media|media_form_unmet|video_codec_unmet|container_unmet|file_extension_unmet|minimum_raster_unmet|system_upscale_forbidden|primary_audio_unmet|max_size_exceeded|requirement_integrity_failure`：stream_file要求普通文件且无disc topology；codec/container/extension/raster按Probe和Handle规范值精确比较；accepted audio为空表示不限制，否则按`MediaProbeEvidence@1`的Primary Audio集合至少一项normalized class命中；workspace output为4K而source低于4K且forbid upscale时失败，direct input比较同一Probe故不伪造upscale；space按product/episode scope比较actual bytes。reason空为passed，否则failed；不得转码音频或修改Requirement。`verificationId=SHA-256(JCS({schema:"libra.product-media-verification-id@1",candidateId,candidateNodeId,candidateBasisDigest,candidateKind,libraRunId,productMaterialHandleId,productMaterialFenceDigest,mediaRequirementDigest,sourceProbeEvidenceDigest,outputProbeEvidenceDigest}))`，basis/evidence/result digest按完整typed值唯一计算，完整value≤`16 KiB`；只有`primary_payload`的workspace output passed Result可作为`WorkspaceProductVerificationSnapshot@1.media`分支进入Product Staging，direct_input的passed Result只供direct-original Package Conformance；非媒体Artifact不得调用本Capability伪造通过 |
| `SelectedProductOutput` | `DraftEnvelope + libraRunId + acceptanceSpecId + mediaRequirementDigest + criteriaId + criteriaDigest + candidateSetDigest + result(selected|not_selected) + selectedCandidateKind? + selectedHandleId? + selectedWorkspaceMediaHandleId? + selectedVerificationId? + selectedVerificationDigest? + selectionReasonCode(selected_by_declared_rank|no_passed_candidate)`。Capability先拒绝Criteria/ref/digest不闭合，再只保留passed candidates，按Criteria显式rank升序、同rank按verificationId UTF-8 bytes确定；有候选时`selectedHandleId=productMaterialHandleId`、`selectedVerificationDigest=SHA-256(JCS(完整selected ProductMediaVerification))`，workspace variant才含selectedWorkspaceMediaHandleId，并填满其余selected字段；无候选时全部selected字段为NULL。`DraftEnvelope.draftId=SHA-256(JCS({schema:"libra.selected-product-output-id@1",libraRunId,criteriaId,candidateSetDigest}))`，`draftDigest=SHA-256(JCS(完整Draft excluding DraftEnvelope.draftDigest))`，完整value≤`16 KiB`；不得使用caller数组顺序、文件大小、时间或未声明“质量偏好”自行排名 |
| `ProductConformanceEvidence` | `VerificationEnvelope + libraRunId + acceptanceSpecId + acceptanceSpecRecordDigest + runExecutionBasisDigest + productSnapshotDigest + productFactSetDigest + evaluatedRequirementSetDigest + unmetRequirementCodes[] + reasonCodes[]`。Capability只验证完整`ProductConformanceInputSnapshot@1`，没有Repository、Owner Store或Query port。六组closed evaluation为：Identity按完整`ResolvedProviderIdentity@1`的namespace/key/season tuple；Structure按`ProductStructureSnapshot@1 + ProductionMaterialManifest@1`的single/season与Episode coverage；Metadata按closed Product Fact union逐项读取required field，并按完整Artifact Verification Result判定NFO renderable、poster/fanart decodable；Mandatory Media与Space要求每个Selected Product携带同Requirement的passed `ProductMediaVerification@1`；Inventory按`ProductInventoryConformanceSnapshot@1`验证Binding、checksum、included-product Artifact和logical layout。field lookup固定为：`tmdb_movie_id|tmdb_series_id|jav_code`读取对应namespace的providerKey，`internal_identity`读取internal Anchor，`actor`读取MediaCastFact非空relation，`season_number|episode_number`读取Identity/Manifest Episode scope，其他field code读取`ProductMetadataFact.descriptiveFacts`同名non-null/non-empty字段；不得模糊匹配或扫描latest Fact。unmet closed set仍为`identity_unmet|season_identity_unmet|structure_unmet|episode_coverage_unmet|metadata_field_unmet|metadata_artifact_unmet|sidecar_unrenderable|image_undecodable|media_form_unmet|video_codec_unmet|container_unmet|file_extension_unmet|minimum_raster_unmet|system_upscale_forbidden|primary_audio_unmet|max_size_exceeded|domain_binding_unmet|checksum_unmet|artifact_materialization_unmet|layout_unmet`并按该顺序去重；缺合法业务Fact/Artifact产生对应unmet，typed对象缺失、union/schema/digest/ref/集合不闭合才以reason `snapshot_integrity_failure`产生failed。无reason且unmet为空为passed；有合法业务unmet为failed。`evaluatedRequirementSetDigest=SHA-256(JCS({schema:"libra.product-conformance-requirements@1",acceptanceSpecId,recordDigest:acceptanceSpecRecordDigest,requirements:input.acceptanceSpec.requirements}))`，`verificationId=SHA-256(JCS({schema:"libra.product-conformance-verification-id@1",libraRunId,runExecutionBasisDigest,productSnapshotDigest,evaluatedRequirementSetDigest}))`；P9-08 Attestation只逐字节引用passed的verificationId、evidenceDigest、productSnapshotDigest与evaluatedRequirementSetDigest |
| `OnDeckProductPackage` | `ManifestEnvelope + onDeckPackageId + packageRevision + libraRunId + runStateRevision + runStateDigest + runExecutionBasisDigest + subjectId + shelfId + acceptanceSpecRef{id,recordDigest} + resolvedIdentitySnapshot{productFactId,factRevision,schemaRef,factValue,factDigest,evidenceDigest} + productStructureSnapshot + runMaterialManifestRef{id,digest} + productMaterialManifest(ProductionMaterialManifest) + productFactManifest + artifactManifest + mediaCastSnapshot{mediaCastFactId,mediaCastFactRevision,schemaRef,factValue,factDigest,evidenceDigest,relations[],relationsDigest} + offloadContextManifest + productionProvenance + productionAttestation + packageDigest`；两个Snapshot必须逐字节等于Product Fact Manifest内对应唯一item，Media Cast的relations从同一factValue确定性展开；全部member/fact/artifact/offload数组分别按8.6.21稳定排序并关系化保存；`packageDigest=SHA-256(JCS(完整nominal typed value excluding ManifestEnvelope.manifestDigest, ManifestEnvelope.publishedAtMs and packageDigest))`且等于Manifest digest；`publishedAtMs`固定为Promotion commit time，只是持久化/审计元数据，不得反向进入Decision预先携带的content digest；禁止用自选字段子集、Package relation aggregate或Receipt摘要替代该完整basis；完整JCS≤`16 MiB`且只经bounded Handoff Port读取，不进入Event Result/Outbox；Delivery Snapshot必须从Libra Owner rows重建同一完整typed value并重算相同`packageDigest`，Arca不回读People、Provider、Foundation Event Result或current Workspace补relation |
...
| `PerceptionObservationPage` | `EvidenceEnvelope + perceptionAcquisitionId + pageOrdinal + source{sourceId,sourceKind,sourceConfigRevision} + cursor{expectedCursorRevision,cursorIn,cursorOut} + observations[{observationId,sourceRecordKey,sourceRecordRevision,sourceRecordDigest,observedAtMs,payloadSchemaRef,payloadDigest,inlinePayload,provenanceDigest}] + observationPageDigest + hasMore`；每项Observation必须包含Normalize所需的bounded typed `inlinePayload`且canonical bytes匹配`payloadDigest`；单项≤`16 KiB`、整页Result≤`64 KiB`。完整Provider response不进入Result；Adapter以字段投影/page budget满足限制，超限稳定失败且不推进cursor，禁止pure Acquire创建Artifact |
| `PerceptionAcquisitionCommitDraft` | `DraftEnvelope + perceptionAcquisitionId + source{sourceId,sourceKind,sourceConfigRevision} + normalizationRuleRef + cursorTransition{pageOrdinal,expectedCursorRevision,cursorIn,cursorOut,observationPageDigest,hasMore} + records[{draftId,recordKind(observation|correction|retraction),sourceRecordKey,sourceRecordRevision,sourceRecordDigest,rating?,watchedState?,observedTitle,observedAtMs,identityAnchors[{anchorKind,anchorValue,confidenceClass,evidenceDigest}],provenanceRef,provenanceDigest}] + sourceLineageRelations[{relationKind(supersedes|retracts),sourceDraftId,targetSourceRecord{sourceRecordKey,sourceRecordRevision,sourceRecordDigest},ruleRevision,evidenceDigest}]`；单Draft只含一个Source的一页；Canonical rating仅`integer 1..5|null`，watchedState仅`boolean|null`；完整Draft digest是Commit Handle payloadDigest |
...
| `PerceptionResolutionDraft` | `DraftEnvelope + queryContract + querySchemaRef + queryInputDigest + factKind + recordSetDigest + ruleRevision + ruleDigest + resultKind(found|not_found) + winningPerceptionId? + resolvedValue? + resolvedProvenance? + reasonCode? + duplicateRelationDrafts[{sourcePerceptionId,targetPerceptionId,ruleRevision,evidenceDigest}]`；`resolvedValue={factKind,value}`且rating value为integer `1..5`、watched value为boolean；`resolvedProvenance={winningPerceptionId,sourceKind,sourceRecordKey,sourceRecordRevision,provenanceRef,provenanceDigest,matchedAnchorEvidence[{anchorKind,strengthRank,evidenceDigest}]}`且Anchor数组按strength/key稳定排序；found必须携带winner/value/provenance且reason为空；not_found不得携带winner/value/provenance，reason固定为`no_matching_record|requested_fact_absent|strongest_value_conflict`；duplicate pair必须规范化且只来自Rule Snapshot的duplicate proof matcher，不因fuzzy match或相同value生成 |
| `PerceptionResolutionRevision` | `DomainFactEnvelope + queryContract + querySchemaRef + queryInputDigest + factKind + recordSetDigest + ruleRevision + ruleDigest + resultKind + winningPerceptionId? + resolvedValue? + resolvedProvenance? + reasonCode? + committedRelationIds[]`；resolved value/provenance沿用Draft的精确schema，完整保存并由Resolution Facade返回，不允许Facade旁读Foundation或重新执行Policy拼装结果 |
| `PersonRegistrationEvidence` | `EvidenceEnvelope + originKind(provider_hint|ondeck_nfo_projection) + originRef + originRevision + originEvidenceDigest + proposedName + aliases[{aliasDisplay,aliasNormalized,provenanceDigest}] + providerIdentities[{provider,namespace,providerKey,provenanceDigest}] + referenceHints[{hintKind,referenceValue,provenanceDigest}]`；数组有界，大型引用只能使用typed handle/ref；referenceHints不是已导入Reference Image，也不是联网图片搜索结果 |
...
| `PersonReferenceRevision` | `DomainFactEnvelope + personId + referenceRevision + operationKind(add_image|release_image) + affectedReferenceAssetId + affectedReferenceFaceId + activeReferenceAssets[{referenceAssetId,artifactHandleId,artifactDigest}] + activeReferenceFaces[{referenceFaceId,referenceAssetId,embeddingHandleId,embeddingDigest,modelRef}] + activeAssetSetDigest + activeFaceSetDigest + referenceSetDigest`；ID排序统一比较exact ID的UTF-8 bytes升序，禁止locale/case-fold；Asset/Face ID重复直接拒绝。`activeAssetSetDigest=SHA-256(JCS({schema:"people.reference-active-assets@1",items:activeReferenceAssets按referenceAssetId排序}))`；`activeFaceSetDigest=SHA-256(JCS({schema:"people.reference-active-faces@1",items:activeReferenceFaces按referenceFaceId排序}))`；`referenceSetDigest=SHA-256(JCS({schema:"people.reference-set@1",personId,activeAssetSetDigest,activeFaceSetDigest}))`；`factDigest=SHA-256(JCS(完整PersonReferenceRevision excluding factDigest))`，其他三个digest及两个完整数组均保留在basis中；Face始终与唯一Asset共同建立/释放 |
...
| `PersonRevision` | `DomainFactEnvelope + operationKind(registration|merge) + registrationOrigin(direct|candidate)? + personId(primary/new target) + canonicalName + aliasSetDigest + providerIdentitySetDigest + directDecisionRef? + acceptedCandidateRef?{candidateKind,candidateId,candidateRevision,candidatePayloadDigest} + affectedPersonRevisions[{personId,revision,status,factDigest}] + mergeRecordRef? + preferenceRevisionRef?`；direct Registration必须有`directDecisionRef`且无Candidate ref，Candidate Registration/Merge反之；Merge Result同时返回target新revision和source terminal revision，marker重放返回同一完整value |
...
1. **Accepted Business DTO**：`SelectedFieldMaterialSet`、`ProcurementRunExecutionBasis`、
   `ProcurementRunSealDecision`、`ProcurementRetryIntent`、`CandidatePackage`、`PrimaryInputManifest`、
   `ShelfStandard`、`AcceptanceSpec`、`FinalInventoryDecision`、`DirectPersonRegistrationDecision`、`PeopleCandidateAcceptanceDecision`、
   `PeopleReferenceMaintenanceDecision`、`DestructionScope`等必须包含对象ID、revision、
...
2. **Typed Handle/Projection**：使用`8.6.18`字段或versioned public Query Projection；不得传Store row、裸路径
   或跨Domain可变对象；
3. **Bounded Intent/Requirement**：`BoundedFingerprintProfile`、`SamplingPlan`、`EncodeIntent`、`RemuxIntent`、
   `MediaRequirement`、`SelectionCriteria`、`ArtifactRequirement`等必须包含`intent/requirementId, revision,
   schemaRef, digest`，且只描述当前单效果所需参数。

Media Production是上述通则的closed specialization，不允许生成器退化为旧generic schema：

- `libra.transcode.input.verify@1`的exact named inputs为`sourceMaterialHandle,sourceProbeEvidence,encodeIntent,
  deviceSnapshot`；`selectedDeviceClass`物化为scalar string，四项都必须进入Event immutable input binding；
- `libra.media.remux@1`的exact named inputs为`sourceMaterialHandle,remuxIntent,outputTarget`；
  `libra.media.transcode@1`追加`deviceSnapshot,transcodeInputVerification`，其passed basis必须逐项命中同一Handle、
  Probe、Intent和Device Snapshot。两项Effect的idempotency key固定为`SHA-256(JCS({schema:
  "libra.workspace-media-effect-key@1",capabilityRef,eventId,sourceHandleDigest,productionIntentDigest,
  outputTargetId,outputTargetDigest}))`；pre-effect fence重验Workspace state/root/target containment，Effect receipt绑定
  target、实际output digest和Foundation Handle registration。重启只恢复或核对同一Effect，不重新选Device/路径；
- `libra.product_media.verify@1`的唯一exact named input为`productMediaCandidateInput`，其closed union完整携带
  direct input或workspace output的Handle/Probe/Requirement；`mediaProbeRef`字符串与第二Handle ID从schema删除；
- `libra.product_output.select@1`只接受完整`ProductOutputSelectionInput@1`；
  `libra.product.conformance.verify@1`只接受完整`ProductConformanceInputSnapshot@1`。这两项pure Executor不得声明
  Domain/Foundation/Platform Store read dependency。

Machine type registry必须为`EncodeIntent@1`、`RemuxIntent@1`与`MediaRequirement@1`分别生成8.6.18的exact
`additionalProperties:false` schema；`EncodeIntent@1.intentDigest`与`RemuxIntent@1.intentDigest`是唯一Intent
摘要字段，旧`digest`、`targetQuality`、generic `audioCodec`或`typedParameters`均不是别名或兼容输入。
`ProductConformanceInputSnapshot@1`与`WorkspaceProductVerificationSnapshot@1`的nested schema必须以exact `$ref/oneOf`引用
`ResolvedProviderIdentity@1`、三个closed Product Fact value、`ArtifactManifestVerification@1`、
`ProductionMaterialManifest@1`、`ArtifactManifest@1`、`SelectedProductOutput@1`与
`ProductMediaVerification@1`；Workspace Product Verification还必须保留本节role-aware
`media|artifact|structural` closed union，Schema materializer遇到同名旧bounded-contract不得合并、降级或生成第二条路径。

`LibraProductionCoordinator`负责在Event Attempt前组装Conformance input，但它不是新Capability、Owner或Store：
它只按Run冻结的Acceptance Spec、显式Product Fact ID/revision、Product Staging Reference、Material Manifest以及
`VerifiedArtifactManifest.items[].verificationResultRef`的精确ID读取typed Owner/Public Result；禁止按Run扫描
latest/current Fact或无关Foundation Result。Coordinator逐项重建完整值、验证schema/digest/ref后形成上述Snapshot，
Runtime把完整Snapshot作为immutable named input并冻结`inputBindingDigest/snapshotDigest`。pure Executor只复验
Snapshot内部闭包并执行六组规则，不再次证明数据库历史，也没有任何Query port；装配阶段缺失、stale或digest冲突
在调用Executor前就是input integrity failure。该Snapshot与passed `ProductConformanceEvidence@1`的
`productSnapshotDigest`随后被P9-08 Attestation逐字节引用，因此Attestation不需要回读Fact/Artifact/Material Store。

`PlatformComputeRuntimePort`是既有Platform typed runtime port族的只读扩充，不属于Catalog Capability。Planner只能
冻结found Snapshot；Device在执行前失效时由fence形成stable failure并交回Runtime/Planner，Executor不得隐藏fallback。
`deviceClass=remote_worker`时，Composition Root只能按Snapshot.workerRef在Attempt开始时取得同revision/capability的
短期`WorkerHandle@1`并放入fence snapshot；WorkerHandle不改变选定Device，也不得指向另一Worker/revision。取得失败或
revision不匹配只产生fence rejection，不能回退本地Device。local Device禁止workerRef/WorkerHandle。
Media Production所有Probe都由既有`shared.material.media.probe@1`产生；转码/Remux Executor不得内部补Probe并把它
伪装为正式Evidence。上述细化不新增Owner、Store、Handoff或Canonical Transaction。
...
Structure/IdentityMetadata`；Schema必须把Member映射、`1..256`总Selection、`1..100`Probe Batch、digest、Rule
...
Reference语义校验必须证明`add_image`的Artifact与Embedding `sourceArtifactSetDigest`精确对应、Face/Vector count
均为1、Asset/Face ID未被使用且expected Person/reference revision仍current；`release_image`必须证明目标Asset
active、Face active且二者唯一关联。Metadata fetch校验必须证明Intent discriminator与Handle kind、Integration
type或Related Reference digest精确匹配。任何一项失败都不得通过Executor默认值、按数组取首项或Store旁读补齐。

PBF-13 Application DTO同样必须生成正式Schema：Run/Workspace/Package/Cleanup Decision不得退化为
`objectIds[] + digest`；`ProductionMaterialManifest`、`OnDeckProductPackage`和Cleanup member的完整成员只可使用本节
closed结构。大Package经Handoff Port按16 MiB上限读取，Commit Event Result只使用bounded Receipt，不能为了通过
Foundation 64 KiB限制截断Package或把Owner业务事实藏入Artifact/Result fallback。
`fx_plan_nodes.input_bindings_json`只保存versioned typed binding refs；Run/Promotion等大Decision由Coordinator在提交
边界按这些精确refs装配。可变current input必须携带expected revision/digest并在Canonical Transaction重验，immutable
input只可从本Domain Owner rows、Foundation Control/Workspace historical rows或正式typed跨Domain Query Result展开；
Capability Executor本身不得读Store。装配后的完整Decision可以超过16 KiB，但不得作为单个Plan JSON/Event Result
持久化；commit成功后的全部业务成员必须由本节relation rows永久重建，commit前崩溃只重新装配并接受同一CAS结果。
...
#### 8.6.21 Libra Run、Workspace、Deliverable与Reclamation typed contract

本节只把Level 3/6已经确认的Libra业务责任变成可持久化、可CAS和可历史重建的formal DTO；这些Application
Decision/Result、Facade Read DTO和内部typed Message不新增Catalog Capability或Result family。除另有说明外，集合
ID按exact UTF-8 bytes升序，禁止locale/case-fold；所有digest均为小写hex `SHA-256(JCS(value))`，digest字段自身不
进入其basis；数组必须已去重，越界或顺序错误稳定拒绝而不是由Commit Participant整理。

**Run input、scope与state：**

- `ProductionMaterialManifest@1`固定为
  `{manifestId,manifestRole(run_input|product_delivery),manifestRevision,libraRunId,
  scopeKind(single|episode_delivery),members[1..1024],memberSetDigest,
  episodeScopeDigest,manifestDigest}`。member逐项为
  `{ordinal,materialKey,role(primary_payload|structural_dependency|metadata_sidecar|poster|fanart|subtitle|
  external_audio|chapter),physicalIdentity{mountScopeId,inode,sizeBytes,fingerprintAlgorithm,contentFingerprint},sizeBytes,
  location{locationKind(domain_binding|workspace_handle),endpointId,location?,rootHandleRef?,relativePath?},
  bindingKind(libra_material_binding|workspace_material_reference),bindingRevision,
  bindingEvidenceDigest,originCandidateDeliveryRef?{intakeDecisionId,offerId,candidatePackageId,packageRevision,
  packageDigest,candidateDeliverySnapshotDigest,relatedReferenceSetDigest},workspaceReferenceId?,
  workspaceMaterialHandle?,admittedControlRevision?,
  admittedControlProjectionDigest?,outputRequirementDigest,
  episodeClaims[0..32 {episodeKey,seasonClaimDigest,claimDigest}],episodeClaimSetDigest,memberDigest}`；
  `claimDigest=SHA-256(JCS({schema:
  "libra.production-material-episode-claim@1",episodeKey,seasonClaimDigest}))`；members按
  materialKey排序后从0赋ordinal，Episode claims按episodeKey排序。run_input及direct-original固定
  `locationKind=domain_binding,bindingKind=libra_material_binding`、origin/admitted Control pair非NULL且workspace/root-relative字段为NULL；Workspace Product固定
  `locationKind=workspace_handle`且location字段为NULL，endpoint/root/relative逐字节等于Handle，并固定
  `bindingKind=workspace_material_reference,originCandidateDeliveryRef/admittedControl pair=NULL`，binding revision/evidence逐字节等于同Run current Product Staging
  Reference，并内联其完整typed Workspace Handle。`memberDigest=SHA-256(JCS(member excluding
  memberDigest))`；`episodeClaimSetDigest=SHA-256(JCS({schema:
  "libra.production-material-episode-claims@1",items:episodeClaims}))`；`memberSetDigest=SHA-256(JCS({schema:"libra.production-material-members@1",items:
  members}))`；`episodeScopeDigest=SHA-256(JCS({schema:"libra.production-episode-scope@1",items:
  去重episodeClaims按episodeKey排序}))`；同一episodeKey被多个Material覆盖时三字段必须逐字节相同，否则Manifest
  稳定拒绝；single必须为空scope，season至少一项；`manifestDigest=SHA-256(JCS(
  完整value excluding manifestDigest))`；`manifestId=SHA-256(JCS({schema:"libra.production-material-manifest-id@1",
  manifestRole,libraRunId,manifestRevision}))`。Run input固定role=`run_input`,revision=1；Product delivery固定
  role=`product_delivery`,revision=Package revision，二者不得复用ID。run_input只允许
  `primary_payload|structural_dependency`；product_delivery至少一个primary，其他role按Spec可选且只有primary允许
  Episode Claim。Run Admission构造run_input时，Physical Identity/size/Location必须逐项取自current immutable
  `libra_material_bindings` revision并满足`PhysicalMaterialIdentity@2`的materialKey公式；该Binding的Handoff A origin
  又必须回指同一historical Candidate Delivery Snapshot，禁止从materialKey反解、读取current Procurement Field或由caller补值。
  Episode Delivery成员本身没有`pending|delivered`状态；Run Input与Product Deliverable各自是immutable Manifest，
  业务推进只由Run revision、Package revision与Arca Inventory事实表达，禁止原地修改Episode member冒充交付。
- 每个member的`outputRequirementDigest`不是caller提供的opaque值，也不是由Planner选择六类Requirement的某个
  子集。它唯一来自同Run immutable `AcceptanceSpec@1`与本member事实。先定义
  `acceptanceRequirementSetDigest=SHA-256(JCS({schema:"libra.acceptance-requirement-set@1",
  requirements:acceptanceSpec.requirements}))`；六类Requirement完整对象必须按8.6.18规则规范化，不能只使用
  `specDigest`替代该typed value。
- `ProductionMaterialOutputRequirement@1`固定为
  `{acceptanceSpecRef{acceptanceSpecId,specRevision,specDigest,recordDigest,productScopeDigest},
  manifestRole(run_input|product_delivery),materialKey,materialRole,
  applicationScope{kind(product_scope|episode_subset|production_support),episodeKeys[],scopeDigest},
  acceptanceRequirementSetDigest,
  outputRequirementDigest}`。`applicationScope.scopeDigest=SHA-256(JCS({schema:
  "libra.production-material-requirement-scope@1",productScopeDigest:acceptanceSpec.productScope.scopeDigest,
  kind,episodeKeys}))`，episodeKeys总是去重并按UTF-8 bytes排序；
  `outputRequirementDigest=SHA-256(JCS(完整value excluding outputRequirementDigest))`。
- 映射是closed且无Planner自由度：single `primary_payload`使用`product_scope+[]`；season
  `primary_payload`使用`episode_subset+本member全部episodeClaims中的episodeKey`，要求1..32项且必须是Acceptance
  Spec Product Scope的非空子集。`structural_dependency`固定`production_support+[]`，明确它只参与生产且不单独
  声称满足最终产品要求；`metadata_sidecar|poster|fanart|subtitle|external_audio|chapter`固定`product_scope+[]`。
  具体责任仍由`materialRole`保留，不能把多种role压成同一digest。run_input只会命中primary/structural两个分支；
  product_delivery按实际role命中对应分支。所有分支都绑定完整六类Requirement；该binding只说明Material属于哪份
  产品要求及其作用范围，不声称单个Material独立满足整份Spec，也不把Acceptance Policy拆成member-local Policy。
- Run Admission从`libra_runs.acceptance_spec_id → libra_acceptance_specs`的immutable Spec、Run Manifest/member/Episode
  relation重算每项typed value；Deliverable Promotion从Package与Run共同引用的同一Spec及Product
  Manifest/member/Episode relation重算。两者都必须逐字节等于持久化`output_requirement_digest`并进入member/
  manifest digest。历史恢复只使用这些Libra Owner rows；不得读取current Spec、caller cache、Foundation Result或
  另一Domain Store补值。该typed binding是application data contract，不新增Capability、Result family或表。
- `LibraRunExecutionBasis@1`固定为
  `{subjectSnapshot{subjectId,intakeRevision,structureKind,contentProfile,continuitySetDigest,episodeScopeDigest},
  decisionHeadSnapshot(SubjectDecisionHeadSnapshot),acceptanceSpec{acceptanceSpecId,specRevision,specDigest,
  recordDigest,productScopeDigest,shelfId},shelfProjection{routingProjectionRevision,projectionDigest,
  standardRevision,standardDigest},productionMaterialManifest,executionBasisDigest}`；Basis完整JCS≤`8 MiB`，
  `executionBasisDigest=SHA-256(JCS(完整value excluding
  executionBasisDigest))`。Run Admission写入后该Basis不可改写，所有成员都能由Run/Manifest/Binding snapshot
  Owner rows恢复；成员引用的随单Related Evidence只通过冻结的exact origin ref调用Handoff A historical
  `CandidateDeliveryPort`并校验snapshot/set digest，不旁读Procurement Store。不得用后来current Subject、Spec、
  Binding或Control补历史。Priority是Run-local scheduling fact，不属于产物目标或Execution Basis；Priority变化不得
  改变`executionBasisDigest`或触发replacement Run。
- `LibraRunExecutionBasisRecord@1`与上述Basis字段逐项相同，但把完整
  `productionMaterialManifest`替换为`productionMaterialManifestRef{manifestId,manifestDigest,memberCount,
  episodeScopeDigest}`；Record本身不另造semantic digest。历史重建必须先读该typed record，再从本Run immutable
  Manifest/member/Episode relation展开完整Basis并重算`executionBasisDigest`；Record缺字段或Manifest不一致是
  integrity failure，不能读取current Subject/Decision Head/Shelf Projection补齐。
- `runScopeDigest=SHA-256(JCS({schema:"libra.run-scope@1",subjectId,structureKind,contentProfile,
  productScopeDigest,episodeScopeDigest,productionMaterialManifestDigest}))`。admission提交前的
  `expectedRunAdmissionHead={headState(absent|present),headRevision,activeScopeSetDigest}`；absent固定revision 0与
  `SHA-256(JCS({schema:"libra.active-run-scopes@1",subjectId,items:[]}))`；present的headRevision必须为正整数。
  `headState=absent`当且仅当不存在`libra_run_admission_heads` row且headRevision=0，不能以存储revision 1伪装
  pre-CAS absent state，也不能建立revision 0 sentinel row。
- `LibraRunAdmissionDecision@1`固定为
  `{decisionId,admissionKind(initial|replacement),subjectId,admissionRevision,libraRunId,
  replacementOfRunRef?,expectedRunAdmissionHead,runExecutionBasis,
  initialPriority{priorityClass(normal|expedited),priorityIntentDigest},runScopeDigest,decisionDigest}`；
  `admissionRevision=expectedHeadRevision+1`，`libraRunId=SHA-256(JCS({schema:"libra.run-id@1",subjectId,
  admissionRevision}))`；先由该ID派生Manifest ID并形成Manifest/Run Scope/Basis；decision digest覆盖完整value。
  replacement ref固定旧`{libraRunId,stateRevision,stateDigest,runScopeDigest,acceptanceSpecId,
  executionBasisDigest}`且只允许相同Subject/Episode交付scope，并要求Acceptance Spec record digest或
  Execution Basis digest至少一项变化；initial不得携带该ref。initial Priority恒为`normal`且intent digest固定为
  `SHA-256(JCS({schema:"libra.priority-intent-empty@1"}))`；replacement必须逐字节复制旧Run current Priority，不能由
  caller借replacement提权。`decisionId=SHA-256(JCS({schema:
  "libra.run-admission-decision-id@1",libraRunId,executionBasisDigest,
  initialPriority.priorityClass,initialPriority.priorityIntentDigest}))`。
  “相同Subject/Episode交付scope”固定比较`subjectId + structureKind + ProductScope.scopeKind + 去重排序episodeKeys`；
  不比较包含Material Manifest digest的`runScopeDigest`，否则合法的Binding/Material变化将永远无法建立replacement。
- `LibraRunAdmissionResult@1`固定为
  `{decisionId,libraRunId,admissionRevision,stateRevision(1),stateDigest,executionBasisDigest,runScopeDigest,
  productionMaterialManifestId,priorityClass,priorityIntentDigest,committedAdmissionHeadRevision,activeScopeSetDigest,
  supersededRunRef?{libraRunId,committedStateRevision,committedStateDigest},resultDigest}`。
...
- `PlatformWorkspaceRootSnapshot@1`固定为
  ...
  snapshotDigest}`；`rootHandleRef=SHA-256(JCS({schema:"platform.workspace-root-handle@1",rootId,
...
- `PBF-17`不改变“全部changed/new Workspace Product必须先进入Product Staging”的既有门禁，只修正其
  verification discriminator：Product verification必须证明该Material role要求的产品性质，而不是把所有文件都当作
  可播放媒体。`WorkspaceProductVerificationSnapshot@1`因此是同一门禁下的role-aware closed union。公共字段固定为
  `{verificationKind(media|artifact|structural),materialRole,libraRunId,workspaceMaterialHandleId,
  workspaceMaterialHandleDigest,workspaceMaterialFenceDigest,schemaRef,verificationId,verificationValue,
  verificationDigest,snapshotDigest}`；三个Handle字段必须逐字节命中同一完整`WorkspaceMaterialHandle@1`，
  `verificationId=verificationValue.verificationId`，`verificationDigest=SHA-256(JCS(verificationValue))`，
  `snapshotDigest=SHA-256(JCS(完整Snapshot excluding snapshotDigest))`，完整Snapshot canonical JCS≤`128 KiB`：
  - `media`恰允许`materialRole=primary_payload`、`schemaRef=ProductMediaVerification@1`，其value必须是
    `libra.product_media.verify@1`的typed `candidateKind=workspace_output,result=passed` Result，并逐项命中同一Run、
    `productMaterialHandleId`、Handle digest及fence；
  - `artifact`恰允许`materialRole=metadata_sidecar|poster|fanart`、`schemaRef=ArtifactManifestVerification@1`，
    追加完整`artifactHandle(ArtifactHandle@1)`与`artifactRequirement(ArtifactRequirement@1)`；value必须来自
    `shared.artifact.manifest.verify@1`且`result=passed`，包含同一Handle/revision/kind/digest与Requirement。
    Commit同时按ID精确读取`fx_artifact_registry`，要求Artifact owner scope属于同一Libra Run/Workspace，并要求
    Artifact的digest algorithm/digest/size逐字节等于Workspace Material Handle的content hash/digest/size；
    role-kind固定映射为`metadata_sidecar→nfo`、`poster→poster`、`fanart→fanart`，不得把Artifact伪装成可播放媒体；
  - `structural`恰允许`materialRole=structural_dependency|subtitle|external_audio|chapter`、
    `schemaRef=ManifestVerification@1`，追加完整bounded `typedManifest`、`manifestContract`与
    `verifiedMemberDigest`；value必须来自`shared.manifest.verify@1`且`result=passed`，Manifest/Contract必须显式
    包含同一Run、同一Material role及同一Workspace Handle digest/fence，Result的manifest/contract basis逐字节匹配。
  任一分支的failed/not_applicable、另一Handle、另一Run、role/schema错配或digest不一致均稳定拒绝。
  Snapshot是Reference Commit的完整正式输入，Commit不旁读Foundation Event Result、caller cache或当前文件Reality；
  提交成功后由Libra reference row永久重建。`product_verification_id=verificationId`，
  `product_verification_digest=snapshotDigest`，不能只保存内部Result digest而丢失role/Handle binding。
- `WorkspaceMaterialReferenceSnapshot@1`固定为
  `{referenceId,workspaceId,libraRunId,materialHandleId,materialKey,workspaceMaterialHandle(WorkspaceMaterialHandle),
  workspaceHandleDigest,referenceRevision,
  state(working|product_staging|released),episodeClaims[0..32 {episodeKey,seasonClaimDigest,claimDigest}],
  episodeScopeDigest,productVerificationRef(WorkspaceProductVerificationSnapshot)?,previousReferenceRevision?,
  committedWorkspaceRevision,
  referenceDigest}`；`referenceId=SHA-256(JCS({schema:"libra.workspace-reference-id@1",workspaceId,
  materialHandleId}))`；Episode claims按episodeKey排序且沿用`ProductionMaterialManifest@1`的单项/set公式，
  `episodeScopeDigest=SHA-256(JCS({schema:"libra.production-episode-scope@1",items:episodeClaims}))`；
  `workspaceHandleDigest=SHA-256(JCS(workspaceMaterialHandle))`且逐字节等于Foundation/Libra row的handle digest；
  `referenceDigest=SHA-256(JCS(完整value excluding referenceDigest))`。
  `LibraWorkspaceMaterialReferenceDecision@1`固定为
  `{decisionId,operation(attach_working|promote_to_product_staging),libraRunId,workspaceId,
  expectedWorkspaceRevision,expectedWorkspaceStateDigest,expectedReference{state(absent|present),revision,digest},
  workspaceMaterialHandle,episodeClaims,episodeScopeDigest,
  productVerificationRef(WorkspaceProductVerificationSnapshot)?,decisionDigest}`；`LibraWorkspaceMaterialReferenceResult@1`固定为
  `{decisionId,workspaceId,workspaceRevision,workspaceStateDigest,referenceSnapshot,
  workspaceMaterialReferenceSetDigest,resultDigest}`。Material Reference只是Libra business relation，文件bytes
  仍由Foundation Workspace handle定位；不得让`workspace_write` Capability暗写Libra Store。
  absent固定`revision=0,digest=SHA-256(JCS({schema:"libra.workspace-reference-absent@1",workspaceId,
  materialHandleId}))`；present必须逐字节等于该reference stream最高revision。`decisionId=SHA-256(JCS({schema:
  "libra.workspace-reference-decision-id@1",operation,workspaceId,
  materialHandleId:workspaceMaterialHandle.handleId,expectedReferenceState:expectedReference.state,
  expectedReferenceRevision:expectedReference.revision,
  workspaceMaterialFenceDigest:workspaceMaterialHandle.fenceDigest}))`；该canonical object恰含上述7个顶层property，
  不嵌套Handle/expectedReference、不使用点号property name。Decision/Result digest均覆盖各自完整value excluding自身digest。
- `workspaceMaterialReferenceSetDigest=SHA-256(JCS({schema:"libra.workspace-reference-set@1",workspaceId,
  items:[每个referenceId最高revision且state!=released的完整Snapshot]按referenceId排序}))`；Workspace
  `stateDigest=SHA-256(JCS({schema:"libra.workspace-state@1",workspaceId,workspaceRevision,state,
  workspaceMaterialReferenceSetDigest,transitionKind,transitionEvidenceDigest}))`。Working→Product Staging只是一条
  append-only relation transition，不移动文件、不改变Material Identity，也不预先取得Deliverable Control。
  两种普通Reference operation都要求Run及Workspace仍active；Workspace一旦reclaiming只能由Cleanup Commit追加released。
  `attach_working`要求expected Reference absent且`productVerificationRef=NULL`；
  attach提交前还必须验证提交后的current non-released Reference总数不超过1024；
  `promote_to_product_staging`要求expected Reference current state=working并携带验证同一Handle且与目标Product
  Material role一致的完整`WorkspaceProductVerificationSnapshot@1`，同时完整Episode claims/scope digest必须逐字节沿用expected Reference。
  普通Reference事务不接受released目标；只有Cleanup Commit能在Deletion/already-absent
  Evidence成立后追加released revision。

**Product Package与Promotion：**

- `ProductStructureSnapshot@1`固定为
  `{structureKind(single|season),contentProfile,productScopeDigest,episodeScopeDigest,primaryMaterialCount,
  structuralDependencyCount,productStructureDigest}`；`productStructureDigest=SHA-256(JCS(完整value excluding
  productStructureDigest))`。它只保存有界结构摘要，完整Material↔Episode N:M成员由
  Product Material Manifest承载，禁止在package row的structure JSON重复内联1024项列表。
- `ProductFactManifest@1`固定为`{manifestId,manifestRevision,libraRunId,items[1..256],factSetDigest,manifestDigest}`，items为
  `{productFactId,factKind,factRevision,schemaRef,factValue,factDigest,evidenceDigest,referenceDigest}`，按
  factKind/id/revision排序；`factValue`是对应immutable `libra_product_fact_revisions.fact_json`的完整typed value、
  单项≤`64 KiB`且必须重算相同`factDigest`；`referenceDigest=SHA-256(JCS(单项 excluding referenceDigest))`，
  `factSetDigest=SHA-256(JCS({schema:"libra.product-fact-set@1",items}))`，manifest digest覆盖完整value excluding
  manifestDigest。`ArtifactManifest@1`固定为
  `{manifestId,manifestRevision,libraRunId,items[0..256],artifactSetDigest,manifestDigest}`，items为
  `{artifactHandleId,artifactKind,artifactRevision,artifactDigest,requirementDigest,materializationState,
  referenceDigest}`，按kind/handle/revision排序，单项reference digest同样排除自身，
  `artifactSetDigest=SHA-256(JCS({schema:"libra.product-artifact-set@1",items}))`，manifest digest覆盖完整value
  excluding自身。Resolved Identity与Media Cast必须分别是Product Fact Manifest内唯一
  `factKind=resolved_identity|media_cast`项；Package的两个对应Snapshot逐字节引用这些item并内联同一
  `factValue`，Media Cast Snapshot还必须从该value无损展开完整relations。
- Product Package内的`ProductionMaterialManifest@1`使用新`manifestId`且`libraRunId`相同；direct-original member
  从Run Material Manifest逐项复制Physical Reality/Binding并使用`assert_existing_input`，Workspace member从current
  Product Staging Reference与Handle逐项复制并使用`acquire_workspace_product`。Product member额外包含
  `controlOperation,expectedControlRevision?,expectedControlProjectionDigest?,committedControlRevision,
  committedControlProjectionDigest`；member/set/manifest digest沿用本节公式且必须与Package relation rows逐字节相等。
- `OffloadContextManifest@1`固定为
  `{manifestId,manifestRevision,libraRunId,members[0..1024],memberSetDigest,manifestDigest}`；member是
  `{ordinal,materialKey,contextRole,physicalIdentity,location,bindingRevision,bindingEvidenceDigest,
  admittedControlRevision,admittedControlProjectionDigest,settlementExpectation,memberDigest}`，按
  materialKey/contextRole排序；member digest覆盖单项excluding自身，set digest使用schema
  `libra.offload-context-members@1`，manifest digest覆盖完整value excluding自身。`settlementExpectation`只是
  `retain|replace_or_move|remove_after_place`的生产交接预期，
  不构成Arca Final Inventory Decision或删除授权。
  上述Product Fact、Artifact、Off-load Context与Product Material四个Manifest的`manifestRevision=packageRevision`；
  前三者的`manifestId=SHA-256(JCS({schema:"libra.product-submanifest-id@1",manifestKind(product_fact|artifact|offload_context),
  libraRunId,packageRevision}))`，Product Material沿用本节既有ID公式。四个ID/digest均写入Package row，不能只以
  relation所属package ID或aggregate digest冒充Manifest identity。
- `ProductionProvenance@1`固定为
  `{libraRunId,runExecutionBasisDigest,acceptanceSpecRecordDigest,workflowPlanRefs[0..256
  {planId,planRevision,planDigest}],productVerificationRefs[0..256 {verificationId,verificationDigest}],
  externalRealityObservationRefs[0..256 {evidenceId,evidenceDigest}],provenanceDigest}`；数组按ID排序且完整JCS≤`64 KiB`，
  只引用已持久化Plan/Evidence/Effect receipt，不内联日志；该字段只证明Libra读取过的外部现实，不授权或声称
  Libra修改Workspace之外的Physical Material。`LibraProductionAttestation@1`固定为
  `{attestationId,libraRunId,onDeckPackageId,acceptanceSpecId,acceptanceSpecRecordDigest,
  productConformanceEvidenceId,productConformanceEvidenceDigest,evaluatedRequirementSetDigest,
  productSnapshotDigest,unmetRequirementCount(0),attestedAtMs,attestationDigest}`；所有字段必须逐字节复制同Run、同Spec、
  `passed`的`ProductConformanceEvidence@1`，`attestationId=SHA-256(JCS({schema:
  "libra.production-attestation-id@1",libraRunId,onDeckPackageId,productConformanceEvidenceId,
  productConformanceEvidenceDigest}))`，`attestationDigest=SHA-256(JCS(完整value excluding attestationDigest))`；只有
  unmet count为0且Conformance reason为空可发布。
- `LibraDeliverablePromotionDecision@1`固定为
  `{decisionId,libraRunRef{libraRunId,stateRevision,stateDigest,executionBasisDigest,runScopeDigest,
  expectedPackageRevisionHead},runMaterialManifestRef{manifestId,manifestDigest},workspaceRef?{workspaceId,
  workspaceRevision,workspaceStateDigest},productStagingReferences[],acceptanceSpecRef{acceptanceSpecId,
  recordDigest},resolvedIdentitySnapshot,productStructureSnapshot,productFactManifest,artifactManifest,
  mediaCastSnapshot,productMaterialManifest,offloadContextManifest,productionProvenance,productionAttestation,
  controlCommitScope{items[1..1024],controlScopeDigest},onDeckPackageId,packageRevision,packageDigest,offerId,
  decisionDigest}`。Product Staging refs按referenceId排序并与Workspace Product members恰好一一对应；每个ref的
  `WorkspaceProductVerificationSnapshot.verificationKind/materialRole`必须与matching Product Material member的role
  一致，媒体、Artifact与结构依赖分别使用各自closed分支；direct-original
  Package允许`workspaceRef=NULL/productStagingReferences=[]`；每个Workspace member的Episode claims聚合必须重算为
  matching Reference的`episodeScopeDigest`且完整claims逐字节相等，不能由caller另配Episode。Control item按materialKey排序，variant固定为
  `assert_existing_input{materialKey,expectedControlRevision,expectedControlProjectionDigest,
  ownerDomain:"libra",ownerScopeType:"subject",ownerScopeId:subjectId}`或
  `acquire_workspace_product{materialKey,expectedControlState:"absent",toOwnerDomain:"libra",
  toOwnerScopeType:"on_deck_package",toOwnerScopeId:onDeckPackageId}`；它与Product Manifest的controlOperation及
  matching `ResponsibilityControlCommitHandle`逐项相等。`controlScopeDigest=SHA-256(JCS({schema:
  "libra.product-control-commit-scope@1",libraRunId,onDeckPackageId,items}))`。
  `expectedPackageRevisionHead`固定为non-negative integer并逐字节等于`libra_runs.package_revision_head`当前数值；
  首个Package固定expected 0。`packageRevision=
  expectedPackageRevisionHead+1`，`onDeckPackageId=SHA-256(JCS({schema:"libra.on-deck-package-id@1",
  libraRunId,packageRevision}))`。该ID是在Control/Manifest构造前由Run内唯一revision确定性预派生的稳定身份，
  不是内容摘要、不是独立Reservation，也不得包含Product Manifest、Off-load Context、Package或Control digest；
  Commit仍必须CAS Run current `package_revision_head`，不得按MAX查询后盲插。
  `offerId`沿用8.5.11公式；`decisionId=SHA-256(JCS({schema:
  "libra.deliverable-promotion-decision-id@1",libraRunId,packageRevision,packageDigest,
  controlCommitScope.controlScopeDigest}))`。Decision digest覆盖完整typed input。
- `PBF-18`固定唯一的非循环构造顺序：先从expected head派生`packageRevision/onDeckPackageId`；再以该ID构造
  `on_deck_package`目标Control scope。`assert_existing_input` member的committed pair逐字节等于事务前已验证的
  current Control；`acquire_workspace_product` member固定形成revision 1，并按8.6.18从精确post-state
  `{controlled,ownerDomain:"libra",ownerScopeType:"on_deck_package",ownerScopeId:onDeckPackageId,
  regionProjection:"production"}`重算真实`committedControlProjectionDigest`。随后才把这些真实committed pair写入
  Product Material members并计算member/set/manifest digest，再计算Attestation、完整Package content digest、Offer与Decision。
  `PBF-18-R1`明确Package content digest覆盖8.6.19列出的完整nominal typed value，但排除两个自摘要字段以及
  只能由Promotion commit写入的`publishedAtMs`；提交后填入该时间不能改变Package/Offer identity，历史读取仍须从
  Owner rows重建完整Package并按相同排除规则复验digest。
  禁止使用`pending`、placeholder、预估或事后覆盖的Control digest；事务必须重算实际post-CAS Control row及同一marker
  historical revision，并与Decision/Product relation逐字节相等。`onDeckPackageId`负责稳定身份，`packageDigest`负责
  完整内容真实性；Handoff B始终以`onDeckPackageId+packageDigest`及逐成员historical Control projection验证交接，
  不把Package ID重新解释为内容摘要。Package head CAS、全部Control CAS/revision、Manifest/Package relation、
  Receipt/marker/Offer/Outbox全有或全无；失败CAS或rollback不得留下预派生ID对应的row或Control。
  Commit后assert member保留原Control revision/projection，acquire member固定形成revision 1；二者的post-state item按materialKey排序形成
  `controlRevisionSetDigest=SHA-256(JCS({schema:"libra.product-control-revision-set@1",onDeckPackageId,
  items:[{materialKey,controlOperation,committedControlRevision,committedControlProjectionDigest}]}))`，并进入Receipt。
- Promotion的durable Event Result是bounded `OnDeckProductPackageCommitReceipt@1`；完整
  `OnDeckProductPackage@1`是Libra-owned Handoff Deliverable，通过`ProductDeliveryPort`从关系表重建，不塞入
  `fx_event_result_bindings`的64 KiB JSON。Catalog中Receipt替换原来错误地把完整Package当作commit output的同一
  Result slot，故Result family计数不变。`LibraProductOfferAvailableMessage@1`固定为
  `{messageKind:"libra.product-offer.available@1",messageId,offerId,onDeckPackageId,packageRevision,packageDigest,
  libraRunId,subjectId,shelfId,acceptanceSpecId,dedupKey}`；`messageId=dedupKey=SHA-256(JCS({schema:
  "libra.product-offer-message-id@1",offerId,packageDigest}))`，consumer固定`ArcaAcceptanceFacade`。
- `ArcaProductAcceptedMessage@1`固定为
...
- `LibraRunDiscardDecision@1`固定为
  `{discardDecisionId,libraRunId,expectedRunStateRevision,expectedRunStateDigest,runScopeDigest,
  originalInputControlScope{items[1..1024],scopeDigest},workspaceCleanupScopeDraft?,workspaceCleanupMemberSetDigest,
  actorId,idempotencyKey,
  decisionDigest}`；Control items逐项来自Run Material Manifest且按materialKey排序。`discardDecisionId=SHA-256(
  JCS({schema:"libra.run-discard-decision-id@1",libraRunId,actorId,idempotencyKey}))`；Decision/Receipt formula与
  8.5.11 row逐项对应。每个Control item固定为
  `{materialKey,expectedControlRevision,expectedControlProjectionDigest,fromOwnerDomain:"libra",
  fromOwnerScopeType:"subject",fromOwnerScopeId:subjectId,operation:"release"}`，必须逐项等于Run Material Manifest
  admitted Control snapshot及当前Foundation Control；`scopeDigest=SHA-256(JCS({schema:
  "libra.run-discard-input-control-scope@1",libraRunId,runScopeDigest,items}))`。匹配的
  `ResponsibilityControlCommitHandle`必须使用operationKind=`release`、同一basis/Control scope/expected revisions；
  committed release set按materialKey排序并进入Receipt，不得由current Subject Binding重新枚举。
  `LibraRunDiscardReceipt@1`固定为
  `{ReceiptEnvelope,discardDecisionId,libraRunId,committedRunStateRevision,releasedInputControlSetDigest,
  cleanupScopeId?,cleanupMemberSetDigest,commitDigest}`；固定`receiptKind=libra_run_discarded,ownerDomain=libra,
  scopeType=libra_run,scopeId=libraRunId,scopeDigest=LibraRunDiscardDecision.decisionDigest`，
  `receiptId=SHA-256(JCS({schema:"libra.run-discard-receipt-id@1",discardDecisionId}))`，
  `commitDigest=SHA-256(JCS(完整value excluding commitDigest))`；scope nullable规则与表一致。
  `LibraWorkspaceCleanupRequestedMessage@1`固定为
  `{messageKind:"libra.workspace-cleanup.requested@1",messageId,libraRunId,cleanupScopeId?,
  cleanupMemberSetDigest,triggerDigest,dedupKey}`，`messageId=dedupKey=SHA-256(JCS({schema:
  "libra.workspace-cleanup-message-id@1",libraRunId,triggerDigest,cleanupMemberSetDigest}))`，consumer固定Libra
  Workspace Reclaimer；它只wake，非空Scope本身已在Discard事务成立。
- `OffloadCompletionQuery@1`固定为`{queryContract:"arca.offload-completion@1",onDeckPackageId,
  expectedPackageDigest}`；`OffloadCompletionReadResult@1`为`found{projectionRevision(1),projectionDigest,
  onDeckPackageId,packageDigest,offloadCompletionFact}`或`not_found{checkedAtMs}`。found的package pair必须逐字节等于
  Query；`projectionDigest=SHA-256(JCS({schema:"arca.offload-completion-projection@1",projectionRevision:1,
  offloadCompletionFact}))`，从immutable `arca_offload_completions` row确定性重建，不要求第二张Projection表；
  不一致是integrity error。Arca `OffloadCompletionPort`只读Owner Fact/Projection，不触发
  On-deck；found历史revision可重放。Libra只把found variant作为offload cleanup trigger，not_found不是失败也不
  建Scope。
- `WorkspaceCleanupScopeAdmissionDecision@1`固定为
  `{decisionId,triggerKind(offload_completed|run_superseded|run_discarded),triggerSnapshot,
  libraRunRef{libraRunId,stateRevision,stateDigest,executionBasisDigest},
  workspaceRef{workspaceId,workspaceRevision,workspaceStateDigest,materialReferenceSetDigest},
  graceDeadlineMs,referenceAudit?{observation1,observation2},eligibilityEvidenceDigest,
  members[0..1024],memberSetDigest,cleanupScopeId,
  decisionDigest}`。`triggerSnapshot`是closed union：offload固定为完整found
  `OffloadCompletionReadResult@1`；superseded固定为`{oldRunRef,replacementRunRef}`且两项互相引用同一replacement；
  discard固定为`{discardDecisionId,expectedFrozenRunRevision,expectedFrozenRunDigest}`，不得内联尚未计算的父
  `discardDecisionDigest`形成自引用。
  offload与superseded variant的两次observation必须间隔至少一个Reclaimer周期、都证明无其他
  active Libra Reference；offload还要求当前时间不早于Completion后24小时，superseded要求replacement Run已成立且
  grace为0；discard variant由Discard事务直接形成且grace为0。superseded trigger固定
  `triggerRef=oldLibraRunId,triggerRevision=terminalStateRevision,triggerDigest=terminalStateDigest`，只清理旧Run Workspace，
  不接管或删除replacement Workspace；它关闭旧Run未被Arca Accepted的Workspace/Product遗留责任。offload固定
  `triggerRef=offloadCompletionFact.factId,triggerRevision=projectionRevision,
  triggerDigest=projectionDigest`；discard固定`triggerRef=discardDecisionId,triggerRevision=1,
  triggerDigest=discardDecisionDigest`。三种映射不得由Store实现自行选择Fact digest或时间戳替代。
  offload的`graceDeadlineMs=offloadCompletionFact.committedAtMs+86_400_000`，superseded/discard固定0；时间计算溢出或
  Completion缺commit time稳定拒绝。
  每个reference observation固定为`{observedAtMs,activeReferenceSetDigest,activeReferenceCount,
  controlProjectionSetDigest,evidenceDigest}`；其中active set只枚举目标Workspace之外、仍引用任一目标materialKey/handle的
  current Libra Reference，合格observation必须`activeReferenceCount=0`且digest为正式empty set；Control set则完整
  枚举目标members的完整available Control Projection（包括uncontrolled、Libra-owned和other-owned）。Admission事务必须从current Reference/Control rows重算并逐字节等于
  第二次observation，变化即stale且不建Scope。discard variant的`referenceAudit=NULL`，其他variant必须非NULL且第二次时间更晚。
  `activeReferenceSetDigest=SHA-256(JCS({schema:"libra.workspace-cleanup-other-reference-set@1",
  targetWorkspaceId:workspaceRef.workspaceId,items:[{workspaceId,referenceId,referenceRevision,referenceDigest}]
  按workspaceId/referenceId排序}))`，空集也使用该公式；
  `controlProjectionSetDigest=SHA-256(JCS({schema:"libra.workspace-cleanup-control-projection-set@1",
  items:[{materialKey,controlDisposition,controlRevision,controlProjectionDigest,ownerDomain?,ownerScopeType?,
  ownerScopeId?}]按materialKey排序}))`；从未存在Control row以revision 0/uncontrolled正式表达，不能省略该item；
  observation的`evidenceDigest=SHA-256(JCS(完整observation excluding evidenceDigest))`。
  `eligibilityEvidenceDigest=SHA-256(JCS({schema:"libra.workspace-cleanup-eligibility@1",triggerKind,
  triggerSnapshot,libraRunRef,workspaceRef,graceDeadlineMs,referenceAudit}))`；它不覆盖members，成员由独立set digest约束。
  offload与superseded variant的`cleanupScopeId=SHA-256(JCS({schema:"libra.workspace-cleanup-scope-id@1",triggerKind,
  triggerRef,triggerRevision,triggerDigest,workspaceId}))`；discard variant固定为`SHA-256(JCS({schema:
  "libra.workspace-cleanup-scope-id@1",triggerKind,discardDecisionId,workspaceId}))`，其row在Decision完成后保存
  `triggerDigest=discardDecisionDigest`，Decision本身不得把该自digest回填为输入。
  member为`{materialHandleId,materialKey,workspaceReferenceId,
  expectedReferenceRevision,expectedReferenceDigest,
  controlDisposition(uncontrolled|libra_owned|other_owned),expectedControlRevision,
  expectedControlProjectionDigest,expectedControlOwnerDomain?,expectedControlOwnerScopeType?,
  expectedControlOwnerScopeId?,cleanupKind:"delete_or_verify_absent",memberDigest}`，按handle ID排序；
  Control字段必须逐字节映射current `MaterialControlProjectionSnapshot@1`：uncontrolled无owner、libra_owned的owner
  domain固定libra、other_owned固定为非libra；Cleanup只能释放libra_owned Control。
  members必须恰好覆盖该Workspace每个reference stream最高revision中`state!=released`的handle，不得漏项、重复或
  把历史released revision重新纳入；`created`要求`1..1024`项，没有此类成员时Decision固定为空数组并且只允许typed
  `no_op`，不能建立空Scope。
  `memberDigest=SHA-256(JCS(member excluding memberDigest))`；`memberSetDigest=SHA-256(JCS({schema:
  "libra.workspace-cleanup-members@1",items:members}))`；offload/superseded admission的`decisionId=SHA-256(JCS({schema:
  "libra.workspace-cleanup-admission-decision-id@1",triggerRef,triggerRevision,triggerDigest,workspaceId,
  memberSetDigest}))`；discard draft的`decisionId=SHA-256(JCS({schema:
  "libra.workspace-cleanup-admission-decision-id@1",triggerKind:"run_discarded",discardDecisionId,workspaceId,
  memberSetDigest}))`。
  `WorkspaceCleanupScopeAdmissionResult@1`固定为
  `{decisionId,resultKind(created|no_op),cleanupScopeId?,scopeStateRevision?,scopeStateDigest?,memberSetDigest,
  memberCount,committedWorkspaceRevision?,committedWorkspaceStateDigest?,resultDigest}`。created固定
  `scopeStateRevision=1`且Workspace双字段非NULL、为reclaiming post-state，no_op要求这些字段全部NULL。offload或superseded没有Workspace Reference时返回semantic no-op且
  不建Scope；discard的同类空结果只由Discard Receipt的nullable scope ref与正式empty member-set digest表达，不另写
  Scope Admission Result。
  `WorkspaceCleanupScopeAdmissionRecord@1`等于Decision除`members`外的全部字段，并增加
  `memberCount/memberSetDigest`；Scope row保存该Record，member rows保存完整members。历史重建必须合并两者并重算
  `decisionDigest`；不得把只有trigger/eligibility摘要的row称为完整Decision。
- `WorkspaceCleanupEffectIntent@1`固定为
  `{intentId,cleanupScopeId,workspaceId,materialHandleId,expectedWorkspaceHandleDigest,expectedReferenceRevision,
  expectedReferenceDigest,controlFence{controlDisposition,expectedControlRevision,
  expectedControlProjectionDigest,expectedControlOwnerDomain?,expectedControlOwnerScopeType?,
  expectedControlOwnerScopeId?},effectMode(delete_or_verify_absent|verify_absent_only),containmentFenceDigest,
  idempotencyKey,intentDigest}`；只授权处理一个Workspace-contained handle，不携带父目录或Material Field/Shelf target路径。
  uncontrolled/libra_owned固定`delete_or_verify_absent`，other_owned固定`verify_absent_only`且文件仍存在时必须返回
  control conflict、不得调用delete。`intentId=idempotencyKey=SHA-256(JCS({schema:
  "libra.workspace-cleanup-effect-intent-id@1",cleanupScopeId,workspaceId,materialHandleId,
  expectedReferenceRevision,expectedReferenceDigest,controlFence,effectMode}))`，`intentDigest=SHA-256(JCS(完整value excluding
  intentDigest))`；同一member retry必须复用相同Intent/Effect idempotency key。
- `WorkspaceMaterialDeletionEvidence@1`固定为
  `{EvidenceEnvelope,effectId,cleanupScopeId,materialHandleId,preDeleteHandleDigest,result(deleted|already_absent),
  postDeleteContainmentProbeDigest,effectReceiptId,evidenceDigest}`；只能由既有幂等filesystem delete effect产生，
  `already_absent`也必须证明目标恰是Workspace-contained handle，不能以父目录扫描代替。Effect成功必须以同一
  receipt把`fx_workspace_materials`从active CAS为reclaimed；Evidence的effect/receipt digest与该terminal row逐字节
  相等，Domain Commit只消费Evidence而不重做物理效果。`other_owned` Intent不得产生`deleted`结果；文件仍存在时
  只能返回control conflict，只有确证already absent才能形成本Evidence。
- `WorkspaceCleanupBlockingEvidence@1`固定为
  `{EvidenceEnvelope,cleanupScopeId,materialHandleId,terminalFailureCode(workspace_path_escaped|identity_mismatch|
  control_conflict|effect_retry_exhausted),lastEffectId?,lastEffectFailureDigest?,observedFenceDigest,evidenceDigest}`；只在
  有界Effect retry耗尽或安全Fence确定冲突后形成，不能把暂时资源等待、可重试I/O或Provider失败标成blocked。
- `WorkspaceCleanupCommitDecision@1`固定为
  `{decisionId,cleanupScopeId,expectedScopeStateRevision,expectedScopeStateDigest,workspaceId,
  expectedWorkspaceRevision,expectedWorkspaceStateDigest,materialHandleId,expectedReferenceRevision,
  expectedReferenceDigest,expectedMemberStateRevision,expectedMemberStateDigest,outcome{kind(deletion_verified|
  terminal_blocked),deletionEvidence?,blockingEvidence?},
  expectedControlFence{materialKey,controlDisposition,revision,projectionDigest,ownerDomain?,ownerScopeType?,
  ownerScopeId?},decisionDigest}`。
  `decisionId=SHA-256(JCS({schema:"libra.workspace-cleanup-commit-decision-id@1",cleanupScopeId,
  materialHandleId,expectedMemberStateRevision,outcomeEvidenceDigest}))`，其中outcomeEvidenceDigest从互斥variant
  唯一取得；Decision digest覆盖完整value，expected Workspace必须处于reclaiming。deletion_verified要求Deletion Evidence且禁止Blocking Evidence；
  terminal_blocked反之。Control fence必须逐字节等于Scope member及current Projection；libra_owned completed
  member在同事务release，uncontrolled不写Control revision，other_owned不释放且只接受`already_absent` Evidence。
  任一Control Projection变化使Decision stale并重新评估Scope，不能在省略release或越权删除的情况下提交。Result使用
  `WorkspaceCleanupCommitReceipt@1`；`cleanupState=completed`表示本member完成，scope是否完成由同事务重算全部
  member terminal state后决定；blocked只终结本member/Scope并保留Reference与Control供人工诊断；完成member必须在同事务追加对应Reference `released` revision，全部member
  completed时再把Libra Workspace与Foundation Registry置为reclaimed。Deletion Effect成功但commit前崩溃时复用相同effect receipt；commit成功后重放
  原Receipt，不再次删除或释放Control。`deletionEvidence.preDeleteHandleDigest`必须逐字节等于expected Reference
  冻结的`workspaceHandleDigest`，Evidence中的scope/handle也必须等于Decision；否则拒绝提交。
  completed非末项追加一条Workspace `state=reclaiming,transitionKind=reference_released` revision；末项追加一条
  `state=reclaimed,transitionKind=reclaimed` revision并让同事务Reference的`committedWorkspaceRevision`指向它，
  不为同一member虚构两条Workspace revision。blocked不追加Workspace/Reference revision。

Cleanup member的`stateDigest=SHA-256(JCS({schema:"libra.workspace-cleanup-member-state@1",cleanupScopeId,
materialHandleId,stateRevision,state,outcomeEvidenceDigest,cleanupReceiptId,committedControlRevision?}))`；Scope的
`stateDigest=SHA-256(JCS({schema:"libra.workspace-cleanup-scope-state@1",cleanupScopeId,stateRevision,state,
memberSetDigest,terminalMemberSetDigest,completedAtMs?}))`，terminal member set按materialHandleId排序。每次member
commit都使Scope state revision加1；blocked member使Scope为blocked，只有全部completed才为completed。

`ProductDeliveryQuery@1`固定为`{queryContract:"libra.product-delivery@1",readPurpose(historical|
acceptance_fence),offerId,onDeckPackageId,expectedPackageRevision,expectedPackageDigest}`；
`ProductDeliveryPort.readPackage(ProductDeliveryQuery@1)`必须返回
`ProductDeliveryReadResult@1(found{onDeckProductPackage,readDigest}|not_found{reasonCode(package_missing),
checkedAtMs})`；found还携带`deliveryFence?{eligibility(eligible|ineligible),reasonCode?,libraRunId,
runState,runStateRevision,runStateDigest,acceptanceSpecId,packageRevisionHead,deliveryReceiptAbsent,
productControlSetDigest?,fenceDigest}`。historical固定Fence为NULL；acceptance_fence只有Run仍active、Spec/Package
current、尚无Delivery Receipt且Package Product/Off-load去重Control仍由Libra持有时为eligible；否则reason固定为
`run_not_active|spec_not_current|package_not_current|delivery_already_terminal|control_not_libra`并返回ineligible，Arca不得
...
PBF-13相关Canonical Transaction的机器表集固定如下；`Result/marker`分别指
`fx_event_result_bindings/fx_commit_markers`，不得因某个variant没有row而从transaction manifest删表：
...
| Workspace Admission | `libra_workspaces,libra_workspace_revisions,fx_workspace_registry` + Result/marker | `libra_runs,libra_run_revisions,libra_run_material_manifests,libra_run_material_members`及Platform read participant的`platform_workspace_roots`；事务内从immutable Primary members重算input bytes/request/required bytes并交叉验证Root/space typed input；不读取Episode Claim，Platform内部resolved path不进入read result或Libra rows | no |
| Workspace Material Reference Commit | `libra_workspaces,libra_workspace_revisions,libra_workspace_material_refs` + Result/marker | `libra_runs,libra_run_revisions,fx_workspace_materials,fx_artifact_registry`及全部write tables；Foundation Material row重建完整Handle，Decision携带完整role-aware Product Verification并冻结到Reference row；`fx_artifact_registry`只在artifact分支按显式Handle ID精确读取，media/structural分支不扫描它；不读Workspace Registry、Foundation Event Result或current Reality | no |
...
Product Metadata variant还必须从Commit Payload/Owner row内完整`VerifiedArtifactManifest`取得去重后的显式
`verificationResultRef`集合，逐项读取对应Work→Plan→Event→`ArtifactManifestVerification@1`，验证Plan input中的完整
Handle/Requirement、Result passed item和`fx_artifact_registry` row；`requirementDigest`必须命中同一
`ProductMetadataDraft.artifactRequirements`成员。`fx_event_result_bindings`同时是Source Basis/Artifact Verification的
read table与当前Commit Result的write table，Coordinator必须按不同`result_id/event_id`区分，不允许覆盖既有Result。
同一variant还必须按Commit Payload的nullable`mediaCastFactRef`执行精确ID读取或精确NULL传播；非NULL Fact必须由同一
`libra_product_fact_revisions` Owner row重建完整`MediaCastFact@1`并重算Fact digest，不能按Run扫描“最新”Media Cast。
任一引用缺失、schema/digest不匹配、revision fence变化或typed Result/Manifest无法由Owner row重建，都使整个
transaction rollback；Product Fact仍引用相关Result期间，Result链与Artifact Registry row不得GC。

`offload_completed`查询结果作为typed immutable input进入Cleanup Scope Admission，不把`arca_*`加入Libra事务
read set；如果该Workspace没有任何未released Reference，Reclaimer返回typed semantic no-op并不建立空Scope。
上述当前variant要求的write row、Control revision、Result、marker或required Outbox缺失都使事务整体回滚；
semantic replay先从Owner rows重建原Decision/Result并校验digest，再返回原结果，不能重新分配ID或重复外部效果。
Cleanup Scope Admission的`no_op`是唯一没有新Owner业务Fact的variant，只从同事务Result/marker重放；这不是以
Foundation Result补读Owner事实。事务manifest列出的variant-superset表不要求无关variant伪造空row。
...
| `capabilities/westernAdultCapabilities.js`：frames、embed、cluster、match、poster、compose、worker register/upload、analysis request/observe/normalize | 11 | Frame/Western product能力归Libra；embedding/cluster/match为shared service-local算法合同；Beta删除Western Worker register/upload与ExternalJob语义，request/observe改签为同一clean Service内Artifact-backed分析链且不保留dual path；`shared.worker.*`若被其他正式能力引用仍只属于Remote Worker协议，不能被Western调用；People只提供有界projection集合，旧execute不得写People Store或传base64/embedding热payload |
...
~~~text
clean Capability refs           111
unique refs                     111
duplicate refs                    0
Result schema families           97
unresolved Result families        0
Effect Classes                    7 / 7 covered
Canonical Transactions           43 / 43 defined
historical registrations         62
historical registrations mapped  62
~~~

111项ref均已绑定固定Effect Class、contract package结构、ExecutionContext、Outcome envelope、typed handle/
...
97的当前值包含`PBF-11-R2-R1`对Handoff A/Handoff B拒绝Receipt的nominal分型；`PBF-12/PBF-13`新增的
application transaction result与Facade Read DTO，以及`PBF-14`新增的Commit payload/selection/input helper DTO，
...
逐表合同共有`178 tables / 178 unique names / 0 invalid prefix / 0 duplicate definition`：
...
- Product Metadata/Media-Cast Fact以Libra relation显式引用其精确closed Source Basis Result（Metadata Observation或
  Western Analysis/Normalize/Match），Fact history不扫描later current Result，也不把Foundation Event Result当成
  无归属缓存；
...
- Perception Acquisition、跨Acquisition cursor head、page CAS、Record/Anchor/Record Relation、typed page receipt与
  commit result形成显式闭合链；Normalize不依赖Provider二次读取，pure Acquire不写Artifact，Canonical rating
  只允许1–5或null，Commit/Result digest均有无自引用的唯一basis；
...
| --- | --- | --- |
| Handoff A Accepted | Candidate Delivery snapshot重建前后、global continuity head CAS前后、continuity match前后、并发Subject/episode/Resolved Identity exact anchor变化、target Intake CAS前后、Decision/Subject/Binding N:M relation participant后、逐项Control CAS/transfer revision及Control set digest组装前后、Result/marker/Outbox前、Control后来再次转移后历史重放 | Snapshot的Offer/Package/Manifest/Location Evidence不一致即fail closed且不旁读`proc_*`修补；exact claim唯一命中且zero overlap才extension；0/N命中、缺失或overlap新建Subject；global head阻止query phantom，target head阻止唯一Subject stale extension；竞态使Decision失效后重新装配；new Subject identity pointer为NULL；Handle/Payload/Control scope、expected与committed projection、from/to scope、marker/basis必须逐项闭合，Control set按唯一formula生成；要么全部不存在，要么global/target revision、Decision及match/overlap Evidence、Subject/claim/Episode scope、每Material Binding及全部Episode relation、Control revision、Receipt/Result/marker/Outbox全部成立；后续current owner变化仍能从historical revision重建同一Receipt；Procurement只异步消费Receipt |
| Procurement Handoff A acceptance consume | Inbox写入前后、open Delivery CAS前后、1/256个Reservation逐项转移中途、Delivery terminal与Inbox result提交前后、迟到Rejected消息 | Delivery、全部Candidate member `transferred+handoff_accepted`、同一Receipt Evidence与Inbox result全有或全无；Material Control完全不改写；相同Accepted message重放同一closure digest，已rejected/成员不完整/digest冲突稳定拒绝；Repository只允许声明的`open → accepted`和`candidate_delivery → transferred`CAS，terminal row不得再UPDATE |
| Handoff A Rejected | Candidate/Material/Control Verification形成前后、Reason/Evidence rows逐项写入中途、Decision/Receipt/Result/marker/Outbox各边界、相同Offer的Accepted竞态 | 只允许closed Handoff A reason；Continuity 0/N/overlap不得误判rejected；要么Decision/全部Reason Evidence/Receipt/Result/marker/Rejected Outbox全部不存在，要么全部成立且typed digest可由Owner rows重建；任何分支均不读写continuity head、不创建Subject/Binding、不转移Control；相同Offer只能有一个terminal Decision，相同marker重放原Receipt/Message |
| Procurement Handoff A rejection consume | Inbox写入前后、open Delivery CAS前后、1/256个Reservation逐项释放中途、Delivery terminal与Inbox result提交前后、迟到Accepted消息 | Delivery、全部Candidate member `released+handoff_rejected`、同一Receipt Evidence与Inbox result全有或全无；Procurement Material Control完全不变；相同Rejected message重放同一closure digest，已accepted/成员不完整/digest冲突稳定拒绝；Repository只允许声明的`open → rejected`和`candidate_delivery → released`CAS，terminal row不得再UPDATE，绝不部分释放或重开Run |
| Material Identity | mount remap/container restart、inode reuse、采样中stat变化、Fingerprint中断与Control acquire前 | 稳定Mount Scope才继承Identity；完成中段最多256 KiB Fingerprint前无Control；采样前后stat fence必须一致；同Fingerprint不同inode仍是不同Identity；任何路径均不得退化为全文件读取 |
| Material Field Profile Hint | create省略/显式mixed/五项enum、Field aggregate与revision 1插入中间、URL/body mismatch、expected revision CAS、same value no-op、same-key replay/conflict、current pointer切换前后崩溃、active Observation/Run存在时修订 | 缺省只规范化为显式mixed且digest唯一；Field/current pointer/Profile revision及Foundation receipt/marker/audit全有或全无；same value不加revision；旧revision immutable且可历史恢复；current修改不改写旧Observation/Run/Retry/Candidate，也不通过Session/caller/current cache注入Triage |
| Field Observation Page | Page DTO/Access/Profile Hint/Request digest验证前后、64 MiB physical-read边界、Field Observation/Profile heads CAS前后、immutable page header与`proc_field_observation_entries` rows写入前后、compact typed Result/marker前后、零Outbox协调、响应前崩溃 | 任一DTO、顺序、continuity、digest、Supporting Work或CAS验证失败整页rollback；entry rows、Page header、Field head、Hint Snapshot revision、current Material physical projection、typed Result和marker全有或全无；Page Result不含文件明细；同一Work每页Snapshot相同，Hint切换后旧Work后续页零写入；同marker重放返回原Result且不推进revision；历史Observation由entry rows重建 |
| Field Eligibility Reconcile | Change Set形成前后、Policy schema/path/precedence边界、Selection与Control snapshot读取前后、Batch提交前、逐项material-local basis/revision重验、事务提交前后、响应前崩溃 | 只按`ExtractionPolicy@1`和固定reason precedence计算；无隐藏duplicate suppression；只处理new/changed/missing/unknown或Policy/Access/Control受影响Material；unchanged basis为no-op且不执行SQL UPDATE；stale Material row不被覆盖并进入summary；一个Batch的applied rows全有或全无；无Event Result/marker/Outbox；重启由current facts与durable cursor重新收敛，不要求每行追平全局Observation revision |
| Procurement Run admission/seal/retry | 1/256/257 member边界、Profile Hint current/terminal Observation一致性与切换、Registry active Rule切换/缺失/digest冲突、Run/Basis/Selection写入前后、逐Control acquire/assert中途、Receipt/marker前后、Candidate publish reservation前后、Seal逐成员Evidence/aggregate digest前后、Retry Intent create/consume head/member snapshot与响应前崩溃 | 空或257项Selection稳定拒绝；新Admission冻结current Profile Hint及使用同一Snapshot的terminal Observation，current Hint变化但未重新Observation时拒绝；新Admission只冻结Registry current active Rule，调用者伪造tuple拒绝，existing Run按保留Profile/Rule entry恢复；1/256项时Run/完整relationized Basis/全部guard/全部Control/Receipt/marker全有或全无；同Field retained Control只assert不伪造revision；Seal只释放未成Package Selection并保留Control/Delivery Reservation，三个digest可在current row后续变化后由immutable relation重建；旧Run始终sealed；Retry Profile/head/set/member digest由rows重算一致，`field_profile_hint_changed` precedence与primary/aggregate映射唯一；一个Intent最多建立一个新Run，任一stale不建部分Run；失败不会自动连锁重试 |
| Procurement Triage pipeline | Probe Batch 1/100/101边界、Selection完整覆盖/缺页/重复member、Playability closed reason、Structure page/cursor、single/season/disc与N:M Episode、显式Western及mixed profile precedence、Profile revision切换、Unit 64 KiB边界、Identity/Manifest并行、两种canonical continuity kind、Candidate Publication逐字段、完整Related Physical Identity/reference digest、Acceptance Basis/Offer ID/Outbox及revision-head CAS前后崩溃 | 现实读取只来自Shared typed Evidence；Batch/Structure page重放保持同digest；Triage只消费Run冻结Snapshot，caller/current Field/hardcoded mixed均拒绝；显式western_adult形成western_adult，mixed永不形成western_adult；Unit与unassigned跨page精确分区Selection；同Member不得进入两个Unit；Handle/Probe/Layout/Rule/Run digest不匹配fail closed；ordinal从0；Claim不升级为Canonical Identity，别名claim kind稳定拒绝；Identity/Manifest任一缺失不发布Package；Publication把Run `candidate_package_revision_head`增量、Package/final Manifest/`proc_candidate_primary_material_episode_claims`及逐列完整Related relation/Reservation/同一`acceptanceBasisDigest+offerId`的Offer/typed Outbox/typed Result/marker全有或全无，同marker返回原package revision/digest且不建立第二Offer；缺任一Related identity/reference字段整项rollback；Candidate/Run/Offer关闭后仍由Owner rows恢复相同Package/Snapshot digest；机器Transaction的8张domain participant表和3张Foundation participant表必须同时回滚或同时可见 |
...
| Western service-local Analysis | Frame Output Target/首帧/末帧写入前后、Embedding与Cluster artifact physical bytes后/Result前、Analysis Artifact physical bytes后/journal后/Result前、Observe与Match Result后/Event success前、People Projection空/变化、restart跨每个点 | 每个Effect以Plan Node+完整input digest+Target digest唯一恢复；Frame/Embedding/Cluster/Analysis Artifact各自只产生一份active Handle与一份durable Result，已存在physical bytes不得导致第二次抽帧/ONNX/聚类/Artifact写；Observe/Match只复用同Run/Basis显式Result refs，旧Plan冻结Projection集合不被current People变化替换；空Projection仍执行Match并以全部cluster unmatched证明`matches=[]`；任一Handle kind/owner/run/workspace/model/config/spec/provenance/digest/fence不一致fail closed，零Product Fact/Package写入；全过程不得出现Worker/ExternalJob/legacy runtime row或调用 |
| Libra Workspace Admission/Reference | Root current config/state切换、旧resolved-path输出拒绝、Root Snapshot query found/stale/inactive、Run Manifest/member缺失或size变化伪造、Primary求和safe-integer边界、Space Evidence admitted/insufficient/expired/bytes溢出、Platform root fence前后、首次aggregate insert后/revision 1 insert前、revision后/commit前、Foundation Workspace Registry前后、Foundation Handle热列/JSON/fence任一不一致、working携带Verification、staging缺失/伪造/另一Handle Verification、media/artifact/structural role错配、Reference attach/promotion中间 | public Port从不返回resolved path；只有事务内从immutable Primary members重算且与request/required bytes逐项匹配的current active Root Snapshot和30秒内admitted Evidence可建立Workspace；Episode Claim不进入字节basis；deferred current-head复合FK允许同事务aggregate-first/revision-second并在commit验证，任一崩溃后两者与Registry/Result/marker全无；禁止关闭FK、sentinel或nullable后补；Reference必须从Foundation row重建完整Handle，promotion只接受同Run/Handle/fence、与Material role一致的完整passed `media|artifact|structural` Product Verification并冻结JSON；Artifact分支还必须命中immutable Artifact Registry Handle及相同bytes identity，NFO/Poster不得伪造Media Probe；文件后来reclaimed仍从Libra row恢复历史Decision；每次只追加一个closed state revision，同handle不同时出现在Working/Product Staging；成功重放不重复附着或移动bytes |
| Libra Deliverable Promotion | numeric package head 0/1及后续CAS、稳定Package ID预派生、Package relation写入中间、Product member Output Requirement重算、direct assert/new Product Control acquire及实际post-state projection前后、Offer/Outbox前 | 首个Package expected head 0并提交revision 1；Package ID只由Run/revision预派生，Control以该ID为scope后再形成真实committed projection与最终Manifest/Package digest，不存在hash固定点或placeholder digest；Package可见时完整Product/Fact/Artifact/Off-load/Requirement snapshot及historical Control可重建且所有Product Material已有Libra Control；任一缺失则Package head/Control/Package/Offer/Result/Outbox全部不可见，同marker返回原Package |
...
| Routing/Template publish | Preview后、revision insert后、current head切换前后、Shelf Standard及Shelf routing projection refresh前后 | Field只见一个current Routing Policy；Template current/binding和每座Shelf的routing projection revision/digest可恢复；Arca不写Routing Priority |
| Libra Routing/Decision Basis/Acceptance Spec | 首次无Decision head、Basis input relation逐项写入、head CAS、Basis后再经历Routing/Spec多次head CAS、进程重启、Assessment/Decision写入、Spec publish及Result/marker前后；Subject Intake/Identity/Episode变化、Field Routing head变化、Arca Projection在Query后变化、同semantic Spec但新provenance | 首次Basis保存`absent/revision 0` Head Snapshot并原子建立revision 1 head；后续Basis保存完整pre-CAS current Snapshot；任意多次Head推进后仍只能从Basis及Owner input rows重建原`DecisionInputSet/inputSetDigest`，不得读current Head或Foundation Result冒充历史；Routing/Spec提交验证当前Head恰为该Basis的确定post-state；Basis及全部typed input rows全有或全无；Routing/Spec只写Libra/Foundation固定表且不读`arca_*`；stale Subject/Policy/head稳定拒绝；Arca Projection变化使Run Creator不建Run并触发新Preparation，不改写历史Decision；同semantic `specDigest`不强制替代Run；相同marker返回原Decision/Spec；`contentProfile=series`与`structureKind=season`不互相改名 |
...
| Extraction Policy @1 | Beta唯一可执行Field开采规则：目录include/exclude、扩展名、最小size与精确materialKey排除；采用Field-root-relative exact matching，不支持glob/regex或历史抑制 | 5.3.1、8.5.11、8.6.18 |
...
| Triage Material Probe Batch | 把Run Selection的`1..256`成员按最多100项分页，并将每个Material、Binding、Read Handle与Shared Media Probe Evidence显式一一绑定的pure输入；不允许数组位置或Store旁读补映射 | 6.3.3、8.6.4、8.6.18 |
...
| Season Continuity Claim | Handoff A两侧共用的唯一exact continuity value；只允许`provider_season_identity|triage_grouping_lineage`，从Triage Unit到Candidate relation再到Libra Subject relation不改名、不映射 | 3.4.2、5.4.1、8.5.11、8.6.18–8.6.19 |
...
| Candidate Delivery Snapshot | Procurement通过CandidateDeliveryPort为Handoff A提供、把Offer/Basis/完整Package（含Related Physical Identity/reference Evidence）、完整Manifest与逐Primary immutable Location Evidence闭合为一个digest的versioned只读Deliverable；关闭后仍由Procurement immutable Owner rows历史重建，Libra不得旁读Procurement Store补值 | 4.4.2–4.4.3、8.2.1、8.4.2、8.5.11、8.6.18 |
...
| Intake Rejection Decision | Libra Intake在Handoff A合同、Material reality或Control前置条件失败时形成的独立typed终态；不借用Continuity Resolution、不创建Subject或推进continuity head | 4.4.3、6.4.2、8.5.4、8.6.18 |
| Procurement Candidate Rejection Closure | Procurement消费Libra rejected terminal message后原子关闭Delivery并释放Candidate Reservation、但保留Procurement Material Control的application transaction | 6.3.3、8.4.2、8.5.4、8.6.18 |
...
| Workspace Cleanup Scope | Libra按Off-load Completion、Run Superseded或Run Discard事实建立、逐项持有Workspace回收资格和受Control Product清理责任的durable范围 | 8.5.11、8.6.7、8.6.21 |
| Libra Run Admission Head | Libra按Subject串行证明当前具有最终提交资格的Run scope集合、分配Run admission revision并阻止single/Season overlap竞态的CAS head | 8.5.4、8.5.11、8.6.21 |
| Libra Run Execution Basis | Run创建时冻结的Subject、Decision Head、Acceptance Spec、Shelf Projection和Production Material Manifest完整初始事实；Run内不可改写；Run-local Priority是独立调度事实，不进入Basis | 6.4.4、8.6.21 |
| Production Material Manifest | Libra为一份Run冻结的完整外部Input/Binding/Control/Episode scope快照；与Procurement Candidate Manifest结构相似但由Libra拥有且不是跨域alias | 3.4.6、8.5.11、8.6.21 |
| Production Material Output Requirement | 从immutable Acceptance Spec完整六类Requirement、Manifest/Material role及该成员application scope确定性派生的member binding；不是Planner选择的Requirement子集或caller opaque digest | 8.5.11、8.6.21 |
| Workspace Material Reference | Libra对Foundation Workspace handle建立的append-only Working/Product Staging/Released业务关系；它不移动bytes或代替Material Control | 6.4.7、8.5.11、8.6.21 |
| Workspace Cleanup Scope Admission | Libra依据typed Off-load Completion、Run Superseded或Run Discard事实、对应grace与reference audit原子冻结可清理Workspace成员的Application transaction | 6.4.10、8.5.4、8.6.21 |
...
| Perception Resolution Input Set | 由typed Query、Owner内完整候选Record/Relation快照和可执行Rule Snapshot组成的三份digest-bound Resolver正式输入；pure Executor不得旁读Store补齐 | 5.9.2、6.8.2、8.6.13、8.6.18 |
| Perception Resolution Input Assembler | User Perception Application中唯一可以按Rule retrieval clauses读取本域Repository、形成完整候选超集但不得决定winner或duplicate的组件 | 6.8.2、8.2.4 |
...
| Metadata Fetch Intent | 将一次Product Metadata观察限定为一个Related NFO或一个精确Provider的bounded typed intent；不得携带fallback或允许Executor选择来源 | 6.4.7.1、8.6.5、8.6.18 |
| Product Fact Source Basis | Media-Cast/Product Metadata Fact提交时冻结的closed typed来源依据；只能是Metadata Observation或Western Analysis/Normalize/Match之一，并以Libra relation显式引用durable Foundation Result | 6.4.7.1、8.5.4、8.5.11、8.6.6、8.6.18–8.6.19 |
| Western Product Metadata Basis | 同Run/Basis的Western Analysis Result集合加唯一Normalize Result；不伪造Provider Observation，Normalize Result必须逐字节等于Commit Draft | 6.4.7.1、8.6.6、8.6.18–8.6.19 |
| Direct Person Registration Decision | 用户直接建立Person首revision的完整typed Decision；不含Candidate、Reference或Media-Cast事实 | 5.9.4、6.8.3、8.6.18 |
| People Reference Maintenance Decision | 以`add_image|release_image`区分、完整绑定Asset/唯一派生Face、Handle、model、revision与终态的People commit payload | 6.8.3、8.5.4、8.6.14、8.6.18 |
| On-deck Person Evidence Projection | Arca从当前Inventory/NFO relation发布给People的分页只读Evidence；People只据此注册Person，不取得Media-Cast写权 | 6.6.2.1、8.2.3、8.6.18 |
| Canonical Digest Rule | typed JSON按合同命名basis执行UTF-8 RFC 8785 JCS后计算SHA-256 lowercase hex；对象内领域digest字段不进入自身digest basis，但`fx_event_result_bindings.result_digest`始终覆盖包含该领域digest字段的完整typed Result；同一typed Result只有一个Foundation storage digest，两层digest不得混用 | 8.5.4、8.5.10、8.6.18–8.6.19 |
| CAS Lifecycle Row | 允许且仅允许表合同closed transition set中expected-state CAS的row；未列列write-once，terminal后只能相同Evidence幂等重放，不等于整表可变 | 8.5.3–8.5.4、8.5.11 |
| Handoff A Control Revision Set | Handoff A Accepted中由完整Control scope、expected/committed historical projection及两端scope形成的稳定集合；Receipt只保存其唯一digest但必须可由Control revision rows重建 | 8.5.4、8.5.11、8.6.19 |
| Shelf Routing Target Projection | Arca发布给Libra的versioned只读Shelf active/current Standard快照；它是Decision Input，不转移Shelf Policy Owner | 8.2.3、8.6.18 |
| Subject Decision Head | Libra按Subject串行切换current Routing Decision、Decision Basis与Acceptance Spec的CAS head；首次Basis才建立，不是Run状态 | 8.5.4、8.5.11 |
| Subject Decision Head Snapshot | Libra在每份Decision Basis中保存的pre-CAS Head immutable typed input；即使current Head后来多次推进，仍可重建历史Input Set与验证Basis来源 | 8.5.4、8.5.11、8.6.18 |
| Routing Assessment Evidence | Routing Resolver按用户Shelf优先级形成的有界连续评估前缀及首个命中/unknown/no-match证据 | 5.4、8.6.18 |
| Acceptance Spec Semantic Digest | 只覆盖目标Shelf、content profile、Product Scope和六类确定产品要求的digest；Policy/query provenance变化但产品要求相同时保持不变 | 5.6.5、8.6.18 |

当前确认状态：

- `8.0`–`8.10`：`ACCEPTED / JOURNEY-AMENDED / POST-BASELINE-DOC-CORRECTED`（2026-07-28；用户确认的基线保持，Level 9反向审计与`PBF-01`–`PBF-23`各项bounded修正已回写）；
...
- 178张关系表的PK、revision、关键列、unique/partial unique、热路径索引和JSON上限已经固化；PBF-10新增
  ...
  PBF-22新增一张Procurement-owned immutable Field Profile Hint revision历史表；
- 当前62项Capability registration、named helper和直接依赖已经完成function-level conservation；
- Level 8 post-amendment closure audit及`PBF-02`–`PBF-23`纵向传播结果为`PASS / NO BLOCKING GAP / NO OPEN BUSINESS DECISION`；各项输入、事务、持久化、Provider port与机器合同的bounded细节以本文对应合同为准；
...
用户可以记录评分、已看状态及Person Preference，也可以处理Person注册、别名、Reference Image或身份合并。
...
- 上传/删除Reference Image；Reference Face是系统内部派生事实，不出现在页面、表单或API中；
- 审阅、接受或忽略Registration/Merge Candidate，并在Merge时处理Preference冲突；
- 查看由Arca公开Projection提供的相关收藏及数量。

用户直接创建Person与上传Reference Image采用连续但独立的两步体验：第一步保存姓名/别名/Provider Identity后
Person立即成立；第二步提示上传一张只含一张人脸的Reference Image。用户可以跳过第二步稍后再补，上传失败
不会回滚Person。图片包含零张或多张人脸时显示明确字段错误，不让用户进入内部Face选择界面。Beta不提供
“搜索网图”按钮；该便利能力的缺失不阻止Person注册、Candidate接受或本地Reference Image上传。
...
表达“允许开采/排除”的结果条件：包含/排除目录边界、允许扩展名、最小文件大小和具体Material排除；不提供
glob、wildcard、regex或按文件名模糊匹配。系统内部
...
Material Field `POST`的closed registration body允许可选`contentProfileHint`；缺失时Host必须在Command schema
验证及`requestDigest`计算前规范化为显式`mixed`，并原子建立Profile Hint revision 1。Material Field `PATCH`
是closed discriminated command：既有`revise_access`与新增`revise_profile_hint`不得混合字段；
public body提供`operation=revise_profile_hint,fieldId,expectedProfileHintRevision,newContentProfileHint,idempotencyKey`，Host据此组装并重算完整
`MaterialFieldProfileHintRevisionCommand@1`后执行expected revision CAS。list/exact
GET及所有成功Command Result都返回`MaterialFieldAdminProjection@1`中的完整current Profile Snapshot。
该修订不增加method+path route，不允许从query string、Session、CapabilityExecutionContext或旧Runtime默认值注入
Hint；旧Observation/Run/Candidate历史不因PATCH改变。
...
随后执行`8.5.4`定义的People Candidate Acceptance事务。`actions/merge`仍以discriminator区分direct与
accept_candidate，不影响Person Registration入口唯一性。
...
`POST .../reference-assets`只接受一张用户已选定的本地图片和idempotency key，返回一个durable Reference
Maintenance operation；内部依次完成import、single-face embedding与原子Reference commit。`DELETE`只按
Reference Asset ID释放图片及其唯一派生Face，不存在Reference Face公开route。联网搜索图片不复用上述上传
endpoint，也不属于Beta API。
...
5. 装载Platform current revision、Capability Registry、Schema Registry和Procurement Triage Rule Registry，验证
   active entry、append-only历史entry、schema与digest兼容；
...
| HELIX_TRIAGE_RULE_REGISTRY_INVALID | Triage Registry active entry缺失、重复/digest不一致，或任一retained Run/Retry引用的已发布entry缺失 | faulted，只读诊断；禁止信任Run/Command携带的Rule tuple |
...
Work terminal signal必须携带精确`ownerDomain,processType,processId,workKind,workId`并只触发该Process Scope
的Owner Reconcile；signal可以丢失且不得成为正确性事实。Active Domain Process fallback在启动时立即执行
一轮，随后固定每30秒运行；每页最多100个Process或5秒，使用`fx_reconcile_cursors`保存Owner不可解释的opaque
cursor。Runtime不得在每个Event结束后调用全体`reconcilePending`，Owner Reconciler也不得为一个Process读取
整个Field、Shelf或全库；必须只读取该Process冻结的relationized scope。cursor只在整页完成后推进，崩溃重放
依靠Owner命令幂等性收敛。
...
People每日补偿扫描使用`OnDeckPersonEvidenceProjection`的持久cursor，每页最多100项；只有新的
projection revision/evidence digest进入Registration Evidence/Candidate准备。Arca Presentation Assessment与
People Candidate扫描可以由新Inventory/Person Evidence Signal提前唤醒，但Signal丢失不得造成漏注册。扫描
不得重新打开物理NFO、跨域读取Arca Store或因同一弱Identity每日重复创建Candidate。

Material Field每轮Observation使用同一目录对账算法，并为每个Regular File重新计算正中间最多256 KiB的bounded
fingerprint；首次与后续轮次不形成两种执行模式。Fingerprint完成且Identity确定前不得取得Procurement Control；
任何受保护commit仍按Level 7重验stat fence及同一有界fingerprint，禁止全文件读取。
...
| Contract and schema | 112 Capability、97 Catalog Result family、178 table、43 Canonical Transaction、113 Admin route、1 public health route及nominal handle/transaction DTO验证 |
...
- Direct Person Registration提交前后；Reference Image已导入但Face检测/Embedding未完成、Embedding完成但Reference commit未成立、Asset/Face/Reference head提交中以及commit成功但响应前；零Face、多Face和release stale revision必须保持无部分Reference Fact；
...
| 10.10 | 九条旅程、112 Capability、178 tables、43 Canonical Transactions、113 Admin routes、1 public health route及clean-cut门禁 | preserved after bounded final-audit closure and `PBF-02`–`PBF-23`（含各节记录的bounded revisions） |
...
RELATIONAL TABLES            post-baseline corrected: 178
...
post-baseline `PBF-01`–`PBF-23`（含各节记录的bounded revisions）已经按同一纪律完成bounded合同闭合；实现、测试或
部署仍未由本文件授权。
...
- Level 8（`8.0`–`8.10`）：`ACCEPTED / JOURNEY-AMENDED / POST-BASELINE-DOC-CORRECTED`（2026-07-28；用户确认基线保持，`PBF-01`–`PBF-23`已闭合）
...
- Post-baseline realizability audit：`PBF-01`–`PBF-23 CLOSED / APPLIED_AND_AUDITED`；PBF-09–PBF-14的持久化与事务细节继续按既有条目生效，PBF-15闭合Media Production/Conformance，PBF-16闭合External Material Acquisition/P5 observe/import连续性，PBF-17闭合role-aware Workspace Product Staging verification，PBF-18（含`PBF-18-R1`）解除On-deck Package ID与post-CAS Control projection之间的hash依赖环、固定非循环原子构造顺序并排除Package commit time对预提交content digest的反向依赖，PBF-19闭合Handoff B Accepted responsibility原子性，PBF-20在既有Arca Binding/Inventory Material row中以bounded typed JSON保存Series N:M Episode Claim Set，PBF-21使Provider Metadata Fetch携带可直接执行的Resolved Provider Identity并固定Artifact生产必须走既有正式Capability，PBF-22以Procurement immutable Profile Hint revision闭合Field→Observation→Run/Retry→Triage连续性，PBF-23把Western Worker/External Job入口改签为clean Service内Artifact-backed Frames/Face/Analysis chain并使空Person Projection集合仍需真实Match Evidence；关系表总数为178，Catalog Result family为97，Canonical Transaction为43。
...
- `PBF-14-R2` bounded machine extraction：两个Product Fact closed variants只在既有Domain Fact Commit内展开；
  当前SSOT extractor结果固定为`112 Capability / 97 Result family / 178 tables / 43 unique Canonical Transactions`。
...
- `PBF-14-R4` bounded Artifact verification continuity：Product Metadata Fact Owner row冻结完整Verified Artifact
  Manifest，每项显式引用typed Verification Result并绑定Draft Requirement与Registry Handle；全部inventory计数不变。
- `PBF-14-R5` bounded Result storage continuity：Artifact/Western领域内digest与Foundation完整Result storage digest
  分离，Artifact Verification固定`1..64`项且≤`65,536` bytes双界；Product Fact marker列的FK/UNIQUE机器约束闭合；
  全部inventory计数不变。
- `PBF-14-R6` bounded Western digest propagation：Western Analysis Variant、Basis、Normalize ref与Product Fact
...
- `PBF-22` bounded Material Field Profile Hint continuity：Procurement新增一张append-only
  `proc_field_profile_hint_revisions` Owner表，注册缺省明确规范化为`mixed` revision 1，current pointer与Profile
  PATCH使用expected revision CAS；完整Snapshot逐字节冻结进Observation Page/Result、Run Basis、Retry head与
  Triage Context。current Hint修订不改写历史，旧Observation Work不得在新current revision下继续，新Run/Retry
  必须使用与current Hint相同的terminal Observation；没有caller cache、current Field旁读或hardcoded `mixed`
  fallback。该修正不新增Domain、Owner、Store、Handoff、Capability、Result family、Canonical Transaction或
  Admin route，只把Procurement表数从15增至16、全局关系表总数从177增至178。
- `PBF-23` bounded Western service-local Analysis continuity：Western唯一正式链改为
  `frames.extract → face.embedding.compute → face.cluster.compute → western.analysis.request/observe`，并从同一
  Cluster分支执行`face.reference.match`；request只生成同Run/Workspace的immutable Western Analysis Artifact，
  observe只从完整typed Handle/Spec形成durable Result。删除Western的Worker Upload/External Job语义且不保留
  legacy/caller-fixture双路径；People只提供`0..256`项有界Projection集合，空集合也必须完成真实Match并以全部
  Cluster unmatched证明`matches=[]`。Product Fact Source ref改为Analysis Artifact Handle，全部Effect受既有
  Journal/Artifact Registry恢复；不新增Domain、Owner、Store、Handoff、Capability、Result family、table或
  Canonical Transaction，核心计数保持`112/97/178/43`。
