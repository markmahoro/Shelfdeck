...
- **Workspace Admission**固定写`libra_workspaces`、`libra_workspace_revisions`、`fx_workspace_registry`及
  ...
  `fx_workspace_materials`、Run current fence和Decision携带的完整typed Verification；Foundation row必须逐字段重建
  `WorkspaceMaterialHandle@1`并重算handle/fence digest，不能信任caller或读取current Registry重造Handle。
  promote的完整`WorkspaceProductVerificationSnapshot@1`必须匹配该Handle/Run且result=passed，并原样冻结进Reference row；
...
`ProductConformanceInputSnapshot@1`的nested schema必须以exact `$ref/oneOf`引用
...
`ProductMediaVerification@1`；Schema materializer遇到同名旧bounded-contract不得合并、降级或生成第二条路径。
...
- `WorkspaceProductVerificationSnapshot@1`固定为
  `{schemaRef:"ProductMediaVerification@1",verificationId,verificationValue(ProductMediaVerification@1),
  verificationDigest}`；`verificationId=verificationValue.verificationId`，`verificationDigest=SHA-256(JCS(
  verificationValue))`，完整Snapshot canonical JCS≤`16 KiB`。它只能来自
  `libra.product_media.verify@1`的typed `candidateKind=workspace_output,passed` Result，并必须逐项匹配同一Run、
  `productMaterialHandleId=WorkspaceMaterialHandle.handleId`、完整Handle JCS digest及`fenceDigest`；direct_input、
  failed/not_applicable、另一Handle、另一Run或
  digest不匹配稳定拒绝。Snapshot是Reference Commit的完整正式输入，Commit不旁读Foundation Event Result、caller
  cache或当前文件Reality；提交成功后由Libra reference row永久重建。
...
- `workspaceMaterialReferenceSetDigest=SHA-256(JCS({schema:"libra.workspace-reference-set@1",workspaceId,
  ...
  `promote_to_product_staging`要求expected Reference current state=working并携带验证同一Handle的
  完整`WorkspaceProductVerificationSnapshot@1`，同时完整Episode claims/scope digest必须逐字节沿用expected Reference。
...
- `LibraDeliverablePromotionDecision@1`固定为
  ...
  decisionDigest}`。Product Staging refs按referenceId排序并与Workspace Product members恰好一一对应；direct-original
...
| Workspace Material Reference Commit | `libra_workspaces,libra_workspace_revisions,libra_workspace_material_refs` + Result/marker | `libra_runs,libra_run_revisions,fx_workspace_materials`及全部write tables；Foundation Material row重建完整Handle，Decision携带完整typed Product Verification并冻结到Reference row；不读Workspace Registry、Foundation Event Result或current Reality | no |
...
RELATIONAL TABLES            post-baseline corrected: 176
