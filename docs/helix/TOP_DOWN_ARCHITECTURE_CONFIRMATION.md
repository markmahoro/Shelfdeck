...
- 本次Selected Field Material Set包含`1..1024`项Primary Material；这是单个Run的原子准入上限，不限制一片
  Material Field的总规模，多个独立生产单位或Season Episode范围必须由多个互不重叠的Run处理；空Selection
  不得建立Run；一个不可拆分的语义生产单位若确实需要超过1024项Primary Material，则以稳定
  `procurement_selection_scope_too_large`封口，不能破坏Control原子性强行拆包；
- Run admission能够针对全部选择成员在同一事务中取得或确认Procurement Control，禁止把同一Run拆成多次
  Control acquire后再声称原子成立；
- 计划选择的Primary Material没有被任何`active|waiting` Run Selection或尚未完成Handoff A Decision的
...
- `helix.transaction.procurement-run-admission@1`
  ...
  fx_material_control_revisions,fx_event_result_bindings,fx_commit_markers]`，`outboxRequired=false`；
...
| `proc_candidate_related_references` | `candidate_package_id FK, reference_id, primary_ordinal, role, material_key, mount_scope_id, inode, content_hash_algorithm, content_hash, endpoint_id, location, checksum_algorithm, checksum_hex, association_evidence_digest, reference_digest` | `PK(candidate_package_id,reference_id)`及复合FK`(candidate_package_id,primary_ordinal) → proc_candidate_primary_materials(candidate_package_id,ordinal)`；逐列无损保存一个`RelatedMaterialReference@1`，`primaryMaterialKey`只由该immutable FK目标重建；Physical Identity/materialKey、SHA-256 checksum和reference digest均按8.6.18重验，`content_hash_algorithm=checksum_algorithm=sha256`且`content_hash=checksum_hex`；published row immutable，Related不进入Material Control；Candidate关闭、Run封口或Offer关闭不得删除，历史Delivery Snapshot仍从本Owner relation恢复 |
...
同为`1..1024`项。Control Authority的目标Owner固定为`material_field/fieldId`：action为`acquire`才新增Control
...
| `WorkspaceMaterialHandle` | `handleId, workspaceId, ownerDomain, processId, endpointId, materialKey,physicalIdentity{mountScopeId,inode,contentHashAlgorithm(sha256),contentHash},rootHandleRef,relativePath,digestAlgorithm(sha256),digestHex,sizeBytes,referenceRevision,accessScope(workspace_material_read),fenceDigest`；`materialKey/physicalIdentity`必须满足`PhysicalMaterialIdentity`公式且`digestHex=contentHash`，endpoint/mount/root handle逐字节等于Workspace Registry/root snapshot；`accessScope`固定只授权后续Capability读取，写新产物使用新的event-scoped output，删除必须另有Cleanup Effect Intent。`handleId=SHA-256(JCS({schema:"foundation.workspace-material-handle-id@1",workspaceId,materialKey,relativePath,referenceRevision}))`；`fenceDigest=SHA-256(JCS({schema:"foundation.workspace-material-handle-fence@1",handleId,workspaceId,ownerDomain,processId,endpointId,materialKey,physicalIdentity,rootHandleRef,relativePath,digestAlgorithm,digestHex,sizeBytes,referenceRevision,accessScope}))`。这里的`referenceRevision`是Foundation Workspace Handle registry revision，不是Libra Workspace Material Reference的业务revision，后者只能由`libra_workspace_material_refs.reference_revision`推进；路径必须在Workspace root内。Workspace write effect只有在bytes关闭、hash/stat与containment验证完成并把完整`WorkspaceMaterialHandle@1`及其JCS digest与Effect receipt原子登记到`fx_workspace_materials`后才能返回Handle；任何consumer均从该Owner row重建并验证完整Handle，不能自行重造fence或信任caller缺失字段 |
...
Field Material冻结完整`PhysicalMaterialIdentity@1 + sizeBytes`到Selected Set与immutable Run member；Candidate
...
| `PrimaryInputManifest` | `ManifestEnvelope + structureKind(single|season) + members[{ordinal,materialKey,role(primary_payload|structural_dependency),physicalIdentity{mountScopeId,inode,contentHashAlgorithm(sha256),contentHash},sizeBytes,bindingRevision,admittedControlRevision,admittedControlProjectionDigest,episodeClaims[{episodeKey,seasonClaimDigest,claimDigest}],memberDigest}]`；最终Manifest只由Candidate Publication commit从同一`CandidateDraft.structureEvidence.unit`、`PrimaryInputManifestDraft`和immutable Run Basis原子建立；members为Run Selection非空子集、最多1024项，按materialKey UTF-8 bytes排序后从0赋ordinal，materialKey唯一；Identity/size逐项等于Run member并满足`PhysicalMaterialIdentity@1`；每个`memberDigest=SHA-256(JCS(单项excluding memberDigest))`，memberCount/membersDigest/manifestDigest必须与完整稳定值匹配 |
...
- `ProductionMaterialManifest@1`固定为
  ...
  external_audio|chapter),physicalIdentity{mountScopeId,inode,contentHashAlgorithm,contentHash},sizeBytes,
  ...
  `libra_material_bindings` revision并满足`PhysicalMaterialIdentity@1`的materialKey公式；该Binding的Handoff A origin
...
| Procurement Handoff A acceptance consume | Inbox写入前后、open Delivery CAS前后、1/1024个Reservation逐项转移中途、Delivery terminal与Inbox result提交前后、迟到Rejected消息 | Delivery、全部Candidate member `transferred+handoff_accepted`、同一Receipt Evidence与Inbox result全有或全无；Material Control完全不改写；相同Accepted message重放同一closure digest，已rejected/成员不完整/digest冲突稳定拒绝；Repository只允许声明的`open → accepted`和`candidate_delivery → transferred`CAS，terminal row不得再UPDATE |
