...
Status: ShelfDeck / Helix architecture SSOT; Levels 0–10 accepted; final full-document audit and post-baseline `PBF-01`–`PBF-22`（含各自已记录的bounded revisions）closed; implementation not authorized by this document.
...
| `libra.media_cast.resolve@1` | `LibraMediaCastSourceBasis(metadata_observation|western_match) + PersonReferenceProjection → MediaCastDraft` | `pure_observation` |
...
| `WesternProductMetadataBasisSnapshot` | `basisId,basisKind(western_analysis),libraRunId,runExecutionBasisDigest,resolvedIdentityDigest,analysisVariantDigest,analysisRefs[1..16]{workId,attemptId,planId,eventId,resultId,resultDigest,inputBindingDigest,externalJobReceiptId,evidenceId,evidenceDigest},normalizeRef{workId,attemptId,planId,eventId,resultId,resultDigest,inputBindingDigest,analysisVariantId,productMetadataDraftDigest},sourceRefsDigest,basisDigest`；analysisRefs逐项对应上述Variant，所有analysisRefs/normalizeRef的`resultDigest`均为对应Foundation完整typed Result storage digest并逐字节命中`fx_event_result_bindings.result_digest`；Analysis Result内部领域`resultDigest`只在展开typed Result时独立验证，不进入外层ref。`evidenceId/evidenceDigest`继续逐字节命中Analysis Result的Evidence Envelope，不受两层Result digest分工影响。normalize固定`libra.western.metadata.normalize@1 → ProductMetadataDraft@1`且Plan input binding逐字节引用同一Variant；Commit Draft必须等于normalize Result。先构造`canonicalSourceRefs[2..17]`：每项固定为`{ordinal,capabilityRef,resultSchemaRef,workId,attemptId,planId,eventId,resultId,resultDigest,sourceRef,sourceOrder,evidenceId,evidenceDigest,inputBindingDigest}`；按Variant既定顺序放入Analysis refs（capability/schema固定Western observe/Result，`sourceRef=externalJobReceiptId`，Evidence非NULL，连续ordinal/sourceOrder=`0..N-1`），最后放入Normalize ref（capability/schema固定Western normalize/Draft，`sourceRef=analysisVariantId`，Evidence为NULL，ordinal/sourceOrder=`N`）。`sourceRefsDigest=SHA-256(JCS({schema:"libra.western-product-metadata-source-refs@1",items:canonicalSourceRefs}))`，`basisId=SHA-256(JCS({schema:"libra.western-product-metadata-basis-id@1",libraRunId,runExecutionBasisDigest,sourceRefsDigest}))`，`basisDigest=SHA-256(JCS(完整value excluding basisDigest))`；完整value≤`64 KiB` |
| `WesternMediaCastBasisSnapshot` | `basisId,basisKind(western_match),libraRunId,runExecutionBasisDigest,resolvedIdentityDigest,matchRef{workId,attemptId,planId,eventId,resultId,resultDigest,inputBindingDigest,evidenceId,evidenceDigest,personMatchEvidenceDigest},matchState(matches_found|no_matches),basisDigest`；Ref固定同Run/Basis的`shared.face.reference.match@1 → PersonMatchEvidence@1`，Evidence字段逐字节等于Result的Evidence Envelope；`matches_found`要求matches非空且Draft逐项解释全部match，`no_matches`要求`matches=[]`并只允许Draft `relations=[]`；`basisId=SHA-256(JCS({schema:"libra.western-media-cast-basis-id@1",libraRunId,runExecutionBasisDigest,matchResultId:matchRef.resultId,matchResultDigest:matchRef.resultDigest}))`，canonical object恰含这五个顶层property；`basisDigest=SHA-256(JCS(完整value excluding basisDigest))`；完整value≤`16 KiB` |
...
- `8.0`–`8.10`：`ACCEPTED / JOURNEY-AMENDED / POST-BASELINE-DOC-CORRECTED`（2026-07-28；用户确认的基线保持，Level 9反向审计与`PBF-01`–`PBF-22`各项bounded修正已回写）；
...
- Level 8 post-amendment closure audit及`PBF-02`–`PBF-22`纵向传播结果为`PASS / NO BLOCKING GAP / NO OPEN BUSINESS DECISION`；各项输入、事务、持久化、Provider port与机器合同的bounded细节以本文对应合同为准；
...
| 10.10 | 九条旅程、112 Capability、178 tables、43 Canonical Transactions、113 Admin routes、1 public health route及clean-cut门禁 | preserved after bounded final-audit closure and `PBF-02`–`PBF-22`（含各节记录的bounded revisions） |
...
post-baseline `PBF-01`–`PBF-22`（含各节记录的bounded revisions）已经按同一纪律完成bounded合同闭合；实现、测试或
部署仍未由本文件授权。
...
- Level 8（`8.0`–`8.10`）：`ACCEPTED / JOURNEY-AMENDED / POST-BASELINE-DOC-CORRECTED`（2026-07-28；用户确认基线保持，`PBF-01`–`PBF-22`已闭合）
