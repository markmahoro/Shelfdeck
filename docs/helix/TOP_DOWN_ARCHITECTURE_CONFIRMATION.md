...
| `fx_workflow_plans` | `plan_id PK, attempt_id FK, planner_ref, planner_version, work_objective_type_ref, work_objective_version, catalog_digest, basis_digest, graph_digest, state, diagnostic_classification, created_at_ms` | `UNIQUE(attempt_id)`；`state`持久化四种Plan Resolution；Plan发布后禁止更新Graph、Objective、Resolution或diagnostic字段 |
| `fx_plan_nodes` | `plan_id FK, node_id, capability_ref, contract_version, input_binding_schema_ref, input_bindings_json, parameter_schema_ref, parameters_json, when_schema_ref, when_json, effect_class, fence_schema_ref, fence_basis_json, resource_demand_schema_ref, resource_demand_json, approval_requirement_ref, authorization_requirement_ref, retry_policy_ref, timeout_policy_ref, output_contract_ref, compensation_for_event_id FK, compensation_contract_ref` | `PK(plan_id,node_id)`；所有JSON分别受`16 KiB`上限；`INDEX(capability_ref,contract_version)`；compensation两列必须同时为空或同时存在，target必须是同一Plan内的预声明Event；Plan发布后这些执行合同字段不可更新 |
...
Plan node input/parameter/Fence/Resource Demand的Schema内容不重复存入`fx_plan_nodes`：不可变Schema由
`capability_ref@contract_version`解析；Plan必须保存已验证的有界值和全部执行合同引用，不能只保存无法反解的Graph digest。
