...
Status: ShelfDeck / Helix architecture SSOT; Levels 0–10 accepted; final full-document audit and post-baseline `PBF-01`–`PBF-11`（含`PBF-09-R1`、`PBF-10-R1`、`PBF-10-R2`、`PBF-10-R3`、`PBF-11-R1`、`PBF-11-R2`、`PBF-11-R2-R1`）bounded corrections closed; implementation not authorized by this document.
...
| `proc_candidate_deliveries` | `offer_id PK, candidate_package_id FK, package_revision, package_digest, acceptance_basis_digest, state(open|accepted|rejected), handoff_decision_id NULL, handoff_decision_digest NULL, handoff_receipt_id NULL, handoff_receipt_digest NULL, terminal_evidence_digest NULL, offered_at_ms, closed_at_ms NULL` | `offer_id`与`acceptance_basis_digest`必须分别等于8.6.18从final `CandidatePackage@1`计算的`CandidateIntakeAcceptanceBasis@1`公式结果，且同事务存在唯一`ProcurementCandidateOfferAvailableMessage@1` Outbox；`UNIQUE(candidate_package_id,package_digest,acceptance_basis_digest)`；同一Candidate至多一个open Offer。open时全部handoff/terminal/closed列为NULL；accepted要求Decision ID、Receipt ID/digest、terminal Evidence及closed time非NULL，Decision digest沿用既有nullable合同；rejected要求全部handoff/terminal/closed列非NULL且逐项等于`LibraCandidateRejectedMessage@1`，`terminal_evidence_digest=receiptDigest`；terminal列写后immutable，相同Offer重放不得新增row、Outbox或改写不同终态；`INDEX(state,offered_at_ms)` |
...
- `8.0`–`8.10`：`ACCEPTED / JOURNEY-AMENDED / POST-BASELINE-DOC-CORRECTED`（2026-07-19；用户确认的基线保持，Level 9反向审计与`PBF-01`–`PBF-11`（含`PBF-09-R1`、`PBF-10-R1`、`PBF-10-R2`、`PBF-10-R3`、`PBF-11-R1`、`PBF-11-R2`、`PBF-11-R2-R1`）bounded修正已回写）；
...
- 不改变Level 8的Capability数量、Owner/Repository和一个SQLite物理库边界；最终全文审计只以已确认语义
  ...
  与`PBF-10`（含`PBF-10-R1`、`PBF-10-R2`、`PBF-10-R3`）Triage typed pipeline、Candidate Publication机器事务表集及Offer/continuity闭合，以及`PBF-11`（含`PBF-11-R1`、`PBF-11-R2`、`PBF-11-R2-R1`）Libra Intake、Related Reference历史重建与两次Handoff Rejected终态闭合，把关系表合同修正为169张并保持112项Capability；
...
| 10.10 | 九条旅程、112 Capability、169 tables、113 Admin routes、1 public health route及clean-cut门禁 | preserved after bounded final-audit closure and `PBF-02`–`PBF-11`（含`PBF-09-R1`、`PBF-10-R1`、`PBF-10-R2`、`PBF-10-R3`、`PBF-11-R1`、`PBF-11-R2`、`PBF-11-R2-R1`） |
...
post-baseline `PBF-01`–`PBF-11`（含`PBF-09-R1`、`PBF-10-R1`、`PBF-10-R2`、`PBF-10-R3`、`PBF-11-R1`、`PBF-11-R2`、`PBF-11-R2-R1`）已经按同一纪律完成bounded合同闭合并记录在Review Section 15；实现、测试或
部署仍未由本文件授权。
...
- Level 8（`8.0`–`8.10`）：`ACCEPTED / JOURNEY-AMENDED / POST-BASELINE-DOC-CORRECTED`（2026-07-19；用户确认基线保持，`PBF-01`–`PBF-11`（含`PBF-09-R1`、`PBF-10-R1`、`PBF-10-R2`、`PBF-10-R3`、`PBF-11-R1`、`PBF-11-R2`、`PBF-11-R2-R1`）已闭合）
