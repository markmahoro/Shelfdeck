...
- `variant=libra_product_metadata_fact@1`使用相同write set并固定`outboxRequired=false`；它额外验证
  `VerifiedArtifactManifest@1`与可选同Run Media Cast Fact，但不得产生Outbox；
...
| `libra_product_fact_revisions` | `product_fact_id PK, libra_run_id FK, fact_kind, fact_revision, aggregate_id NULL, schema_ref, fact_json, fact_digest, evidence_digest, source_basis_kind(metadata_observation|western_analysis|western_match) NULL, source_basis_id NULL, source_basis_digest NULL, source_ref_count, commit_payload_schema_ref NULL, commit_payload_digest NULL, event_fence_digest NULL, commit_marker NULL, result_digest NULL, committed_at_ms` | `fact_kind`必须是Catalog已注册的Product Fact kind，不由Store发明open value；Fact JSON上限`64 KiB`且大Artifact只含handle；`media_cast|product_metadata`按8.6.19固定`product_fact_id/aggregate_id/revision/commit/evidence/result`，要求全部新增commit/Source Basis列非NULL。Product Metadata只允许`metadata_observation|western_analysis`，Media Cast只允许`metadata_observation|western_match`；Observation basis的`source_ref_count=1..16`；Western Product Metadata basis为`2..17`（至少一项Analysis Result加恰一项Normalize Result）；Western Media-Cast basis恰为1项Match Result。其他已注册kind继续服从各自既有精确合同，允许这些PBF-14专用列为NULL且count=0，不能被本修正反向套用未定义公式；非NULL `commit_marker`显式FK到`fx_commit_markers`且唯一，`result_digest`逐字节等于marker所指`fx_event_result_bindings`；`UNIQUE(libra_run_id,fact_kind,fact_revision)`；`INDEX(libra_run_id,fact_kind,fact_revision)` |
...
`product_metadata` variant还要求Manifest内每项Artifact Handle可由`fx_artifact_registry`验证，且可选
`mediaCastFactRef`命中同一Run immutable `MediaCastFact`。任何不一致都在提交前拒绝，不能由Executor修补Draft。
...
| `VerifiedArtifactManifest` | `manifestId,libraRunId,items[0..256]{ordinal,artifactHandleId,artifactKind,artifactRevision,artifactDigest,requirementDigest,verificationEvidenceId,verificationEvidenceDigest,referenceDigest},artifactSetDigest,manifestDigest`；items按`artifactKind,artifactHandleId,artifactRevision`的UTF-8 bytes排序且ordinal从0连续；每项必须命中`fx_artifact_registry` immutable Handle，`referenceDigest=SHA-256(JCS(完整item excluding referenceDigest))`，`artifactSetDigest=SHA-256(JCS({schema:"libra.verified-artifact-set@1",items}))`，`manifestId=SHA-256(JCS({schema:"libra.verified-artifact-manifest-id@1",libraRunId,artifactSetDigest}))`，`manifestDigest=SHA-256(JCS(完整value excluding manifestDigest))`；只携带handle/evidence，不内联二进制，完整value≤`256 KiB` |
...
`fx_event_result_bindings`同时是选中Observation的read table与当前Commit Result的write table，Coordinator必须按
不同`result_id/event_id`区分，不允许覆盖Observation Result。任一引用缺失、schema/digest不匹配、revision fence
变化或typed Result无法由Owner row重建，都使整个transaction rollback。
