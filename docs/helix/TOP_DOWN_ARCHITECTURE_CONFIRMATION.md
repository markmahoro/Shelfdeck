...
| `EncodeIntent` / `RemuxIntent` | 两者公共字段为`intentId,revision,schemaRef,libraRunId,sourceHandleDigest,mediaRequirementDigest,outputContainer(matroska),outputExtension(mkv),intentDigest`。Encode追加`video{codec(hevc),rateControlMode(target_size|quality_bound),targetVideoBitrateBps?,qualityBound?,preserveRaster,forbidUpscale},audio{mode(copy)},subtitle{mode(copy)},deviceClass`；target_size恰含positive targetVideoBitrate，quality_bound恰含bounded qualityBound，二者互斥；`preserveRaster=true,forbidUpscale=true`，audio永远copy。Remux追加`streamPolicy(copy_all_supported)`且不得含Device/rate-control字段。`intentId=SHA-256(JCS({schema:"libra.media-production-intent-id@1",semanticIntent:完整value excluding intentId and intentDigest}))`，`intentDigest=SHA-256(JCS(完整value excluding intentDigest))`；完整值≤`16 KiB`。Planner只能从Acceptance Spec与probe Evidence编译Intent，Executor不得改写Intent或补第二种生产动作 |
...
| `MediaRequirement` | `requirementId,revision,schemaRef(libra.media-requirement@1),acceptanceSpecId,acceptanceSpecRecordDigest,contentProfile,structureKind,mandatoryMedia,space,requirementDigest`；`revision=AcceptanceSpec.specRevision`，其余Spec identity/content/structure字段逐字节复制同一immutable Spec；`mandatoryMedia`与`space`逐字节复制`AcceptanceSpec@1.requirements`的两个完整对象，字段、enum、排序与8.6.18的`AcceptanceRequirementSet@1`一致，不接受三个opaque sub-digest或任意parameters；`requirementId=SHA-256(JCS({schema:"libra.media-requirement-id@1",acceptanceSpecId,revision,mandatoryMedia,space}))`，`requirementDigest=SHA-256(JCS(完整value excluding requirementDigest))`，完整值≤`16 KiB` |
...
| `ProductConformanceInputSnapshot` | `snapshotId,libraRunId,runExecutionBasisDigest,acceptanceSpec(AcceptanceSpec),resolvedIdentitySnapshot,productStructureSnapshot,productFactSnapshots[1..64]{productFactId,factKind,factRevision,schemaRef,factValue,factDigest,evidenceDigest},verifiedArtifactManifest,productMaterialManifest,selectedProducts[1..32]{selectedProduct(SelectedProductOutput),verification(ProductMediaVerification),workspaceHandleDigest?},inventoryBindingSnapshots[1..1024]{materialKey,bindingRevision,bindingDigest,controlRevision,controlProjectionDigest},productFactSetDigest,selectedProductSetDigest,inventoryBindingSetDigest,productSnapshotDigest,snapshotDigest`；每项selectedProduct必须`result=selected`且引用同一passed verification；workspaceHandleDigest只允许workspace_output且逐字节匹配Workspace Reference，direct_input必须NULL并匹配Run immutable Input Material。Fact按`factKind,productFactId,factRevision`、selected product按`selectedHandleId,selectedVerificationId`、Inventory Binding按`materialKey,bindingRevision`的UTF-8 bytes/整数升序，tuple唯一；Fact/Artifact/Material/selected verification必须逐字节命中同Run immutable Owner rows及Package staging inputs；Acceptance Spec为Run Basis冻结的完整revision，不是current/latest。`productFactSetDigest=SHA-256(JCS({schema:"libra.product-conformance-fact-set@1",items:productFactSnapshots}))`；`selectedProductSetDigest=SHA-256(JCS({schema:"libra.product-conformance-selected-product-set@1",items:selectedProducts}))`；`inventoryBindingSetDigest=SHA-256(JCS({schema:"libra.product-conformance-inventory-binding-set@1",items:inventoryBindingSnapshots}))`；`productSnapshotDigest=SHA-256(JCS({schema:"libra.product-conformance-product-snapshot@1",resolvedIdentitySnapshot,productStructureSnapshot,productFactSetDigest,verifiedArtifactManifestDigest:verifiedArtifactManifest.manifestDigest,productMaterialManifestDigest:productMaterialManifest.manifestDigest,selectedProductSetDigest,inventoryBindingSetDigest}))`；`snapshotId=SHA-256(JCS({schema:"libra.product-conformance-input-id@1",libraRunId,runExecutionBasisDigest,acceptanceSpecRecordDigest:acceptanceSpec.recordDigest,productSnapshotDigest}))`，`snapshotDigest=SHA-256(JCS(完整value excluding snapshotDigest))`；完整值≤`4 MiB`，通过Plan named input binding引用，不进入Event Result/Outbox。缺relation或digest不一致是input integrity failure，Executor不得扫描Owner Store或Foundation Result补值 |
...
| `ProductConformanceEvidence` | `VerificationEnvelope + libraRunId + acceptanceSpecId + acceptanceSpecRecordDigest + runExecutionBasisDigest + productSnapshotDigest + productFactSetDigest + evaluatedRequirementSetDigest + unmetRequirementCodes[] + reasonCodes[]`。Capability只从`ProductConformanceInputSnapshot@1`完整值执行六组closed evaluation：Identity比对resolved identity kind/provider/season；Structure比对single/season及Episode coverage；Metadata逐项验证required field、Artifact kind/render/decodable Evidence；Mandatory Media与Space要求每个Selected Product携带同Requirement的passed `ProductMediaVerification`；Inventory验证Domain Binding、checksum、materialized Artifact及layout。Metadata field lookup固定为：`tmdb_movie_id|tmdb_series_id|jav_code`读取对应Provider Identity，`internal_identity`读取Resolved Identity，`actor`读取MediaCastFact非空relation，`season_number|episode_number`读取Product Structure/Episode scope，其他field code读取`ProductMetadataFact.descriptiveFacts`同名non-null/non-empty字段；不得按模糊别名猜测。Identity kind也按同一Provider/Resolved Identity tuple精确匹配。unmet closed set为`identity_unmet|season_identity_unmet|structure_unmet|episode_coverage_unmet|metadata_field_unmet|metadata_artifact_unmet|sidecar_unrenderable|image_undecodable|media_form_unmet|video_codec_unmet|container_unmet|file_extension_unmet|minimum_raster_unmet|system_upscale_forbidden|primary_audio_unmet|max_size_exceeded|domain_binding_unmet|checksum_unmet|artifact_materialization_unmet|layout_unmet`，按该顺序去重；Snapshot缺失/摘要不符不伪装为unmet，而以reason `snapshot_integrity_failure`产生failed。无reason且unmet为空为passed；有合法业务unmet为failed。`evaluatedRequirementSetDigest=SHA-256(JCS({schema:"libra.product-conformance-requirements@1",acceptanceSpecId,recordDigest:acceptanceSpecRecordDigest,requirements:input.acceptanceSpec.requirements}))`，`verificationId=SHA-256(JCS({schema:"libra.product-conformance-verification-id@1",libraRunId,runExecutionBasisDigest,productSnapshotDigest,evaluatedRequirementSetDigest}))`，Evidence/Result digest按Envelope统一公式，完整value≤`16 KiB`；P9-08 Attestation只能逐字节引用passed的verificationId、evidenceDigest及evaluatedRequirementSetDigest |
