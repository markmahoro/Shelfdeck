...
- Adult Provider只使用当前官方ThePornDB Bearer REST最小子集：
  ...
  `/jav/{identifier}`，并同时重验returned id与sku。`scene.id`必须是1..256
  UTF-8 bytes的真实string；object/number/null/empty/oversize均fail closed，
  search row非法时不得发起exact-read transport。performer identifier同样禁止
  object coercion。generic metadata与Product路径共享同一REST client，禁止把sku
  猜作path identifier；
...
H1.2 replacement实现closure commit为`b5cc89ae`（前序REST identifier
continuity `a9b1f993`）。focused H1.1/H1.2/P5回归
