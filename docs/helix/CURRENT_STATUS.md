...
- 2026-07-11 Atomic Capability Runtime closure 已完成：Capability Catalog 使用 nominal + structural schema、版本化 ports、effect/resource/approval/fencing/cancel contract；不可变 Graph 持久化合同签名快照并拒绝版本漂移。Executor 已按 Basedata、Metadata、Western Adult、Transcode、Upgrade、Media Asset 和 Maintenance 物理模块拆分。
- 最终自动证据：Service `205/205` 通过，Admin Web production build 通过，disposable real FFmpeg Graph 覆盖 precheck/attempt/verify/select/disposition/preview/replace/cleanup/post-effect，Artifact materialize 实测覆盖 rebind 后 current source。静态审计覆盖内部 polling、Executor 间调用、Task/Event 写入、跨域 signal、Inventory drift 和重复 shared effect。
- 明确删除：旧系统从未具备可执行 Subtitle Provider，`subtitle.search/download/verify` schema-only 名称已移除；中文字幕 Objective 稳定 blocked，不静默满足。Series/Season Kairox Task 继续由 Helix playable Episode + Libra scope 取代。
- 残余风险只属于后续验收环境：真实 MoviePilot、成人 Provider/Worker、NAS QSV 与大规模性能尚未重新 E2E。它们不能反向否定本次代码 closure，也不能作为恢复 E2E 或生产的授权。localhost `18181` 与生产 ShelfDeck 继续停止。
