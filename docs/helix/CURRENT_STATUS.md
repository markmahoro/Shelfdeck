...
Status: Helix-beta 范围已收窄为仅 Movie 全链路。Movie Procurement与Movie Libra封口保持有效；Movie Arca已完成Handoff B Acceptance、On-deck、Shelf Entry、Deck Fact、Beta Aftercare、Off-deck及Shelf Deregistration完整闭环。当前精确状态为`MOVIE COLLECTION LIFECYCLE READY THROUGH SHELF DEREGISTRATION`。当前生产镜像为 `markmahoro/shelfdeck:helix-beta-20260829-79fc055d1`（无损升级，无 `--helix-clean-init`），health `ok` / `helix-clean-v3`。Intake 席位 3，Workspace `/transcode` rev 2。Helix-beta 验收行见现行基线，尚未把任何 `HB-*` 标为验收 `PASS`。

Last updated: 2026-08-29

## 0. Production lossless upgrade — PROD-023

2026-08-29 已部署 `helix-beta-20260829-79fc055d1`（git `79fc055d1`，tar SHA-256 `a4da3eedccb224639a1177a8f7e595254617f740c18d098ab24357c5b96a0427`），未 `--helix-clean-init`。readiness `helix-clean-v3` / 184 tables / findings []。health `ok`。放弃整理改为页面 Dialog，不再用浏览器 `window.confirm`。请 Ctrl+F5。Field/Shelf/`/transcode`/集成未改。
...
时按current Control projection digest刷新Eligibility。`UAT-070` 已由 commit `efaf2d827` 修复并通过失败库克隆 RESTART/FACT；`UAT-064` 同一真实证人在安全重启后自然完成，执行中与完成态 API/FACT 都满足命题。2026-08-23 Product Owner 明确接受两项现有证据并要求关闭；未取得的 UI 截图仍如实记录。当前总账为70/70。
...
## 0. Recorded product gaps — UAT-050–070
...
关闭作业基线：`docs/helix/acceptance/UAT_CLOSURE_BASELINE.md`（`UAT-001`–`UAT-070` 共 70 行）。已删除不可靠的 `helix-beta-user-e2e` workflow。正式关闭立即汇报且不暂停；确认关闭时发现新产品缺陷则暂停并先登记新 UAT。单元测试不能把任何一行标为 `PASS`。
...
- `UAT-064` 已关闭：Formation 步骤从冻结 Plan/Work/Event FACT 推导真实 CPU/GPU 与成品验证状态；commit `daaef8c3d`。真实证人执行中为 GPU running / output verify pending，安全重启后 GPU、输出验证与上架均完成；Product Owner 接受现有 API/FACT 证据。
...
- `UAT-070` 已关闭：新 Metadata Work 创建时读取并冻结当前 Integration Handle；单个 reconcile scope 失败不再阻断 startup 或跳过失败 cursor。commit `efaf2d827`，真实失败库克隆 RESTART/FACT 已通过，Product Owner 接受现有证据。
