...
| 桌面依赖媒体服务（门禁）      | `docs/requirements/REQ_FEATURE_desktop-requires-media-service.md` · `docs/design/DESIGN_DESKTOP_MEDIA_SERVICE_AVAILABILITY.md`                                                                                                                                                                                                                              | 桌面壳层以 `GET /v1/health` 为可达判据；服务不可用时强门禁；与配置保存成功语义一致；开发变量见 `DEV_SETUP`                 |
