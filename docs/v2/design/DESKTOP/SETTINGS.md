...
| 配置域 | 存储位置 | 管理方 | 示例 |
|--------|----------|--------|------|
| **desktop 本地设置** | electron-store (本地) | SETTINGS 模块 | serviceUrl, playerExePath, pathMappings |
...
| `localPathMapFrom` | `""` | 空表示不做映射 |
| `localPathMapTo` | `""` | 空表示不做映射 |
...
- preload 只暴露 `shelfdeckSettings` 三个方法，不暴露 store 对象本身
- 键名白名单？当前不做键名校验（设置面板是唯一的调用方）
...
| 本地路径映射（源） | localPathMapFrom | text input | 如 D:\media |
| 本地路径映射（目标） | localPathMapTo | text input | 如 \\NAS\media |
...
用户点击"保存" → 逐个调用 shelfdeckSettings.set()
    → 成功 → 显示"已保存"（2s 后消失）
    → 失败 → 显示错误提示
...
1. electron-store 更新
2. 主进程广播 `cp:updated`
3. CONNECTION 模块重新读取 baseUrl
4. ConnectionGate 触发健康检查
