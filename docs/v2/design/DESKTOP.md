...
│ - 读取 service 连接配置（通过 shelfdeckMedia） │
│ - 用户交互（打分、观看标记、任务下发）          │
│ - 本地设置编辑（通过 shelfdeckSettings）       │
└─────────────────────────────────────────────┘
```
...
│  - 通过 shelfdeckSettings 读写本地设置        │
│  - 通过 shelfdeckMedia 感知连接状态           │
└──────────────────────────────────────────────┘
        │ 依赖（import）
        ▼
┌──────────────────────────────────────────────┐
...
| 子模块 | 目录 | 职责 |
|--------|------|------|
| **CONNECTION** | `src/connection/` + `electron/shelfdeckConnection.js` | service 地址解析（env > store > default）、健康检查轮询、连接门禁 |
...
│   │   └── transcodePool.ts       # 编码资源池数据模型 + 展示辅助
│   │
│   ├── utils/                      # 纯工具函数
│   │   ├── doubanMatch.ts         # 豆瓣片名匹配（客户端侧展示用）
│   │   └── format.ts              # 格式化辅助
│   │
│   ├── dev/                        # 仅开发模式使用
...
```
apiClient 轮询 GET /v1/tasks（间隔 400ms）
    │
    ▼
...
UI 层进入媒体库管理页
    │
    ▼
apiClient.getLibraryCache() → 返回 items[] + cachedAt
    │
    ▼
UI 层展示列表，叠加本地标注（评分、观看状态）
    │
    ▼
用户修改评分 → apiClient.patchItemRatings() → service 写入
```

### 4.4 连接健康监控

```
connection/health.ts 定时轮询 GET /v1/health
    │
    ▼
ConnectionGate 组件根据健康状态决定显示内容：
  - healthy → 渲染正常页面
  - unhealthy → 显示连接引导界面
```
...
| 路径映射 | SETTINGS (electron-store) | 本地磁盘 | 本地路径 → NAS 路径映射 |
...
| 调用方 ↓ / 被调用方 → | CONNECTION | API_CLIENT | SETTINGS | models | utils |
|---|---|---|---|---|---|
| **UI (pages)** | ConnectionGate | apiClient | shelfdeckSettings | MediaTask, ManagedMediaItem | doubanMatch, format |
| **UI (components)** | — | apiClient | shelfdeckSettings | MediaTask, ManagedMediaItem | doubanMatch, format |
| **API_CLIENT** | baseUrl, apiKey | — | — | API 类型 | — |
| **CONNECTION** | — | — | 读取 serviceUrl/apiKey | — | — |
...
| service 连接管理 | `DESKTOP/CONNECTION.md` | v2 编写中 |
| REST API 客户端层 | `DESKTOP/API_CLIENT.md` | v2 编写中 |
| 配置持久化 | `DESKTOP/SETTINGS.md` | v2 编写中 |
| UI 组件与布局 | `DESKTOP/UI.md` | v2 编写中 |
