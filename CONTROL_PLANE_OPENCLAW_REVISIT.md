...
> **读者**：产品/架构/后端/客户端实现者。  
> **非目标**：替代 `PROJECT_MANAGEMENT.md` 的里程碑表，也不替代 `TASK_CENTER_FULL_LOGIC.md` 的可执行条文；本文描述 **目标态与接口形态**，具体任务状态机仍以任务中心 SSOT 为准。
...
| # | 方向 | 要点 |
|---|------|------|
| **1** | **架构前后端分离** | 前端（Electron）负责观影与全功能 UI；后端（媒体控制面）负责任务持久化、7×24 转码 Worker、Emby 批量操作、对外 API。**后端部署优先支持：Windows 原生/服务化 与 飞牛 fnOS 上 Docker（及 Compose）**。 |
...
│  — 业务读写：优先调用「媒体控制面」HTTP API（配置、队列、重温、任务状态）    │
└────────────────────────────┬────────────────────────────────────┘
                             │ HTTPS / 内网
┌────────────────────────────▼────────────────────────────────────┐
│  媒体控制面（后端）                                                  │
...
- **演进**：将「任务持久化、调度决策、转码执行」从 **渲染进程 + IPC** 逐步迁移为 **调用控制面 API**；本机 IPC 收缩为 **播放相关**（`launchPlayer`、可选本地探测等）——**路径映射规则的权威在控制面**，见 **§3.4**。
...
- **REST 契约（机器可读 SSOT）**：仓库内 [`docs/api/openapi.yaml`](docs/api/openapi.yaml)（索引、IPC→REST 对照与 lint 说明见 [`docs/api/README.md`](docs/api/README.md)）。具体路径、方法、鉴权与错误体以 OpenAPI 为准；本文仅描述战略分工，若与 OpenAPI 冲突则 **先修订 OpenAPI 与本文至一致再实现**。
...
Emby 与本地盘符/SMB/容器挂载 **不是同一套字符串**；若用户只在客户端配好「能起播放器」的映射，而控制面未配 **可读写的媒体路径规则**，会出现 **「能播但预检/转码/替换失败」**。采纳以下约定，避免两套互不同步的「暗映射」：

| 原则 | 说明 |
|------|------|
| **配置权威在控制面** | 媒体路径映射、`transcodeTempRoot` 等与 **读源、写临时、替换** 相关的项，以 **控制面持久化配置为唯一真相（SSOT）**。 |
| **Electron 只做展示与编辑** | 设置页 **读写均经 API 落到控制面**；不在 `localStorage` 另存一套与 Worker 对打的映射真相。 |
| **UI 写清楚** | 主配置区展示 **「控制面 / Worker 用」** 的映射（及临时目录等）；仅在 **控制面运行环境与用户桌面不一致**（如 Docker 在 NAS、桌面在 Windows）时，再展开 **可选的「本机播放附加映射」**，并与主表 **明确区分**，避免用户只配到一半。 |
| **预检与 Worker 同源** | 转码预检、验收、替换路径解析 **与控制面使用同一套规则**；禁止客户端一套、服务端再硬编码一套导致行为分叉。 |
...
| 层级 | 职责 | 载体 |
|------|------|------|
| **MCP Tools** | **可执行、可审计** 的原子或高阶操作：查库、写入重温、入队洗版/压缩、查询任务状态等。描述与 JSON Schema 需 **自解释**，便于模型选型。 | 媒体控制面 MCP Server |
...
- **当前**：`mvp` 以 Electron 主进程 `embyService` + `preload` IPC 为主；任务与豆瓣等大量在渲染进程与 `localStorage`。
- **迁移策略**：引入 **领域服务层** 与 **单一后端真相**；新功能（重温队列、MCP）**优先后端实现**，前端改为 API 消费者。转码执行迁往控制面容器后，与「7×24」一致。
- **任务中心条文**：洗版/删除/转码状态名、Flow 与验收规则仍以 `TASK_CENTER_FULL_LOGIC.md` 为 SSOT；后端实现应对齐该文档，避免双规格。
- **HTTP 接口条文**：与 Emby 无关的控制面自有 REST 以 [`docs/api/openapi.yaml`](docs/api/openapi.yaml) 为 SSOT（当前 Electron 发版仍以前端 IPC 为主，见模块化 PRD **§11.2**）。
...
| 阶段 | 内容 |
|------|------|
| **P0** | 控制面 MVP：鉴权、配置、Emby 只读搜索、重温队列 REST；Electron 只读展示重温（或双写过渡期）。 |
| **P1** | MCP Server 与 P0 同源调用；OpenClaw 联调「经典回顾」主路径；转码 Worker 迁入控制面（飞牛 Compose + Windows profile）。 |
